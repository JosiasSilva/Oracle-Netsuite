			...
			try
			{
				...
				else
				{
					...
					for(var i=0; i < attributeNodes.length && i < 1; i++)
					{
						...
						if(locationType.indexOf("Drop Box")!=-1)
						{
							nlapiLogExecution("debug","Drop Box Location","Script will Ignore.");
							continue;
						}
						
						var match = true;
						
						address1 = address1.replace(/\bN\b/g,'North');
						address1 = address1.replace(/\bS\b/g,'South');
						address1 = address1.replace(/\bE\b/g,'East');
						address1 = address1.replace(/\bW\b/g,'West');
						...
						shipValidateObject.address1 = shipValidateObject.address1.replace(/\bN\b/g,'North');
						shipValidateObject.address1 = shipValidateObject.address1.replace(/\bS\b/g,'South');
						shipValidateObject.address1 = shipValidateObject.address1.replace(/\bE\b/g,'East');
						shipValidateObject.address1 = shipValidateObject.address1.replace(/\bW\b/g,'West');
            ...
            if (csFulStatus)
            {
              ...
              if (arrStatus != "")
              {
                if (index ==- 1)
                {
                  ...
                  nlapiLogExecution("debug", "field: CS Fulfillment Status Changed  at:", new Date());
                }
              }
