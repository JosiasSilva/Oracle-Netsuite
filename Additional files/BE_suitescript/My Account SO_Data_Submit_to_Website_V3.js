 ...
 * Script Type   : Suitlet
 * Script Name   : My Account SO_Data_Submit_to_Website.js 
 * Created Date  : Aug 24, 2016
 * Last Modified Date :Sep 8, 2016
* SB Url: /app/common/scripting/script.nl?id=998
 **/
...
function MyAccount_PushSOData_to_Website(request,response)
{
	try
		{
			var soId= request.getParameter("record");
           		nlapiLogExecution("debug","salesorder Id is: "+soId,soId);
			...
			if ((orderType==1 || orderType==5) && soStatus!="Pending Approval" && MyAccount_WebInfo=='T' && (MyAccount_lastUpdated=="" || MyAccount_lastUpdated==null) )
			{				
				PushSODataNStoPortal(soObj);
				nlapiSubmitField('salesorder',soId,['custbody_my_account_info_on_web_via_so'],['YES']);
				nlapiLogExecution("Debug"," SO data Succuessfully Pushed To Website:",soId);
			}
			else if ((orderType==1 || orderType==5) && soStatus!="Pending Approval" && MyAccount_WebInfo=='T' && (MyAccount_lastUpdated!="" || MyAccount_lastUpdated!=null))
			{
				PushSODataNStoPortal(soObj);
				nlapiSubmitField('salesorder',soId,['custbody_my_account_info_on_web_via_so'],['YES']);
				nlapiLogExecution("Debug","Updated SO data again Succuessfully Pushed To Website:",soId);
			}
			else
			{

				nlapiSubmitField('salesorder',soId,['custbody_my_account_info_on_web_via_so'],['No']);
				nlapiLogExecution("debug","SO Data not matched with condition:",soId); 
                        	response.sendRedirect("RECORD","salesorder",soId,false,'');
			}
		}
		catch(err)
		{
			...
			nlapiSubmitField('salesorder',soId,['custbody_my_account_info_on_web_via_so'],['No']);
			nlapiLogExecution("Debug","SO data not integrate successfully to website",soId);
                        response.sendRedirect("RECORD","salesorder",soId,false);
		}
	}
...
function PushSODataNStoPortal(soObj)
{
	try
	{
		...
		var soId=soObj.getFieldValue("id");
		nlapiLogExecution("DEBUG", "SO initiated to Push To Portal for soId:"+soId,soId);
		...
		if(soId!='' && soId!=null)
		{
			...
			var web_resize_eligibility = soObj.getFieldText("custbody_web_resize_eligibility"); //Added by Yagya Kumar 25 May 2017
			    ...
			    var order_type=type_of_order;
				 ...
				 for ( var i = 1 ; i<= itemCount  ; i++)
				 {
					...
					{	
						...
						{						
							// var itemname=soObj.getLineItemValue('item','item_display',i);							 
							 var itemPrice=soObj.getLineItemValue('item','amount',i);
                ...
                // Ended on 19/08/2016
                    ...
                    {							 
							//var itemname=soObj.getLineItemValue('item','item_display',i);							
							 ...
							 //Added by Rahul Panchal as per NS-416 on 27/10/2016
							  var center_Gem=soObj.getLineItemValue('item','custcol_center_gem',i);
							 //var center_Gem=soObj.getLineItemText('item','custcol_center_gem_item',i);
							 var center_Gem_item;
							 ...
							 {								 
								 ...
								 else
								 {
										...
										//center_Gem_item=[center_Gem[0] + , + center_Gem[1]];
            ...
            // Added on 19/08/2016 by Rahul Panchal
                ...
                // ended on 19/08/2016
				 ...
				 //Customer Deposit
					...
					var searchResult=[];
					var searchid = 0;
						filters[0] = nlobjSearchFilter("salesorder",null,"is",soId);
					...
					//Item Fulfillment
					var filters = [];
					filters[0] = nlobjSearchFilter("createdfrom",null,"is",soId);									
				 ...
				 // Setting of Headres
					...
					if(response1.code==200)
					{
						...
						nlapiLogExecution("debug", "SO successfully pushed to Portal.", +soId,soId);
						...
						nlapiSubmitField("salesorder",soId,"custbody_my_account_info_last_updated",MyAccount_lastUpdated);
	...
	{             
	     nlapiLogExecution("debug","Error occur during Salesorder Push from NS to portal is : ",err.message); 
             //response.sendRedirect("RECORD","salesorder",soId);             
             response.sendRedirect("RECORD","salesorder",soId,false);        
	}
