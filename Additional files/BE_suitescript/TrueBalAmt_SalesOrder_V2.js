   ...
   if( type=="create"  || type=="edit" || type=="xedit")  
	{
		...
		try{
					...
					//Get True Bal Adjustment
					 var TrueBalAdjustAmount= nlapiLookupField('salesorder',soId,'custbody_truebalanceadjustment'); 
					 if(TrueBalAdjustAmount=='' || TrueBalAdjustAmount==null)
					 {
						 TrueBalAdjustAmount=0;
					 }
			         var TrueBalWebsiteAdjustAmount= nlapiLookupField('salesorder',soId,'custbody_website_truebalance_adj_amt'); 
					 if(TrueBalWebsiteAdjustAmount!='' && TrueBalWebsiteAdjustAmount!=null)
					 {
						 TrueBalAdjustAmount=TrueBalWebsiteAdjustAmount;
					 }
			         else
					 {
					   TrueBalAdjustAmount=0;
                   ...
                   var i=0;
					...
					if( type=="create"  || type=="edit" || type=="xedit")
					{
						...
						//Added by Pankaj jadaun : Req:-CS Fulfillment Status - Pending Balance 
							var fulfillment_status= nlapiLookupField ( 'salesorder' , soId , 'custbody140')						  
							if(trueBalanceAmt>=5)
							{
								if(fulfillment_status!=null && fulfillment_status!='')
								{
									fulfillment_status=fulfillment_status.split(',');	
									if(fulfillment_status.indexOf('18')==-1)
									{
									  fulfillment_status.push(18);
									}
								}
								else
								{
						...
						nlapiSubmitField("salesorder",soId,["custbody_truebalanceamount","custbody_truebalanceadjustment","custbody_website_truebalance_amt","custbody140"], [currencySymbol+" "+numberWithCommas(trueBalanceAmt),TrueBalAdjustAmount,trueBalanceAmt,fulfillment_status]);
   ...
   try
    {
        ...
        if(TrueBalAmtField != '' && RunoutRingField!='')
        {
