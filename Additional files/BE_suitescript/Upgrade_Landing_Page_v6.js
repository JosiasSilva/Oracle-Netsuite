...
function Upgrade_Landing_Page(soRec,soId)
{
  var form = nlapiCreateForm('NEW UPGRADE');
  form.addSubmitButton('Save');
  //form.addButton('custpage_back', 'Back', "window.history.go(-1);");
  var backDataUrl_InitialPage =  backDataUrlInitialPage(soId);
  nlapiLogExecution("Debug","Back Data Url for Upgrade Landing Page ",backDataUrl_InitialPage);
  ...
  //Create Field Groups
  form.addFieldGroup("upgrade","Order Information");
  form.addFieldGroup("delivery","Delivery/Shipping");
  //form.addFieldGroup("items","Item Details");Commented by Ravi on 03/04/2017      
  //Ended

  //Exchange Section Form Fields Started
  var fld = form.addField("custpage_customer","select","Customer","customer","upgrade");
  fld.setMandatory(true);
  fld.setDefaultValue(soRec.getFieldValue("entity"));
  fld.setDisplayType("disabled");

  /*fld = form.addField("custpage_date","date","Date",null,"upgrade").setDisplayType("hidden");
  fld.setMandatory(true);
  var today = new Date();
  ...
  fld = form.addField("custpage_created_from","select","Created From","salesorder","upgrade");
  fld.setMandatory(true);
  fld.setDisplayType("inline");
  fld.setDefaultValue(soId);

  /* fld = form.addField("custpage_orig_sales_rep","select","Original Sales Rep","employee","upgrade");
  fld.setMandatory(true);
  //fld.setDisplayType("inline");
  ...
  //Original Sale Rep
  fld = form.addField("custpage_orig_sales_rep","select","Original Sales Rep",null,"upgrade");
  if(results)
  {
    fld.addSelectOption("","");
  ...
  //New Sale Rep
 /* fld = form.addField("custpage_new_sales_rep","select","Rep in Communication",null,"upgrade").setDisplayType("hidden");
  if(results)
  {
    fld.addSelectOption("","");
    for(var x=0; x < results.length; x++)
  ...
  fld.setDefaultValue(soRec.getFieldValue("salesrep"));
  */
  fld = form.addField("custpage_actual_ship_date","date","Actual Ship Date",null,"upgrade");
  fld.setMandatory(true);
  fld.setDefaultValue(nlapiLookupField("salesorder",soId,"actualshipdate"));
  fld.setDisplayType("disabled");     

   
  // fld = form.addField("custpage_rep_in_comm", "select", "Rep in Comm", "employee", "upgrade").setDefaultValue(nlapiGetUser()); // Added: NS-1363 - Oct 08, 2018 - Nikhil
  
  /* Start: Added by Nikhil on December 13, 2018 for NS-1363 */
  fld = form.addField("custpage_rep_in_comm", "select", "Rep in Comm", "employee", "upgrade"); // Added: NS-1363 - Oct 08, 2018 - Nikhil
  ...
  fld = form.addField("custpage_so_imp_notes","textarea","Original Sales Order Notes",null,"upgrade");
  fld.setDefaultValue(soRec.getFieldValue("custbody58"));
  fld.setDisplayType("disabled");

  fld = form.addField("custpage_reason_for_return","select","Reason For Return","customlist21","upgrade");
  fld.setMandatory(true);

  fld = form.addField("custpage_type_of_send_back","select","Type of Send Back","customlist22","upgrade");
  fld.setMandatory(true);
  fld.setDefaultValue("6"); //Default to EXCHANGE

  //Added by Ravi on 03/04/2017
  fld = form.addField("custpage_or_notes","textarea","OR Notes",null,"upgrade"); 
  var or_notes = nlapiLookupField('salesorder',soId,'custbody254');
  fld.setDefaultValue(or_notes);  
  //Ended

  //Added by Sandeep - NS-593
  fld = form.addField("custpage_drop_off","select","Drop Off",'customlist_drop_off',"upgrade");
  ...
  fld = form.addField("custpage_exchange_notes","longtext","Exchange Order Notes",null,"upgrade");
  fld.setMandatory(true);

  //fld = form.addField("custpage_place_of_sale","select","Place of Sale","classification","upgrade").setMandatory(true);
  fld = form.addField("custpage_place_of_sale","select","Place of Sale","null","upgrade").setMandatory(true);//Added on 03/06/2017
  ...
  fld = form.addField("custpage_amount_paid","float","Amount Paid By Customer",null,"upgrade").setDisplayType("hidden");;
  fld.setMandatory(true);
  //fld.setDefaultValue(nlapiLookupField("salesorder",sales_order,"custbody55"));

  fld = form.addField("custpage_block_auto_emails","checkbox","Block Auto Emails",null,"upgrade");
  //Exchange Ended
  ...
  */
  ...
  var upgrade_NewItemOrdered_Tab = form.addTab('custpage_upgrade_new_item_ordered_tab', '');
  var upgrade_ItemReturningToBe_SubTab = form.addSubTab('custpage_upgrade_item_returning_to_be_subtab','');
  upgrade_ItemReturningToBe_SubTab.setLabel('Item Returning to BE');

  //Items Returning Started
  var itemList = form.addSubList("custpage_items_returning","inlineeditor","Item returning to BE",'custpage_upgrade_item_returning_to_be_subtab');
  ...
  itemList.addField("custpage_items_returning_amount","currency","Amount").setDefaultValue('0');   
  ...
  var itemArr=SO_itemreturn_exchange(soRec);//same for both exchange and upgrade
  ...
  var itemList1 = form.addSubList("custpage_new_items","inlineeditor","New Item(s) Ordered",'custpage_upgrade_new_item_ordered_tab');
  ...
  fld =form.addField('custpage_upgrade_id','text').setDisplayType('hidden').setDefaultValue('6');
  form.setScript("customscript_postsale_exchange_client");


  // highlight required field background color
