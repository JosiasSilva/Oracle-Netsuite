 ...
 */
function calcSOContributionMargin(rec_type,rec_id)
{
	try
	{	
		nlapiLogExecution("debug","Sales Order Internal ID",rec_id);
		setContributionMargin(rec_id)
	}
	catch(err)
	{
...
function setContributionMargin(soID)
{
	try
	{
		...
		columns.push(new nlobjSearchColumn("custbody_total_item_price"));
		...
		if(results!=null)
		{
			...
			nlapiLogExecution("debug","Currency",results[0].getValue("currency"));
			nlapiLogExecution("debug","Exchange Rate",results[0].getValue("exchangerate"));
			
			if(results[0].getValue("currency")=="2")
			{
				//For GBP orders, get the total order price from the body Total Price field and convert to USD
				revenue = results[0].getValue("custbody_total_item_price") * results[0].getValue("exchangerate");
				nlapiLogExecution("debug","GBP Revenue Figure",revenue);
			}
			
			for(var x=0; x < results.length; x++)
			{
				if(results[x].getValue("currency")!="2")
				{
		...
		for(var x=0; x < lineItems.length; x++)
		{
			...
			else
			{
				...
				if(lineItems[x]["po_status"]!="fullyBilled")
				{
					...
					//Check PO to ensure Mold or Customer Item line items are not hanging open
					var filters = [];
					filters.push(new nlobjSearchFilter("internalid",null,"is",lineItems[x]["po"]));
					filters.push(new nlobjSearchFilter("mainline",null,"is","F"));
					filters.push(new nlobjSearchFilter("taxline",null,"is","F"));
					filters.push(new nlobjSearchFilter("shipping",null,"is","F"));
					var billedFormula = "NVL({quantity},0) - NVL({quantitybilled},0)";
					filters.push(new nlobjSearchFilter("formulanumeric",null,"greaterthan","0").setFormula(billedFormula));
					filters.push(new nlobjSearchFilter("item",null,"noneof",["59143","66394"]));
