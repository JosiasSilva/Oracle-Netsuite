...
function priority_PD_PO_Link_PO_Edit(type)
{
  if(type=='edit'){
    ...
    for(var x=0; x < Purchase.getLineItemCount("item"); x++)
    {
      items.push(Purchase.getLineItemValue("item","item",x+1));
    }
    if(items.length>0)
    {
      var filters = [];
      ...
      if(results)
      {
        ...
        var parentItem = results[0].getValue('parent');//new add
        //if(parentItem==null || parentItem=='')
        //	continue;
        var Item = nlapiLoadRecord(results[0].getRecordType(),parentItem);
        var activeStatus=nlapiLookupField(results[0].getRecordType(),parentItem,'custitem160');
        if((!activeStatus) || (activeStatus=='-1'))
        {
          nlapiLogExecution('DEBUG','internal id',parentItem);
          ...
          //If Design Approval Status is changed to Finished Item Approved (35) or No Production Needed (36)
          if((desApprStatus == '36' && nlapiGetOldRecord().getFieldValue('custbody41')!='36') || (desApprStatus == '35' && nlapiGetOldRecord().getFieldValue('custbody41')!='35'))
          {
