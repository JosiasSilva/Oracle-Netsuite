...
function beforSubmit(type)
{
  ...
  if(type=='delete')
  {
    ...
    nlapiLogExecution("debug","deleting item...",parentID);
    var searchResult=nlapiSearchRecord('customrecord_melee_item',null,new 	nlobjSearchFilter('custrecord_melee_item',null,'is',parentID), new nlobjSearchColumn('internalid'));
    if(searchResult)
    {
      for(var c=0;c<searchResult.length; c++)
      {
        nlapiDeleteRecord('customrecord_melee_item', searchResult[c].id);
      }
    }
  }
}