...
function generateVendorBill(vendor,billNumber)
{
	...
	try
	{
		...
		if(results)
		{
			if(results[0].getValue("aplocked")=="T" || results[0].getValue("closed")=="T")
			{
				//If account is closed or locked then update to current accounting period
				var today = new Date();
				var newPeriod = "";
				nlapiLogExecution("debug","Today (Month)",today.getMonth());
				...
				newPeriod += today.getFullYear();
				
				nlapiLogExecution("debug","newPeriod",newPeriod);
				
				vendorBill.setFieldText("postingperiod",newPeriod);
			}
		}
