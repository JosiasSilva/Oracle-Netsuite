function  Repair_Final_Create_Label_Send_Email(form)
{
  try
  {
    var  CC =[];
    ...
    nlapiLogExecution("Debug","Repair Order Delivery Information in else block",JSON.stringify(retObj));
    ...
    soRec.setFieldValue("custbody87",3);//Type of Order = Repair added as per feedback document of 20 jun 2017
    soRec.setFieldValue("custbody6",retObj.deliveryDate);
    soRec.setFieldValue("custbody82",retObj.deliveryDateFirm);
    soRec.setFieldValue("custbody254",retObj.orNotes);
    ...
    soRec.setFieldValue("custbody138",3);
    nlapiLogExecution("debug","Place of Sale",retObj.placeofsale);
    ...
    // soRec.setFieldValue("custbody142",11);//repair/resize status
    ...
    {
      var itemId = request.getLineItemValue("custpage_customer_item_returning_be", "custpage_rtn_item", i);
    ...
    var shipService = setShipService(repairOrderID);
    ...
    var type="repair";
    var retlblStatus = createReturnLabelPdf(shipService[1],shipService[0],type,repairOrderID);    
    nlapiLogExecution("Debug","Tracking No",retlblStatus[0]);
    nlapiLogExecution("Debug","File Id",retlblStatus[1]);
    nlapiSubmitField('salesorder',repairOrderID,['custbody123','custbody137'],[retlblStatus[0],retlblStatus[1]]);
    //
    ...
    sendEmailWithAttachment(retlblStatus[1],CC,repairOrderID,toEmail,emailType,copyEmailTemplateInternalId);
  }
  catch(err)
  {
    nlapiLogExecution("error","Repair Page Processing Error in else block final page submission","Details: " + err.message);
  }
}