...
function transferOrders() 
{
  try {
   ...
   // var mySearch = nlapiLoadSearch(null, 6307);// For sandbox
    var mySearch = nlapiLoadSearch(null, 8813);// For production
    var searchresult = [];
    ...
    if (searchCount > 0) {
      ...
      if (itemArrLoc) {
        var search_memo = nlapiGetContext().getSetting('SCRIPT', 'custscript_search_title_to');
        nlapiLogExecution('Debug','Search title',search_memo);
        var location_id = nlapiGetContext().getSetting('SCRIPT', 'custscript_to_location_to');
        nlapiLogExecution('DEBUG','Location through parameter',location_id);
        
         var from_loc = nlapiGetContext().getSetting('SCRIPT', 'custscript_from_location_to');
