      ...
      try
      {
        ...
        if((retData[i].status ==0 || retData[i].status ==2) && (retData[i].ews_id ==null  || retData[i].ews_id ==''))
        {
          ...
          if(clientphone.length < 22)
          {
            ...
            if(retVal)
            { 
              // Added new logic for NS-1171
              // in case of already existing appointment in netsuite
              var filters = new Array();
              filters[0] = new nlobjSearchFilter('custevent_setsterappid', null, 'is',retData[i].id);  
              var search_setster_app_id = nlapiSearchRecord('calendarevent', null, filters, null);
              nlapiLogExecution("DEBUG","JSON Search Results for Existing Setster Appointment Id in Calendar Event",JSON.stringify(search_setster_app_id));
              if(search_setster_app_id != null)
                continue;

              // in case of update start date in netsuite
              var filters = new Array();
              filters[0] = new nlobjSearchFilter('custevent_customer_email', null, 'is',retData[i].client_email);                
              filters[1] = new nlobjSearchFilter('startdate', null,'noton', startDate);		
              filters[2] = new nlobjSearchFilter('custevent_calendar_location', null, 'is', location);
              var search_setster_start_date = nlapiSearchRecord('calendarevent', null, filters, null);
              nlapiLogExecution("DEBUG","JSON Search Results for update start date in Calendar Event",JSON.stringify(search_setster_start_date));
              if(search_setster_start_date != null)
                continue;
              ...
              else
              {
                ...
                nlapiLogExecution("DEBUG","JSON Search Result of Existing Customer",JSON.stringify(results));
                if(results)
                {
