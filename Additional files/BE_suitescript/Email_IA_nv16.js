...
function Generate_IA_Form(record)
{
  ...
  // Email IA for Earrings & Pendants

  var templateId_email_ia_for_earrings_pendants = nlapiGetContext().getSetting("SCRIPT","custscript_email_ia_earrings_pendants");
  var templateFile_email_ia_for_earrings_pendants = nlapiLoadFile(templateId_email_ia_for_earrings_pendants);
  var template_email_ia_for_earrings_pendants = templateFile_email_ia_for_earrings_pendants.getValue();
  var email_ia_for_earrings_pendants = Handlebars.compile(template_email_ia_for_earrings_pendants);

  //Start Here [NS-1128]
  // For Earrings
  var email_ia_cyo_earrings_xml = Email_CYO_Earrings(order,currency,pages,email,today,xml,email_ia_for_earrings_pendants,isIABlockCode);
  nlapiLogExecution("Debug","XML of Email IA CYO Earrings :",email_ia_cyo_earrings_xml);
  if(email_ia_cyo_earrings_xml !='')
    return email_ia_cyo_earrings_xml;
  // For Pendants
  var email_ia_cyo_pendants_xml = Email_CYO_Pendants(order,currency,pages,email,today,xml,email_ia_for_earrings_pendants,isIABlockCode);
  nlapiLogExecution("Debug","XML of Email IA CYO Pendants ",email_ia_cyo_pendants_xml);
  if(email_ia_cyo_pendants_xml !='')
    return email_ia_cyo_pendants_xml;
  // End Here
...
function isIABlocked(orderId)
{
  ...
  {
    var order = nlapiLoadRecord("salesorder",orderId);
    var orderType = nlapiLookupField("salesorder",orderId,"custbody87");
    // Repair = 2 & Resize = 3
    ...
    for(var x=0; x < order.getLineItemCount("item"); x++)
    {
      ...
      var myItemId=order.getLineItemValue("item","item",x+1);
      //	nlapiLogExecution("debug","My Item Id:"+myItemId+", Item Category:"+category,myItemId);				
      var is_IA_Block=nlapiLookupField("item",myItemId,"custitemblock_insurance_appraisal");

      if(is_IA_Block=='' || is_IA_Block==null)
      {
        is_IA_Block='F';
      }
