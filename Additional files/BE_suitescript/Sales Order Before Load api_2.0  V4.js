       ...
       function(search,log,serverWidget,runtime,record,url,redirect) {
         function beforeLoad(context) {
           ...
           if(context.type == context.UserEventType.VIEW|| context.type == context.UserEventType.EDIT)
           {
             ...
             var get_value_cads=false;
             if(request)
             {
               var get_cad=  request.parameters.CADS;
               if(get_cad=='F')
               {
                 get_value_cads=true;
               }
             ...
             if(!get_value_cads)
             {
               ...
               {
                 log.debug({ title: 'Cads_SO Error => SO ID '+SalesRecord.id, details: er.message});
                 get_value_cads=true;
               }
             }
             if(get_value_cads)
             {
               ...
               //log.debug({ title: 'HighlightNotEligibleForReturn  => SO ID '+SalesRecord.id, details:'OK'});
               ...
               // log.debug({ title: 'Highlight_custbody319  => SO ID '+SalesRecord.id, details:'OK'});
               try{Highlight_SO_Fields(search,form,serverWidget,SalesRecord,log); }catch(er){}//  Script 607  //OK
               // log.debug({ title: 'Highlight_SO_Fields  => SO ID '+SalesRecord.id, details:'OK'});
               try{IA_Requested_HighLight(context,serverWidget,SalesRecord,log); }catch(er){} //Script Use-865    //OK
               // log.debug({ title: 'IA_Requested_HighLight  => SO ID '+SalesRecord.id, details:'OK'});
               try{Highlight_CustomerName_On_SO(form,serverWidget,customer_data); }catch(er){} // Script Use 971   //OK
               // log.debug({ title: 'Highlight_CustomerName_On_SO  => SO ID '+SalesRecord.id, details:'OK'});
               ...
               //log.debug({ title: 'BackStock_Automation_UE_BL  => SO ID '+SalesRecord.id, details:'OK'});
             }
             else
             {
               var mode_value=true;
               ...
               redirect.toRecord({
                 ...
                 id : SalesRecord.id,
                 parameters: {'CADS':'F'} ,
                 isEditMode:mode_value
               });
             }
             log.debug({ title: 'End Time  => SO ID '+SalesRecord.id, details: new Date()});
           }
         }
         return {
...
function Cads_SO(form,serverWidget,search,SalesRecord,record,log) {
    ...
    {	
      ...
      if(ring_size_value)
      {
        ...
        if(cads.length>0)
        {
          ...
          log.debug({ title: 'cads_get.length Value   => SO ID '+SalesRecord.id, details: cads_get.length});
          if(cads_get.length>3000)
          {
            cads_get=cads_get.substring(0,500);
          }
          log.debug({ title: 'cads_get.length Value   => SO ID '+SalesRecord.id, details: cads_get.length});
          ...
          log.debug({ title: 'cads_get New Value   => SO ID '+SalesRecord.id, details: cads_get});
          log.debug({ title: 'chkCadValue Old Value   => SO ID '+SalesRecord.id, details: chkCadValue});
  ...
  if(cads_index_value_check.length>0)
  {
    ...
    for (var x = 0; x < lineItemCount; x++) {
      ...
      if(index_value>=0)
      {
        //log.debug({ title: 'value SalesRecord_Update', details: cads_index_value_check[index_value]});
