    ...
    function fieldChanged(scriptContext) {
      ...
      itemObj.vendor = currentRecord.getValue({fieldId:'entity'});
    	...
    	if(sublistName == 'item'){
            ...
            var categoryListVal =  currentRecord.getCurrentSublistValue({sublistId:sublistName, fieldId:'custcol_category'});
            var itemLink = currentRecord.getCurrentSublistValue({sublistId:sublistName,fieldId:'custcolitem_link'});
              console.log('itemLink : ' + itemLink);
            if (itemLink !==  undefined && itemLink != ''){
              
              var item_desc = search.lookupFields({type:search.Type.INVENTORY_ITEM,id:itemLink,columns:['purchasedescription']});
              
            }
           // currentRecord.getCurrentSublistValue({sublistId:sublistName, fieldId:'description'});
    		if(chkBoxVal == true){
    		itemObj.item = currentRecord.getCurrentSublistValue({sublistId:sublistName,fieldId:'item'});
              var item_desc_obj = search.lookupFields({type: search.Type.INVENTORY_ITEM,id:itemObj.item,columns:['parent']});
              
              itemObj.line = currentRecord.getCurrentSublistIndex({sublistId:'item'});
              
              if (categoryListVal == 2){
                     if (item_desc){
                itemObj.censtonedesc = item_desc.purchasedescription;
                     }
              }
              
              if(item_desc_obj.parent[0] !== undefined){
                //console.log(item_desc_obj.parent[0].value);
                itemObj.item = item_desc_obj.parent[0].value;
              }
      ...
      // Code change for multi select
      
      /*else if (fieldName == 'custpage_sku'){
        var sku_first =[];
       var pro_obj =  dialog.create({title:'Add Another SKU?',message: 'CLick OK- Add another , Cancel- Not right now',buttons: [{ label: 'OK', value: 1 },{ label: 'Cancel', value: 0 }]}).then(function(result){
         
 if(result == 1){
 alert ("I am in OK")


if (currentRecord.getValue({fieldId:'custpage_hidden_mul_sku'}) !== ''){
  alert("There is a value in OK");
 sku_first.push(currentRecord.getValue({fieldId:'custpage_hidden_mul_sku'}));
 //sku_first.push(",");
 }
sku_first.push(currentRecord.getValue({fieldId:'custpage_sku'}));
         ...
         //var sku_temp = [];
         if (currentRecord.getValue({fieldId:'custpage_hidden_mul_sku'}) !== ''){
           alert("I am in value for Cancel");
         sku_first.push(currentRecord.getValue({fieldId:'custpage_hidden_mul_sku'}));
         //sku_temp.push(",");
         }
           ...
           //var final_sku = JSON.stringify(sku_temp)
         //currentRecord.setText({fieldId:'custpage_hidden_mul_sku',text:final_sku});
       }
         
 
       });
       
      var sku_final = JSON.stringify(sku_first);
         
currentRecord.setText({fieldId:'custpage_hidden_mul_sku',text:sku_final});
      }*/
  ...
  function saveAndAdd(){
    ...
    var temp_name_var = rec.getValue({fieldId:'custpage_name'});
    
    if (temp_name_var === '') {
      dialog.create({title: 'Missing Mandatory Field',message:'Please enter value for Name'});
      return false;
    }
    
    var mul_line_count = rec.getLineCount({sublistId:'custpage_parent_sku_sublist'});
    
    for(var l=0;l < mul_line_count;l++){
      item_sku_val.push(rec.getSublistValue({sublistId:'custpage_parent_sku_sublist',fieldId:'custpage_sku',line:l}));
    }
   ...
   // moldRec_obj_sa.catalogVal = rec.getValue({fieldId:'custpage_catalog'});
   moldRec_obj_sa.catalogVal = moldRec_obj_sa.nameVal
...
var rec = currentRecord.get();
    ...
    var sin_line_count = rec.getLineCount({sublistId:'custpage_parent_sku_sublist'});
    
    for(var m=0;m < sin_line_count;m++){
      sin_item_sku_val.push(rec.getSublistValue({sublistId:'custpage_parent_sku_sublist',fieldId:'custpage_sku',line:m}));
    }
   ...
   for (var j = 0; j<temp_mold_arr.length;j++){
    ...
    mold_obj.shapeText = rec.getText({fieldId:'custpage_shape'});
    	...
    	mold_obj.skuValue = sin_item_sku_val;
       
       // sku_temp_ctrl = sku_temp_ctrl.replace(/[\uE000-\uF8FF]/g, ",");
       // sku_temp_ctrl = sku_temp_ctrl.replace("\\u000",",");
       
        //log.debug("Mold SKU Value",mold_obj.skuValue);
        
    	// mold_obj.nameVal = mold_obj.lenValue + " "+ "x"+ " "+ mold_obj.widthValue + " " + mold_obj.shapeText;
    	mold_obj.nameVal = rec.getValue({fieldId:'custpage_name'});
        mold_obj.vendorVal = rec.getValue({fieldId:'custpage_hidden_vendor'});
       mold_obj.itemVal = rec.getValue({fieldId:'custpage_hidden_info'});
      // mold_obj.catalogVal = rec.getValue({fieldId:'custpage_catalog'});
      mold_obj.catalogVal = mold_obj.nameVal;
     ...
     */
      function validateLine(scriptContext) {
        var currentRecord = scriptContext.currentRecord;
        var sublistName = scriptContext.sublistId;
     if (sublistName == 'item'){console.log("I am being called correctly");
      
      var catalogText= currentRecord.getCurrentSublistValue({sublistId:sublistName,fieldId:'custcol17'});
     // var tempCatalogText = currentRecord.getCurrentSublistValue({sublistId:sublistName,fieldId:'custcol17'});
   //  var tempCatalogText = ""
     // var catalogText = "";
      
    var arrCatalog = catalogText.split("\n"); 
	console.log('length: ' + arrCatalog.length + ', arrCatalog: ' + arrCatalog); 
      if(catalogText){ // change by Bharath
     if (catalogText.indexOf("\n") == -1) // Bharath
		 catalogText += "\n"; 
      }
      
      
     // currentRecord.setCurrentSublistValue({sublistId:sublistName,fieldId:'custcol17',value:''});
      ...
      // if (sublistName === 'item'){
         ...
         //alert(item_val);
		 if(item_val)
           item_parent = search.lookupFields({type:search.Type.INVENTORY_ITEM,id:item_val,columns:['parent']});
         ...
         var multi_Val =  JSON.parse(currentRecord.getValue({fieldId:'custpage_multiple_save'}));
           ...
           if (mold_val == true) {
             ...
             if (item_val == single_val[j].itemVal){
				 console.log(' inside single save:');
              // if (catalogText != single_val[j].catalogVal){ // Bharath // test : commented : satya
                 //alert("I should be here once only");
                if (arrCatalog.indexOf(single_val[j].catalogVal) == -1){ // test : satya
					console.log('catlog not found in single save: ' + single_val[j].catalogVal);
					catalogText += single_val[j].catalogVal;
					catalogText += "\n";
               }
              ...
              if(item_parent.parent[0] !== undefined){
                if(item_parent.parent[0].value == single_val[j].itemVal){
					console.log(' inside single save Parent:');
                  //if (catalogText != single_val[j].catalogVal){ // Bharath // test : commented : satya
					if (arrCatalog.indexOf(single_val[j].catalogVal) == -1){ // test : satya	
						catalogText += single_val[j].catalogVal;
						catalogText += "\n";
             ...
             if(multi_Val) {
               ...
               for (var i = 0; i<multi_Val.length;i++){
                 ...
                 if (item_val == multi_Val[i].itemVal){
                   //catalogText += "\n";
				   if (arrCatalog.indexOf(multi_Val[i].catalogVal) == -1){ // test : satya
						catalogText = catalogText + multi_Val[i].catalogVal + "\n";
						console.log(' inside multi save:');
					}// test : satya
                 }
                if(item_parent.parent[0] !== undefined){
					if(item_parent.parent[0].value == multi_Val[i].itemVal){
						if (arrCatalog.indexOf(multi_Val[i].catalogVal) == -1){ // test : satya
							catalogText += multi_Val[i].catalogVal + "\n";
							console.log(' inside multi save:parent');
						} // test : satya
					}
				}   
               }
            ...
            // currentRecord.setCurrentSublistText({sublistId:sublistName,fieldId:'custcol17',text:""});
             
               currentRecord.setCurrentSublistText({sublistId:sublistName,fieldId:'custcol17',text:catalogText});
             
             
             //currentRecord.setCurrentSublistText({sublistId:sublistName,fieldId:'custcol17',text:catalogText});
            // currentRecord.setCurrentSublistValue({sublistId:sublistName,fieldId:'custcol17',value:catalogText});
            // currentRecord.setCurrentSublistText({sublistId:sublistName,fieldId:'custcol_catalog_temp',text:catalogText});
           }
       }
	return true;
    }
