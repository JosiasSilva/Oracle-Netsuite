...
function IA_Print_Queue(request,response)
{
	if(request.getMethod()=="GET")
	{
	  var orders=[];
		...
		var numOrders = 0;		
		var filters = [];
		//filters.push(new nlobjSearchFilter("internalid",null,"anyof",[9445164,9380829,9352516]));
	...
	else
	{
		...
		var load_data=[];
		load_data.push({
		haloMeleeSearchResult:haloMeleeSearchResult(),
		salesOrder:[]		
		})
...
function PassData(load_data,AllorderId)
{
		...
		if(results!=null)
		{
		   ...
		   for(var s=0;s<results.length;s++)
			...
			nlapiLogExecution("debug","load_data Collection Test:=",JSON.stringify(load_data));
