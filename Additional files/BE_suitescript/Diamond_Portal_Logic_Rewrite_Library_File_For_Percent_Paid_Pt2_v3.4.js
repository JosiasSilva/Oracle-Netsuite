var scriptId = "2133";
function Diamond_Portal_Logic_Rewrite_Push_To_Portal_For_Percent_Paid(cdpId,type,recordType,contextType) 
{
  try 
  {
    ...
    if (type == 'create' || type == 'edit' || type == 'xedit') 
    {
      ...
      if(cdpId)
      {
        ...
        nlapiLogExecution("DEBUG","Diamond Status old & new value without Json (Temp) Info from system notes",diamond_status_old_new_value);
        nlapiLogExecution("DEBUG","Diamond Status old & new value Json  (Temp) Info from system notes",JSON.stringify(diamond_status_old_new_value));
        ...
        nlapiLogExecution("DEBUG","CS Status old & new value without Json (Temp) Info from system notes",cs_status_old_new_value);
        nlapiLogExecution("DEBUG","CS Status old & new value Json  (Temp) Info from system notes",JSON.stringify(cs_status_old_new_value));
        ...
        sys_note_date_per_page = per_paid_old_new[2];
        ...
        if(recordType == "customerdeposit" || recordType =="customerrefund" || recordType == "salesorder")
        {
          ...
          percent_paid = new_paid;//DP-474
          nlapiLogExecution("DEBUG","Paid Percentage Info After Assigning new % paid","% Paid After Assigning new % paid is : " + percent_paid + " having cdp " + cdpId);
        ...
        {
          // var certificate_status = item_obj.getFieldValue('custitemcertificate_status');// commented old code
          var certificate_included = item_obj.getFieldValue('custitemcertificate_included');// added new logic for Pt2
          ...
          if (check_diamond_group == 'B' && (qnty_on_hand > 0 || qty_in_transit >0))// for point b
          {
            ...
            //if(certificate_status == 5) commented old code
          ...
          if(type == "edit" && recordType == "salesorder" && contextType =="userinterface")
          {
            if(cdp_obj.getFieldValue('custrecord_item_status') =='' || cdp_obj.getFieldValue('custrecord_item_status')==null)
            {
              nlapiLogExecution("DEBUG","Step 5 A",cdpId + ' having item status '+ cdp_obj.getFieldValue('custrecord_item_status'));
              return ;
            }
          }
        }
        //Start DP-498
        var push_to_portal_via_schedule_context = false;
        nlapiLogExecution("DEBUG","Step 5 B",cdpId + ' Push to portal via schedule context ' + push_to_portal_via_schedule_context);
        if (group == 'A' || group_a == 'T') 
        {
          if(type == "edit" && recordType == "salesorder" && contextType =="scheduled")
          {
            if(percent_paid > 20 && diamond_status == 1 && so_status == 'pendingFulfillment')
            {
              push_to_portal_via_schedule_context = true;
              nlapiLogExecution("DEBUG","Step 5 C",cdpId + ' Push to portal via schedule context is ' + push_to_portal_via_schedule_context);
            }
          }
        }
        if(contextType == "scheduled" && push_to_portal_via_schedule_context == false )
        {
          nlapiLogExecution("debug","Step 5D Stopping script execution.","Context: " + contextType + ' and push to portal via schedule context is ' + push_to_portal_via_schedule_context);
          return;
        }
        ...
        if(override_mapping_logic=='F')
        {
          if (group == 'A' || group_a == 'T') 
          {
            ...
            //if(percent_paid > 20 && diamond_status != 5 && so_status == 'pendingFulfillment') commented old logic
          ...
          {
            //fullyBilled
            // if ( (request_type == 1 || request_type == 8) && diamond_status != 5 && so_status == 'fullyBilled' && (new_bill_date == null || new_bill_date == '')) // for point 1 commented old condition
            group = 'B';
            ...
            //else if (request_type == 7 && percent_paid > 20 && diamond_status != 5 && cs_status == 4 && so_status == 'pendingFulfillment' && new_bill_date != null && new_bill_date != '') // for point 2 commented old condition
          ...
          else 
          {
            ...
            if(request_type ==1 || request_type == 8)
            {
              ...
              else if (percent_paid >= 20)// added new logic during QA as per Eric new req. on 30 AUG 2018
              {
                diamond_status = '';//null
                portal_request_type = 3;//Sold
                item_status = 1;//Not Confirmed
            ...
            else if (request_type == 7) 
            {
              ...
              else if (percent_paid > 20 && cs_status == 4)// for poit 11
              {
                //AND new Item Status = ‘Confirmed’
                if(new_item_status == 2)
                {
          ...
          }//End of else part & Neither Group A or Group B Diamonds points
        ...
        if(response.code=='200')
        {
          ...
          nlapiLogExecution("DEBUG","Check for Portal Action Needed having Confirmation Needed(2)",ac_needed + " having cdp " + cdpId);
          ...
          cdp_obj.setFieldValue('custrecord238',"889");
          ...
          // Set Diamond ETA from sales order NS-1387
          if((recordType == "salesorder" && type == "edit") || (recordType =="customerdeposit" || recordType =="customerrefund"))
            //SetDiamondETAFromSalesOrder(cdpId,type);
            if(cdpId!='' && cdpId!=null)
            {
