 ...
 * FILE NAME   			: Email_IA_For_CYO_Pendants.js
 ...
 * Suite Script URL 	: https://system.sandbox.netsuite.com/app/common/scripting/script.nl?id=454&whence=
 * File URL Sandbox		: https://system.sandbox.netsuite.com/app/common/media/mediaitem.nl?uploadrectype=script&id=22328721
 * File URL Production	:
 */

function Email_CYO_Pendants(order,currency,pages,email,today,xml,email_ia_for_earrings_pendants,isIABlockCode)
{
  ...
  {
    return Email_IA_CYO_Pendants(order,currency,pages,email,today,xml,email_ia_for_earrings_pendants,isIABlockCode);
  }
  else 
    return '';
}
function Email_IA_CYO_Pendants(order,currency,pages,email,today,xml,email_ia_for_earrings_pendants,isIABlockCode)
{
  ...
  if(cyo_pendants_pdf_arr.length>0)
  {
    ...
    xml += email_ia_for_earrings_pendants(cyo_pendants_pdf_json_obj);
  }

  nlapiGetContext().setSetting('SESSION','custpage_hIACode',isIABlockCode);
  xml += "</body></pdf>";
  nlapiLogExecution("DEBUG","Email IA xml for CYO Pendants",xml);
  return xml;
