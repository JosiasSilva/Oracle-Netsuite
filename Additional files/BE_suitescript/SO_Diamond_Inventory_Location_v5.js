			...
			if(location!=null)
			{
				...
				for(var x=0; x < order.getLineItemCount("item"); x++)
				{
					...
					if(category=="7")
					{
						...
						// *** New Logic Added 02-14-2015 ***
						
						//Set to Preferred Production Vendor Location (USNY and CH only)
						// 1. All AY stones
						// 2. A stones for the following vendors - Kapu, SDIL, Diarough, S. Narendra, Mehta Brothers
						if((name.charAt(name.length-1)=="Y" && name.charAt(name.length-2)=="A") || (name.charAt(name.length-1)=="A" && (diamond_vendor=="161462" || diamond_vendor=="518900" || diamond_vendor=="3575" || diamond_vendor=="584840" || diamond_vendor=="1707524")))
						{
							if(prodLocation=="1" || prodLocation=="6")
								order.setLineItemValue("item","location",x+1,prodLocation);
						}
						
						//Set any of the following stones to San Francisco
						// 1. All Y stones
						// 2. A stones from vendors excluding the ones above
						if((name.charAt(name.length-1)=="Y" && name.charAt(name.length-2)!="A") || (name.charAt(name.length-1)=="A" && diamond_vendor!="161462" && diamond_vendor!="518900" && diamond_vendor!="3575" && diamond_vendor!="584840" && diamond_vendor!="1707524"))
						{
							order.setLineItemValue("item","location",x+1,"2");
						}
