function Exchange_DropOff_One(dropOff,form)
{
  var rmaID = "";
  ...
  try
  {
   ...
   // var original_salesrep = request.getParameter("custpage_orig_sales_rep");
    //var new_salesrep = request.getParameter("custpage_new_sales_rep");// commented by feedback document additional requirements
    ...
    var country_hid = request.getParameter("custpage_country_hidden");//Added by Ravi
    ...
    nlapiLogExecution("debug","(POST) Items Returning Retrieved Exchange",JSON.stringify(items_exchange_ra));
  ...
  try
  {
    ...
    if(items_return_ra.length>0 || items_exchange_ra.length>0)
    {
      if(rma==null || rma=="")
      {
        ...
        // nlapiLogExecution("debug","Line Item Count",ra.getLineItemCount('item'));
        //Check if transaction ID already exists
        var tranid = ra.getFieldValue("tranid");
        ra.setFieldValue("tranid",checkRaNumber(tranid,1));
        //nlapiLogExecution("Debug","Original Salesrep",original_salesrep);
       ...
       // ra.setFieldValue("salesrep",original_salesrep); //1. Sales rep will always go on the return auth
        ...
        //ra.setFieldValue("custbody247",new_salesrep);// commented by feedback document additional requirements
        ra.setFieldValue("custbody138",return_shipping_label);
        ra.setFieldValue("custbody_drop_off",dropOff);
        ra.setFieldValue("custbody87","4"); //Type of Order = Exchange
        ...
        for(var x=0;x<items_return_ra.length;x++)
        {
          ...
          ra.setCurrentLineItemValue("item","item",items_return_ra[x].item);
          ra.setCurrentLineItemValue("item","quantity",items_return_ra[x].quantity);
          ra.setCurrentLineItemValue("item","amount",items_return_ra[x].saleamount);//updated as per client test case excel sheet 9-22-2017
          ra.setCurrentLineItemValue("item","rate",items_return_ra[x].saleamount);//Added by Ravi 
          ra.setCurrentLineItemValue("item","description",items_return_ra[x].desc);
          ra.setCurrentLineItemValue("item","custcol_full_insurance_value",items_return_ra[x].insurance);
          if(items_return_ra[x].itemsku!='' && items_return_ra[x].itemsku!=null)
            ra.setCurrentLineItemText("item","custcol_item_sku_so",items_return_ra[x].itemsku);
        ...
        for(var x=0;x<items_exchange_ra.length;x++)
        {
          ...
          ra.setCurrentLineItemValue("item","amount",items_exchange_ra[x].saleamount);//updated as per client test case excel sheet 9-22-2017
  ...
  try
  {
    ...
    if(items_return_resize.length>0 || items_return_repair.length>0 || items_return_cust_be.length>0 || new_items.length>0)
    {
     ...
     // newOrder.setFieldValue("salesrep",original_salesrep);
      //newOrder.setFieldValue("custbody247",new_salesrep);// commented by feedback document additional requirements
      ...
      if(orNotes!='')
        newOrder.setFieldValue("custbody254",orNotes); //or notes
      else
        newOrder.setFieldValue("custbody254",''); //or notes
      ...
      for(var x=0; x < items_return_cust_be.length; x++)//customer item at be
      {
      ...
      for(var x=0; x < new_items.length; x++)//new order item
      {
        newOrder.selectNewLineItem("item");
      ...
      {
        //nlapiLogExecution("Debug","Disable Created From for Item Returing to BE",items_return_cust_be[i].disablecreatedfrom);
      ...
      {
        // nlapiLogExecution("Debug","Disable Created From for Resize",items_return_resize[i].disablecreatedfrom);
      ...
      {
        //nlapiLogExecution("Debug","Disable Created From  for Repair",items_return_repair[i].disablecreatedfrom);
