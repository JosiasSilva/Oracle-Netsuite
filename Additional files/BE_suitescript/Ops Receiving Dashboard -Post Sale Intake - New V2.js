function Sale_intake_List(form,obj_get_result,number_of_result,tab_name)
{
  ...
  Sublist_post_sale_intake.addField('custpage_entityid_post_sales','text').setDisplayType("hidden");
  Sublist_post_sale_intake.addField('custpage_tranid_post_sales','text').setDisplayType("hidden");
...
function All_result_fill_sales_intake(results,Sublist_value,number_of_result)
{
  ...
  if(results)
  {
    try
    {
      var col= results[0]?results[0].getAllColumns():[];
      for(var i=0;i<results.length;i++)
      {
        ...
        Sublist_post_sale_intake_Arr.push({
		  ...
		  custpage_tranid_post_sales:results[i].getValue('tranid','custrecord_transaction_record'),
		  custpage_entityid_post_sales:results[i].getText('entity','custrecord_transaction_record')
        });
      }
    }
    catch(er){
