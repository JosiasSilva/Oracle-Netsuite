  ...
  try
  {
    if(type=="create" || type=="edit" || type=="specialorder")
    {
      if(rectype=="salesorder")
      {
        ...
        for(var x=0; x < pos.length; x++)
        {
          ...
          if(ddChanged==true || so_important_notes_changed==true)
          {
            ...
            {
              Updated_Date_Time(type,pos[x]);
              PushPOToPortal(pos[x],"585");
            }
      ...
      else if(rectype=="purchaseorder")
      {
        ...
        {
          //Modified by Sandeep for NS-1148
          var old_dd_push_reason=nlapiGetOldRecord().getFieldValue("custbody251");
          if(old_dd_push_reason=='')
            old_dd_push_reason=null;

          if(old_dd_push_reason!=dd_rush_reason)
            dd_rush_reason_changed = true;
        ...
        if(order!=null && order!="")
        {
          ...
          //Edited by Sandeep for NS-1148
          var soRec = nlapiLoadRecord("salesorder",order);
          //nlapiSubmitField('purchaseorder',nlapiGetRecordId(),'custbody251', soRec.getFieldValues('custbody251')); // by tanuja on 23/7/18 for prevent reverting of DD push reason field

          if(dd_rush_reason_changed)
          {
            //Since DD Push Reason is multi-select, we need to load record and use nlapiSetFieldValues
            //var soRec = nlapiLoadRecord("salesorder",order);//commented For NS-1148
              ...
              if(isGemPO && location=="2")
              {
                ...
                else
                {
                  if(ddChanged)
                    ...
                    nlapiSubmitField("purchaseorder",results[x].getValue("purchaseorder",null,"group"),["custbody103","custbody82","custbody58","custbody251"],[gemstone_status,delivery_date_firm,so_important_notes,soRec.getFieldValues('custbody251')]);
                }
              }
              else
              {
