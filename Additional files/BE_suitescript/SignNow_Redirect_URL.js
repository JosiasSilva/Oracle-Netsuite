function SignNow_Signed_Redirect(request,response)
{
	var documentId = request.getParameter("docid");
	
	var filters = [];
	filters.push(new nlobjSearchFilter("custrecord_signnow_document_id",null,"is",documentId));
	
	var results = nlapiSearchRecord("customrecord_custom_signnow_document","customsearchsignnow_pending_signature",filters);
	...
	response.sendRedirect("EXTERNAL","http://www.brilliantearth.com");
}
