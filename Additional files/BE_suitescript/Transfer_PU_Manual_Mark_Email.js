		...
		try
		{
			...
			if(context.getExecutionContext()=="userinterface")
			{
				...
				if(ifRec.getFieldValue("custbody89")=="2" && nlapiGetOldRecord().getFieldValue("custbody89")!="2")
				{
					...
					if(soId!=null && soId!="")
					{
						//Check if it's a pick-up order for LA or Boston. If so, need to do inventory transfers.
						...
						if(soFields.custbody53=="T" && (soFields.custbody_pickup_location=="2" || soFields.custbody_pickup_location=="3"))
						{
							var puLocation;
