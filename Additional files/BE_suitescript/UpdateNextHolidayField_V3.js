...
function UpdateField(NextHolidayDate, location, Arr, name) {
    try {
        ...
        for (var f = 0; f < NextHolidayDate.length; f++) {
            var HolidayDate = NextHolidayDate[f];

            var filterExpr = [
                ['custbody_next_holiday_date', 'noton', NextHolidayDate], 'AND', ['memorized', 'is', 'F']
            ];
            if (location != '' && location != null) {
                filterExpr = [
                    ['custbody_delivery_location', 'is', location], 'AND', ['custbody_next_holiday_date', 'noton', NextHolidayDate], 'AND', ['memorized', 'is', 'F']
                ];
            }
            //var search = nlapiLoadSearch(null,8262);//Sandbox
            var search = nlapiLoadSearch(null, 8492); // Production
            //search.setFilterExpression(filterExpr);
            ...
            do {
                ...
                for (var p = 0; p < resultSet.length; p++) {
...

                    {
                        ...
                        if (resultSet[p].getValue('type') == 'SalesOrd')
                            type = 'salesorder';
                        var flag = true;
                        if (location != null && location != '' && location != resultSet[p].getValue('custbody_delivery_location'))

                        {
                            flag = false;
                        }
                        if (flag) {
                            nlapiSubmitField(type, resultSet[p].getId(), 'custbody_next_holiday_date', HolidayDate);
                            nlapiLogExecution("debug", "Updated " + name + " : " + resultSet[p].getId(), HolidayDate);
                        }
                        Re_Shedule();
                    } catch (a) {
                        nlapiLogExecution("error", name + " field not update #" + resultSet[p].getId(), a.message);
                    }
                }
