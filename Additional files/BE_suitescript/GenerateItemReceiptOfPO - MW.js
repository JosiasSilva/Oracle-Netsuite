 ...
 * Script URL : /app/common/scripting/script.nl?id=1017
 * Button URL : /app/site/hosting/scriptlet.nl?script=1017&deploy=1
 * Save Search URL : /app/common/search/searchresults.nl?searchid=4722
 */

//Create a Suitelet Script that generate an Item Receipt
/*function GenerateItemReceipt(request,response)
{
	var form = nlapiCreateForm("Create Item Receipt of POs for MW");
	...
	else
	{
		try
		{
			...
			var mySearch = nlapiLoadSearch(null,4722); //Save search id of POs list
			var searchresult = [];
...
function GenerateItemReceipt(request, response)
{
	...
	else if(request.getMethod() == 'POST')
	{
                ...
                params['custscript_savedsearch_id2'] = 4722;
		var result = nlapiScheduleScript('customscript1055', 'customdeploy1',params);  // 1055 is internal id of the scheduled script that you wish to reschedule and 1 is deployment id
