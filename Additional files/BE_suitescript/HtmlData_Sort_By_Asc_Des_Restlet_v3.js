...
function HtmlData_Sort_By_Asc_Des_Restlet(datain) {
  ...
  try {
    ...
    if (data != null && data != '' && data.length > 0) {
      html += "<input type='hidden' id='temp' ><style type='text/css'>#customers th { vertical-align: middle!important; text-align: center!important;}#customers td, #customers th { vertical-align: top;border: 1px solid #ddd !important;text-align: left;padding: 3px;}   \
.SO_Item{margin:-6px;}.SO_Item tr{background-color: transparent !important;}\
#customers tr:nth-child(even) {background-color: #9FD9E0;} #customers th {padding-top: 11px;padding-bottom: 11px;background-color: #088888 !important ;color: white;}   \
\
table#customers {   \
font-size: 14px;   \
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (so_data != null && so_data != '' && so_data.length > 0) {
          ...
          for (var z = 0; z < so_data.length; z++) {
            ...
            var Parent = so_data[z].parent;
            nlapiLogExecution("Error", "Parent Item Id: ", JSON.stringify(Parent));
            ...
            if (match != null && match != '' && match.length > 0) {
              var getparent = itemPulled_addItems(Parent); //Function Call for parent Items...
              //var back_stock_item_internalid_arr =[];
              //var filters=[];
            ...
            if (item_pulled_line_item_parent_matches_parent_of_backstock_match_item == 'T') {
              ...
              html += '<td class="Nested_td ItemPulled"><a style="color: #607799;"onclick="return ItemPulled_Add(this, ' + so_data[z].ItemId + ');" id="ItemPulled_' + i + '_' + z + '" href="#" data-value="">Add Item</a></td>';
...
function BackStock_Data(datain) {
  try {
    ...
    var validSOs=[];
    validSOs.push('0');
    var mySearch = nlapiLoadSearch(null, 'customsearch_backstock_dashboard_2');
    var searchresult = [];
    ...
    while (resultslice?resultslice.length >= 1000 : false);
    var searchCount = searchresult.length;
    if (searchCount > 0) {
      for (var j = 0; j < searchresult.length; j++) {
        validSOs.push(searchresult[j].id);
      }
      nlapiLogExecution("Error", "valid SOs", JSON.stringify(validSOs));
    }
    var dynamicSearch=nlapiCreateSearch('salesorder',[
      ["internalid",'anyof',validSOs],"and",
      ["item.custitem20","anyof","7"],
      "AND",
      ["item.custitem5","anyof","8"]
    ],
                                        [
      new nlobjSearchColumn("tranid"), 
      new nlobjSearchColumn("custbody227"), 
      new nlobjSearchColumn("entity"), 
      new nlobjSearchColumn("custbody58"), 
      new nlobjSearchColumn("itemid","item",null), 
      new nlobjSearchColumn("salesdescription","item",null), 
      new nlobjSearchColumn("baseprice","item",null), 
      new nlobjSearchColumn("custbody_backstock_item_matches"), 
      new nlobjSearchColumn("internalid","item",null), 
      new nlobjSearchColumn("parent","item",null), 
      new nlobjSearchColumn("custbody338"),
    ...
    for(var c=0; c<2; c++)
    {
      ...
      var searchid1 = 0;
      do {
        //var AllRows = results.getResults(searchid, searchid + 1000);//old logic
        var AllRows = results.getResults(searchid, searchid + 50)?results.getResults(searchid, searchid + 50) : []; //---------------------------------------------------------------->>*<<
        nlapiLogExecution("Error", "AllRows", AllRows.length);
        if (AllRows != null && AllRows != '' && AllRows.length > 0) {
          ...
          for (var rs in AllRows) {
            ...
            if (parent != '' && parent != '' && internalid != '' && internalid != null) {
              ...
              nlapiLogExecution('error', 'resultMatch(test)', JSON.stringify(resultMatch)); //-----------------------------------------
            ...
            resultsArr.push({
              ...
              OrderStatus :AllRows[rs].getText(allCol[1])? AllRows[rs].getText(allCol[1]).replace(",", ",<br/>"): AllRows[rs].getText(allCol[1]),
              Name: link_cust,
...
function itemPulled_addItems(parent) //add function to gets the parent items with Parent Items via Item Pulled for task:NS-1229 by Aradhana
{
  var getParentId = '';
  try {
    var filters = new Array();
    filters[0] = new nlobjSearchFilter('internalid', 'parent', 'is', parent);

    // Define return columns
    var columns = new Array();
    columns[0] = new nlobjSearchColumn('itemid', null, null);
    columns[1] = new nlobjSearchColumn('internalid', null, null);
    columns[2] = new nlobjSearchColumn('name', null, null);

    // Create the saved search
    var search = nlapiSearchRecord('inventoryitem', null, filters, columns);
    for (var i = 0; i < search.length; i++) {
      var parentId = search[i].getValue(columns[0]);
      getParentId += " <option value=" + i + ">" + parentId[i] + "</option>";
    }
  } catch (exp) {
    nlapiLogExecution('error', 'Error On Item Pulled Parent Item:-', exp.message);
  }
  return getParentId;
}