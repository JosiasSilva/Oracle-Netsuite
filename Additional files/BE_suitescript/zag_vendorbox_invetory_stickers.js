...
function(serverWidget, search) {
    ...
    function onRequest(context) {
        ...
        custForm.clientScriptModulePath = 'SuiteScripts/print_zpl_label_cl.js';
        ...
        totalSelected_nrp.updateDisplayType({
            displayType: serverWidget.FieldDisplayType.HIDDEN});
        var count_rp = custForm.addField({id:'custpage_count_rp',type:'text', label: 'Total PO Count: ', container: 'custpage_tab_ready_to_pull'});
        count_rp.updateDisplayType({
    ...
    function getDataFromSearch(custForm){
    ...
    function parseSoResults() {
        ...
        log.debug('ALL SOKey', 'SOKey : ' + SOKey);
  ...
  //End of function to get Holding Binder from SO
    ...
    /*Function to fetch all SO Ids and push into an array*/
    function getSalesOrderIds(vendrBoxInvSrchObj){
    	var PO_ResultSet = vendrBoxInvSrchObj.run();
    	var PO_CurrentRange = PO_ResultSet.getRange({start : 0, end : 1000});
    	var startIndex = 0;
    	var endIndex = 0;
    	
    	while ( endIndex < PO_CurrentRange.length ) {
    		var result = PO_CurrentRange[endIndex];
 			var salesOrderId = result.getValue({name: "createdfrom", summary: search.Summary.GROUP});
 			if(salesOrderId){
	 			var intrIdFound = SO_InternalIds.indexOf(salesOrderId);
	  			if(intrIdFound < 0)
	  				SO_InternalIds.push(salesOrderId);
 			}
 			startIndex++; endIndex++;
        	if( endIndex == 1000 ) {   // check if it reaches 1000
        		endIndex = 0;          
        		PO_CurrentRange = PO_ResultSet.getRange({start : startIndex, end : startIndex + 1000});
               }
        	}
    }
    /*End of Function to fetch all SO Ids and push into an array*/  
    
    /*Function to fetch Holder Binder from SO*/
    function findHoldBinder(SO_ResultSet,salesOrderOnPO,itemLinkOnPO ){
      log.debug('salesOrderOnPO', 'salesOrderOnPO : ' + salesOrderOnPO + ' :itemLinkOnPO ' +itemLinkOnPO);
