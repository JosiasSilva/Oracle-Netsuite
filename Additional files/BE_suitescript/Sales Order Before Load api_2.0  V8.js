           ...
           if(context.type == context.UserEventType.VIEW|| context.type == context.UserEventType.EDIT)
           {
             ...
             if(get_cad!="F")
             {
               ...
               try
               {
                 ...
                 var falg_submit={ falg_submit_value:false};



                 try{ Get_Mols_SO(search,SalesRecord,log,falg_submit); //Script Use 606	
                    }catch(er){
                      log_message+="\n Get_Mols_SO  Error  => "+er.message; 
                    }
                 try{ Cads_SO(search,SalesRecord,log,falg_submit); //Script Use 1152	
                    }catch(er){
                 ...
                 if(falg_submit.falg_submit_value)
                 {
                   SalesRecord.save({
                     enableSourcing: true,
...
function Get_Mols_SO(search,SalesRecord,log,falg_submit)
{
    ...
    for (var x = 0; x < lineItemCount; x++) {
      ...
      if(index_value>=0)
      {
        ...
        SalesRecord.setSublistValue({
          sublistId: 'item',
          fieldId: 'custcol_molds_so',
          line: x,
          value: set_molds_value
        });

        falg_submit.falg_submit_value=true;
...
function Cads_SO(search,SalesRecord,log,falg_submit) {
...
function set_status_field(search,SalesRecord,log,falg_submit){
  ...
  //var order_status = search.lookupFields({type: 'salesorder',id:salesOrderID ,columns: 'custbody227'});  
  ...
  if(!terms){
    if(layaway == 'F' && (rate_customdeposit_total < 0.5)){
  ...
  if((temp_ring_stack == 'F' && CheckCombination.RingStack ) ||( temp_ring_stack=='T' && !CheckCombination.RingStack ))
  {
    Highlight_status_change_flag = true;
  }
  ...
  if(Highlight_status_change_flag)
  {
    try
    {
      ...
      var get_old_Highlight_status=order_status;//SalesRecord.getValue("custbody227");
      ...
      {
        if(string_old_status !=string_new_status || !get_old_ring_stack)
        {
