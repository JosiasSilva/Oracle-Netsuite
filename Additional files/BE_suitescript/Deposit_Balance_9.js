	...
	if(type=="create" || type=="edit" || type=="copy" || type=="view")
	{
		...
		if(IsEmpty(customer) != "")
		{
			...
			try{
				...
				var cust_deposits = 0.00;
				var filters = [];
				filters.push(new nlobjSearchFilter("entity",null,"is",customer));
				filters.push(new nlobjSearchFilter("mainline",null,"is","T"));
				filters.push(new nlobjSearchFilter("status",null,"anyof",["CustDep:A","CustDep:B"]));
				var cols = [];
				cols.push(new nlobjSearchColumn("fxamount",null,"sum"));
				var results = nlapiSearchRecord("customerdeposit",null,filters,cols);
				if(results)
				{
					cust_deposits = results[0].getValue("fxamount",null,"sum");
					if(cust_deposits==null || cust_deposits=="")
						cust_deposits = 0.00;
				}
				
				if (cust_deposits == "" || cust_deposits == 0) {
					...
					var fieldStr = "<p style='background-color: lightpink; font-weight: bold;'>Deposit Balance: $" + addCommas(cust_deposits) + "</p>"
					nlapiSetFieldValue("custbody_deposit_balance", fieldStr)
				}
			} 
			catch (err) {
