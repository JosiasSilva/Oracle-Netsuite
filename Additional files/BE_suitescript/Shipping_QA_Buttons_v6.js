function Shipping_QA_Button_UE(type)
{
	if(type=="view")
	{
		try
		{
			...
			if((order.getFieldValue("custbody_shipping_qa_employee")==null || order.getFieldValue("custbody_shipping_qa_employee")=="") && (order.getFieldValue("orderstatus")=="B" || order.getFieldValue("orderstatus")=="D" || order.getFieldValue("orderstatus")=="E"))
			{
...
function qaorder(order,user)
{
	try
	{
		...
		var orderFields = nlapiLookupField("salesorder",order,["custbody140","custbody194","custbody_website_truebalance_amt"],true);
		var truebalance = orderFields.custbody_website_truebalance_amt;
		...
		if(custbody194!=null && custbody194!="" && custbody194.substring(0,3)!="DNU" && custbody194!="10")
			alertMsg+= "Order is marked with Delivery Instructions: " + custbody194;
		...
		if(results)
		{
			...
			{
				nlapiLogExecution("debug","FedEx Tracking # Found on Item Fulfillment",itemfulfillment.getFieldValue("custbody_fedex_ws_tracking_number"));
		...
		else
		{
			...
			if(results2)
			{
				...
				{
					nlapiLogExecution("debug","FedEx Tracking # Found on Item Fulfillment (Results2)",results2[0].getValue("custbody_fedex_ws_tracking_number"));
		...
		nlapiLogExecution("debug","Tracking #",tracking);
