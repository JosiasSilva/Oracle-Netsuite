...
var FEDEX_RETURN_TAG_PRODUCTION = "18934560";
var FEDEX_RETURN_TAG_SANDBOX = "17840010";
...
function Return_Label_Email_Queue(request,response)
{
  if(request.getMethod()=="GET")
  {
    try
    {
      ...
      list.addField("custpage_ship_from","textarea","Ship To");
      list.addField("custpage_bill_to","textarea","Bill To");			
      ...
      fld.setDisplayType("inline");
      list.addField("custpage_colour_flag","text","custpage_colour_flag").setDisplayType('hidden');
      ...
      filters.push(new nlobjSearchFilter("type",null,"anyof","SalesOrd").setLeftParens(2));
      filters.push(new nlobjSearchFilter("mainline","custbody_created_from","is","T").setRightParens(1).setOr(true));
      filters.push(new nlobjSearchFilter("type",null,"anyof","RtnAuth").setLeftParens(1));
      filters.push(new nlobjSearchFilter("mainline","createdfrom","is","T").setRightParens(2));
      ...
      cols.push(new nlobjSearchColumn("formulatext").setFormula("CASE WHEN {type}='Sales Order' THEN {custbody_created_from.shipaddress} ELSE {createdfrom.shipaddress} END"));

      cols.push(new nlobjSearchColumn("formulatext").setFormula("CASE WHEN {type}='Sales Order' THEN {custbody_created_from.billaddress} ELSE {createdfrom.billaddress} END"));

      var results = nlapiSearchRecord("transaction",null,filters,cols);
      if(results)
      {
        var result_col=results[0].getAllColumns();
        for(var x=0; x < results.length; x++)
        {
          var template = "";
          ...
          /*
					...
					*/
          var colour_flag='0';
          var ship_to=results[x].getValue(result_col[11]);
          if(ship_to)
          {
            ship_to=ship_to.toLowerCase();
            if(ship_to.search('united states')==-1)
            {
              colour_flag='1';
            }
          }
          orders.push({
            ...
            custpage_ship_from : results[x].getValue(result_col[11]),
            custpage_bill_to : results[x].getValue(result_col[12]),
            custpage_colour_flag:colour_flag
          });
        }
...
function Gen_Return_Label(internalid,recordType,transId,shipMethod,insuranceAmount)
{
  ...
  //var templateID = nlapiGetContext().getSetting("SCRIPT","custscript_queue_fedex_return_xml");
  ...
  var file = nlapiCreateFile(transId + " return_label.pdf","PDF",imageXml);
