//var templateID_SB = "17835989";
//var templateID_PROD = "24241134";

var templateID, templateFile, template, xmlTemp;
...
function Track_Package(packageId,statusOnly)
{
	...
	//Save XML Response to folder in sandbox
	if(nlapiGetContext().getEnvironment()=="SANDBOX")
	{
		var xmlContents = cResp.getBody();
		
		var responseFile = nlapiCreateFile(trackObj.trackingnumber + "_Response.txt","PLAINTEXT",xmlContents);
	...
	var trackDetailsNodes = nlapiSelectNodes(xmlBody,"//SOAP-ENV:Envelope/SOAP-ENV:Body/*[name()='TrackReply']/*[name()='CompletedTrackDetails']/*[name()='TrackDetails']");
	nlapiLogExecution("debug","# TrackDetails Nodes: " + trackDetailsNodes.length);
	
	var trackDetailsNode = trackDetailsNodes[0];
	
	//Check for errors and handle appropriately
	var errors = [];
	var errorNodes = nlapiSelectNodes(trackDetailsNode,".//*[name()='Notification']");
	for(var x=0; x < errorNodes.length; x++)
	{
		...
		if(code=="6035" && message=="Invalid tracking numbers.   Please check the following numbers and resubmit."/*&& lessThan24Hours==false*/)
		{
			nlapiSubmitField("customrecord_custom_package_record",packageId,"custrecord_valid_fedex_tracking_id","2");
			return true;
		}
		else if(code=="9040" && message=="This tracking number cannot be found. Please check the number or contact the sender."/*&& lessThan24Hours==false*/)
		{
			nlapiSubmitField("customrecord_custom_package_record",packageId,"custrecord_valid_fedex_tracking_id","2");
			return true;
		}
	...
	if(statusOnly==false)
	{
		...
		var statusDetailNode = nlapiSelectNode(trackDetailsNode,".//*[name()='StatusDetail']");
		...
		var locationNode = nlapiSelectNode(trackDetailsNode,".//*[name()='StatusDetail']/*[name()='Location']");
		...
		var ancillaryDetailsNode = nlapiSelectNode(trackDetailsNode,".//*[name()='StatusDetail']/*[name()='AncillaryDetails']");
		...
		var weightNode = nlapiSelectNode(trackDetailsNode,".//*[name()='PackageWeight']");
		...
		var serviceNode = nlapiSelectNode(trackDetailsNode,".//*[name()='Service']");
		...
		var shipperNode = nlapiSelectNode(trackDetailsNode,".//*[name()='ShipperAddress']");
		...
		var destinationNode = nlapiSelectNode(trackDetailsNode,".//*[name()='DestinationAddress']");
		...
		var idNodes = nlapiSelectNodes(trackDetailsNode,".//*[name()='OtherIdentifiers']/*[name()='PackageIdentifier']");
		...
		var specialHandlingsNodes = nlapiSelectNodes(trackDetailsNode,".//*[name()='SpecialHandlings']");
		...
		var eventsNodes = nlapiSelectNodes(trackDetailsNode,".//*[name()='Events']");
		...
		var dateTimeNodes = nlapiSelectNodes(trackDetailsNode,".//*[name()='DatesOrTimes']");
		if(dateTimeNodes!=null)
		{
			nlapiLogExecution("debug","dateTimeNodes.length: " + dateTimeNodes.length);
			
			for(var x=0; x < dateTimeNodes.length; x++)
			{
				var dateTimeType = nlapiSelectValue(dateTimeNodes[x],".//*[name()='Type']");
				var dateTimeValue = nlapiSelectValue(dateTimeNodes[x],".//*[name()='DateOrTimestamp']");
				
				if(dateTimeType=="ESTIMATED_DELIVERY" || dateTimeType=="ACTUAL_DELIVERY")
				{
					nlapiLogExecution("debug","ESTIMATED_DELIVERY",timestampToDate(dateTimeValue));
					
					if(pckg.getFieldValue("custrecord_standard_transit_dd")==null || pckg.getFieldValue("custrecord_standard_transit_dd")=="")
					{
						pckg.setFieldValue("custrecord_standard_transit_dd",timestampToDate(dateTimeValue));
					}
					
					pckg.setFieldValue("custrecord_scheduled_delivery_cpr",timestampToDate(dateTimeValue));
				}
				
				if(dateTimeType=="ACTUAL_DELIVERY")
				{
					pckg.setFieldValue("custrecord_package_latest_activity","15");
				}
			}
		...
		nlapiLogExecution("debug","Finished dateOrTime Nodes");
	...
	else
	{
		...
		if(statusDetailNode!=null)
		{
			...
			if(cpr==null)
				cpr = nlapiLoadRecord("customrecord_custom_package_record",packageId);
		...
		var dateTimeNotes = nlapiSelectNodes(trackDetailsNode,".//*[name()='DatesOrTimes']");
		if(dateTimeNotes!=null)
		{
			for(var x=0; x < dateTimeNodes.length; x++)
			{
				var dateTimeType = nlapiSelectValue(dateTimeNotes[x],".//*[name()='Type']");
				var dateTimeValue = nlapiSelectValue(dateTimeNotes[x],".//*[name()='DateOrTimestamp']");
				...
				if(dateTimeType=="ESTIMATED_DELIVERY" || dateTimeType=="ACTUAL_DELIVERY")
				{
					...
					if(cpr.getFieldValue("custrecord_standard_transit_dd")==null || cpr.getFieldValue("custrecord_standard_transit_dd")=="")
					{
						cpr.setFieldValue("custrecord_standard_transit_dd",timestampToDate(dateTimeValue));
					}
					
					cpr.setFieldValue("custrecord_scheduled_delivery_cpr",timestampToDate(dateTimeValue));
				}
				
				if(dateTimeType=="ACTUAL_DELIVERY")
				{
					cpr.setFieldValue("custrecord_package_latest_activity","15");
				}
			}
		}
...
function Update_Packages()
{
	...
	filters.push(new nlobjSearchFilter("custrecord_undelivered",null,"is","F"));
	filters.push(new nlobjSearchFilter("custrecordreturned_to_shipper_tracking",null,"isempty"));
