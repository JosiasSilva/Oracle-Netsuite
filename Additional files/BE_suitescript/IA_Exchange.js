function IA_Exchange(order,xml,pages,today,customer_name,currency,email,iaTemp,isIABlock,isIABlockCode,iaBlockMsg,countgemStoneArr,chkCategory)
{
  var diamondID = "";
  var sapphireID = "";
  var image = "";
  ...
  for(var i=0; i < order.getLineItemCount("item"); i++)
  {
    ...
    if(sub_category=="7")
    {
      ...
      nlapiLogExecution("debug","test 888");
  ...
  for(var x=0; x < order.getLineItemCount("item"); x++)
  {
    ...
    if(category=="2")
    {
      ...
      nlapiLogExecution("debug","count gem Stone Arr",countgemStoneArr.length);
  ...
  if(isIABlockCode=='1' || isIABlockCode=='2' || isIABlockCode=='3')
    ...
    else
    {
      if(category=='2' || category=='3')
      {
        ...
        response.setContentType("PDF","Insurance_Appraisal.pdf","inline");
        response.write(file.getValue());
          nlapiLogExecution("DEBUG","INFORMATION","IA EXCHANGE LIB FILE (TEST 1)");
      }
      else
      {
        if(chkCategory==true && (countgemStoneArr.length==0 || countgemStoneArr.length==1))
        {
          ...
          response.setContentType("PDF","Insurance_Appraisal.pdf","inline");
          response.write(file.getValue());
            nlapiLogExecution("DEBUG","INFORMATION","IA EXCHANGE LIB FILE(TEST 2)");
        }
        else
        {
          var errorMsg = "";
          ...
          response.setContentType("PDF","Insurance_Appraisal.pdf","inline");
          response.write(file.getValue());
           nlapiLogExecution("DEBUG","INFORMATION","IA EXCHANGE LIB FILE (TEST 3)");
        }
      }
    }
  return true;
