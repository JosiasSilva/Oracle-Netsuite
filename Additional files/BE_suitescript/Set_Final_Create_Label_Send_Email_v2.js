function Set_Final_Create_Label_Send_Email(form)
{
  try
  {
    ...
    soRec.setFieldValue("custbody87",17);// Type Of Order = Set [added by feedback document of 20 jun 2017]
    ...
    for(var x=0; x < request.getLineItemCount('custpage_new_item_order'); x++)
    {
      ...
      soRec.setCurrentLineItemValue("item","item",request.getLineItemValue("custpage_new_item_order","custpage_new_item_item",x+1));
      ...
      soRec.setCurrentLineItemValue("item","rate",request.getLineItemValue("custpage_new_item_order","custpage_new_item_rate",x+1));//Added by Ravi
      soRec.setCurrentLineItemValue("item","amount",request.getLineItemValue("custpage_new_item_order","amount",x+1));
      // soRec.setCurrentLineItemValue("item","custcol_full_insurance_value_2",request.getLineItemValue("custpage_new_item_order","custpage_new_items_production_insurance_value",x+1));
      soRec.setCurrentLineItemValue("item","custcol_full_insurance_value",request.getLineItemValue("custpage_new_item_order","custpage_new_items_production_insurance_value",x+1));
      soRec.setCurrentLineItemValue("item","custcol_center_gem_item",request.getLineItemValue("custpage_new_item_order","custpage_center_stone_sku",x+1));
      center_stone_sku =request.getLineItemValue("custpage_new_item_order","custpage_center_stone_sku",x+1);
    ...
    var  setOrderID = nlapiSubmitRecord(soRec,true,true);

    var shipService = setShipService(setOrderID);
    form.addField("custpage_tranid","text","Transaction #").setDisplayType("hidden").setDefaultValue(shipService[1]);
    form.addField("custpage_service_type","text","Ship Service").setDisplayType('hidden').setDefaultValue(shipService[0]);
    ...
    var type= "set";
    var retlblStatus = createReturnLabelPdf(shipService[1],shipService[0],type,setOrderID);    
    nlapiLogExecution("Debug","Tracking No",retlblStatus[0]);
    nlapiLogExecution("Debug","File Id",retlblStatus[1]);
    nlapiSubmitField('salesorder',setOrderID,['custbody123','custbody137'],[retlblStatus[0],retlblStatus[1]]);
    //

    var redirectUrl =  nlapiResolveURL("RECORD" , "salesorder" , setOrderID , "view" ) ;
    ...
    sendEmailWithAttachment(retlblStatus[1],CC,setOrderID,toEmail,emailType,copyEmailTemplateInternalId);


  }
  catch(err)
  {
    nlapiLogExecution("error","Set Item(s) else block ","Details: " + err.message);
    return true;
  }
}