function IA_Print_Queue(request,response)
{
	...
	else
	{
		...
		//Added by Shiv
		var hmSearchList=[];
			hmSearchList=haloMeleeSearchResult();
		//End
...
var flag_value=false;
			for(var x=0; x < results.length; x++)
			{
				...
				{
 flag_value=true;
nlapiLogExecution("debug","category 3");
				...
				{
 flag_value=true;
nlapiLogExecution("debug","category 24");
					//Estate Rings
...
nlapiLogExecution("debug","hmSearchList Count:"+hmSearchList.length);
...
function haloMeleeSearchResult()
{
		var grpItemArr=[];
		var filters = [];    	
		...
		if(searchResult.length>0)
		{
			...
			//nlapiLogExecution("debug","Array List:",JSON.stringify(haloMeleeArr));
			
		}
               nlapiLogExecution("debug","1111");
		return haloMeleeArr;
}
/* --Old colde
