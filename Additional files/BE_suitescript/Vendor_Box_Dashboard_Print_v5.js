...
function Vendor_Box_Dashboard_Print(request,response)
{
    ...
    //nlapiLogExecution("DEBUG","Results",JSON.parse(results));
	if(results)
	{
	...
	for(var x=0; x < vendors.length; x++)
	{
		...
		if(results)
		{
			var totalBox = Box_Counter(results); // Added: ZAG : As per NS 1485
			var boxInsurance = 0.00;
			var boxCounter = 1;
			
			//xml += "<p text-align='center'>Sending from SF " + nlapiEscapeXML(results[0].getText("mainname")) + " - Box # " + boxCounter + "</p>";
			xml += "<p text-align='center'>Sending from SF " + nlapiEscapeXML(results[0].getText("mainname")) + " - Box # " + boxCounter + " of " + totalBox + "</p>"; // Added: ZAG : As per NS 1485
			...
			for(var i=0; i < results.length; i++)
			{
				...
				var tempBoxInsurance = boxInsurance + itemInsurance;
				//boxInsurance += parseFloat(itemInsurance);
				
				if(tempBoxInsurance >= 45000.00 && i!=0)
				{
					xml += "</table>";
...
// Start: ZAG : As per NS 1485
function Box_Counter(resultset){
	var boxInsurance = 0.00;
	var Counter = 1;
	for(var i=0; i < resultset.length; i++){
		var itemInsurance = resultset[i].getValue("custcol_full_insurance_value");
		...
		if(tempBoxInsurance >= 45000.00 && i!=0){
			Counter++;
			boxInsurance = 0.00;
			boxInsurance += parseFloat(resultset[i].getValue("custcol_full_insurance_value"));
		}else{
			boxInsurance += parseFloat(itemInsurance);
		}
	}
