function Exchange_Page(request,response)
{
	if(request.getMethod()=="GET")
	{
		try
		{
			...
			/*************** Start Added By Yagya Kumar for NS-1033 on 27 Dec 2017 **************************/
			//fld = form.addField("custpage_return_shipping_label","select","Return Shipping Label","customlist126","delivery").setMandatory(true);
			var fil_arr = ['2','3','5','6','7'];
			fld = form.addField("custpage_return_shipping_label","select","Return Shipping Label",null,"delivery").setMandatory(true);
			fld.addSelectOption('','');
			var result  = nlapiLoadRecord('customlist',126);
			for(var z = 1; z <= result.getLineItemCount('customvalue'); z++)
			{
				var id = result.getLineItemValue('customvalue','valueid',z);
				if( fil_arr.indexOf(id) == -1)
				{
					fld.addSelectOption(id,result.getLineItemValue('customvalue','value',z));
				}
			}
			/*************** End Added By Yagya Kumar for NS-1033 on 27 Dec 2017 **************************/			
			// Below code has commented by Rahul Panchal as per NS-820 on dated 03 August 2017
	...
	else
	{
		try
		{
			...
			nlapiLogExecution("debug","Items Returned",JSON.stringify(items_returned));
		...
		try
		{
			...
			if(rma==null || rma=="")
			{
				...
				ra.setFieldValue("custbody_drop_off",dropOff);// added on 25 march 2017 [NS-644]
				...
				for(var x=0; x < ra.getLineItemCount("item"); x++)
				{
					...
					for(var z=0; z < items_returned.length; z++)
					{
						...
						{
							nlapiLogExecution("debug","Item Returned Matches RMA Line Item");
							...
							{
								nlapiLogExecution("debug","New Delta MGR Entry","Category: " + itemCat + "   |   $ Returned: " + parseFloat(ra.getLineItemValue("item","amount",x+1)));
			...
			else
			{
				...
				var rmaRec = nlapiLoadRecord("returnauthorization",rmaID);
				var currentNum = rmaRec.getFieldValue("tranid");
				var newNum = currentNum.replace("RT","EX");
				rmaRec.setFieldValue("tranid",newNum);
				rmaRec.setFieldValue("custbody35","6"); //Type of Sendback = Exchange
				
				var keepMelee = false;
				for(var x=0; x < rmaRec.getLineItemCount("item"); x++)
				{
					var found = false;
					for(var z=0; z < items_returned.length; z++)
					{
						if(rmaRec.getLineItemValue("item","item",x+1)==items_returned[z])
						{
							nlapiLogExecution("debug","Item Returned Matches RMA Line Item");
							
							var itemCat = nlapiLookupField("item",rmaRec.getLineItemValue("item","item",x+1),"custitem20");
							if(itemCat=="2")
								keepMelee = true;
							...
							for(var t=0; t < delta_mgr.length; t++)
							{
								if(delta_mgr[t].category == itemCat)
								{
									delta_mgr[t].returned += parseFloat(rmaRec.getLineItemValue("item","amount",x+1));
									delta_found = true;
									break;
								}
							}
							...
							if(delta_found==false)
							{
								nlapiLogExecution("debug","New Delta MGR Entry","Category: " + itemCat + "   |   $ Returned: " + parseFloat(rmaRec.getLineItemValue("item","amount",x+1)));
								
								delta_mgr.push({
									category : itemCat,
									returned : parseFloat(rmaRec.getLineItemValue("item","amount",x+1)),
									ordered : 0
								});
							}
					...
					if(!found)
					{
						...
						{
							items_not_returned.push({
								id: rmaRec.getLineItemValue("item","item",x+1),
								desc: rmaRec.getLineItemValue("item","description",x+1),
								val: rmaRec.getLineItemValue("item","amount",x+1)
							});	
						}
					}
				}
				...
				rmaID = nlapiSubmitRecord(rmaRec,true,true);
			}
		}
		catch(err)
		{
		...
		try
		{
			...
			newOrder.setFieldValue("custbody_created_via_exchange_page","T");
			newOrder.setFieldValue("custbody_linked_rma",rmaID);
			...
			//modification start by sanjeet for NS-1033
          ...
          //modification end by sanjeet for NS-1033
			...
			for(var x=0; x < new_items.length; x++)
			{
				...
				nlapiLogExecution("debug","New Order Currency",newOrder.getFieldValue("currency"));
				nlapiLogExecution("debug","New Order Ship Country",newOrder.getFieldValue("shipcountry"));
				
				if(newOrder.getFieldValue("currency")=="2" && newOrder.getFieldValue("shipcountry")=="GB")
				{
					nlapiLogExecution("debug","Setting Line Item Price to " + new_items[x].rate);
					newOrder.setCurrentLineItemValue("item","custcol_item_price",new_items[x].rate);
				}
