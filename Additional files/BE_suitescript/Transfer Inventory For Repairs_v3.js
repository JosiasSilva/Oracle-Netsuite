...
function transferInventoryForRepairs(type) {
    ...
    try {
        ...
        var vendorNameArray = ["Unique New York", "GM Casting House Inc", "Miracleworks", "Trung Do's JD & Co.", "Jewel-Craft Inc", "Harout R", "Sasha Primak", "Benchmark", "Overnight", "Choice Jewelers", "Endless Designs", "Guild and Facet","Forgeous Jewelers"];
        var vendorLocationArray = ["BE Fulfillment-NY", "BE Fulfillment-CH", "BE Fulfillment-MW", "BE Fulfillment-TD", "BE Fulfillment-JC", "BE Fulfillment-HR", "BE Fulfillment-SP", "BE Fulfillment-BM", "BE Fulfillment-OV", "BE Fulfillment-CJ", "BE Fulfillment-ED", "BE Fulfillment-GF","BE Fulfillment-FOR"];
        var transferItemLocationArray = [6, 1, 9, 8, 12, 11, 16, 17, 22, 21, 20, 19, 24];

        /* ----------------- Ended by YAGYA 1st July 2016 --------------- */

        var searchResult = new Array();
        // Loading the Given saved search Estate Ring Repair - Sent to TD  
        //var searchObj = nlapiLoadSearch(null, 'customsearch2115'); //old saved search on live
		var searchObj = nlapiLoadSearch(null, 'customsearch5012'); //new saved search on live
        ...
        if (searchResult != null && searchResult != '') {
            ...
            for (var polen = 0; polen < po_without_duplicate.length; polen++) {
                ...
                try {
                   ...
                   //Added by vinay for task NS-897
                    recordObj.setFieldValue('custbody_transaction_link_inv', po_without_duplicate[polen]);
                     /*--------vinay End-------*/
                    ...
                    if (vendorPos == -1) {
                        //recordObj.setFieldText('transferlocation', tolocation);
						recordObj.setFieldText('transferlocation', "Check Out");
                    }
                    else {
                    ...
                    for (var line = 1; line <= itemscount; line++) {
						...
						{
							GemSentToVendorDate.push(new Date(date_sent_from_sf));
						}
					...
					//Added new code 04oct2016
					recordObj.setFieldText('location', item_link_loc); 
					nlapiLogExecution('debug',"Item Link Location for item : "+line, item_link_loc );
					//Ended
					...
					/*------------------Add By YAGYA 29June 2016-----------------*/
					var LatestDate = nlapiDateToString (new Date(Math.max.apply(null,GemSentToVendorDate)));
                    if (LatestDate != '' && LatestDate != null)
					{
                        nlapiSubmitField("purchaseorder", po_without_duplicate[polen], "custbody321", LatestDate);
                        nlapiLogExecution("debug", "Submit purchase order Id :" + po_without_duplicate[polen],LatestDate);						
					}
                    /*------------------End Add By YAGYA-----------------*/
