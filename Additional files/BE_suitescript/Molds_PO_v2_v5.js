    ...
    try
    {
      ...
      for(var x=0; x <po.getLineItemCount("item"); x++)
      {
        if(po.getLineItemValue("item","custcol13",x+1)=="T" && (po.getLineItemValue("item","custcol17",x+1)!=null && nlapiGetLineItemValue("item","custcol17",x+1)!=null) && (nlapiGetLineItemValue("item","custcol_mold_new",x+1)==null || nlapiGetLineItemValue("item","custcol_mold_new",x+1)==null) && (po.getLineItemValue("item","custcolitem_link",x+1)==null || po.getLineItemValue("item","custcolitem_link",x+1) ==''))
        {
          ...
          var trim_str = removeSpecialChars(mold_name); // Added by Bharath (03/22/2019)
          //nlapiLogExecution('debug',"Mold Names",trim_str);
          var trim_str_arr = trim_str.split(",");
          trim_str_arr.pop();
          //nlapiLogExecution('debug',"Trim String array",trim_str_arr);
          //nlapiLogExecution('debug',"Trim String array length",trim_str_arr.length);
          ...
          for(var j=0;j<trim_str_arr.length;j++){
          var filters = [];
          nlapiLogExecution('debug',"Trim String array value",trim_str_arr[j]);
          filters.push(new nlobjSearchFilter("name",null,"is",trim_str_arr[j]));
          ...
          else
          {
			...
			//var trimmed_str = removeSpecialChars(trim_str_arr)  // Added by Bharath (03/20/2019)
            moldRecord.setFieldValue("name",trim_str_arr[j]); // Added by Bharath (03/20/2019)
        ...
        else if(po.getLineItemValue("item","custcol13",x+1)=="T" && (po.getLineItemValue("item","custcol17",x+1)!=null && nlapiGetLineItemValue("item","custcol17",x+1)!=null) && (nlapiGetLineItemValue("item","custcol_mold_new",x+1)==null || nlapiGetLineItemValue("item","custcol_mold_new",x+1)==null) && (po.getLineItemValue("item","custcolitem_link",x+1)!=null && po.getLineItemValue("item","custcolitem_link",x+1)!=''))
        {
          ...
          // END HERE
          var mold_name = po.getLineItemValue("item","custcol17",x+1);
          nlapiLogExecution("DEBUG","MOLD NAME (MOLD)",mold_name);// Added by Bharath (03/22/2019)
          var trim_str_item = removeSpecialChars(mold_name);
          var trim_str_il_arr = trim_str_item.split(",");
          trim_str_il_arr.pop();
          var item = po.getLineItemValue("item","item",x+1);
          ...
          for(var j = 0 ; j<trim_str_il_arr.length;j++){
          var filters = [];
          filters.push(new nlobjSearchFilter("name",null,"is",trim_str_il_arr[j]));
          filters.push(new nlobjSearchFilter("custrecord_mold_vendor",null,"is",vendor));
          filters.push(new nlobjSearchFilter("custrecord_mold_item",null,"is",parent));
          var results = nlapiSearchRecord("customrecord_mold",null,filters);
          nlapiLogExecution('debug',"Results",JSON.stringify(results));
          ...
          else
          {
            ...
            moldRecord.setFieldValue("name",trim_str_il_arr[j]); // Added by Bharath (03/20/2019)
