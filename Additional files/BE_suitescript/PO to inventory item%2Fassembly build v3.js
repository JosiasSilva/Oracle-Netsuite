/*=====This script uses saved search(customsearch_po_modified_within24hours_2 sandbox) retrieves all PO modified within 24 hours and snyc that PO fields
...
function UpdatePOtoInvAssmItems() {
    try {
        var mySearch = nlapiLoadSearch(null, '9801'); // For Sandbox #customsearch_po_modified_within24hours_2
        var searchresult = [];
        ...
        if (searchCount > 0) {
            for (var j = 0; j < searchresult.length; j++) {
                ...
                var Center_Stone_min_size_mm = searchresult[j].getValue('custbodycenter_stone_min_size_mm', 'custitemcustitem_pd_po_link');
                var gender = searchresult[j].getValue('custbodycustbody_gender', 'custitemcustitem_pd_po_link');
                var typeOfSetting = searchresult[j].getValue('custbodycenter_setting_style', 'custitemcustitem_pd_po_link');
                ...
                try {
                    ...
                    activeStatus = nlapiLookupField('inventoryitem', orderId, 'custitem160');
                } catch (ex) {
                    inventoryItem = nlapiLoadRecord('assemblyitem', orderId);
                    activeStatus = nlapiLookupField('assemblyitem', orderId, 'custitem160');
                }

                if (inventoryItem && (!activeStatus || activeStatus == '-1')) {
                    ...
                    if (Category == 2 || Category == 3) {
                        //(halo_style,finish,Accent_Setting_Style) has been commented here  as per NS-1181 because according to Rachel these fields are not syncing and it should sync in all cases.
                    ...
                    inventoryItem.setFieldValue('custitemcenterstoneminsize', Center_Stone_min_size_mm);
                    inventoryItem.setFieldValue('custitem3', gender);
                    inventoryItem.setFieldValue('custitem6', typeOfSetting);
