function Randomize_POs_for_Testing()
{	
	var pos = [];
	
	//Get list of Purchase Orders from saved search
	var search = nlapiLoadSearch("purchaseorder", "customsearch_random_po_testing");
	...
	for(var x=0; x < posToUpdate.length; x++)
	{
		try
		{
			...
			nlapiSubmitField("purchaseorder",posToUpdate[x],"custbody_random_testing","T");
		}
		catch(err)
		{
			nlapiLogExecution("error","Error Marking PO " + posToUpdate[x] + " for Testing","Details: " + err.message);
		}
	}
}
