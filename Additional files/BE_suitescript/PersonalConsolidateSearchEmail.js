...
function consolidatedSearchPersonalEmail()
{
	...
	{		
	   ...
	   ArrSearchIdNames.push('2825#1# Setster Appts Not Confirmed');
	   ArrSearchIdNames.push('1500#5# Open (CS) Ops/CS Status');
	   ArrSearchIdNames.push('1329#5# All Appointments - no items');
	   ArrSearchIdNames.push('2113#4# Customer Follow Up'); //created error to get EtityId of Employee directly So we get via Customer
	   ...
	   ArrSearchIdNames.push('1531#24# Diamond Unavailable-Options pending customer decision');
			   ...
			   do{ 
				   ...
				   { 
					  ...
					  if(searchId=='2113')
					  {
                        ...
                        salesRepId=nlapiLookupField('customer',custId,'salesrep');// Get salesRepId w.r.t Customer Id for searchId-2113;
				  ...
				  for(var j=0;j<salesRepArrayResults.length;j++) //j<salesRepArrayResults.length
				  {
					...
					if((empId!='' && empId!=null) && empId!='Unassigned' && empId!='None')
					{
						if(empId!='7820') 
						{
							...
							{
								searchResultArr.push({empId: empId, searchId:searchId, searchName: searchName,empLeads:empLeads});		
							}		
                        }						
				...
				if(resultArr.length>0)
				{
					...
					{ 
						...
						{	
						  ...
						  var url="https://system.netsuite.com/app/common/search/searchresults.nl?searchid="+eSearchId;
						...
						if(eCount>=1)   //  Note:- Allowed to receive email for All Sales Rep who having at least one search result && eId=='3365149'
						{
							...
							{				
								 ...
								 // var emailTo='sprastogi@inoday.com'; 
