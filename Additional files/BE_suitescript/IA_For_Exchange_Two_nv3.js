function IA_For_Exchange_Two(order,xml,pages,today,customer_name,currency,email,iaTemp,countgemStoneArr,chkCategory,isIABlockCode,iaBlockMsg,isIABlock)
{
  var diamondID = "";
  var sapphireID = "";
  var image = "";
  ...
  nlapiLogExecution("DEBUG","Check Gem Stone in Sales Order having Order Type Exchange",chkGemStone);
  if(chkGemStone == false) return false;
  var orderId = nlapiLookupField("salesorder",soId,"custbody_created_from");
  nlapiLogExecution("DEBUG","Created From Field SO Id Exchange Two Lib File",orderId);
  ...
  for(var x=0; x < order.getLineItemCount("item"); x++)
  {
    ...
    nlapiLogExecution("debug","BLOCK INSURANCE APPRAISAL Exchange Two",isIABlock);
    ...
    if(category=="2")
    {
      ...
      nlapiLogExecution("DEBUG","Count gem Stone length IA_For_Exchange_Two()",countgemStoneArr.length);
  ...
  nlapiLogExecution("Debug","INSURANCE APPRAISAL Message Exchange Two",iaBlockMsg);
  nlapiLogExecution("Debug","INSURANCE APPRAISAL CODE Exchange Two",isIABlockCode);
  ...
  nlapiLogExecution("DEBUG","Gem Stome Length (2 or more than 2) Exchange Two",countgemStoneArr.length);
  if(chkCategory==true && (countgemStoneArr.length>=2))
  {
    var errorMsg = "";
    errorMsg += "<p>Instruction!<br/>IA is not allowed for following conditions:";      
    errorMsg += "<br/>There are more than one center gemstone on the original order"; 
    errorMsg += "</p>";
    nlapiLogExecution("error","IA Instruction Error",errorMsg);
    xml += errorMsg;
    xml += "</body></pdf>";
  ...
  {
    xml += "</body></pdf>";
    nlapiLogExecution("DEBUG","XMl for Exchange Two",xml);
    var file = nlapiXMLToPDF(xml);
    response.setContentType("PDF","Insurance_Appraisal.pdf","inline");
    response.write(file.getValue());
    nlapiLogExecution("DEBUG","IA Exchange For Two(Test 2)","Center gem stone having ring in sales order associated with created from field");	
    return true;
  }
  else
    return false;
