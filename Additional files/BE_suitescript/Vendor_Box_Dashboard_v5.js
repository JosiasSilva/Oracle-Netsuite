function Vendor_Box_Dashboard(request,response)
{
	if(request.getMethod()=="GET")
	{
		...
		cols.push(new nlobjSearchColumn("custbody244"));
		...
		if(results)
		{
			for(var x=0; x < results.length; x++)
			{
				...
				lines.push({
					...
					custpage_int_notes : results[x].getValue("custcol_vbd_int_notes"),
		...
		list.addField("custpage_int_notes","textarea","Internal Notes").setDisplayType("entry");
				...
				if(request.getLineItemValue("custpage_list","custpage_changed",x+1)=="T")
				{
					...
					for(var i=0; i < po.getLineItemCount("item"); i++)
					{
						if(po.getLineItemValue("item","line",i+1)==request.getLineItemValue("custpage_list","custpage_po_line",x+1))
						{
							...
							po.setLineItemValue("item","custcol_vbd_int_notes",i+1,request.getLineItemValue("custpage_list","custpage_int_notes",x+1));
							break;
						}
					}
