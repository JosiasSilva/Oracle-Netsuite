        ...
        if(search_record)
        {
          ...
          for(var a=0;a<purchase_ord.length;a++)
          {
            ...
            for(var b=0;b<get_line.length;b++)
            {
              ...
              if((get_line[b].item=='1087131' ||  get_line[b].item=='1093360') && (get_record_type=='purchaseorder' && ( type=='specialorder' || type=='dropship')))
              {
                var search_record=nlapiSearchRecord('item',null,
                                                    new nlobjSearchFilter('internalid',null,'anyof',get_line[b].item),new nlobjSearchColumn('purchasedescription'));
                var desc='';
                if(search_record)
                {
                  desc=search_record[0].getValue('purchasedescription');
                }
                po_load.setLineItemValue('item','description',find_index,desc );
              }
              else
              {
