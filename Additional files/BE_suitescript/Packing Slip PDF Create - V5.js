function PDF_file_Create(get_line_item,vendor_text,box_rec_Id,total_insu,packing_slip_id,po_push,type_call,box_count,box_rec_number)
{
  if(packing_slip_id)
  {
  ...
  if(get_obj)
  {
    ...
    xml += Get_Fotter(vendor_text,box_rec_Id,get_obj_box,box_count,box_rec_number).replaceAll("&","");
    xml += '</macro>';
    xml += '</macrolist>';   
    xml += '</head>';
...
function Get_Fotter(vendor_text,box_rec_Id,get_obj_box,box_count,box_rec_number)
{
  var xml = '<table width="100%" border="0" cellpadding="1" cellspacing="1" style="border-top:0px #959799 ;padding: 0.1in;">';
  xml += '<tr>';
  xml += '<td>'+vendor_text+' | Box # '+box_rec_Id+'<br/>';
  // Added by Bharath - BE -30 (1/21/2019)
   xml += 'Box # '+ box_rec_number+' of '+box_count;
