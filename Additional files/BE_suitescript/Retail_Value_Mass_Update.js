...
function Retail_Value_UE(rec_type,rec_id)
{
	try
	{
		...
		{
			if(order.getLineItemValue("item","createpo",x+1)!=null && order.getLineItemValue("item","createpo",x+1)!="")
			{
				nlapiLogExecution("debug","Purchase Order",order.getLineItemValue("item","createpo",x+1));
				po_records.push(order.getLineItemValue("item","createpo",x+1));
			}
		}
		
		if(po_records.length > 0)
			po_records = po_records.getUnique();
			
		po_count = po_records.length;
		*/
			...
			if(po_count <= 1)
			{
				...
				for(var x=0; x < order.getLineItemCount("item"); x++)
				{
					if(order.getLineItemValue("item","itemtype",x+1)=="InvtPart")
					{
						...
						if(category==TYPE_RING || category==TYPE_PENDANT || category==TYPE_EARRINGS || category==TYPE_FINISHED || category==TYPE_PEARLS)
						{
						...
						{
							order.setLineItemValue("item","custcol_full_insurance_value",x+1,(order.getLineItemValue("item","amount",x+1)));
							totalAmount += parseFloat(order.getLineItemValue("item","amount",x+1));
						}
					}
				}
			...
			else
			{
				...
				for(var x=0; x < order.getLineItemCount("item"); x++)
				{
					if(order.getLineItemValue("item","itemtype",x+1)=="InvtPart")
					{
						var item = order.getLineItemValue("item","item",x+1);
						var category = nlapiLookupField("item",item,"custitem20");
						nlapiLogExecution("debug","Item Substring",order.getLineItemText("item","item",x+1).substring(0,3));
						...
						{
							nlapiLogExecution("debug","BE1 Item");
							var subAmount = 0.00;
							...
							{
								order.setLineItemValue("item","custcol_full_insurance_value",x+1,subAmount + parseFloat((order.getLineItemValue("item","amount",x+1))));
								totalAmount += parseFloat(order.getLineItemValue("item","amount",x+1));
							}
						}
		...
		else if(order_type==WEDDING_BAND || order_type==ENGRAVING)
		{
			...
			if(results)
			{
				...
				for(var x=0; x < originalOrder.getLineItemCount("item"); x++)
				{
					...
					{
						var item = originalOrder.getLineItemValue("item","item",x+1);
						var category = nlapiLookupField("item",item,"custitem20");
						if(category==TYPE_RING || category==TYPE_PENDANT || category==TYPE_EARRINGS || category==TYPE_FINISHED || category==TYPE_PEARLS)
						{
						...
						{
							originalOrder.setLineItemValue("item","custcol_full_insurance_value",x+1,(originalOrder.getLineItemValue("item","amount",x+1)));
							totalAmount += parseFloat(originalOrder.getLineItemValue("item","amount",x+1));
						}
					}
				}
				order.setFieldValue("custbody_full_insurance_amount",totalAmount);
