		...
		if(IsEmpty(customer) != "")
		{
			...
			try{
				...
				if(IsEmpty(customer_fields["balance"]) == "" || customer_fields["balance"] == 0){
					...
					var fieldStr = "<p style='background-color: lawngreen; font-weight: bold;'>Balance: $" + addCommas(customer_fields["balance"]) + "</p>"
				}
				...
				if(IsEmpty(soBalance) == "" || soBalance == 0){
					...
					//Set field to customer balance if > $0.00 and highlight in yellow and bold text
					fieldStr+= "<p style='background-color: yellow; font-weight: bold;'>New Balance: $" + addCommas(soBalance) + "</p>"
				...
				if(IsEmpty(customer_fields["consolbalance"]) == "" || customer_fields["consolbalance"] == 0){
					...
					//Set field to customer balance if > $0.00 and highlight in yellow and bold text
					var fieldStr = "<p style='background-color: lawngreen; font-weight: bold;'>Consolidated Balance: $" + addCommas(customer_fields["consolbalance"]) + "</p>"
					nlapiSetFieldValue("custbody_customer_consolbalance", fieldStr)
				}
