	...
	if(name=="custrecord_box_location")
	{
		try
		{
			...
			if(to_location_txt.substring(0,2)!="BE")
			{
				...
				cols.push(new nlobjSearchColumn("country","address"));
				...
				if(results)
				{
					nlapiLogExecution("debug","Address : Country",results[0].getValue("country","address"));
					nlapiLogExecution("debug","Address : Country Code",results[0].getValue("countrycode","address"));
					nlapiLogExecution("debug","Country",results[0].getValue("country"));
					...
					nlapiSetFieldValue("custrecord_box_state",stateAbbrev(results[0].getValue("state","address")),true,true);
					nlapiLogExecution("debug","State populated");
					nlapiSetFieldValue("custrecord_box_zip",results[0].getValue("zip","address"),true,true);
					nlapiLogExecution("debug","Zip populated");
					
					nlapiSetFieldText("custrecord_box_country",results[0].getValue("country","address"),true,true);
					nlapiLogExecution("debug","Country populated");
					
					nlapiSetFieldValue("custrecord_box_phone",results[0].getValue("phone","address"),true,true);
					nlapiLogExecution("debug","Phone populated");
					
					nlapiSetFieldValue("custrecord_box_addressee",results[0].getValue("addressee","address"),true,true);
					nlapiLogExecution("debug","Addressee populated");	
				}
			}
		}
		catch(err)
