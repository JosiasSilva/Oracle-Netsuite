			...
			for (var i = 1; i <= item_count; i++) {
				...
				if ((category == 15 || category == 18 || category == 20 || category == 31 || category == 14 || category == 8 ||
					...
					if (search) {
						...
						var itm_pref_loc = nlapiLookupField('inventoryitem', itemid, 'preferredlocation');
						var itm_record = nlapiLoadRecord('inventoryitem', itemid);
						var loc_count = itm_record.getLineItemCount('locations');

						for (var i = 1; i <= loc_count; i++) {
							var locid = itm_record.getLineItemValue('locations', 'location', i);

							if (locid == itm_pref_loc) {
								var inhand = itm_record.getLineItemValue('locations', 'quantityonhand', i);
								var qtycommited = itm_record.getLineItemValue('locations', 'quantitycommitted', i);
								nlapiLogExecution('debug', 'inhand=' + inhand);
								nlapiLogExecution('debug', 'qtycommited=' + qtycommited);
								if (inhand >= qtycommited) {
									gemResPagecreation.setFieldValue('custrecord_grp_cs_status', 24); //sales Rep
									nlapiLogExecution('debug', 'cs status updated ');
								}
