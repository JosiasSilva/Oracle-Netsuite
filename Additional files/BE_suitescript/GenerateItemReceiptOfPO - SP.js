 ...
 * Script URL : /app/common/scripting/script.nl?id=1021
 * Button URL : /app/site/hosting/scriptlet.nl?script=1021&deploy=1
 * Save Search URL : /app/common/search/searchresults.nl?searchid=4726
 */

//Create a Suitelet Script that generate an Item Receipt
/*function GenerateItemReceipt(request,response)
{
	var form = nlapiCreateForm("Create Item Receipt of POs for SP");
	...
	else
	{
		try
		{
			...
			var mySearch = nlapiLoadSearch(null,4726); //Save search id of POs list
			var searchresult = [];
...
function GenerateItemReceipt(request, response)
{
	...
	else if(request.getMethod() == 'POST')
	{
                ...
                params['custscript_savedsearch_id6'] = 4726;
		var result = nlapiScheduleScript('customscript1059', 'customdeploy1',params);  // 1059 is internal id of the scheduled script that you wish to reschedule and 1 is deployment id
