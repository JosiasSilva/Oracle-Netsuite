...
function Cads_SO(form,serverWidget,search,SalesRecord,record,log) {
  ...
  for (var x = 0; x < lineItemCount; x++) {
    ...
    var parent_data = search.lookupFields({type: 'item',id:item ,columns: ['parent']});
    var parent = Get_Field_Value(parent_data.parent);
    if (!parent)
      continue;
    if ((parent) && (description)) {
      var des = description.split('Size');
      log.debug({ title: 'des', details: des});
      var ring_size_value=des[1];
      ring_size_value= parseFloat(ring_size_value); 
      //log.debug({ title: 'ring_size_value', details: ring_size_value});
      if(ring_size_value)
      {
        search.create({
          "type": "customrecord_internal_cad",
          "filters": [
            ['custrecord_icr_parent_sku', search.Operator.ANYOF, parent], 'and',
          ...
          cads.push(result.getValue("custrecord_dimensions_n_shape"));       
          return true;	
        });
      }
      if(cads.length>0)
      {
        //log.debug({ title: 'cads search', details: cads});
        var chkCadValue =SalesRecord.getSublistValue({sublistId: 'item',fieldId: 'custcol_cad_so',line: x}); 
        if(chkCadValue)
        {
          chkCadValue = chkCadValue.replace(/; \n/g,',');
        }
      ...
      //  log.debug({ title: 'cads_get', details: cads_get});
        ...
        {
          //log.debug({ title: 'cads_get SEARCH', details: cads_get});
          //log.debug({ title: 'chkCadValue OLD LINE ITEM', details: chkCadValue});
  ...
  if(cads_index_value_check.length>0)
  {
    ...
    for (var x = 0; x < lineItemCount; x++) {
      ...
      //log.debug({ title: 'CADS', details: '4'});
    }
    SalesRecord_Update.save({
      enableSourcing: true,
      ignoreMandatoryFields: true
    });
    //log.debug({ title: 'CADS', details: '5'});
    ...
    //log.debug({ title: 'CADS', details: '6'});
    return false;
  }
  return true;
}
