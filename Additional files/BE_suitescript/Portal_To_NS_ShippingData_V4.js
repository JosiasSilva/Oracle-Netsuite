		...
		try
		{
			if(datain.cdp_data != null)
			{
				for(var i=0; i < datain.cdp_data.length; i++)
				{
					...
					var itemId = cdpObj.getFieldValue("custrecord_be_diamond_stock_number");
					...
					/***************************Started Added by Yagya on 2nd Dec 2017 for task DP-375*****************/
					/*
					...
					*/
					var flag = Check_Bill(itemId);
					nlapiLogExecution('debug','Bill found or not ' + flag);
					if(flag)
					{
						PushPaymentToPortal(datain.cdp_data[i].cdp_id);
					}
					/***************************End Added by Yagya on 2nd Dec 2017 for task DP-375*****************/
...
{
	var url = "https://testportal.brilliantearth.com/api/order-notify/"; // for Production
	//Setting up Headers
...
function Check_Bill(itemId)
{
	var flag = false;
	try
	{
		var cdp_Obj = nlapiLookupField('inventoryitem',itemId, ['custitem107','custitem110']);
		var Purchase_Order_Date = cdp_Obj.custitem107;
		if( Purchase_Order_Date )
		{
			var fil = [];
			fil.push(nlobjSearchFilter('item',null,'anyOf',itemId));
			var col = [];
			col.push(new nlobjSearchColumn('trandate'));
			var searchRecord = nlapiSearchRecord('vendorbill', null, fil, col);
			if( searchRecord )
			{				
				var Vendor_Return_Date  = cdp_Obj.custitem110;
				Vendor_Return_Date  = new Date(Vendor_Return_Date);
				Purchase_Order_Date = new Date(Purchase_Order_Date);
				for(var k=0; k < searchRecord.length; k++)
				{
					var Bill_Date = searchRecord[k].getValue('trandate'); 
					if( Bill_Date )
					{
						Bill_Date = new Date(Bill_Date);					
						if(Bill_Date >= Purchase_Order_Date && (Vendor_Return_Date != "Invalid Date" || Bill_Date > Vendor_Return_Date) )
						{
							flag = true;							
							nlapiLogExecution('debug', 'Found Vendor Bill #'+ searchRecord[k].getId());
							break;
						}
					}
				}			
	...
	{
		nlapiLogExecution('debug', 'Error in finding bill', b);
	}
	return flag;
}
