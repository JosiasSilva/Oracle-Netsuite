...
function Vendor_Box_Dashboard(request,response)
{
	...
	{
		var so_status = ""; //Added by ajay 06Jan 2017
		var lines = [];
		...
		if(results)
		{
			...
			{
              nlapiLogExecution('DEBUG','Status',results[x].getValue("status","createdfrom"));
				var notes = "";
				...
				lines.push({
					...
					custpage_po_number : "<a href='/app/accounting/transactions/purchord.nl?id=" + results[x].getId() + "'>" + results[x].getValue("tranid") + "</a>",
					...
					custpage_so_status : results[x].getValue("status","createdfrom"), //Added by ajay 03Jan 2017
					custpage_so_number : "<a href='/app/accounting/transactions/salesord.nl?id=" + results[x].getValue("createdfrom") + "'>" + results[x].getValue("tranid","createdfrom") + "</a>",
					...
					custpage_so_imp_notes : results[x].getValue("custbody58"),
					custpage_gem_notif : results[x].getValue("custbody244")
				});
		...
		list.addField("custpage_po_number","text","Purchase Order");
		...
		list.addField("custpage_so_imp_notes","textarea","SO Important Notes");
		...
		list.addField("custpage_created_from","select","Created From","salesorder").setDisplayType("hidden");
		list.addField("custpage_so_status","text","SO Status").setDisplayType("hidden"); //Added by ajay 03Jan 2017
		list.addField("custpage_so_number","text","Sales Order");
		list.addField("custpage_gem_notif","select","Customer Selected Gem Notification","customlist337");
		...
		try
		{
			for(var x=0; x < request.getLineItemCount("custpage_list"); x++)
			{
				if(request.getLineItemValue("custpage_list","custpage_changed",x+1)=="T")
				{
					...
					po.setFieldValue("custbody244",request.getLineItemValue("custpage_list","custpage_gem_notif",x+1));
