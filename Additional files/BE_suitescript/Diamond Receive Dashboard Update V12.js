...
function Diamond_Receive_Dashboard(request, response) {
  if (request.getMethod() == "GET") {
	...
	list.addField("custpage_imagery_needed", "checkbox", "Imagery Needed").setDisplayType("hidden");//ZAG: Added : NS 1484
    ...
    list.addField("custpage_colour_flag_dd_of_following_day", "text").setDisplayType("hidden");
    ...
    if (result != null) {
      ...
      for (var q = 0; q < result.length; q++) {
        ...
        var colour_flag_dd_of_following_day='0';
        var delivery_date=result[q].delivery_date;
        if(delivery_date)
        {
          var get_day= nlapiStringToDate (nlapiDateToString (new Date() )).getDay();
          if(get_day==6)
          {
            var new_delivery_date_get_one=   nlapiDateToString ( nlapiAddDays ( new Date() , 3));
            var new_delivery_date_get_two=   nlapiDateToString ( nlapiAddDays ( new Date() , 1));
            if(new_delivery_date_get_one==delivery_date || new_delivery_date_get_two==delivery_date)
            {
              colour_flag_dd_of_following_day='1';
            }
          }
          else if(get_day==2 || get_day==3|| get_day==4||  get_day==5)
          {
            var new_delivery_date_get=   nlapiDateToString ( nlapiAddDays ( new Date() , 1));
            if(new_delivery_date_get==delivery_date)
            {
              colour_flag_dd_of_following_day='1';
            }
          }
        ...
        if(result[q].cdp_diamond_status!='1' && result[q].cdp_diamond_status!='9')
        {
          colour_flag_dd_of_following_day='1';
        }
        lines.push({
          ...
          custpage_inventory_location_old: result[q].po_inventory_location,
          ...
          custpage_colour_flag_dd_of_following_day:colour_flag_dd_of_following_day,
		  custpage_imagery_needed: result[q].imagery_needed // zag : Added : NS 1484
        });
    ...
    //  try 
      {
        ...
        if (request.getLineItemValue('custpage_list', 'custpage_certificate_included', x)  ) {

          var custom_diamond_id = request.getLineItemValue("custpage_list", "custpage_diamond_id", x);
          if (custom_diamond_id) {
          ...
          if (request.getLineItemValue('custpage_list', 'custpage_so_id', x)  && request.getLineItemValue("custpage_list", "custpage_item_id", x)) {
            SO_json = ({
              ...
              diamond_in_hold_binder:request.getLineItemValue("custpage_list", "custpage_diamond_in_hold_binder",x)
            });
          }
...
function Fatch_Data(request) {
  ...
  //Start : ZAG : 01/10/2019 : Add column custrecord_imagery_needed as per NS 1484
  var column_1 = new Array();
  column_1.push(new nlobjSearchColumn('custrecord_imagery_needed'));
  searchResult.addColumns(column_1);
  //End : ZAG : 01/10/2019 :
  ...
  if (results) {
    ...
    for (var j = 0; j < results.length; j++) {			
      ...
      {
        po_inventory_location='2';
        /*
        if(results[j].getValue("location", "CUSTRECORD_DIAMOND_PO_NUMBER"))
        {
        ...
        var result_obj = ({
          ...
          po_inventory_location: po_inventory_location,
          ...
          delivery_date:results[j].getValue("custbody6","CUSTRECORD_DIAMOND_PO_NUMBER"),
		  imagery_needed:results[j].getValue('custrecord_imagery_needed') // zag : Added
        });
  ...
  if (results) {
    for (var s = 0; s < results.length; s++) {
      ...
      if(Line_Id_Uniqe.indexOf(get_line_uniq_id)<0)
      {
        ...
        po_inventory_location='2';
        /*
        if(results[s].getValue("location"))
        {
        ...
        */
        result.push({
          ...
          status_cdp_so:'',
          delivery_date:results[s].getValue("custbody6"),
        });        
        if (So_ref && So_ref_text && So_ref_text.indexOf('Sales Order #') != -1) {
          sales_order.push(So_ref);
        }
