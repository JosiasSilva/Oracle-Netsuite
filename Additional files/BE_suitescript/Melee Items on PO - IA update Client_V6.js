  ...
  try 
  {
    ...
    if (allInput.length > 0) 
    {
      ...
      if (flag && poId != '' && poId != null &&  postData != '' && postData != null && postData.length>0) 
      {
        ...
        var itemLink = element.getAttribute('data-itemLink');
        var chk_store = 'F';
        ...
        JsonObj.push({
          ...
          chk_store	    	:	chk_store,
          MeleeItems		: 	postData,
          new_measurements  :   document.getElementById('measurements_id').value,
          itemLink          :   itemLink
        });
...
function postRESTletYagya(dataIn) 
{
  ...
  {        
    ...
    if (poId != null && poId != '' && MeleeItems != null && MeleeItems != '' && MeleeItems.length > 0) 
    {
      ...
      var storeMeleeCount = dataIn[0].chk_store;
      var new_center_size = dataIn[0].new_measurements;
      var itemLinkId = dataIn[0].itemLink;
      if(itemLinkId!='' && itemLinkId !=null)
      {
        if(storeMeleeCount == 'T')
          nlapiSubmitField("inventoryitem",itemLinkId,"custitem_new_measurements",new_center_size);
      }
      Update_Stores_Halo_Melee_Count(poId,storeMeleeCount,new_center_size);
      return ss;
    }
  } 
  catch (ex) 
