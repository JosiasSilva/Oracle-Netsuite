...
function updateItemPricing(itemObj,startAt,currentLine)
{
	try
	{
		...
		if(item.price_14kr!=null && item.price_14kr!="")
			metalTypes.push(7);
		if(item.price_14k!=null && item.price_14k!="")
		{
			metalTypes.push(6);
			metalTypes.push(5);
		}
		
		var useEternity = item.use_eternity;
		if(useEternity=="no" || useEternity=="" || useEternity==null)
			useEternity = false;
		else if(useEternity=="yes")
			useEternity = true;
		...
		cols.push(new nlobjSearchColumn("custitem2")); //Size
		...
		do{
			...
			for(var x=0; x < results.length; x++)
			{
				...
				switch(results[x].getValue("custitem1"))
				{
					...
					case "6":
					case "5":
						itemRec.setLineItemValue("price","price_1_",1,item.price_14k);
						break;
				}
				
				if(useEternity==true)
				{
					var size = parseFloat(results[x].getText("custitem2"));
					var sizeToUse;
					...
					else
					{
						...
						sizeToUse = size.toString();
					}
						
					switch(results[x].getValue("custitem1"))
					{
						case "1":
							itemRec.setLineItemValue("price","price_1_",1,item["price_e_18kw_"+sizeToUse]);
							break;
						case "2":
							itemRec.setLineItemValue("price","price_1_",1,item["price_e_18ky_"+sizeToUse]);
							break;
						case "3":
							itemRec.setLineItemValue("price","price_1_",1,item["price_e_platinum_"+sizeToUse]);
							break;
						case "4":
							itemRec.setLineItemValue("price","price_1_",1,item["price_e_palladium_"+sizeToUse]);
							break;
						case "7":
							itemRec.setLineItemValue("price","price_1_",1,item["price_e_14kr_"+sizeToUse]);
							break;
					}
				}
