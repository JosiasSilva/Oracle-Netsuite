...
function Page_Init_Fun(type) {
  ...
  if(nlapiGetFieldValue('custpage_drop_off')=='1')
	...
	/* End: Added on October 15, 2018 for NS-1363*/
...
function Post_Sale_Line_Init_Fun(type)
{
  ...
  /*
   * 2  Setting with large center stone
   * 3  Ring with no large center stone
   * 32 Preset engagement ring
   * 35 Certed preset engagement ring
   * 36 Uncerted preset engagement ring
   * 24 Antique
   */
  ...
  {
    /*
     * Do not allow user to edit line item if item category is not 
     * 2   Setting with large center stone
     * 3   Ring with no large center stone
     * 32  Preset engagement ring
     * 35  Certed preset engagement ring
     * 36  Uncerted preset engagement ring
     * 24  Antique
     * if chk_be variable found true(T) i.e. item category is not 2,3,32,35,36,24
     */
    ...
    {
      /*
       * For new line item having no item if user click OK or Cancel
       */
      if(Shipping_to_BE=='T')
      {
        nlapiDisableLineItemField('custpage_items', "sublist_shipping_to_be", false);
  ...
  /*
 * Disable Created From Field when user enter item manually
 * By Default Initially this (sublist_created_from_disable) field keep false(uncheck)
 * When user enter item manually then this field(sublist_created_from_disable) becomes checked
 */
...
function Post_Sale_Resize_Fld_Changed_Fun(type,name)
{
  ...
  if(type=="custpage_items" && name=="sublist_item")
  {
    ...
    if(itemId!=null && itemId!='')
    {
      ...
      //document.getElementsByClassName("listinlinefocusedrow")[2].click();
      highLightBackgroundColor();
    }
  }
  ...
  if(type=="custpage_items" && name=="sublist_shipping_to_be" )
  {
    ...
    if(Shipping_to_BE=='T')
    {
      ...
      // if(itemId !=null && itemId !="")
      //document.getElementsByClassName("listinlinefocusedrow")[2].click();
      //jQuery(".listinlinefocusedrow").click();
      // $(".listinlinefocusedrow").trigger("click");
    }
    else
    {
      nlapiDisableLineItemField('custpage_items', "sublist_shipping_to_be", false);
    ...
    else if(nlapiGetCurrentLineItemValue("custpage_items","sublist_created_from_disable")=='F')
    {
      nlapiDisableLineItemField('custpage_items', "sublist_created_from", true);
    }*/
    nlapiDisableLineItemField('custpage_items', "sublist_item_link", true);
  ...
  if(type=="custpage_items" && name=="sublist_new_size")
  {
    /*var itemId = nlapiGetCurrentLineItemValue("custpage_items","sublist_item");
    if(itemId !=null && itemId !="")
      var  itemcat = nlapiLookupField("inventoryitem",itemId,"custitem20");
    ...
    //if(qty==null || qty =='')
    // document.getElementsByClassName("listinlinefocusedrow")[5].click();
  ...
  if(type=="custpage_items" && name=="sublist_qty")
  {
    ...
    }else
    {
      ...
      {
        if(related_sales_order == null || related_sales_order =='')
...
// On Save
function Post_Sale_Resize_Save_Record_Fun(type,name)
{
  ...
  for(var i=0;i<nlapiGetLineItemCount("custpage_items");i++)
  {
    ...
    if(sublist_shipping_to_be =='T')
    {
      ...
      var  itemcat = nlapiLookupField("inventoryitem",itemId,"custitem20");
      if(nlapiGetLineItemValue("custpage_items", "sublist_new_size", i+1)==""||nlapiGetLineItemValue("custpage_items", "sublist_new_size", i+1) == null)
      {
        alert("Ring size is mandatory")
        return false;
      }
      else if(nlapiGetLineItemValue("custpage_items", "sublist_sale_amount", i+1)==""||nlapiGetLineItemValue("custpage_items", "sublist_sale_amount", i+1) == null)
      {
        alert('Sale amount is mandatory')
        return false;
      }
      else if(nlapiGetLineItemValue("custpage_items", "sublist_qty", i+1)==""||nlapiGetLineItemValue("custpage_items", "sublist_qty", i+1) == null)
      {
      ...
      else if(nlapiGetLineItemValue("custpage_items", "sublist_created_from_disable", i+1) == 'T')
      {
        if(itemcat!=2)
        {
          ...
          { 
            alert("Related Sales order is mandatory for item:"+"\n"+itemIdText);
            return false;
          }
        }
      }
      ...
      if(itemcat == 2)
      {
        ...
        {
          alert('Center Stone Link is mandatory for item :' + '\n'+itemIdText);
          return false;
        }
      }
      //}
	...
	try
	{
		var actualShipDate = nlapiGetFieldValue('custpage_actual_ship_date');
		if(outsideResizePolicy(actualShipDate))
		{
			alert('Contact OM for potential metal credit');
		}
	}
	catch(ex)
	{
		nlapiLogExecution('Debug', 'Exception in Policy Check Client Function', ex.message);
	}
...
function SO_SetDeliveryDate()
{
  ...
  //var delivery_date = nlapiDateToString(nlapiAddDays(curr_date,56));// 106  28 100
...
function setDeliveryDate_by_Pickup_Location(delivery_date,chkHolidayPopup)
{
  ...
  if(pickupLocation =="8") //Denver
  {
    ...
    {
      //alert('Pick-ups are not available on Tuesday & Wednesday for Denver');
      if(ddName=="Monday")
      {
        delivery_date=nlapiStringToDate(delivery_date);
        delivery_date=nlapiAddDays(delivery_date, 2);
...
{
	nlapiLogExecution('Debug', 'Actual Ship Date:', actualShipDate);
	try
	{
