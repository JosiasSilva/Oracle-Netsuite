...
function Bar_Graph_Weekly_Orders_Greater_Than_Zer(request,response)
{
	try
	{
	...
	try
	{
		...
		cols[1] = new nlobjSearchColumn("tranid",null,"count")
		...
		if(results)
		{
			for(var x=0; x < results.length; x++)
			{
				...
				graphData[x]["revenue"] = results[x].getValue("tranid",null,"count")
	...
	try
	{
		...
		graphCode += "data.addColumn('number','# Orders');"
		...
		graphCode += "chart.draw(data,{width:650,height:400,title:'Weekly Orders By Sales Rep (" + startDate + " - " + endDate + ")',hAxis:{title:'Sales Rep',format:'###,###.##',slantedTextAngle:90},legend:'none',tooltipTextStyle:{color: 'black',fontName: 'Arial',fontSize:11}});"
		graphCode += "}"
