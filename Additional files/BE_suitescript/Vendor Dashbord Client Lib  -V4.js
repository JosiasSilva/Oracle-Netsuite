...
function List_Without_box_record(chek_type)
{
  var record_item=[];
  var vendor_id=[];
  // Added by Bharath - BE-30 (01/20/2018)
  var total_count = nlapiGetLineItemCount('custpage_vendor_list');
      ...
      if(get_vendor=='153')
      {
        if(send_record.length==0)
        {
          ...
          send_record.push({
            ...
            "box_id":0,
            "line_item":[],
            "box_count":record_item[a].box_count,
            "box_number":record_item[a].box_number
          });
        }
