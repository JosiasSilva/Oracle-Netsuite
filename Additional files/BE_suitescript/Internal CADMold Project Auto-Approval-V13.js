                ...
                for (var m = 0; m < data_sales_order.length; m++) {
                    try {
                        var get_parent_item = nlapiSearchRecord('salesorder', null, [
                            ...
                            //commented by sanjeet 6th August 2018
                            // new nlobjSearchColumn('vendor', 'item'),
                            //End	
                        ]);
                        var col = get_parent_item[0].getAllColumns();
                        var ParentItem = get_parent_item[0].getValue(col[0]);
                        //Code added by sanjeet 7th August
                        var ItemId = get_parent_item[0].getValue('item');
                        var get_vendor_line;
                        var search_vendor = nlapiSearchRecord('item', null, new nlobjSearchFilter('internalid', null, 'anyof', ItemId), new nlobjSearchColumn('vendor'));
                        if (search_vendor) {
                            get_vendor_line = search_vendor[0].getValue('vendor');
                        }

                        //End
                        var pegHead = nlapiLookupField('item', ParentItem, 'custitempeg_head');
                        ...
                        if (get_so_measurement) {
                            if (pegHead == 'T') {
                                create_po_so_update_cads(get_vendor_line, get_parent_item[0].getValue('item'), data_sales_order[m], '', '4', ''); //NS-1385
                                continue;
                            }
                            //Molds comparison
                            ...
                            if (get_molds_obj) {
                                ...
                                var get_mold_id = get_molds_obj[4]; //NS-1385
                                create_po_so_update_mods(get_vendor, get_item_id, data_sales_order[m], get_mold_id); //NS-1385
                                continue;
                            }
                            //Cads comparison  
                            ...
                            if (get_cads_obj) {
                                ...
                                var get_cad_id = get_cads_obj[4]; //NS-1385
                                create_po_so_update_cads(get_vendor, get_item_id, data_sales_order[m], '42', '2', get_cad_id); //NS-1385
                                continue;
                            }
                            //No peg head,No Mold,No Cad then it will set CAD Needed
                            create_po_so_update_cads(get_vendor_line, get_parent_item[0].getValue('item'), data_sales_order[m], '39', '3', ''); //NS-1385
                            //End
                        }
...
function Create_PO(get_vendor, get_item_id, so_id, status_value) {
    ...
    if (get_vendor && get_item_id) {
        ...
        var mold_cad_link = nlapiLookupField('salesorder', so_id, 'custbody_cadmoldhyperlink');
        if (search_po) {
            for (var b = 0; b < search_po.length; b++) {
                nlapiSubmitField('purchaseorder', search_po[b].getId(), ['custbody41', 'custbody_cadmoldhyperlink'], [status_value, mold_cad_link]);
...
function Get_Mold_width_length(get_parent_item, get_so_shape, get_measurment) {
    if (get_parent_item) {
        for (var a = 0; a < get_parent_item.length; a++) {
            ...
            //Lines added by sanjeet 6th August
            var line_item = get_parent_item[a].getValue('item');

            var get_vendor;
            var search_vendor = nlapiSearchRecord('item', null, new nlobjSearchFilter('internalid', null, 'anyof', line_item), new nlobjSearchColumn('vendor'));
            if (search_vendor) {
            ...
            if (parent_item && get_vendor) {
                ...
                if (search_mold) {
                    ...
                    if (temp.length > 0) {
                        ...
                        var mold_Id = temp[0].getId(); //NS-1385
                        //Line commented by sanjeet 6th August
                        //var get_vendor = get_parent_item[a].getValue('vendor', 'item');

                        //End
                        var get_item = get_parent_item[a].getValue('item');
                        //NS-1385
                        if (get_width && get_length && get_vendor && get_item && mold_Id) {
                            return [get_width, get_length, get_vendor, get_item, mold_Id];
                            //NS-1385
                        }
                    }
                }
            ...
            if (parent_item && get_vendor) {
                ...
                if (search_cad) {
                    ...
                    if (temp.length > 0) {
                        ...
                        var cad_Id = temp[0].getId(); //NS-1385
                        //Line commented by sanjeet 6th Aug 2018
                        // var get_vendor = get_parent_item[a].getValue('vendor', 'item');
                        //End 6th Aug 2018
                        var get_item = get_parent_item[a].getValue('item');
                        //NS-1385
                        if (get_width && get_length && get_vendor && get_item && cad_Id) {
                            return [get_width, get_length, get_vendor, get_item, cad_Id]; ////NS-1385
                        }
                    }
                }
            }
...
function create_po_so_update_cads(get_vendor, get_item, so_id, status_value, cad_result_id, get_cad_id) {
    if (get_vendor && get_item && so_id) {

        var so_fields = nlapiLookupField('salesorder', so_id, ['custbody87', 'custbody227', 'custbody285', 'custbodyops_cs_status', 'custbody111', 'custbodyfraud_check_new', 'custbody152']);
        ...
        var mod_notes = so_fields.custbody152;
        nlapiLogExecution("debug", "Mod Notes", mod_notes);

        //Type of Order = New Order, Order Status includes Ready to Order and does not include New Setting Added or Info Needed from CS, Stop WB Workflow = F, Layaway = F, Fraud Check New is not Approval Pending FC, Ops CS Status is not Customer Contacted, Additional Fees/Modifications is blank
        if (type_of_order.indexOf('1') > -1 && order_status.indexOf('8') > -1 && order_status.indexOf('6') == -1 && order_status.indexOf('7') == -1 && so_fields.custbody285 == 'F' && layaway == 'F' && fraud_check_new != '4' && ops_cs_status != '3' && (mod_notes == null || mod_notes == '')) {
            // nlapiSubmitField ( 'salesorder' ,  so_id, 'orderstatus' , 'B') ;
            //nlapiSubmitField('salesorder', so_id, ['orderstatus', 'custbody41'], ['B', status_value]);
            ...
            if (get_cad_id) {
                obj_so.setFieldValue('custbody_cadmoldhyperlink', '/app/common/custom/custrecordentry.nl?rectype=523&id=' + get_cad_id); //NS-1385
            } else {
                obj_so.setFieldValue('custbody_cadmoldhyperlink', ''); //NS-1385	
            }
            var today = new Date();
            today = nlapiDateToString(today, "date");
            obj_so.setFieldValue('custbodyapproval_date', today);
...
function create_po_so_update_mods(get_vendor, get_item, so_id, get_mold_id) {
    if (get_vendor && get_item && so_id && get_mold_id) {
        nlapiLogExecution("debug", "mold record id", get_mold_id);
        ...
        if (type_of_order.indexOf('1') > -1 && order_status.indexOf('8') > -1 && order_status.indexOf('6') == -1 && order_status.indexOf('7') == -1 && so_fields.custbody285 == 'F' && layaway == 'F' && fraud_check_new != '4' && ops_cs_status != '3' && (mod_notes == null || mod_notes == '')) {
            ...
            obj_so.setFieldValue('custbodyapproval_date', today);
            if (get_mold_id) {
                obj_so.setFieldValue('custbody_cadmoldhyperlink', '/app/common/custom/custrecordentry.nl?rectype=146&id=' + get_mold_id);
            } else {
                obj_so.setFieldValue('custbody_cadmoldhyperlink', '');
            }
            var item_count = obj_so.getLineItemCount('item');
            for (var l = 1; l <= item_count; l++) {
                var check_item_id = obj_so.getLineItemValue('item', 'item', l);
