...
var	gbShowDebugDialogs		= false			// Show (else suppress) debug dialogs? 
...
var	gnMsecBounceTabs		=   17			// How often to update bouncing tabs (msec.) // TO_DO: Not used 
// */ 
...
var	gsBrowserFamily			= ( gsDocType != "panel" )									? "?"	// ?  = None or unknown 
							...
							:															  "FF"	// FF = Firefox 
		// TO_DO: Can't use "<br>" for some reason; causes an error in logme() at innerHTML = join(...) 
var	gsLogNewline			= ( gsBrowserFamily == "IE" ) ? " -- " : "\n" 
var	gsAppletFile			= "JSjava2.jar"		// TO_DO: unsigned 
...
var	gsAssetsBaseURL			=   "https://ns.shoretelsky.com/"  // "https://user.m5net.com/m5connect/"	// "https://alumnus.alumni.caltech.edu/~dzobel/m5/assets/" 
...
var	gsLogoLoadedURL			= gsAssetsBaseURL + "M5_1.gif" 
	    // TO_DO: Using -1 for dimensions causes a slight hiccup when swapping images out and in; need to maintain width and height even when image is absent 
		// TO_DO: There is a hiccup anyway, just because the image placeholder shows momentarily, but at least the screen doesn't jump  
 ...
 */ 
                   	   	// TO_DO: Have a separate field for internal reference, rather than using the label as the reference in panel.js 
                   	    // TO_DO: Scale the different types so that their widths are in comparable units 
                   	    // TO_DO: This should really list all the properties of a phone call, including the ones that are not presented onscreen 
var	gasCallField	= 
								// Editable? 
	// Label			|  W x H | E | Type 
   	[ null 
...
//	, "Company			| 40	 | y | select customer"		// TO_DO: This needs to cover customers, partners, vendors, etc. 
...
var	gasCallProperty	= 
	...
	, "disposition		|			|" 
	, "duration			| 0			| 0" 
	, "priority			| MEDIUM	| MEDIUM"	// TO_DO: Use goCallPriorityIndex.medium 
	, "result			|			|" 
	, "secCallStart		| 0			| 0" 
	, "secCallTotal		| 0			| 0" 
	, "secHoldTotal		| 0			| 0" 
	, "status			| COMPLETE	| COMPLETE"	// TO_DO: Use goCallStatusIndex.complete		// Will always be Completed, since the call (or at least an attempt) actually took place 
	, "subject			|			| {Call on} m/d/yy {at} h:nn aa"	// TO_DO 
	] 
...
var	gaoStyleSheetRule =  
	...
	// Classes defined by NetSuite 
	  ...
	  // TO_DO: _X means "ignore"; probably just comment these out 
	,	{ _X : ".smalltextnolink" 
		, fontWeight		: "bold" 
		} 
	...
	,	{ _ : ".debugInfo" 
		, color				: "#808" 
		, fontSize			: "6pt" 
		, readOnly			: true	// TO_DO: does this do anything? 
		} 
	,	{ _ : ".debugLog" 
		, color				: "#090" 
		, fontSize			: "6pt" 
 ...
 */ 
// TO_DO: Use shorthand (set to null ) 
// TO_DO: Some of these can be handled by gaoCallProperty 
...
var	goCallInfoInit	= 
	...
	, Idle	:		// Set when entering "Idle" state 
		{ _					: null 
		, timer				: 0		// TO_DO: Roll into secCall* 
		...
		, recID				: null 
		, rPhoneCall		: null 
		, matchesIndex		: null			// TO_DO: unused 
		, matchesText		: ""			// TO_DO: unused 
		, contactIndex		: null 
		, contactText		: "" 
		, companyIndex		: null 
		, companyText		: "" 
...
/** 
 *	Custom lists. 
 *	TO_DO: Read these from NetSuite instead of hard-wiring them. 
 */ 
 
var	gaoCustomList = 
	[ null 
	,	// TO_DO: Does priority exist as a list in NetSuite?  Anyway, it's not a custom list 
		{ _	: "priority" 
		, "LOW"		: "Low" 
		, "MEDIUM"	: "Medium" 
		, "HIGH"	: "High" 
		} 
	,		// TO_DO: Someday, read correctly from NetSuite 
...
var gasStateMachine	= 
	...
	,											   "| A Autocommit		| ^" 
	, "Busy			 | IO...4 | Red	   | Incomplete	| B Commit			| Store"	// TO_DO: Should H flag be set? 
	,											   "| A Autocommit		| ^" 
 ...
 */ // strings_EN_us.js
 // strings_EN_us.js
// TO_DO: Still need to extract some of the strings in callInfo.js, defines.js, panel.js, and preprocess.js // strings_EN_us.js
 // strings_EN_us.js
goText[ "EN_us" ] = // strings_EN_us.js
 ...
 // strings_EN_us.js
	...
	, noMatches				: "(No matches)"		// TO_DO: "Unknown number" // strings_EN_us.js
 // strings_EN_us.js
...
// */ // preprocess.js
 // preprocess.js
	...
	goCallResult		= getCustomList( gsNetSuiteListPrefix + "cti_call_result"      )	// TO_DO: THIS IS UNUSED // preprocess.js
...
// */ // preprocess.js
 // preprocess.js
		sKey = sKey.replace( / /g, "" ).toLowerCase()	// TO_DO: Would be nicer to camel-case it // preprocess.js
 // preprocess.js
...
{ // preprocess.js
 ...
 // preprocess.js
		...
		if ( s.charAt( 0 ) != "|" ) { // preprocess.js
			...
			gaoStateMachine[ sState ] = // preprocess.js
				...
				, bOffHook			: left(( oFlags._H != "" ), 1 )		// TO_DO: Why is this a string instead of a bool? // preprocess.js
 ...
 // preprocess.js
	...
	for ( gsInitialState in gaoStateMachine ) // preprocess.js
		break; // preprocess.js
 // preprocess.js
	// TO_DO: For debugging, might want to run through all transitions and confirm that they go to a real state // preprocess.js
	// TO_DO: For debugging, catch when a transition or a state has multiple definitions // preprocess.js
} // preprocess.js
 // preprocess.js
 // preprocess.js
function // preprocess.js
...
{ // panelBuilder.js
 ...
 // panelBuilder.js
	for ( var sOverride in // panelBuilder.js
		...
		var	sQ	= ""	// TO_DO: ( eval( "typeof(" + a[ 0 ] + ")" ) == "string" ) ? '"' : "" // panelBuilder.js
 // panelBuilder.js
...
// */ // panelBuilder.js
 ...
 // panelBuilder.js
		...
		// TO_DO: Should be able to use a relative URL; e.g., /images/java/JSjava.jar // panelBuilder.js
/*	// TO_DO: Handshake isn't working right, so let's just inject the applet in the panel's JavaScript, rather than in the panel builder  // panelBuilder.js
...
// */ // panelBuilder.js
 ...
 // panelBuilder.js
			...
			// TO_DO: fix name and ID // panelBuilder.js
...
//TO_DO: choose			    	+ ' class="rndbuttoninpt"' // panelBuilder.js
 ...
 // panelBuilder.js
					with ( addField( sName, sNSType, label, source, sTabName )) { // panelBuilder.js
						// TO_DO: STRANGE! The isOneOf fails, even though it certainly returns TRUE // panelBuilder.js
...
//	TO_DO: delete			for ( var j in gaoTester ) // panelBuilder.js
...
//			if ( gnDebug != 0 ) // panelBuilder.js
//				if ( false )	// TO_DO: It's not there anymore // panelBuilder.js
...
//			display data in columns // panelBuilder.js
 // panelBuilder.js
/* // panelBuilder.js
		// TO_DO: For now, add some invisible fields just so the tab will be displayed // panelBuilder.js
...
// */ // panelBuilder.js
 // panelBuilder.js
			var	oSL	= addSubList( getNsObjectName( "list", "log", 1 ), "staticlist", goText.logTitle, getNsObjectName( "tab", "", "log" ))	// TO_DO: Label isn't appearing? // panelBuilder.js
...
{ // panelBuilder.js
 ...
 // panelBuilder.js
 // panelBuilder.js
var createLayout = layOutPanel	// TO_DO: Temporary // panelBuilder.js
...
// kch			popUpDebug( "dhz off hook w=" + bWasOffHook + ", i=" + bIsOffHook + "..." + sState + " " + getCallInfo( iCall, "bOffHook" )) // panel.js
 ...
 // panel.js
			...
			setCallInfo( iCall, "title", "" )	// So Title doesn't show "null" in IE. TO_DO: Better way to do this is to fix setCallInfo()  // panel.js
 // panel.js
//			displayCallInfo( iCall ) // panel.js
 // panel.js
			...
			// TO_DO: Would be nice to put focus in Number field automatically, but this doesn't work // panel.js
			// Set focus to the "Number" field on tab 1 // panel.js
			// getNsObject( "field", 1, "number", "_fs" ).focus() // panel.js
 // panel.js
			// TO_DO: To put focus in Number field manually, why do we first have to click somewhere else (e.g., in comments field)? // panel.js
		} // panel.js
 // panel.js
...
{ // panel.js
 ...
 // panel.js
/* // panel.js
 * // TO_DO: Do a similar thing with the hold timer -- will need additional flags // // panel.js
 ...
 * TO_DO: Unused else { // Otherwise, if the new state defines a timer and the // panel.js
...
{ // panel.js
 ...
 // panel.js
	...
	if ( typeof( oTrigger ) == "undefined" || oTrigger == null ) { 	// TO_DO: // panel.js
																	// Make a // panel.js
																	// function // panel.js
																	// that does // panel.js
																	// this // panel.js
...
{ // panel.js
	...
	console.log("aNamePart: " + aNamePart)
	console.log("sValue: " + sValue)
	console.log("field changed: " + sVisibleText)
	
	console.log(o.name.endsWith("speakingto"))
	console.log(o.name == "custpagem5_field_1_speakingto")
 ...
 // panel.js
		...
		if ( fldnam == getNsObjectName( "select", "debug", 1 )) { // panel.js
			if ( sValue != "" ) { // panel.js
				// TO_DO: Tear down any previous subscribe // panel.js
 ...
 // panel.js
/* // panel.js
 	// TO_DO: Need to update this every time he types something in the empty field or clears the non-empty field // panel.js
...
{ // panel.js
	...
	// TO_DO: Sort entries; get rid of sorting arrow in column headers // panel.js
	var	aa	= [] // panel.js
...
//		popUpDebug(sInitial) // panelInit.js
		// TO_DO: This fails when we set innerHTML, but it works when we just do a document.write // panelInit.js
...
{ // panelInit.js
	...
	for ( var i = gnLogRows; o; o = o.nextSibling ) { // panelInit.js
		// Every row is followed by a sibling that is not a row // TO_DO: So then what is it? // panelInit.js
...
*/ // panelInit.js
 // panelInit.js
 // panelInit.js
// TO_DO: Bad function name // panelInit.js
 // panelInit.js
function // panelInit.js
...
{ // panelInit.js
	...
	if ( typeof( sVersion ) != "string" ) { // panelInit.js
		// TO_DO: Should be allowed to use tryCatchLog here, but we're running into bug #87 // panelInit.js
//		setTimeout( 'tryCatchLog("' + "tryToGetAppletVersion()" + '")', 250 ) // panelInit.js
		setTimeout( "tryToGetAppletVersion()", 250 ) // panelInit.js
		return // panelInit.js
	} // panelInit.js
 // panelInit.js
//	popUpDebug( "Applet is active; version = " + sVersion )	// TO_DO: Show this somewhere // panelInit.js
 // panelInit.js
 ...
 // panelInit.js
	...
	var	oUser	= getUsersM5Info( nlapiGetUser() )	// TO_DO: Returns -4 on error // panelInit.js
 ...
 // panelInit.js
	// Log in // panelInit.js
	logme( 1, "Logging in user " + oUser.userID + " with phone number " + formatPhone( oUser.phone ))// TO_DO: temporary // panelInit.js
...
{ // panelInit.js
 ...
 // panelInit.js
	// TO_DO: Google Chrome does not support resizeTo and moveTo in top-level windows // panelInit.js
 // panelInit.js
...
// */ // panelInit.js
 ...
 // panelInit.js
	...
	if ( gnDebug != 0 ) { // panelInit.js
		...
		window.scrollBy( 0, ganPanelDim[ 1 ] + ganPanelDebugHeight + 1 )	// TO_DO: Choose a better number? // panelInit.js
	} // panelInit.js
 // panelInit.js
	...
	for ( var iCall = gnCalls + 1; --iCall; ) // panelInit.js
		eval( // panelInit.js
			eval( "Synccustpagem5_field_" + iCall + "_speakingto" ) // panelInit.js
				...
				.replace('contact.nl','custjob.nl')
				//.replace('?target=','?stage=lead&target=')
				.replace('?target=','?stage=lead&ignore=')
...
insertLogo( // panelInit.js
	bDim ) // panelInit.js
{ // panelInit.js
	// TO_DO: We are actually replacing, not inserting // panelInit.js
 // panelInit.js
/* // panelInit.js
...
/* // panelInit.js
	...
	getNsObject( "tab", "", "1", "txt" ) // panelInit.js
		...
		.previousSibling	// TO_DO: Maybe there's a cells[] solution that's tidier // panelInit.js
		//  into which we now insert the logo // panelInit.js
		.innerHTML // panelInit.js
			= '<img src="' + (( bDim ) ? gsNetSuiteImageDimURL : gsNetSuiteImageURL ) + '" width=32 height=20>' // panelInit.js
// */ // panelInit.js
 // panelInit.js
	// TO_DO: Maybe there's a cells[] solution that's tidier // panelInit.js
	// Find the hyperlinked text in the tab for call 1 // panelInit.js
...
{ // panelInit.js
 ...
 // panelInit.js
	...
	findElementByID( "tb_sp_7" ) // panelInit.js
		...
		.nextSibling	// TO_DO: Maybe there's a cells[] solution that's tidier // panelInit.js
...
//	console.log ( "aColor[0]  " + aColor[0]);
//	console.log ( "aColor[1]  " + aColor[1]);
//	console.log ( "aColor[2]  " + aColor[2]);
	
 // panelUtil.js
	// Color the tab // panelUtil.js
	var	s	= "#" + aColor[ 1 ] // panelUtil.js
	
	if ( s == "#" ) // panelUtil.js
		s = "" // panelUtil.js
	
			
	var a = getNsObject( "tab", "", iCall, "txt" ); // panelUtil.js
	a.style.color = "#333333";
	//a.className = 'formtabtext'; //clear formtabtexton and formtabtextoff !important
	//getNsObject( "tab", "", iCall, "txt" ).style.color = "#444444" // panelUtil.js
 // panelUtil.js
	s	= "#" + aColor[ 1 ] // panelUtil.js
	if ( s == "#" ) // panelUtil.js
	   	s = "" // panelUtil.js
	var	asTabPart	= [ "lnk" ] // [ "lt", "lnk", "rt" ] // panelUtil.js
	for ( var i in asTabPart )
	{ // panelUtil.js
		var o = getNsObject( "tab", "", iCall, asTabPart[ i ]); 
		//o.style.backgroundColor = s + " !important"  // panelUtil.js
		o.style.backgroundColor = s  // panelUtil.js
		o.style.padding = '3px 8px 3px 8px';
		o.style.textAlign = 'center !important';
		o.style.height = 'auto';
		o.className = "";  //clear formtabon and formtaboff !important
		//o.style.background = "none !important"; 
	}
 // panelUtil.js
...
{ // panelUtil.js
 ...
 // panelUtil.js
	...
	while ( iCallBegin <= iCallEnd ) // panelUtil.js
		// TO_DO: This list is not a scriptable sublist according to NetSuite, so refreshing it // panelUtil.js
...
{ // panelUtil.js
	...
	for ( var i in a ) { // panelUtil.js
		...
		// TO_DO: Alternatively, we could just try to find the parameter both with and without our prefix, or we could pre-attach the prefix // panelUtil.js
 ...
 // panelUtil.js
//TO_DO: Needed? Clean up // panelUtil.js
...
{ // panelUtil.js
 ...
 // panelUtil.js
/* // panelUtil.js
	// TO_DO: Have another URL parameter that determines whether to drop old log entries // panelUtil.js
...
{ // dialerBuilder.js
 ...
 // dialerBuilder.js
	...
	var	oUser	= getUsersM5Info( nlapiGetUser() )	// TO_DO: Returns -4 on error // dialerBuilder.js
 ...
 // dialerBuilder.js
		var	sName	= getNsObjectName( "dialer", "panel", 1 )		// TO_DO: Better name? // dialerBuilder.js
...
{ // dialer.js
 ...
 // dialer.js
	// TO_DO: resizing is broken // dialer.js
	return // dialer.js
 // dialer.js
	// TO_DO: Google Chrome does not support resizeTo and moveTo in top-level windows; may not be an issue, since dialer is generally launched from a NetSuite page // dialer.js
 // dialer.js
...
{ // dialer.js
 ...
 // dialer.js
	...
	if ( oButton ) { // dialer.js
		...
		// ... TO_DO: Search for call panel window // dialer.js
			// TO_DO: If we were able to open the window, close it right away // dialer.js
...
{ // harness.js
 ...
 // harness.js
	...
	// TO_DO: Should really only do this if applet has told us it's alive // harness.js
	switch ( sTag ) { // harness.js
 // harness.js
 ...
 // harness.js
	case "Rrespformat": // harness.js
	    ...
	    break; // harness.js
 // harness.js
	case "web":		// TO_DO: Unused // harness.js
	    ...
	    break; // harness.js
 // harness.js
	case "subscribe":	// TO_DO: Unused // harness.js
	    // Collect web service name and parameters in raw, tilde-separated format // harness.js
	    var sWebSvcReqURL	= element( nInstance, "params" ).value // harness.js
 ...
 *  TO_DO: Actually, this is now called by the JavaScript when it's about to // harness.js
...
{ // callInfo.js
 ...
 // callInfo.js
		...
		// TO_DO: What if a state change comes in just after call goes on-hook?  // callInfo.js
		// TO_DO: Does off-hook even matter? // callInfo.js
...
//			break // callInfo.js
		...
		// TO_DO: This will be fooled if multiple calls with the same direction and remote number are outstanding // callInfo.js
 ...
 // callInfo.js
	...
	// TO_DO: What if this is a repetition of state info we already have? // callInfo.js
 ...
 // callInfo.js
	...
	// TO_DO: Need a more elegant way to test for "jumped from Idle to some other state" // callInfo.js
...
//		setCallInfo( iCall, "timer", -( nowSec() - o.duration )) // callInfo.js
		// TO_DO: Unfortunately, no way to know how long we've been on hold // callInfo.js
//		if ( o.state == "HOLDING" ) // callInfo.js
//			setCallInfo( iCall, "secHoldStart", nowSec() )	// This is done for us automatically // callInfo.js
	// TO_DO dhzdhz: Every time we go off hold, accumulate hold timer into call record, in case call panel has to be restarted before end of call // callInfo.js
 // callInfo.js
	// If we have never seen this call before, fill in additional info // callInfo.js
	if ( getCallInfo( iCall, "callID" ) == "" ) {		// TO_DO: Null would be better, but setCallInfo likes to translate null to empty string  // callInfo.js
 ...
 // callInfo.js
		...
		if ( sSubject == "" ) // callInfo.js
			sSubject = "Call on " + formatDateTime( dStartTime, "m/d/yy @ h:nn aa" ).replace( /@/, "at" ) // callInfo.js
				// + (( gnDebug > 0 ) ? " (M5 ID " + o.ID + ")" : "" )	// TO_DO: Delete the M5 ID from this and move into goText // callInfo.js
 // callInfo.js
...
// */ // callInfo.js
 ...
 // callInfo.js
	// TO_DO: If clearing option list, do a different test; then again, vValueWas will never be the same as vValue, which is null // callInfo.js
...
// */ // callInfo.js
 ...
 // callInfo.js
	...
	if ( !bClearingOptionList && bIsNull ) // callInfo.js
		// TO_DO: If the property is numeric, set it to 0, not the empty string; and what about booleans? and should we use the default values from the tables in defines.js? // callInfo.js
		// TO_DO: Actually, don't automatically set numerics to 0 (e.g., recID and callID); it would be better for each property to define its own "here's what null means" // callInfo.js
		vValue = "" // callInfo.js
 // callInfo.js
 ...
 // callInfo.js
		...
		/*else if ( isOneOf( sRecType, ",lead" )) { // callInfo.js
			// Set "speaking to" and "contact" to this person // callInfo.js
			oSet.speakingto = oSet.lead = nRecID // callInfo.js
			// Set "company" to the company he's associated with (if any) // callInfo.js
			oSet.company = oRec.getFieldValue( "company" ) // callInfo.js
		}// callInfo.js*/
 // callInfo.js
 ...
 // callInfo.js
		...
		{
			sRecType = "contact"
			oRec = nlapiLoadRecord(sRecType, vValue)
		}
 ...
 // callInfo.js
		...
		if ( vValue == "" || vValue == 0 ) // callInfo.js
			break; // callInfo.js
		// TO_DO: Sometimes throws an exception (e.g., if it's a company but not a customer) // callInfo.js
...
// */ // callInfo.js
		if ( gnDebug > 0 ) { // callInfo.js
			...
			var	s	= "" // callInfo.js
				...
				// TO_DO: Make a guess at which phone line the call is on // callInfo.js
...
//		+ right( 100 + dStartTime.getUTCSeconds()  , 2 ) // NetSuiteDB.js
 // NetSuiteDB.js
	...
	return ( sStartTimeUTC + "99" + right( "000000000" + nCallID, 9 ))	// TO_DO: Is this length excessive? // NetSuiteDB.js
} // NetSuiteDB.js
 // NetSuiteDB.js
 // NetSuiteDB.js
function // NetSuiteDB.js
...
//		, owner												: nlapiGetUser()	// "Created By": Set automatically // NetSuiteDB.js
		...
		, title												: "-"				// TO_DO: Evidently can't be empty string // NetSuiteDB.js
 // NetSuiteDB.js
...
updatePhoneCallRecord( // NetSuiteDB.js
	iCall ) // NetSuiteDB.js
{ // NetSuiteDB.js
	console.log("updatePhoneCallRecord: Loading record")
	...
	var	oRec	= (( recID != null ) && (recID != ""))  ? nlapiLoadRecord( "phonecall", recID ) : null // NetSuiteDB.js
	if ( oRec == null ) { // NetSuiteDB.js
		console.log("Call #" + iCall + ": No phonecall record with ID " + recID)
 ...
 // NetSuiteDB.js
	...
	// TO_DO: Does NetSuite get confused if we cross out of daylight-saving time and the clock goes backward? // NetSuiteDB.js
 // NetSuiteDB.js
	var	nCompanyRef		= null //= getCallInfo( iCall, "company" ) // NetSuiteDB.js
	var	nContactRef		= getCallInfo( iCall, "contact" ) // NetSuiteDB.js
	var nLeadRef 		= getCallInfo( iCall, "lead")
	
	console.log(nCompanyRef)
	console.log(nContactRef)
	console.log(nLeadRef)
	console.log(leadID)
	if ( nullif( nCompanyRef, "" ) == null ) // NetSuiteDB.js
		nContactRef = null // NetSuiteDB.js
		
	if (nCompanyRef == null && leadID != null) {
			nCompanyRef = leadID
			nCompanyRef = leadID
	}
 // NetSuiteDB.js
...
try
{
	...
	console.log("commitPhoneCallRecord")
 // NetSuiteDB.js
 ...
 // NetSuiteDB.js
	...
	oFields["custeventcti_call_speaking_to"] = null
...
// */ // NetSuiteDB.js
	...
	console.log("commitPhoneCallRecord: returned " + recID + "!")
...
{ // NetSuiteDB.js
 ...
 // NetSuiteDB.js
		// TO_DO: Copying these fields from the record to the call info structure must be done in a specific order, // NetSuiteDB.js
		...
		for ( var e in o ) // NetSuiteDB.js
			if ( o[ e ] != "matches" )		// TO_DO: Should be able to handle this, but it's going from a text field to a lookup value // NetSuiteDB.js
...
{ // NetSuiteDB.js
 ...
 // NetSuiteDB.js
	...
	for ( var iHit in aoHit ) { // NetSuiteDB.js
		...
		// TO_DO: We only want contact, employee, or customer // NetSuiteDB.js
...
{ // NetSuiteDB.js
	...
	// TO_DO: Creation of this array can be done once, at startup // NetSuiteDB.js
...
*/ // NetSuiteDB.js
		             // TO_DO     if ( sUser == null || sPassword == null || sNumber == null ) // NetSuiteDB.js
 // NetSuiteDB.js
 // NetSuiteDB.js
...
{ // M5.js
	// TO_DO: Should really just check for this once // M5.js
...
processCallsInfo( // M5.js
	sSerializedResponse ) // M5.js
{ // M5.js
	// TO_DO parse out as new status and log it // M5.js
...
{ // M5util.js
	...
	// TO_DO: Might be better to redefine javaScriptCallsOutToJava() such that if we don't pass User= and Password= at all (not just empty, as we do now) and we need them, then Java just recopies its existing ones // M5util.js
	args.unshift( "User", "", "Password", "" ) // M5util.js
	var	sParams	= "" // M5util.js
	var	bKey	= true // M5util.js
	// TO_DO: Have to do this way because for ( var i in args ) gives i out of order (0, 1, 4, 2, 3) in IE!  Are there other places we should do this? // M5util.js
	var	n		= args.length // M5util.js
	for ( var i = 0; i < n; ++i ) { // M5util.js
		var	sParam	= args[ i ]	// TO_DO: URL-encode? // M5util.js
...
{ // M5util.js
	...
	var	sCommandAndParams	=		// TO_DO: This may not be necessary // M5util.js
...
//			.replace( / *~ */g, "&" ) // M5util.js
 ...
 // M5util.js
	...
	var sCParam             = ( sCommand == "dial" ) ? " " + sUser : ""    // TO_DO: add phone number being called // M5util.js
 ...
 // M5util.js
 // M5util.js
// TO_DO: Rename; also parses listCalls response // M5util.js
// TO_DO: only works if format is key/value // M5util.js
 // M5util.js
function // M5util.js
...
//		s = sPhone.charAt( 0 ) + ""-"	// TO_DO: Or would we rather just not see the country prefix? // util.js
...
{ // util.js
	...
	var baseName = "document" // util.js
	  ...
	  , hName // util.js
 // util.js
	  // TO_DO: Merge this big "if" with the following one // util.js
 ...
 // util.js
	try { // util.js
		...
		if ( sCatchFn != null ) { // util.js
			...
			sRV += "\n--------" // util.js
			eval( sCatchFn + '("' + sRV.replace( /\\/g, "\\\\" ).replace( /\n/g, "\\n" ).replace( /\r/g, " " ).replace( /"/g, '\\"' ) + '")' )	// TO_DO: This is basically stringJavaToJavaScript // util.js
		} // util.js
	} // util.js
 // util.js
	return ( vRV ) // util.js
...
{ // util.js
	...
	if ( goBounceConst == null ) { // util.js
		...
		// TO_DO: Math appears to be slightly wrong here; amplitude gets too high // util.js
 ...
 // util.js
		...
		console.log (sScreenElementName);
 ...
 *	TO_DO: Can we hook a function that is already hooked? // hookFunc.js
 */ // hookFunc.js
 // hookFunc.js
...
{ // hookFunc.js
 ...
 // hookFunc.js
	...
	//  TO_DO: This is not implemented // hookFunc.js
 ...
 // hookFunc.js
		...
		// TO_DO: If we allow multiple hooking, this jumps all the way back to the original hooked function // hookFunc.js
...
// TO_DO: Get rid of this if unused // embedApplet.js
...
{ // embedApplet.js
 ...
 // embedApplet.js
	...
	// TO_DO: Why do width and height appear in three separate places here? // embedApplet.js
	...
	var	oAppletAttribs	= // embedApplet.js
		{ _					: null // embedApplet.js
			// TO_DO: Probably don't need to have separate IDs for IE/non-IE // embedApplet.js
		, id				: elementName( nInstance, "applet" )// ( xapplet_fu.isIE() ) ? "object" : "embed" ) // embedApplet.js
		, width				: 1		// TO_DO: This is the dim that actually matters // embedApplet.js
		, height			: 1 // embedApplet.js
 ...
 // embedApplet.js
	// TO_DO: Don't need? // embedApplet.js
	// TO_DO: Fix alternate message // embedApplet.js
...
var gjReqInfo        	= [] // appletIF.js
 // appletIF.js
 // appletIF.js
// TO_DO: Do as actual object // appletIF.js
 // appletIF.js
...
//	    popUpDebug( "No Java control function for instance #" + nInstance ) // appletIF.js
 ...
 // appletIF.js
	...
	while ( args.length ) { // appletIF.js
	    var sParam  = args[ 0 ]   // TO_DO: will this work with null? // appletIF.js
...
//	        sParam = '"' + sParam + '"' // appletIF.js
 ...
 // appletIF.js
	// TO_DO: Convert the command and the parameter to Java strings? // appletIF.js
 // appletIF.js
	logme( 10, "tell applet " + nInstance + ': "' + sCommand + '" ' + sParams.replace( /,/g, " " )) // appletIF.js
	// TO_DO: We sometimes get an error here: "element( nInstance, gsAppletPrefix )[ javaCF ] is not a function" // appletIF.js
...
//	setTimeout( "dhz9blah()", 5000 ); return ('') // appletIF.js
 ...
 // appletIF.js
		try { // appletIF.js
			...
			// TO_DO: We've hard-wired the applet method name rather than using the variable javaCF // appletIF.js
...
{ // appletIF.js
 ...
 // appletIF.js
	case "show response": // appletIF.js
		showHttpResponse( nInstance, 0, sParam )		// TO_DO: nStatus = 0? // appletIF.js
		break; // appletIF.js
 // appletIF.js
...
//	    	popUpDebug('dhz throw away ' + nInstance + ',' + nID) // appletIF.js
		...
		if ( bTimedOut ) { // appletIF.js
			;	    //TO_DO what if timed out? // appletIF.js
			break; // appletIF.js
		} // appletIF.js
 // appletIF.js
