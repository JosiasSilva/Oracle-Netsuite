...
function Diamond_Receive_Dashboard(request, response) {
    ...
    if (request.getMethod() == "GET") {
        ...
        //  list.addField("custpage_recive_checkss", "multiselect", "item","item");
        ...
        if (result != null) {
            ...
            if (set_vendor_data) {
                ...
                nlapiGetContext().setSetting('SESSION', 'custpage_vendor_data', JSON.stringify(vendors));
            } else {
                var vendors = JSON.parse(nlapiGetContext().getSetting('SESSION', 'custpage_vendor_data'));
                for (var x = 0; x < vendors.length; x++) {
                    form_vendor_fld.addSelectOption(vendors[x].id, vendors[x].name, false);
                }
...
function Redirect_to_onther_script() {
    var param = new Array();
    param['custpage_search_vendor'] = request.getParameter("custpage_search_vendor");
    param['custpage_form_date'] = request.getParameter("custpage_form_date");
    param['Save_Data'] = '1';
    response.sendRedirect("SUITELET", "customscript_update_po_v1", "customdeploy_update_po_v1", false, param);
}
...
function Fatch_Data(request) {
    ...
    if (update_data != null) {
        ...
        for (var s = 0; s < update_data.length; s++) {
            ...
            if (join_id) {
                //PO_ID.push(join_id);
				Line_Id_Uniqe.push(join_id+json_obj.line_number);
            }
    ...
    search_1.push(new nlobjSearchFilter('status', 'custrecord_diamond_po_number', 'anyof', ['PurchOrd:B','PurchOrd:E']));
	...
	search_1.push(new nlobjSearchFilter('applyingtransaction', 'custrecord_diamond_po_number', 'anyof', '@NONE@'));
	search_1.push(new nlobjSearchFilter('formulanumeric',null, 'greaterthan', '0').setFormula("CASE  WHEN {custrecord_be_diamond_stock_number}={custrecord_diamond_po_number.item} THEN 1 ELSE 0 END"));
    if (PO_ID.length > 0) {
	...
	//col.push(new nlobjSearchColumn("item", "CUSTRECORD_DIAMOND_PO_NUMBER"));
    ...
    if (results != null) {
        ...
        for (var j = 0; j < results.length; j++) {
			...
			var value_check=results[j].getValue(col[2])+results[j].getValue(col[14]);
			var check_index_number=Line_Id_Uniqe.indexOf(value_check);
			if(check_index_number <0)
			{			
            var result_obj = ({
				...
				check_duplicate:results[j].getValue(col[2])+results[j].getValue(col[5]) + results[j].getValue(col[14])
        ...
        result = removeDuplicates(result, 'check_duplicate');
    }

    search_2.push(new nlobjSearchFilter('type', null, 'anyof', 'PurchOrd'));
    search_2.push(new nlobjSearchFilter('status', null, 'anyOf', ['PurchOrd:B','PurchOrd:E']));
	...
	search_2.push(new nlobjSearchFilter('applyingtransaction', null, 'anyof', '@NONE@'));
    var col = new Array();
    ...
    if (results != null) {
        for (var s = 0; s < results.length; s++) {
			var value_check=results[s].getId()+results[s].getValue(col[11]);
			var check_index_number=Line_Id_Uniqe.indexOf(value_check);
			if(check_index_number <0)
			{			
            result.push({
                cdp_vendor: results[s].getValue(col[0]),
...
function AllSalesOrder() {
    ...
    filter.push(new nlobjSearchFilter('custcol_category', null, 'anyOf', [2, 12]));
