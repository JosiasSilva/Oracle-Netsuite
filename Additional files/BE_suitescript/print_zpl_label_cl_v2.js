 ...
 */
define(['N/url','N/error', 'N/record', 'SuiteScripts/moment.min.js'],

function(url, error, record, moment) {
  	var cntxt;
    ...
    function fieldChanged(context) {
      //console.log(context);
    	...
    	//Start : 1109
    	var subListId;
    	if (context.fieldId =='custpage_date_rp') {
	    	subListId = 'custpage_list_ready_to_pull';
	        var date_rp= curRec.getValue({fieldId: 'custpage_date_rp'});
	        //console.log (subListId + ' : Date' + date_rp);
	        var date_rpt = moment(date_rp).format("M/D/YYYY");
	        var sublistId_splits = "#" + subListId + "_splits";
			jQuery(sublistId_splits).html('<div>Loading...</div>');
			//console.log (subListId + ' : date_rpt' + date_rpt);
			document.getElementById('server_commands').src="/app/site/hosting/scriptlet.nl?script=2424&deploy=1&r=T&machine="+subListId+"&inputRTPdate="+date_rpt;
	    	} 
	    
    	if (context.fieldId =='custpage_date_nrp') {
	    	subListId = 'custpage_list_not_ready_to_pull';
	        var date_nrp= curRec.getValue({fieldId: 'custpage_date_nrp'});
	        //console.log (subListId + ' : Date' + date_nrp);
	        var date_nrpt = moment(date_nrp).format("M/D/YYYY");
	        var sublistId_splits = "#" + subListId + "_splits";
			jQuery(sublistId_splits).html('<div>Loading...</div>');
			
			document.getElementById('server_commands').src="/app/site/hosting/scriptlet.nl?script=2424&deploy=1&r=T&machine="+subListId+"&inputNRTPdate="+date_nrpt;
	    	}
    	// End : 1109
