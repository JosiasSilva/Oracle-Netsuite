function Print_Cert_Label_Button(type,form)
{
	if(type=="view")
	{
		try
		{
			...
			form.addButton("custpage_print_cert","Print Certificate Label","var newWin = window.open('" + url + "','printCertWin','height=400,width=400'); setTimeout(function(){ newWin.close(); }, 10000);");
		}
		catch(err)
		{
...
function Gen_Diamond_Intake_Label_SL(request,response)
{
	...
	if(results)
	{
		var zpl = '';
			...
			{
				zpl+= '^FO20,220^FD' + results[0].getValue("custbody110") + " " + 'Drop Ship' + '^FS\n';
				zpl+= '^FO20,260^FD' + results[0].getText("custbody_drop_ship_vendor") + '^FS\n';
			}
			else
			{
