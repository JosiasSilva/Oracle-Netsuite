...
function BackStock_Data(datain)
{
  try {
    ...
    if(datain.match_col_asc_des == "true")
    {
      ...
      columns.push(new nlobjSearchColumn("custrecord_saved_search_id"));
      var saved_search_id_result = nlapiSearchRecord("customrecord_temp_saved_search",null,null,columns);
      if(saved_search_id_result)
      {
        var saved_search_id = saved_search_id_result[0].getValue("custrecord_saved_search_id");
        if(saved_search_id !=0)
        {
          var mySearch = nlapiLoadSearch(null, saved_search_id);
          mySearch.deleteSearch();
          nlapiLogExecution("Error","Temp Saved Search for match col(Test 1)","Temp Saved Search for match col has been deleted successfully");
        }
      }
      var saved_search_id_desc = create_Saved_Search_Match_Col_DES();
      Search = nlapiLoadSearch(null, saved_search_id_desc);
    }
    else if(datain.match_col_asc_des== "false")
    {
      ...
      if(saved_search_id_result)
      {
        ...
        if(saved_search_id!=0){
          var mySearch = nlapiLoadSearch(null, saved_search_id);
          mySearch.deleteSearch();
          nlapiLogExecution("Error","Temp Saved Search for Match (Test 2)","Temp Saved Search for match col  has been deleted successfully");
        }
      }
      var saved_search_id_asc = create_Saved_Search_Match_Col_ASC();
      Search = nlapiLoadSearch(null, saved_search_id_asc);
    }//End Match Sort
    ...
    else if(datain.customer_col_asc_des == "true")
    {
      ...
      if(saved_search_id_result)
      {
        ...
        if(saved_search_id !=0)
        {
          var mySearch = nlapiLoadSearch(null, saved_search_id);
          mySearch.deleteSearch();
          nlapiLogExecution("Error","Customer Temp Saved Search (Test 1)","Temp Saved Search for Customer has been deleted successfully");
        }
      }
      var saved_search_id_desc = create_Saved_Search_Customer_Col_DES();
      Search = nlapiLoadSearch(null, saved_search_id_desc);
    }
    else if(datain.customer_col_asc_des== "false")
    {
      ...
      if(saved_search_id_result)
      {
        ...
        if(saved_search_id!=0){
          var mySearch = nlapiLoadSearch(null, saved_search_id);
          mySearch.deleteSearch();
          nlapiLogExecution("Error","Temp Saved Search for Customer (Test 2)","Customer Temp Saved Search has been deleted successfully");
        }
      }
      var saved_search_id_asc = create_Saved_Search_Customer_Col_ASC();
      Search = nlapiLoadSearch(null, saved_search_id_asc);
    }//End sort customer
    ...
    else if(datain.order_status_col_asc_des == "true")
    {
      ...
      if(saved_search_id_result)
      {
        ...
        if(saved_search_id !=0)
        {
          var mySearch = nlapiLoadSearch(null, saved_search_id);
          mySearch.deleteSearch();
          nlapiLogExecution("Error","Temp Saved Search for Order Status (Test 1)","Temp Saved Search for Order Status has been deleted successfully");
        }
      }
      var saved_search_id_desc = create_Saved_Search_Order_Status_Col_DES();
      Search = nlapiLoadSearch(null, saved_search_id_desc);
    }
    else if(datain.order_status_col_asc_des== "false")
    {
      ...
      if(saved_search_id_result)
      {
        ...
        if(saved_search_id!=0){
          var mySearch = nlapiLoadSearch(null, saved_search_id);
          mySearch.deleteSearch();
          nlapiLogExecution("Error","Temp Saved Search for Order Status (Test 2)","Temp Saved Search for Order Status has been deleted successfully");
        }
      }
      var saved_search_id_asc = create_Saved_Search_Order_Status_Col_ASC();
      Search = nlapiLoadSearch(null, saved_search_id_asc);
    ...
    else if(datain.item_col_asc_des == "true")
    {
      ...
      if(saved_search_id_result)
      {
        ...
        if(saved_search_id !=0)
        {
          var mySearch = nlapiLoadSearch(null, saved_search_id);
          mySearch.deleteSearch();
          nlapiLogExecution("Error","Temp Saved Search for Item (Test 1)","Temp Saved Search for Item has been deleted successfully");
        }
      }
      var saved_search_id_desc = create_Saved_Search_Item_Col_DES();
      Search = nlapiLoadSearch(null, saved_search_id_desc);
    }
    else if(datain.item_col_asc_des== "false")
    {
      ...
      if(saved_search_id_result)
      {
        ...
        if(saved_search_id!=0){
          var mySearch = nlapiLoadSearch(null, saved_search_id);
          mySearch.deleteSearch();
          nlapiLogExecution("Error","Temp Saved Search for Item (Test 2)","Temp Saved Search for Item has been deleted successfully");
        }
      }
      var saved_search_id_asc = create_Saved_Search_Item_Col_ASC();
      Search = nlapiLoadSearch(null, saved_search_id_asc);
    ...
    else if(datain.amount_col_asc_des == "true")
    {
      ...
      if(saved_search_id_result)
      {
        ...
        if(saved_search_id !=0)
        {
          var mySearch = nlapiLoadSearch(null, saved_search_id);
          mySearch.deleteSearch();
          nlapiLogExecution("Error","Temp Saved Search for Amount (Test 1)","Temp Saved Search for Amount has been deleted successfully");
        }
      }
      var saved_search_id_desc = create_Saved_Search_Amount_Col_DES();
      Search = nlapiLoadSearch(null, saved_search_id_desc);
    }
    else if(datain.amount_col_asc_des== "false")
    {
      ...
      if(saved_search_id_result)
      {
        ...
        if(saved_search_id!=0){
          var mySearch = nlapiLoadSearch(null, saved_search_id);
          mySearch.deleteSearch();
          nlapiLogExecution("Error","Temp Saved Search for Amount (Test 2)","Temp Saved Search for Amount has been deleted successfully");
        }
      }
      var saved_search_id_asc = create_Saved_Search_Amount_Col_ASC();
      Search = nlapiLoadSearch(null, saved_search_id_asc);
...
function create_Saved_Search_Match_Col_ASC()
{
  ...
  var newFilterExpression =[["type","anyof","SalesOrd"],"AND",["status","anyof","SalesOrd:A"],"AND",["custbody111","is","F"],"AND",["custbody227","noneof","1","5"],"AND",["custbody87","noneof","2","3","4","5","20"],"AND",["custbody_backstock_item_match_status","anyof","@NONE@"],"AND",["custbody_percent_paid","isnotempty",""],"AND",[["custbody_pot_backstock_ring_match","is","T"],"OR",["custbody338","is","T"]],"AND",["item.custitem20","anyof","2","3","7","8","14","15","18","20","31"],"AND",["custbody227","anyof","8"]];

  search.setFilterExpression(newFilterExpression);
  search.setIsPublic(true);
  var search_id = search.saveSearch('Backstock Dashboard Script #1230 v3 temp', 'customsearch_backstock_dashboard_v3_temp');
  nlapiLogExecution("Error","Temp Saved Search Id ASC",search_id);
  nlapiSubmitField("customrecord_temp_saved_search",1,"custrecord_saved_search_id",search_id);
...
function create_Saved_Search_Match_Col_DES()
{
  ...
  search.setFilterExpression(newFilterExpression);
  search.setIsPublic(true);
  var search_id = search.saveSearch('Backstock Dashboard Script #1230 v4 temp', 'customsearch_backstock_dashboard_v4_temp');
  nlapiLogExecution("Error","Temp Saved Search Id DESC",search_id);
  nlapiSubmitField("customrecord_temp_saved_search",1,"custrecord_saved_search_id",search_id);
...
function create_Saved_Search_Customer_Col_ASC()
{
  ...
  search.setIsPublic(true);
  var search_id = search.saveSearch('Backstock Dashboard Script #1230 v3 temp', 'customsearch_backstock_dashboard_v3_temp');
  nlapiLogExecution("Error","Temp Saved Search Id ASC for Customer",search_id);
  nlapiSubmitField("customrecord_temp_saved_search",1,"custrecord_saved_search_id",search_id);
...
function create_Saved_Search_Customer_Col_DES()
{
  ...
  search.setIsPublic(true);
  var search_id = search.saveSearch('Backstock Dashboard Script #1230 v4 temp', 'customsearch_backstock_dashboard_v4_temp');
  nlapiLogExecution("Error","Temp Saved Search Id DESC for Customer",search_id);
  nlapiSubmitField("customrecord_temp_saved_search",1,"custrecord_saved_search_id",search_id);
...
function create_Saved_Search_Order_Status_Col_ASC()
{
  ...
  search.setIsPublic(true);
  var search_id = search.saveSearch('Backstock Dashboard Script #1230 v3 temp', 'customsearch_backstock_dashboard_v3_temp');
  nlapiLogExecution("Error","Temp Saved Search Id ASC for Order Status",search_id);
  nlapiSubmitField("customrecord_temp_saved_search",1,"custrecord_saved_search_id",search_id);
...
function create_Saved_Search_Order_Status_Col_DES()
{
  ...
  search.setIsPublic(true);
  var search_id = search.saveSearch('Backstock Dashboard Script #1230 v4 temp', 'customsearch_backstock_dashboard_v4_temp');
  nlapiLogExecution("Error","Temp Saved Search Id DESC for Order Status",search_id);
  nlapiSubmitField("customrecord_temp_saved_search",1,"custrecord_saved_search_id",search_id);
...
function create_Saved_Search_Item_Col_ASC()
{
  ...
  search.setIsPublic(true);
  var search_id = search.saveSearch('Backstock Dashboard Script #1230 v3 temp', 'customsearch_backstock_dashboard_v3_temp');
  nlapiLogExecution("Error","Temp Saved Search Id ASC for Item",search_id);
  nlapiSubmitField("customrecord_temp_saved_search",1,"custrecord_saved_search_id",search_id);
...
function create_Saved_Search_Item_Col_DES()
{
  ...
  search.setIsPublic(true);
  var search_id = search.saveSearch('Backstock Dashboard Script #1230 v4 temp', 'customsearch_backstock_dashboard_v4_temp');
  nlapiLogExecution("Error","Temp Saved Search Id DESC for Item",search_id);
  nlapiSubmitField("customrecord_temp_saved_search",1,"custrecord_saved_search_id",search_id);
...
function create_Saved_Search_Amount_Col_ASC()
{
  ...
  search.setIsPublic(true);
  var search_id = search.saveSearch('Backstock Dashboard Script #1230 v3 temp', 'customsearch_backstock_dashboard_v3_temp');
  nlapiLogExecution("Error","Temp Saved Search Id ASC for Amount",search_id);
  nlapiSubmitField("customrecord_temp_saved_search",1,"custrecord_saved_search_id",search_id);
...
function create_Saved_Search_Amount_Col_DES()
{
  ...
  search.setIsPublic(true);
  var search_id = search.saveSearch('Backstock Dashboard Script #1230 v4 temp', 'customsearch_backstock_dashboard_v4_temp');
  nlapiLogExecution("Error","Temp Saved Search Id DESC for Amount",search_id);
  nlapiSubmitField("customrecord_temp_saved_search",1,"custrecord_saved_search_id",search_id);
