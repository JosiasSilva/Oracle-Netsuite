  ...
  try 
  {
    ...
    if (type == 'create' || type == 'edit' || type == 'xedit') 
    {
      ...
      if(cdpId)
      {
        ...
        nlapiLogExecution("DEBUG","Temp old Percentage Paid Info with %(Change from UI) ",temp_old_paid);
        ...
        nlapiLogExecution("DEBUG","Temp new Percentage Paid Info with % (Change from UI) ",temp_new_paid);
        //% Paid
        if(temp_new_paid!='' && temp_new_paid!=null)
        {
          temp_new_paid=temp_new_paid.split('%')[0];
          nlapiLogExecution("DEBUG","Temp new Percentage Paid Info without %(Change from UI) ",temp_new_paid);
        }
        if(temp_old_paid!='' && temp_old_paid!=null)
        {
          temp_old_paid=temp_old_paid.split('%')[0];
          nlapiLogExecution("DEBUG","Temp old Percentage Paid Info without %(Change from UI) ",temp_old_paid);
        }
        if(temp_new_paid != temp_old_paid)
        {
          new_paid = temp_new_paid;
          nlapiLogExecution("DEBUG","New paid Percentage Paid Info (Change from UI) ",new_paid);
        }
        ...
        {
          // Item Status
          temp_new_item_status = new_obj.getFieldValue('custrecord_item_status');
          temp_old_item_status = old_obj.getFieldValue('custrecord_item_status');
          if(temp_new_item_status != temp_old_item_status)
          {
            old_item_status = temp_old_item_status;
            new_item_status = temp_new_item_status;
            nlapiLogExecution("DEBUG","Old Item Status Info (Change from UI)","Old Item Status is : "+old_item_status);
            nlapiLogExecution("DEBUG","New Item Status Info (Change from UI)","New Item Status is : "+new_item_status);
          }
          // Action Needed
          temp_new_action_needed = new_obj.getFieldValue('custrecord_action_needed');
          temp_new_action_needed = temp_new_action_needed.replace(/\u0005/g,',');
          temp_old_action_needed = old_obj.getFieldValue('custrecord_action_needed');
          temp_old_action_needed = temp_old_action_needed.replace(/\u0005/g,',');
          if(temp_new_action_needed != temp_old_action_needed)
          {
            ...
            nlapiLogExecution("DEBUG","Old Action Needed Info (Change from UI) ","Old Action Needed is : "+old_action_needed);
            nlapiLogExecution("DEBUG","New Action Needed Info (Change from UI)","New Action Needed is : "+new_action_needed);
          }
        }
        if(temp_new_item_status == temp_old_item_status)
        {
          ...
          nlapiLogExecution("DEBUG","Item Status old & new value without Json (Temp) Info from system notes",item_status_old_new_value);
          nlapiLogExecution("DEBUG","Item Status old & new value Json  (Temp) Info from system notes",JSON.stringify(item_status_old_new_value));
          ...
          else 
          {
            ...
            if(request_type ==1)
            {
              ...
              else if(new_paid>20)
              {
                ...
                item_status = 1;			//	Not Confirmed
                action_needed = 2 ; //	 Confirmation Needed
                nlapiLogExecution("DEBUG","New Paid step 27",cdpId);
  ...
  {   
    ...
    {    
      ...
      {  
        ...
        //var url = "http://testportal.brilliantearth.com/api/item/"; // Test portal api
