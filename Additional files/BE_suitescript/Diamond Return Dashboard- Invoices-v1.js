        ...
        try {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                if (results[i].getValue(col[1])) {
                    link = '<a href=' + nlapiLoadFile(results[i].getValue(col[1])).getURL() + ' target="_blank" style="color:blue;" > ' + results[i].getText(col[1]) + '</a>';
                }

                sublist_invoices_Arr.push({
                    custpage_id: results[i].getValue(col[0]),
                    custpage_invoice: link,
                    custpage_item_total: results[i].getValue(col[3]),
                    custpage_vendor: '<a href="/app/common/entity/vendor.nl?id=' + results[i].getValue(col[2]) + '" target="_blank" style="color:blue;" > ' + results[i].getText(col[2]) + '</a>',
