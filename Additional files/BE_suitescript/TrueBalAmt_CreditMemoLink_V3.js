...
function updateCreditMemoLinkAmt(cmlId)
{
	...
	{  
		...
		var creditMemoId=cmlObj.getFieldValue('custrecord_credit_memo_link_cm');
		var customerId=cmlObj.getFieldValue('custrecord_credit_memo_link_customer');
		...
		if(so_currencyId==cm_currencyId)
		{
			...
			var cmlAmount=0;
			//cmlAmount=cm_consolidatebalance;//commented per Rachel feedback on task NS-338
            if(cm_overallbalance!='' && cm_overallbalance!=null)
			{			
				cmlAmount=cm_overallbalance;
			}	
