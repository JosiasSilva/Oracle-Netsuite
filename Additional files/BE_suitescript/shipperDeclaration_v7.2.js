...
function UserEvent(type,form)
{
  try
  {
    ...
    if (type =='view')
    {
      ...
      {
        var url = nlapiResolveURL("SUITELET","customscript_shipper_decl_suitelet","customdeploy_shipper_decl_suitelet");
        url += "&record=" + sId+"&page=home&adds=T";		
        form.addButton("custpage_btn", "Shipper's Declr - OLD", "window.open('" + url + "', 'salesord', 'location=no,menubar=yes,resizable=yes,scrollbars=yes,status=no,titlebar=yes,toolbar=yes,height=600,width=700')");
      }	
    }
  }
  catch(err)
