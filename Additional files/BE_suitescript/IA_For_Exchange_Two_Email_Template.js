function IA_For_Exchange_Two_Email_Template(order,xml,pages,today,currency,email,iaTemp,countgemStoneArr,chkCategory,pageData,isIABlockCode)
{
  var diamondID = "";
  var sapphireID = "";
  var image = "";
  ...
  for(var x=0; x < order.getLineItemCount("item"); x++)
  {
    ...
    if(category=="2")
    {
      ...
      nlapiLogExecution("DEBUG","Gem Stone Count for IA_For_Exchange_Two() Email Template in Original Sales Order",countgemStoneArr.length);
  ...
  nlapiLogExecution("Debug","GemStone Count For Exchange Two Email Template in Original Sales Order(Test)",countgemStoneArr.length);
  nlapiLogExecution("Debug","Check Category Value(T/F) For Exchange Two(Test)",chkCategory);
  ...
  {
    nlapiGetContext().setSetting('SESSION','custpage_hIACode',isIABlockCode);
    nlapiLogExecution("debug","isIABlockCode Set Value:"+isIABlockCode,soId);
    xml += "</body></pdf>";
    return xml;
  }
  else
  ...
  else
  {
    ...
    else
    {
      ...
      errorMsg += "Original Sales Order contains only one gem stone";    
      errorMsg += "</p>";
