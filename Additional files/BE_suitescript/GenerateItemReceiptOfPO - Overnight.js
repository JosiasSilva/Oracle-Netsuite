 ...
 * Script URL : /app/common/scripting/script.nl?id=1061
 * Button URL : /app/site/hosting/scriptlet.nl?script=1061&deploy=1
 * Save Search URL : /app/common/search/searchresults.nl?searchid=4944
 */

//Create a Suitelet Script that generate an Item Receipt
/*function GenerateItemReceipt(request,response)
{
	var form = nlapiCreateForm("Create Item Receipt of POs for Overnight");
	...
	else
	{
		try
		{
			...
			var mySearch = nlapiLoadSearch(null,4727); //Save search id of POs list
			var searchresult = [];
			...
			{ 
				nlapiLogExecution("debug"," Length : ", searchresult.length);	
				var governancePoint = nlapiGetContext().getSetting('SCRIPT','custscript_governance_point');
				governancePoint = (governancePoint!=null && governancePoint!=undefined) ? parseInt(governancePoint) : 0;
				nlapiLogExecution("debug","Reschedule Point is :",governancePoint);
				
				for( var m = governancePoint; m < searchresult.length; m++) 
				{
					var Results = searchresult[m].getAllColumns();
					var Id = searchresult[m].getId();
					CreateItemReceiptOfPO(Id);
					
					//For Rescheduling concept
					if ( context.getRemainingUsage() <= 0 && (m+1) < searchresult.length )
					{
						var params = new Array();
						params['custscript_governance_point'] = i;
...
function GenerateItemReceipt(request, response)
{
	...
	{
                var params = new Array();
                params['custscript_savedsearch_id8'] = 4944; //saved search id
		var result = nlapiScheduleScript('customscript1062', 'customdeploy1',params);  // 1062 is internal id of the scheduled script that you wish to reschedule and 1 is deployment id
