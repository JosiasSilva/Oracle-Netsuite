function Push_To_Portal_Via_WorkFlow(cdpId,type,recordType,contextType) 
{
  try 
  {
    nlapiLogExecution("DEBUG","type (Lib) (WorkFlow)" , type);
    //if (type == 'create' || type == 'edit' || type == 'xedit') 
    //{
    nlapiLogExecution("DEBUG","Cdp Id (Lib) (WorkFlow) " , cdpId);
    nlapiLogExecution("DEBUG","Context Type (Lib) (WorkFlow) " , contextType);
    nlapiLogExecution("DEBUG","Record Type (Lib) (WorkFlow) " , recordType);
    if(cdpId)
    {
      ...
      if(response.code=='200')
      {
        ...
        cdp_obj.setFieldValue('custrecord238',"Work Flow # 2592");
        ...
        /*if(contextType == "workflow" &&  portal_request_type == 3)
        {
          nlapiLogExecution("DEBUG","Portal Request Type", portal_request_type + " having cdp " + cdpId);
          cdp_obj.setFieldValue('custrecord165',portal_request_type);
        }
        else*/
        cdp_obj.setFieldValue('custrecord165',portal_request_type);
        nlapiLogExecution("DEBUG","Portal Request Type (String)", String(portal_request_type) + " having cdp " + cdpId);
        cdp_obj.setFieldValue('custrecord267',String(portal_request_type));
