...
function PushCdpDataNSToPortal(type)
{
    if(type == "create" || type == "edit"  || type == "xedit")	
	{
		try
		{
			...
			{                           
			   ...
			   if(onVendorPortal=='T')
			   {
				...
				{   
					...
					/*--------------------------------------------------Start Added By Yagya 30 July 2016--------------------------------*/
					...
					/*--------------------------------------------------End Added By Yagya 30 July 2016--------------------------------*/
										...
										if(old_ItemStatus == 2) //confirmed
								                {
									              ...
									              SendJsonData(cdpId,vendorId,portalRequestTypeId,actNeedStr,item_status,vendorPortal,manual_override,paymentTerms);
								               }
								               else
								               {
                                                                                     portalRequestTypeId = new_PortalReqType;							
			...
			SetDiamondETA(type); // Added code for set Diamond ETA field value
			nlapiLogExecution('debug','Diamond ETA has been set with calculated/manual data for cdp Id is :',cdpId);
		}
		catch(err)
		{
...
function SendJsonData(cdpId,vendorId,portalRequestTypeId,actNeedStr,item_status,vendorPortal,manual_override,paymentTerms)
{
...
function SetDiamondETA(type)
{
    try
    {
		...
		if(type == "create" || type == "edit" || type == "xedit")
		{
			...
			nlapiLogExecution("debug","Item Status is :",itemStatus);
                        var old_itemStatus = nlapiGetOldRecord().getFieldValue("custrecord_item_status");
			var new_itemStatus = nlapiGetNewRecord().getFieldValue("custrecord_item_status");
                        nlapiLogExecution("debug","Old Item Status is :",old_itemStatus);
			nlapiLogExecution("debug","New Item Status is :",new_itemStatus);
