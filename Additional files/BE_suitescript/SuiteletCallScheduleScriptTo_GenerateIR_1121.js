...
function CreateItemReceipt()
{
	try
	{
	    ...
	    var searchId = nlapiGetContext().getSetting('SCRIPT','custscript_savedsearch_id_Forgeous');
	    ...
	    if ( searchresults == null )
	     ...
	     var reschedulePoint = nlapiGetContext().getSetting('SCRIPT','custscript_reschedule_point_forgeous');
	    ...
	    for ( var i = reschedulePoint; i < searchresults.length; i++ )
	    {
		    ...
		    if ( context.getRemainingUsage() <= 0 && (i+1) < searchresults.length )
		    {
                                ...
                                params['custscript_reschedule_point_forgeous'] = i;
