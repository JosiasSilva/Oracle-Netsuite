...
function UpdateInvoiceRecord(datain) {
  ...
  nlapiLogExecution('DEBUG', "939 Portal json data is : ", JSON.stringify(datain));
  ...
  try {
    if (datain != null) {
      ...
      // START HERE Added new logic as per DP-481
      var fileIdStr = "";
      ...
      nlapiLogExecution("debug", "file str is :", fileIdStr);
      var ret_cdp_len = Check_Cdp_Arr_Len(datain);
      nlapiLogExecution("debug", "No of CDP :", ret_cdp_len);
      if(ret_cdp_len == 0)
      {
        err.status = "failed";
      ...
      else if(ret_cdp_len>=1)
      {
        // create a custom record to store datain data,invoice date,fileIdArr for using in schedule script
        //create search for above custom record
        Create_CR_For_Limit_Exceeded(datain,fileIdStr,invoiceDate);
        nlapiLogExecution("debug", "Information", "Data will be process shortly by Schedule Script");
        err.status = "OK";
        err.message = "Data will be process shortly";
        return err;
        // create a custom record to store error log 
        // create save search for error log
      }
      // END HERE
  ...
  /*var val = 0;
    switch (chk_mnth) {
        case "0":
            ...
            break;
        case "1":
            val = "217"; //Changed by Eric 01/05/2018
            break;
        case "2":
            val = "200";
            break;
...
// Added new function for updating action needed field value on 23 apr 2018 for [DP-426]
...
function Check_Cdp_Arr_Len(datain)
{
  if (datain.cdp_data != null) {
    var count = 0;
    for (var i = 0; i < datain.cdp_data.length; i++) {
      nlapiLogExecution("debug", "CDP Id in Check_Cdp_Arr_Len() fun is :", datain.cdp_data[i].cdp_id);
      if (!datain.cdp_data[i].cdp_id) {
        return 0;  
      }
      count++;
...
function Create_CR_For_Limit_Exceeded(datain,fileIdArr,invoiceDate)
{
  try
  {
    var crObj = nlapiCreateRecord('customrecord_cr_for_limit_exceeded');
    crObj.setFieldValue('custrecord_datain_data',JSON.stringify(datain));
    crObj.setFieldValue('custrecord_datain_invoice_date',invoiceDate);
    crObj.setFieldValue('custrecord_datain_file_id',fileIdArr);
    nlapiSubmitRecord(crObj);
    nlapiLogExecution("debug", "Custom Record Information for Limit Exceeded","Record has been created Successfully");
  }
  catch(ex)
  {
    nlapiLogExecution("debug", "Error Message",ex.message);
  }
}