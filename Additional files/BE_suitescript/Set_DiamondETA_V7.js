					...
					if(vendorId != '')
					{
						...
						if(businessDays != '')
						{
							if(deadLine != null)
							{
								...
								{						
									if(shipDays != null)
									{
										...
										if(count == 1 && (shipDaysArr[0] == 1 || shipDaysArr[0] == ''))  // N/A condition
										{
                                            ...
                                            //Added by ajay 18Jan 2017
											diamondETA = nlapiAddDays(diamondETA,1);
											diamondETA = nearest_business_day(diamondETA,day);
											//Ended by ajay 18Jan 2017
                                ...
                                // Adding by ajay Holiday Table logic as per NS-235 on 21Dec 2016
								var location_holi = Check_All_Location(nlapiDateToString(diamondETA));
								if(!location_holi)
								{
									newDay = diamondETA.getDay();																	
									if(newDay <= 4)
									...
									nlapiSubmitField("customrecord_custom_diamond",Id,"custrecord_diamond_eta",nlapiDateToString(diamondETA));
									nlapiSubmitField("customrecord_custom_diamond",Id,"custrecord_date_expected_at_be",nlapiDateToString(diamondETA));
									nlapiLogExecution(log_type,"Diamond ETA Field has been set with calculated data after filter through holiday table is .", nlapiDateToString(diamondETA));
									nlapiLogExecution('debug','Today is Holiday Table with  All location','The script has been prevented to execute for all location');
								}
								else
								{
									nlapiSubmitField("customrecord_custom_diamond",Id,"custrecord_diamond_eta",nlapiDateToString(diamondETA));
		    ...
		    else
		    {
				 if(type != "xedit")
				 {
					  nlapiSubmitField(nlapiGetRecordType(),Id,"custrecord_diamond_eta",new_diamond_eta);
                   	  nlapiSubmitField(nlapiGetRecordType(),Id,"custrecord_date_expected_at_be",new_diamond_eta);
...
//Added by ajay 18Jan 2017
function nearest_business_day(day,currentDay) {
...
function GetDiamondETA(day,businessDays,currentDate)
{
 ...
 // Adding Holiday Table logic as per NS-235 by Rahul Panchal
function Check_All_Location(dia_eta_date)
{

 var secrh_holiday=  nlapiSearchRecord('customrecordholiday_table_child',null,[new nlobjSearchFilter ('custrecord_holiday_date' ,null, 'on' ,dia_eta_date )],[new nlobjSearchColumn ( 'custrecord_location' , 'custrecord_holiday_table_join_2' ),new nlobjSearchColumn ( 'custrecord_all_location')] );
var location_holi=new Array();
if(secrh_holiday!=null)
  {
     var check_all_location_T= secrh_holiday[0].getValue('custrecord_all_location')  
    if(check_all_location_T=='T')
    {
        return false;
    }
    for(var holi=0;holi<secrh_holiday.length;holi++)
    {
        location_holi.push(secrh_holiday[holi].getValue('custrecord_location' , 'custrecord_holiday_table_join_2'));
    }   
  }
  
  return location_holi;
