...
function repairPage(request,response)
{
	if(request.getMethod()=="GET")
	{
		try
		{
			...
			fld.setDefaultValue(soRec.getFieldText("shipcountry"));
	...
	else
	{
		try
		{
			...
			soRec.setFieldValue("tranid",orderNumber + "RP");
			...
			for(var x=0; x < originalOrder.getLineItemCount("item"); x++)
			{
				if(originalOrder.getLineItemValue("item","itemtype",x+1)=="InvtPart")
				{
					...
					{
						originalOrder.setLineItemValue("item","custcol_full_insurance_value",x+1,parseFloat(originalOrder.getLineItemValue("item","amount",x+1)));
			...
			if(!updateAddress && zipcode != soRec.getFieldValue("shipzip"))
				updateAddress = true;
			if(!updateAddress && country != soRec.getFieldText("shipcountry"))
				updateAddress = true;
			...
			if(updateAddress)
			{
				...
				var shipaddress = "";
				shipaddress += soRec.getFieldValue("shipaddressee")+ "\n";
