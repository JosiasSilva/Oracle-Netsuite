function bindPOslist(form, obj_result, number_of_result) {
	...
	sublistPOs.addField('custpage_highlight_po', 'textarea', 'Highlight').setDisplayType('hidden');//Priti
    ...
    if (results) {
        ...
        try {
            ...
            for (var i = 0; i < results.length; i++) {
				var purchaseOrderId=results[i].getId();
				var messageSearch = nlapiSearchRecord("message",null,[["transaction.internalid","anyof",purchaseOrderId],"AND",["transaction.mainline","is","T"]]);
				var Count=0;
				if(messageSearch)
				{
					Count=messageSearch.length;
				}
				var color;
				if(Count>=1){
					color="green";
