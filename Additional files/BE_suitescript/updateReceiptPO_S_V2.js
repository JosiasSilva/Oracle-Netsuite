...
function updateReceiptPO(request, response) {
    ...
    if (requestData_Multi != null) {
        ...
        if (requestData != null) {
			...
			var record_mode=request.getParameter('type');
			nlapiLogExecution ('debug','record_mode', record_mode );
			if(record_mode=='edit')
			{
				try {
                //var poID = PurchaseOrder[0].ID;
					...
					try {
						//Added by ajay 08May 2017
						var receipt = null;
						// Added By Rahul Panchal on dated 19/7/2017 as per NS-796
						var PO_DROP_SHIP_MATERIALS=nlapiLookupField('purchaseorder',poID,'custbody39');
						//nlapiLogExecution("debug","test dd21",PO_DROP_SHIP_MATERIALS);
						var ven_inv_loc='';
						if(PO_DROP_SHIP_MATERIALS!='')
						{
							//nlapiLogExecution("debug","test dd22");
							var vendorId=nlapiLookupField('purchaseorder',poID,'entity');
							ven_inv_loc=nlapiLookupField("vendor",vendorId,"custentity138");
							//nlapiLogExecution("debug","test dd23",ven_inv_loc);
						}
						else
						{
							ven_inv_loc=2;
							//nlapiLogExecution("debug","test dd24",ven_inv_loc);
						}
						...
						if (k == 0) {
							...
							UpdateItemReceipt(irId,ven_inv_loc);
						}
						//Ended by ajay 08May 2017
					} catch (sdaa) {
			...
			else if(record_mode=='view')
			{
				...
				{
					nlapiLogExecution("debug","Error occuring in View Mode",eee.message);
				}
				nlapiSetRedirectURL('RECORD', 'purchaseorder' ,poID,false,null);
			}
...
function UpdateItemReceipt(irId,ven_inv_loc) {
