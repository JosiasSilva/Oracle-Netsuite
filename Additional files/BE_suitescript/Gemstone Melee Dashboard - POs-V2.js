    ...
    if (results) {
        ...
        try {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                sublistPOs_Arr.push({
                    custpage_item: "<a href='/app/common/item/item.nl?id="+results[i].getId()+"' target='_blank' style='color:blue'>"+results[i].getValue(col[0])+"</a>",
                    custpage_item_id: results[i].getId(),
                    custpage_po_document_no: "<a href='/app/accounting/transactions/purchord.nl?id="+results[i].getValue(col[2])+"' target='_blank' style='color:blue' >"+results[i].getValue(col[1])+"</a>",
                    custpage_po_id: results[i].getValue(col[2]),
                  custpage_date_created:results[i].getValue(col[3]),
                  custpage_item_desc:results[i].getValue(col[4])
                 });
            }
        } catch (er) {
