...
var location_id_obj=['','2','4','11'];
var location_text_obj=['','San Francisco', 'Item Sent to SF (Ops Only Status)','Denver'];
...
function Update_Team(type,name)
{
  if(name=='custpage_team')
  {
    ...
    jQuery.ajax({
      type: 'POST',
      url: '/app/site/hosting/scriptlet.nl?script=1169&deploy=1&data_save=T',
      data: send_data,
      dataType: "text",
...
function Update_Order_Post_Sale_Intake()
{
  ...
  for(var z=1;z<=nlapiGetLineItemCount('custpage_post_sale_intake');z++)
  {
	...
	var customer=nlapiGetLineItemValue('custpage_post_sale_intake','custpage_entityid_post_sales',z);
	var tranid=nlapiGetLineItemValue('custpage_post_sale_intake','custpage_tranid_post_sales',z);
    ...
    {
      //Determine which post-sale intake label(s) to print
	  var or_items_label = false;
	  var status_label = false;
	  var undelivered_label = false;
	  
	  if(custpage_undelivered=='T')
	  {
	  	undelivered_label = true;
	  }
	  
	  if(custbody142!=custbody142_old)
	  {
	  	status_label = true;
	  }
	  
	  if(custpage_select_materials_id_old!=custpage_select_materials_id)
	  {
	  	or_items_label = true;
	  }
      ...
      data_update.push({
		...
		or_items_label:or_items_label,
		or_items:custpage_select_materials_id,
		status_label:status_label,
		undelivered_label:undelivered_label,
		customer:customer,
		tranid:tranid
      });
...
function Call_Ajax_data_update(data_update,sublist)
{
  for(var c=0;c<data_update.length;c++)
  {
	...
	//Handle post-sale intake stickers
	var zpl = '';
	
	for(var c=0; c < data_update.length; c++)
	{
		var reasonForSendingBack = nlapiLookupField("transaction",data_update[c].order_id,"custbody35");
		
		if(data_update[c].or_items_label===true && reasonForSendingBack!="2" && reasonForSendingBack!="28")
		{
			var cleaner = false, cert = false, promo = false, antique = false, work = false;
			
			//alert("OR Items: " + data_update[c].or_items);
			
			if(data_update[c].or_items)
			{
				var temp_custpage_select_materials_id = data_update[c].or_items.split(',');
			
				for(var p=0; p < temp_custpage_select_materials_id.length; p++)
				{
					if(temp_custpage_select_materials_id[p]=='5')
						cleaner = true;
					if(temp_custpage_select_materials_id[p]=='7')
						cert = true;
					if(temp_custpage_select_materials_id[p]=='14')
						promo = true;
					if(temp_custpage_select_materials_id[p]=='13')
						antique = true;
					if(temp_custpage_select_materials_id[p]=='12')
						work = true;
				}
			}
			...
			zpl+= '^XA\n';
			zpl+= '^CF0,40\n';
			zpl+= '^FO20,40^FD' + data_update[c].tranid + '^FS\n';
			zpl+= '^FO20,90^FD' + data_update[c].customer + '^FS\n';
			zpl+= '^CFF,30\n';
			if(cleaner==true)
				zpl+= '^FO20,140^FDCleaner^FS\n';
			if(cert==true)
				zpl+= '^FO20,180^FDCert^FS\n';
			if(promo==true)
				zpl+= '^FO20,220^FDPromotional Item^FS\n';
			if(antique==true)
				zpl+= '^FO20,260^FDAntique Appraisal^FS\n';
			if(work==true)
				zpl+= '^FO20,300^FDWork and Release of Liability^FS\n';
			zpl+= '^XZ';
		}
	
		if(data_update[c].status_label===true)
		{
			zpl+= '^XA\n';
			zpl+= '^CF0,40\n';
			zpl+= '^FO20,40^FDVendor:^FS\n';
			zpl+= '^FO20,100^FDPO#:^FS\n';
			zpl+= '^CF0,40\n';
			zpl+= '^FO20,160^FD' + data_update[c].tranid + '^FS\n';
			zpl+= '^XZ';
		}
	
		if(data_update[c].undelivered_label===true)
		{
			zpl+= '^XA\n';
			zpl+= '^CF0,40\n';
			zpl+= '^FO20,40^FD' + data_update[c].tranid + '^FS\n';
  	...
  	if(zpl!='')
	{
		...
		zplRecord.setFieldValue("custrecord_zpl_filename",data_update[0].tranid + ".zpl");
		nlapiLogExecution("debug","File Name",data_update[0].tranid + ".zpl");
  ...
  jQuery.ajax({
    ...
    success: function(msgesem) {
      ...
      if(data_send_obj.length==temp_data_obj.length)
      {
        ...
        else if(sublist=='custpage_all_packages_parrent' || sublist=='custpage_all_packages_rec_drop_off' )
        {
          flag=false;
        ...
        {
          var url_hit='/app/site/hosting/scriptlet.nl?script=1169&deploy=1&r=T&machine=custpage_all_packages';
          window.opener.document.getElementById('server_commands').src=url_hit;
        }
...
function Update_Bulk_Custom_Package_Record()
{
  var data_update=new Array();
  temp_data_obj=new Array();
  var get_team=nlapiGetFieldValue('custpage_team_bulk');
  if(get_team)
  {
    for(var a=1;a<=nlapiGetLineItemCount('custpage_bulk_package');a++)
    {

      var tracking_number=nlapiGetLineItemValue('custpage_bulk_package', 'custpage_tracking_number', a);
      if(tracking_number)
      {
        temp_data_obj.push('');
        data_update.push({
    ...
    if(temp_data_obj.length>0)
    {
      ...
      Call_Ajax_data_update(data_update,'custpage_all_packages_parrent');
    }
  }
}
//Bulk Receive Drop off - NS-1344
function UpdateRecordBulkRecDropOff()
{
  var data_update=new Array();
  temp_data_obj=new Array();
  
    for(var a=1;a<=nlapiGetLineItemCount('custpage_bulk_rec_drop_off');a++)
    {

      var transaction_number=nlapiGetLineItemValue('custpage_bulk_rec_drop_off', 'custpage_transaction_number', a);
    ...
    {
      document.getElementById("div__body").innerHTML = "Please wait..";
      Call_Ajax_data_update(data_update,'custpage_all_packages_rec_drop_off');
    }
