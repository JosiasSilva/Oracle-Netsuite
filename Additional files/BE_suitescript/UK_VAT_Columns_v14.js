function UK_VAT_Columns(type)
{
	if(type=="create" || type=="edit")
	{
		try
		{
			...
			var shipCountry = order.getFieldValue("shipcountry");
			nlapiLogExecution("debug","Ship Country",shipCountry);
			
			if(currency=="2") //GBP orders only
			{
				var totalPrice = 0;
				var totalVAT = 0;
				var totalDuty = 0;
				...
				{
					if(order.getLineItemValue("item","itemtype",x+1)=="Discount")
						continue;
					...
					if(amount==0.00)
					{
						...
						//var qb = parseFloat(nlapiGetCurrentLineItemValue("item","quantitybackordered"));
						//nlapiLogExecution("debug","qb",qb);
						//if(qb!=null && qb!="" && qb!="NaN" && !isNaN(qb) && isFinite(qb))
						//	nlapiSetCurrentLineItemValue("item","quantitybackordered",qb.toFixed(5));
						nlapiSetCurrentLineItemValue("item","custcol_item_price",0.00);
						nlapiSetCurrentLineItemValue("item","custcol_vat_amount",0.00);
						nlapiSetCurrentLineItemValue("item","custcol_duty_amount",0.00);
					...
					if(itemPrice!=null && itemPrice!="" && itemPrice!=0)
					{
						...
						//var qb = parseFloat(nlapiGetCurrentLineItemValue("item","quantitybackordered"));
						//nlapiLogExecution("debug","qb",qb);
						//if(qb!=null && qb!="" && !isNaN(qb) && isFinite(qb))
						//	nlapiSetCurrentLineItemValue("item","quantitybackordered",qb.toFixed(5));
			...
			else if(currency!="2" && shipCountry=="GB")
			{
				for(var x=0; x < order.getLineItemCount("item"); x++)
				{
					nlapiSelectLineItem("item",x+1);
					...
					//if(qb!=null && qb!="" && qb!="NaN" && !isNaN(qb) && isFinite(qb))
					//	nlapiSetCurrentLineItemValue("item","quantitybackordered",qb.toFixed(5));
					nlapiSetCurrentLineItemValue("item","custcol_item_price",nlapiGetCurrentLineItemValue("item","rate"));
					nlapiCommitLineItem("item");
				}
			}
		}
