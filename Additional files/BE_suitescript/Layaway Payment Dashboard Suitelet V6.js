...
function Layaway_Data(month, date11) {
    ...
    try {
        ...
        var user = nlapiGetUser();
        // nlapiLogExecution('DEBUG', 'user#', user);
       var DepositArr2= request.getParameter("DepositArr");
       nlapiLogExecution("Debug", "DepositArr2", DepositArr2);
        var filters = new Array();
        filters[0] = new nlobjSearchFilter('custrecord_user', null, 'is', user);
        if (month != '' && month != null) {
            filters[1] = new nlobjSearchFilter('custrecord_monthlypaymentdate', null, 'is', month);
        ...
        if (DepositArr2 !='' && DepositArr2!=null){
          filters[2] = new nlobjSearchFilter('custrecord_jsondata', null, 'is', DepositArr2);
          isdep=2;
        }
