		...
		try
		{
			if(datain.cdp_data != null)
			{
				for(var i=0; i < datain.cdp_data.length; i++)
				{
					...
					var TempData = cdpObj.getFieldValue("custrecord_temp_data");
					var TempDate = nlapiDateToString(new Date(), 'datetime');
					TempData = TempData +'Portal push Data ('+TempDate +')<br>'+JSON.stringify(datain)+'<br><br>';
					cdpObj.setFieldValue("custrecord_temp_data",TempData);
...
function PushPaymentToPortal(cdpId)
{
	...
	var TempData = nlapiLookupField('customrecord_custom_diamond',cdpId,'custrecord_temp_data');
	var TempDate = nlapiDateToString(new Date(), 'datetime');
	TempData = TempData +'Portal Resuest Data ('+TempDate +')<br>'+myJSONText+'<br><br>';
	nlapiSubmitField('customrecord_custom_diamond',cdpId,'custrecord_temp_data', TempData);
	...
	if(response.code=='200')
	{
		...
		var TempData = nlapiLookupField('customrecord_custom_diamond',cdpId,'custrecord_temp_data');
		var TempDate = nlapiDateToString(new Date(), 'datetime');
		TempData = TempData +'Portal Resuest Data ('+TempDate +')<br>'+JSON.stringify(response)+'<br><br>';
		nlapiSubmitField('customrecord_custom_diamond',cdpId,['custrecord_action_needed','custrecord_temp_data'], [retData,TempData]);       
