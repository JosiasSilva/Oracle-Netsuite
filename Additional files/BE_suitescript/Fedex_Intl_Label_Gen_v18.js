...
function Gen_Fedex_Label(shipId,autoIF)
{
	...
	cols.push(new nlobjSearchColumn("custbody_fedex_location_type"));
	...
	if(results)
	{
		...
		for(var x=0; x < results.length; x++)
		{
			...
			else if(results[x].getValue("isspecialorderitem","item")=="T" && (results[x].getValue("custcol_category")=="4" || results[x].getValue("custcol_category")=="5" || results[x].getValue("custcol_category")=="6"))
			{
			...
			else if((results[x].getValue("custbody87")=="4" || results[x].getValue("custbodyloose_gem_order")=="T") && results[x].getText("custcol_category").substring(0,5)=="Loose")
			{
				//Loose Gem Orders
			...
			else if(postSale==true && results[x].getValue(amountField)==0.00 && results[x].getValue("custcol_category")!="12" && results[x].getValue("custcol_category")!="1" && results[x].getValue("custcol_category")!="23" && results[x].getValue("custcol_category")!="30")
			{
			...
			else if(results[x].getValue(amountField)!=0.00 && results[x].getValue("custcol_category")!="12" && results[x].getValue("custcol_category")!="1" && results[x].getValue("custcol_category")!="23" && results[x].getValue("custcol_category")!="30")
			{
				...
				if(results[x].getText("custcol_category").indexOf("Loose")==-1)
				{
					...
					nlapiLogExecution("debug","Category",results[x].getText("custcol_category"));
					
					if(results[x].getValue("custcol_category")=="21")
						continue;
			...
			else if(results[x].getValue(amountField)==0.00 && results[x].getValue("custcol_category")!="12" && results[x].getValue("custcol_category")!="1" && results[x].getValue("custcol_category")!="23" && results[x].getValue("custcol_category")!="30")
			{
				...
				if(results[x].getText("custcol_category").indexOf("Loose")==-1 && results[x].getValue("item")!="53547")
				{
					//All other Loose categories...
					items.push({
						description : results[x].getValue("memo"),
			...
			else if(results[x].getValue("custitem_ci_fee","item")=="T" && results[x].getValue("amount") > 0)
			{
				//CI Fee/Discount
				items.push({
					description : results[x].getValue("memo"),
	...
	var fedexLocationType = results[0].getValue("custbody_fedex_location_type");
