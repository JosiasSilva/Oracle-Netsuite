function salesRepMetricsGraph(portlet, column)
{
	...
	try
	{
		...
		if(monthIndex.toString().length==1)
			monthFilter = "0" + monthIndex.toString()
		else
			monthFilter = monthIndex.toString()
		...
		filters[0] = new nlobjSearchFilter("formulatext", null, "is", monthFilter)
		filters[0].setFormula("TO_CHAR({datecreated},'mm')")
		nlapiLogExecution("error", "DEBUG", "Month Filter: " + monthFilter)
		if(repDefined)
		{
