    		...
    		bccFld.updateBreakType({breakType: serverWidget.FieldBreakType.STARTROW});
             ...
             var hidenReplyOpField= form.addField({id:'custpage_replyop_click',type:serverWidget.FieldType.TEXT,label: 'hdnReplyOp'})
    		   ...
    		   if(customerEmail)
				...
				rec_frm_customer_email.defaultValue = search.lookupFields({type:search.Type.SALES_ORDER,id:recordid,columns:['entity']}).value;
				}
...
nlapiCommitLineItem ( "custpage_attachment_sublist" ); }}}</script>';
    	...
    	else
    	{
          ...
          paramsForSuitelet.custparam_flag_reply = flag_for_reply_op;
           ...
           paramsForSuitelet.custparam_recipient = recp_val;
    		    ...
    		    var cc=request.parameters.custpage_cc ;
    		    var bcc=request.parameters.custpage_bcc;
    		    if(!bcc)
    		      bcc=null;
    		    if(!cc)
    		      cc=null;
      ...
      entitySearchObj.run().each(function(result){
		...
		if (temp_rec_val_frm_search !== null && temp_rec_val_frm_search !== undefined && temp_rec_val_frm_search !== ''){
          email.send({author:25266151,recipients:temp_rec_val_frm_search,subject:subject,body:body,cc:cc,bcc:bcc,relatedRecords:records,attachments:filefromCabinet});
		}
		else {
			email.send({author:25266151,recipients:temp_rec_val_frm_email,subject:subject,body:body,cc:cc,bcc:bcc,relatedRecords:records,attachments:filefromCabinet});
		}
    		     ...
    		     //var htmlHeader = form.addField({id:'custpage_popup_close', type:serverWidget.FieldType.INLINEHTML,label:'Inline'});
    		      hdnSendAjaxCall.defaultValue = '<input id="prodId" name="prodId" type="hidden" value="https://666639-sb1.app.netsuite.com/app/site/hosting/scriptlet.nl?'+request.parameters.entryformquerystring+'"><script>'+
