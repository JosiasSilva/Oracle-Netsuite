...
function SetDiamondETAFromSalesOrder(cdpId,type)
{
  try
  {
    var Id = cdpId;
    ...
    nlapiLogExecution('Debug', 'Action Needed value is : ', actionNeeded);
    ...
    nlapiLogExecution("debug","Action Needed Array is :", JSON.stringify(arr_actionNeeded));
    ...
    {
      nlapiLogExecution('Debug', 'Diamond ETA', 'Entering IF');
      if(new_portal_request_type == 3 || new_portal_request_type == 4 || portalReqType == 3 || portalReqType == 4)		//New Codition					
      {
        nlapiLogExecution('Debug', 'Portal Request Type', 'Entering IF');
        if((old_diamond_status != 1 && new_diamond_status == 1 && (new_portal_request_type ==3 || new_portal_request_type ==4 || portalReqType == 3 || portalReqType == 4)) || (old_item_status != 2 && new_item_status == 2 && itemStatus == 2  && old_diamond_status != new_diamond_status && (new_portal_request_type == 3 || new_portal_request_type==4 || portalReqType == 3 || portalReqType == 4)))	//Changed New condition	
        {
          nlapiLogExecution('Debug', 'Diamond Status & Portal Request Type', 'Entering IF');
          if(actionNeededBolVal == true)
          {
            nlapiLogExecution('Debug', 'Action Needed', 'Entering IF');
            var vendorId = nlapiLookupField("customrecord_custom_diamond",Id,"custrecord_custom_diamond_vendor");
          ...
          }   // End if condition for Action Needed Value            
        }// End of if condition for Diamond Status & Portal Request Type
      } // End Request Type
