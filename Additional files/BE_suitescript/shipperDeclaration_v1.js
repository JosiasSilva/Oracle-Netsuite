 ...
 * SB URL-1: https://system.sandbox.netsuite.com/app/common/scripting/script.nl?id=990
 * SB URL-2: https://system.sandbox.netsuite.com/app/common/scripting/script.nl?id=991 
*/
...
function UserEvent(type,form)
{
	try
	{
		...
		if (type =='view')
		{
			...
			url += "&record=" + sId;							
            form.addButton("custpage_btn", "Shipper's Declr", "window.open('" + url + "', 'salesord', 'location=no,menubar=yes,resizable=yes,scrollbars=yes,status=no,titlebar=yes,toolbar=yes,height=500,width=700')");
		}		
	}
	catch(err)
	{
...
function ShippersDeclaration_BySuitelet(request,response)
{
	...
	//nlapiLogExecution("debug","SO Id is :",sId);
    if(request.getMethod() == "GET")
	{
		try
		{
			var form = nlapiCreateForm('Items Returning to BE');
			var fld = form.addField('custpage_sofield', 'text', 'Record Id');				
			fld.setDisplayType('hidden');                                
			fld.setDefaultValue(sId);
			...
			//sublist.addField("sublist_check","checkbox"," "); //Returning to BE Checkbox
...
function pdfView(lines,request,response,orderId)
{
	try
	{
		...
		var soFields = ['entity', 'currency','billaddress', 'shipaddress','custbodyloose_gem_order','custbody110'];
	 ...
	 ///End Adds
	   ...
	   itemArr1.sort(function(a, b) {
		...
		if(isCheck_wbo=='T')
		{			
			htmlStr +="<table border='0' align='center' cellpadding='2' cellspacing='1' width='500' style='font-family:Arial, Helvetica, sans-serif'><tr><td width='30px'><input type='checkbox' id='chk_wto' name='chk_wto' checked='1' style='width:25px; height:25px;'/></td>";//" text='The goods did not conform to contract specifications given by the importer to the manufacture or supplier, or because of a fault or defect in the goods.'/>";
			htmlStr +="<td class='pt10' >The goods did not conform to contract specifications given by the importer to the manufacture or supplier, or because of a fault or defect in the goods.</td>";
			htmlStr +="</tr></table>";
		}
		else
		{
			htmlStr +="<table border='0' align='center'  cellpadding='2' cellspacing='1' width='500' style='font-family:Arial, Helvetica, sans-serif'><tr><td width='30px'><input type='checkbox' id='chk_wto' name='chk_wto' style='width:25px; height:25px;'/></td>";//" text='The goods did not conform to contract specifications given by the importer to the manufacture or supplier, or because of a fault or defect in the goods.'/>";
			htmlStr +="<td class='pt10'>The goods did not conform to contract specifications given by the importer to the manufacture or supplier, or because of a fault or defect in the goods.</td>";
			htmlStr +="</tr></table>";			
		}
		...
		htmlStr +="<tr><td colspan='3'></td></tr>";
		htmlStr +="<tr><td width='225'></td><td align='left' colspan='2' class='pt10'>Company: Brilliant Earth</td></tr>";
		htmlStr +="<tr><td colspan='3'></td></tr>";	
		htmlStr +="<tr><td colspan='3'></td></tr>";		
		htmlStr +="</table>";
		...
		//file.setFolder(7975400); 7671491
