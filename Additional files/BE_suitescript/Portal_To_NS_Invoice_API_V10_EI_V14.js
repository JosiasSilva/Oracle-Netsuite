        ...
        if (pos.length > 0 && flag == 0) {
          ...
          if (results) {
            ...
            if (count == results.length) //Added by ajay
            {
              ...
              {
                nlapiLogExecution("debug","Script Remaining Usage before creating vendor bill",nlapiGetContext().getRemainingUsage());
                ...
                if(nlapiGetContext().getRemainingUsage()>=20)
                {
                  ...
                  nlapiLogExecution("debug","Script Remaining Usage after creating vendor bill",nlapiGetContext().getRemainingUsage());
                  ...
                  if (id > 0) {
                    ...
                    for (var f = 0; f < cdp_item_map.length; f++) {
                      ...
                      nlapiLogExecution("debug","Script Remaining Usage before updating cdp :" + cdp_item_map[f],nlapiGetContext().getRemainingUsage());
                      if(nlapiGetContext().getRemainingUsage()>=10)
                      {
                        ...
                        nlapiLogExecution("debug","Script Remaining Usage after updating cdp with action needed :" + cdp_item_map[f],nlapiGetContext().getRemainingUsage());
                      }
                      else
                      {
                        nlapiLogExecution("debug","Script Remaining Usage in else block (1)",nlapiGetContext().getRemainingUsage());
                        nlapiLogExecution("debug","Script Remaining Usage Info (1)","Script Remaining Usage has been finished");
                        jsonobj = {
                          "status": "failed",
                          "message": "Script Remaining Usage has been finished 1"
                        }
                        return jsonobj;
                      }
                    }
                  } //End submit record condition	
                }// End if part script remaining  usage
                else
                {
                  nlapiLogExecution("debug","Script Remaining Usage in else block (2)",nlapiGetContext().getRemainingUsage());
                  nlapiLogExecution("debug","Script Remaining Usage Info (2)","Script Remaining Usage has been finished");
                  jsonobj = {
                    "status": "failed",
                    "message": "Script Remaining Usage has been finished 2"
                  }
                  return jsonobj;
                }
