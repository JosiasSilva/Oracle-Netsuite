...
{	
	...
	cols.push(new nlobjSearchColumn("custbody87","createdfrom")); //Order Type
	...
	if(results)
	{
		...
		//If IF drop ship field is null, use IF date as ship date on label
		if(results[0].getValue("custbody39")==null || results[0].getValue("custbody39")=="")
		{
			nlapiLogExecution("debug","Using Item Fulfillment Date as Ship Date");
			shipDate = nlapiStringToDate(results[0].getValue("trandate"));
		}
		...
		var shipService;
		nlapiLogExecution("debug","Shipping Method from IF",results[0].getValue("shipmethod"));
		...
		if(results[0].getValue("custbody87","createdfrom")=="1" && insurance <= 250)
			labelObj.dsr = "";
