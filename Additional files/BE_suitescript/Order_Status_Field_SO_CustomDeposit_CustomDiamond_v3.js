...
function customDeposit_update_SO_OrderStatus(){
	...
	//Added by Rahul Panchal as Per NS-887 on dated 13/10/2017
	var cd_payment_method=customDeposit.getFieldValue('paymentmethod');
    if(cd_payment_method!=3){
   ...
   /* nlapiLogExecution('DEBUG', 'Setting to be added later', 'Setting to be added later: ' + setting_to_be_added_later);
    ...
    for(var i = 1;i <= relationRecordCount;i++){
        ...
        if(curr_item_type == "Customer Deposit"){
			var curr_item_internalid = salesOrder.getLineItemValue('links','id',i);
            var curr_item_total = getRealAmount(curr_item_internalid);//reference from Global_Config_v2.js
