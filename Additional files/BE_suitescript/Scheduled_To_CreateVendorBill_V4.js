	...
	try
	{
	   var internalid=1;
		do
		{
			...
			filter[1]=nlobjSearchFilter("internalidnumber",null,'greaterthan',internalid);
			var column = [];
			column[0] = new nlobjSearchColumn("custrecord_portal_invoice_no");	
            column[1] = new nlobjSearchColumn("internalid").setSort(true);				
			var Records = nlapiSearchRecord("customrecord_pending_inv_for_cdp",null,filter,column);
			if(Records != null)
			{
				for(var i=0; i< Records.length; i++)
				{
					internalid=Records[i].getId()
					Re_Shedule();
