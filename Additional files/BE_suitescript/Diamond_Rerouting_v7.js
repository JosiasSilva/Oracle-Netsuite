 ...
 * 10-26-2016: Add logic for additional vendors - Miracleworks, Sasha Primak, and Guild+Facet (NS-447)
 * 12-05-2016: Remove Sasha Primak and Guild+Facet (v5)
 */
		...
		try
		{
			...
			for(var x=0; x < order.getLineItemCount("item"); x++)
			{
				...
				if(category=="7")
				{
					...
					if(name.charAt(name.length-1)=="Y" || name.charAt(name.length-1)=="A" || name.charAt(name.length-1)=="N")
					{
						...
						//if((name.charAt(name.length-1)=="A" && isDropShip=="T") || (name.charAt(name.length-1)=="Y" && name.charAt(name.length-2)=="A")) // ZAG: updated the criteria as per NS-1502 
						if((name.charAt(name.length-1)=="A" && isDropShip=="T") || (name.charAt(name.length-1)=="Y" && name.charAt(name.length-2)=="A" && isDropShip=="T")) 
						{
							...
							{
								case "1587345": //Sasha Primak
									location = "16"; //BE Fulfillment-SP
									break;
						...
						else if(name.charAt(name.length-1)=="N" || name.charAt(name.length-1)=="Y" || (name.charAt(name.length-1)=="A" && isDropShip=="F"))
						{
						...
						//Update Item Record
						nlapiSubmitField("inventoryitem",order.getLineItemValue("item","item",x+1),"preferredlocation",location);
						
						//Update CDP Record(s)
						var filters = [];
						filters.push(new nlobjSearchFilter("custrecord_be_diamond_stock_number",null,"is",order.getLineItemValue("item","item",x+1)));
						var results = nlapiSearchRecord("customrecord_custom_diamond",null,filters);
						if(results)
						{
							for(var i=0; i < results.length; i++)
