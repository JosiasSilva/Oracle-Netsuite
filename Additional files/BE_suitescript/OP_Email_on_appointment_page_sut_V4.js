...
function(file, record, search, serverWidget,Url,render,email,http,redirect) {
    ...
    function onRequest(context) {
		...
		var temp_rec_val_other_rec; // added: 03/19/2019
        var reply_as_op = '0';
    	...
    	if(request.method === 'GET')
    	  {
    		...
    		bccFld.updateBreakType({breakType: serverWidget.FieldBreakType.STARTROW});
    		 ...
    		 var customerEmail=request.parameters.customerEmail;
              ...
              //  var getSubUrl = Url.resolveScript({scriptId:'customscript_get_sub_body_from_renderer',deploymentId:'customdeploy_get_sub_body_from_renderer',returnExternalUrl:false});
...
nlapiCommitLineItem ( "custpage_attachment_sublist" ); }}}</script>';
    	...
    	else
    	{
		  ...
		  var oth_rec_mail = request.parameters.custpage_recipient; // added: 03/19/2019
...
// Added by Bharath (03/19/2019)
				
          if (cc_temp){
			  log.debug("CC Temp",cc_temp);
      ...
      entitySearchObj.run().each(function(result){
        cc.push(result.getValue({name:'internalid'}));
      });   
		  }
		  
		  if (bcc_temp){
			  log.debug("BCC Temp",bcc_temp);
          var bentitySearchObj = search.create({
							   type: "entity",
							   filters:
							   [
								  ["email","is",bcc_temp]
      ...
      bentitySearchObj.run().each(function(result){
        bcc.push(result.getValue({name:'internalid'}));
      });   
		  }
		  
		  if(oth_rec_mail){
			 log.debug("Other recipient",oth_rec_mail);
          var o_entitySearchObj = search.create({
							   type: "customer",
							   filters:
							   [
								  ["email","is",oth_rec_mail]
							   ],
							   columns:
							   [
								  search.createColumn({name: "internalid", label: "Internal ID"})
	   ...
	   o_entitySearchObj.run().each(function(result){
        temp_rec_val_other_rec = result.getValue({name:'internalid'});
      });   				
							
		 }
		  
		  // End of addition by Bharath (03/19/2019)
		...
		if (temp_rec_val_frm_search !== null && temp_rec_val_frm_search !== undefined && temp_rec_val_frm_search !== ''&& temp_rec_val_frm_search !== '-4'){ // Added -4 condition , REVERT if necessary
          ...
          email.send({author:25266151,recipients:temp_rec_val_frm_search,subject:subject,body:body,cc:cc,bcc:bcc,relatedRecords:{transactionId:records.transaction},attachments:filefromCabinet});
            ...
            if (oth_rec_mail == temp_rec_val_frm_email){
			email.send({author:25266151,recipients:temp_rec_val_frm_email,subject:subject,body:body,cc:cc,bcc:bcc,relatedRecords:{transactionId:records.transaction},attachments:filefromCabinet});
				...
				log.debug("Temp Other reciever",temp_rec_val_other_rec);
			email.send({author:25266151,recipients:temp_rec_val_other_rec,subject:subject,body:body,cc:cc,bcc:bcc,relatedRecords:{transactionId:records.transaction},attachments:filefromCabinet});	
			}
		}
