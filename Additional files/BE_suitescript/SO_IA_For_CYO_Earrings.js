 ...
 * FILE NAME   			:	SO_IA_For_CYO_Earrings.js
 ...
 * File URL	Sandbox		: https://debugger.sandbox.netsuite.com/app/common/media/mediaitem.nl?uploadrectype=script&id=22328723
 * File URL	Production  :
 */

function SO_IA_CYO_Earrings(order,currency,pages,email,customer_name,today,so_ia_for_earrings_pendants,type)
{
  ...
  {
    return SO_IA_CYO_Earrings_Xml(order,currency,pages,email,customer_name,today,xml,so_ia_for_earrings_pendants,type);
  }
  else 
    return '';
}
function SO_IA_CYO_Earrings_Xml(order,currency,pages,email,customer_name,today,xml,so_ia_for_earrings_pendants,type)
{
  ...
  for(var x=0; x < order.getLineItemCount("item"); x++)
  {
    ...
    if(category==7)
    {
      ...
      replacementAmount += parseFloat(order.getLineItemValue("item","rate",x+1));
      nlapiLogExecution("debug","Replacement amount for diamond(1 & 2)",replacementAmount);
  ...
  if(cyo_earrings_pdf_arr.length>0)
  {
    ...
    nlapiLogExecution("DEBUG","Total Replacement Amount having (2 diamond price + earring price)",replacementAmount);
    ...
    xml += so_ia_for_earrings_pendants(cyo_earrings_pdf_json_obj);
  }
  nlapiLogExecution("DEBUG","SO/IA XML for CYO Earrings",xml);
  return xml;
}
