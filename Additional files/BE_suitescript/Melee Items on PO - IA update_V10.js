              ...
              if(HaloSetting == 'T' || Eternity == 1)
              {
                ...
                if(HaloSetting == 'T' && Category == '2')
                {
                  ...
                  if(ItemLink != '' && ItemLink != null)
                  {
                    ...
                    shapeText = nlapiLookupField('inventoryitem', ItemLink,'custitem5',true);
                    ...
                    {
                      if(Page(parent,shape,custitem30,poId))
                      {
                        flag = true;
                      }
                      else
                ...
                else if(type == 'view' )
                {
                  ...
                  if(MStatus != 'T')
                  {
                    ...
                    if(MCount>0)
                    {
                      ...
                      var center_shape_fld='';
                      var measurements_fld='';
                      center_shape_fld = '<tr><td style="color:blue;font-weight:bold;font-size:14px">Center Shape:</td><td><label type="text" id="center_shape_id">'+ shapeText +'</label></td></tr>';
                      measurements_fld = '<tr><td style="color:blue;font-weight:bold;font-size:14px">Measurements: </td><td><input type="text" id="measurements_id" value="'+ custitem30 +'"/></td></tr>';
                      ...
                      var table='<table class="MeleeItemsCSS" style="width:600px; border: none;"><tr><td><input type="submit" class="submitMelee" value="Save" data-type="'+MeleeFlag+'" data-poId="'+poId+'" data-MainItem="'+ItemId+'"  data-melee="'+TotalQty+'" data-itemLink="'+ItemLink+'" onclick="return submitMelee(this);" />|<input type="submit" class="submitClose" value="Close" onclick="return PopupMelee_Items_Show();" /></td><td data-src="" id="wait" ></td></tr><tr><td></td><td><b>Current Quantity</b></td><td><b>Quantity</b></td></tr>'+data+ html_store_melee_count+center_shape_fld+measurements_fld+'</table>';
                      ...
                      var html='<script type="text/javascript"> function PopupMelee_Items_Show(){ jQuery("#testItem").toggle(); return false;}; </script><div id="testItem" style="display: none;" >'+html1+'</div>';
...
function Page(parent,shape,custitem30,poId)
{
  var flag = true;
        ...
        if(shape == 1)
        {
          ...
          for(var l=0; l< result.length; l++)
          {
            ...
            if( center_size )
            {
              ...
              {
                // start here added new logic as per new requirement for [NS-1323]
                ...
                nlapiLogExecution('debug','Matching Halo Melee Count Page Url (1)',matching_halo_melee_count_page_url);
                var matching_halo_melee_count_rec = nlapiLookupField('purchaseorder',poId,'custbody_matching_halo_melee_count_rec');
                nlapiLogExecution('debug','Value of Matching Halo Melee Count Page Url (1)',matching_halo_melee_count_rec );
                if(matching_halo_melee_count_rec == '' || matching_halo_melee_count_rec==null)
                {
                  var refId = nlapiLookupField('customrecord_refresh_po_for_halo_url',1,'custrecord_refresh_id_for_halo_url');
                  if(refId ==0)
                  {
                    nlapiSubmitField("purchaseorder",poId,'custbody_matching_halo_melee_count_rec' ,matching_halo_melee_count_page_url);
                    nlapiSubmitField("customrecord_refresh_po_for_halo_url",1,"custrecord_refresh_id_for_halo_url",1);
                    var field = form.addField("custpage_po_page_refresh","inlinehtml");
                ...
                {
                  nlapiSubmitField("customrecord_refresh_po_for_halo_url",1,"custrecord_refresh_id_for_halo_url",0);
                }
                flag = false;
                nlapiLogExecution('debug','flag value (1)',flag +  " for PO having PO Id "+ poId);
                return false;
                // End Here

              }// End of innermost if block having range condition
            }// End of outermost if block having center size condition
          }// End of for loop
        }// End of if block having shape condition
        else
        {
          ...
          for(var l=0; l< result.length; l++)
          {
            ...
            if(center_size)
            {
              ...
              if((parseFloat(m1)== parseFloat(r1_20_min_1) || parseFloat(m1)== parseFloat(r3_20_max_1)) || (parseFloat(m1)>parseFloat(r1_20_min_1)&& parseFloat(m1)<parseFloat(r3_20_max_1)))
              {
                ...
                {
                  // Start here added new logic as per new req. for NS[1323]
                  nlapiLogExecution('debug','Matched halo melee count page ID (range 1 & range 2) : ',result[l].getId());
                  matching_halo_melee_count_page_url= nlapiResolveURL ('record' , 'customrecord_halo_melee' , result[l].getId());
                  nlapiLogExecution('debug','Matching Halo Melee Count Page Url (2)',matching_halo_melee_count_page_url);
                  var matching_halo_melee_count_rec = nlapiLookupField('purchaseorder',poId,'custbody_matching_halo_melee_count_rec');
                  nlapiLogExecution('debug','Value of Matching Halo Melee Count Page Url (2)',matching_halo_melee_count_rec );
                  if(matching_halo_melee_count_rec=='' || matching_halo_melee_count_rec == null)
                  {
                    var refId = nlapiLookupField('customrecord_refresh_po_for_halo_url',1,'custrecord_refresh_id_for_halo_url');
                    if(refId ==0)
                  ...
                  {
                    nlapiSubmitField("customrecord_refresh_po_for_halo_url",1,"custrecord_refresh_id_for_halo_url",0);
                    nlapiLogExecution('debug','new test 4' );
                  }
                  flag = false;
                  nlapiLogExecution('debug','flag value (2)',flag +  " for PO having PO Id "+ poId);
                  return false;
                  // End Here
                }
              }
              if((parseFloat(m1)== parseFloat(r5_10_min_1) || parseFloat(m1)== parseFloat(r7_10_max_1)) || (parseFloat(m1)>parseFloat(r5_10_min_1)&& parseFloat(m1)<parseFloat(r7_10_max_1)))
              {
                if((parseFloat(m2)== parseFloat(r8_10_min_2) || parseFloat(m2)== parseFloat(r6_10_max_2)) || (parseFloat(m2)>parseFloat(r8_10_min_2)&& parseFloat(m2)<parseFloat(r6_10_max_2)))
                {
                  ...
                  matching_halo_melee_count_page_url= nlapiResolveURL ('record' , 'customrecord_halo_melee' , result[l].getId());
                  nlapiLogExecution('debug','Matching Halo Melee Count Page Url (3)',matching_halo_melee_count_page_url);
                  var matching_halo_melee_count_rec = nlapiLookupField('purchaseorder',poId,'custbody_matching_halo_melee_count_rec');
                  nlapiLogExecution('debug','Value of Matching Halo Melee Count Page Url (3)',matching_halo_melee_count_rec );
                  if(matching_halo_melee_count_rec == '' || matching_halo_melee_count_rec==null)
                  {
                    var refId = nlapiLookupField('customrecord_refresh_po_for_halo_url',1,'custrecord_refresh_id_for_halo_url');
                    if(refId ==0)
                  ...
                  flag = false;
                  nlapiLogExecution('debug','flag value (3)',flag +  " for PO having PO Id "+ poId);
                  return false;
                  // End here 
                }
              }
            }// End of if part for center_size
          }// End of For Loop
        }// End of else part having non-round shape
      }// End of if part for result != null && result != '' && result.length > 0
    }// End of if part for parent && custitem30
  }
  catch(W)
  {
