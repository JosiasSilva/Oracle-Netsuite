...
function process(email)
{
  try
  {
    ...
    finally
    {
      ...
      // Start if either cssupport@brilliantearth.com email or search email address is primary recipient on an email
      ...
      // End here if either cssupport@brilliantearth.com email or search email address is primary recipient on an email
      if(toEmails.length>0)
        nlapiLogExecution("DEBUG","To Emails(Primary recipient)",toEmails);
      // Start here if either cssupport@brilliantearth.com email or search email address is cc'd on an email
      if(chk_toAddress == false)
      {
        ...
        if(toEmails.length>0)
          nlapiLogExecution("DEBUG","Primary recipient(2)",toEmails);
        if(ccEmails.length>0)
          nlapiLogExecution("DEBUG","Cc Emails",ccEmails);
      }// End here if either cssupport@brilliantearth.com email or search email address is cc'd on an email
      ...
      // No of To emails should not be greater than 10
      for(var e=0; e<toAddress.length;e++)
      {
        toEmailCount++;
        if(toEmailCount>10)
          break;
        if(e==0)
          primary_recipient.push(toAddress[e].getEmail());
        else
          other_recipient_add_to_cc.push(toAddress[e].getEmail());
      }
      other_recipient_add_to_cc = other_recipient_add_to_cc.join(",");
      nlapiLogExecution("DEBUG","Primary recipient(To..)",primary_recipient);
      nlapiLogExecution("DEBUG","Other recipients added into the Cc col in NS ",other_recipient_add_to_cc);
      // End Here
      ...
      for(var x=0; x < ccAddress.length; x++)
      {
        ...
        if(ccEmailsCount>10)
          break;
        // In case of primary recipient = empty or blank
        if(primary_recipient.length ==0)
        {
          if(x==0)
            primary_recipient.push(ccAddress[x].getEmail());
          nlapiLogExecution("DEBUG","Primary recipient(Cc..)",primary_recipient);
        }
        else
          ccEmailsNS.push(ccAddress[x].getEmail());
        nlapiLogExecution("DEBUG", "Cc Email Address(s) Result", "Cc Email Address(s) #"+(x+1)+" : "+ccAddress[x].getEmail());
      }
      ccEmailsNS = ccEmailsNS.join(",");
      nlapiLogExecution("DEBUG","Added Cc emails into the Cc col in NS",ccEmailsNS);
      if(tocs_terms==true || tocss==true)
      {
        ...
        // Start here for existing customer
        if(results)
        {
          try
          {
            ...
            //Specifically, if cssupport@brilliantearth.com is cc'd on an email, they should appear in the Last Email Recipient field, in addition to the other email recipients.
            if(toEmail.length>0)
            {
        ...
        }// End here for existing customer
        else
        {
          // Start here for new customer
          try
          {
        ...
        }// End Here for New Customer
      }// End here To check css and cs-terms are true
    }// End Finally
  }
  catch(err)
  {
