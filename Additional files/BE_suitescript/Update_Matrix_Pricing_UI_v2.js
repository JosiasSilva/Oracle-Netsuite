...
function updateMatrixPricingUI(request,response)
{
	if(request.getMethod()=="GET")
	{
		...
		{
			//Show User UI Form
			var form = nlapiCreateForm("Update Matrix Pricing CSV Upload");
		...
		{
			nlapiLogExecution("error","Update Matrix Pricing UI Error","Details: " + err.message);
			return true;
		}
	}
	else
	{
		try
		{
			...
			params["custscript_csv_pricing_update_user"] = nlapiGetUser();
			nlapiScheduleScript("customscript_csv_update_matrix_pricing",null,params);
			
			//Show User In Progress Page
			var form = nlapiCreateForm("Matrix Pricing Upload In Progress");
			var fld = form.addField("custpage_details","inlinehtml","Details");
			fld.setDefaultValue("<p>Your CSV upload of matrix pricing is underway and in progress. You will be notified upon completion or should any errors be encountered.</p>");
			response.writePage(form);
		}
		catch(err)
		{
