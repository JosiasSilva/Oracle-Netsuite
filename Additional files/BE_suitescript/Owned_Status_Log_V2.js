function Owned_Status_Log(type)
{
	...
	if(type=="create")
	{
		if(recordType=="inventoryitem")
		{
			...
			if(item.getFieldValue("custitem20")=="7")
			{
				try
				{
					...
					log.setFieldValue("custrecord_memovsownedfin",item.getFieldValue("custitem_memovsownedfin"));
					log.setFieldValue("custrecord_soldvsnotsoldfin",item.getFieldValue("custitem_soldvsnotsoldfin"));
					log.setFieldValue("custrecord_billedvsnotbilledfin",item.getFieldValue("custitem_billedvsnotbilledfin"));
					// End of addition
		...
		else if(recordType=="itemreceipt" || recordType=="itemfulfillment")
		{
			...
			cols.push(new nlobjSearchColumn("custitem_memovsownedfin","item"));
			cols.push(new nlobjSearchColumn("custitem_soldvsnotsoldfin","item"));
			cols.push(new nlobjSearchColumn("custitem_billedvsnotbilledfin","item"));
			...
			if(results)
			{
				for(var x=0; x < results.length; x++)
				{
					...
					// Added new fields here
					log.setFieldValue("custrecord_memovsownedfin",results[x].getValue("custitem_memovsownedfin","item"));
					log.setFieldValue("custrecord_billedvsnotbilledfin",results[x].getValue("custitem_billedvsnotbilledfin","item"));
					log.setFieldValue("custrecord_soldvsnotsoldfin",results[x].getValue("custitem_soldvsnotsoldfin","item"));
					// End of Addition
		...
		try
		{
			if(item.getFieldValue("custitem20")=="7")
			{
				...
				if(item.getFieldValue("custitemsoldvsnotsold")!=oldItem.getFieldValue("custitemsoldvsnotsold"))
					update = true;
				//Added new conditions here
				if(item.getFieldValue("custitem_memovsownedfin")!=oldItem.getFieldValue("custitem_memovsownedfin"))
					update = true;
				if(item.getFieldValue("custitem_billedvsnotbilledfin")!=oldItem.getFieldValue("custitem_billedvsnotbilledfin"))
					update = true;
				if(item.getFieldValue("custitem_soldvsnotsoldfin")!=oldItem.getFieldValue("custitem_soldvsnotsoldfin"))
					update = true;
				// End of addition	
				if(update==true)
				{
