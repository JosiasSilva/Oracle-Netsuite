function VRA_Overhaul_Diamonds(request,response)
{
	...
	//Create Inventory Transfer to Move Inventory from SF to TBD
	/*
	var transfer = nlapiCreateRecord("inventorytransfer");
	transfer.setFieldValue("location","2"); //San Francisco
	transfer.setFieldValue("transferlocation",""); //TBD
	transfer.setFieldValue("memo","Transfering inventory for diamond VRA overhaul processing.");
	...
	for(var x=0; x < returns.length; x++)
	{
		for(var i=0; i < returns[x].items.length; i++)
		{
			...
			transfer.setCurrentLineItemValue("inventory","item",returns[x].items[i].id);
			transfer.setCurrentLineItemValue("inventory","adjustqtyby","1");
			transfer.commitLineItem("inventory");
		}
	}
	
	var transferId = nlapiSubmitRecord(transfer,true,true);
	*/
	...
	for(var x=0; x < returns.length; x++)
	{
		...
		for(var i=0; i < returns[x].items.length; i++)
		{
			if((vraTotal + returns[x].items[i].amt) > 70000.00 && vra.getLineItemCount("item") > 0)
			{
	...
	for(var x=0; x < vraIDs.length; x++)
	{
		...
		if(integratedShipping==true)
		{
			...
			if(costTotal > 50000)
				vraIF.setCurrentLineItemValue("packagefedex","reference1fedex","RE2014"+(parseInt(costTotal) - 50000)+"200305-T"+nlapiLookupField("vendorreturnauthorization",vraIDs[x],"tranid")); //Reference Information
			else
