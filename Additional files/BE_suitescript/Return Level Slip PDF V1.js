...
function After_submit(type)
{
  if (type == 'create') {
    ...
    if(custbody_val.custbody138=='5' && type_creted=='SalesOrd')
    {
      ...
      nlapiSetRedirectURL ( 'suitelet' , 'customscript_return_level_slip_pdf' , 'customdeploy_return_level_slip_pdf' ,null,{order:nlapiGetRecordId ( )} ) ;
...
function FedEx_Return_Label(request,response)
{
  ...
  try
  {
    ...
    //var custpage_record_type =request.getParameter("type");
    ...
    try
    {
      ...
      if(custpage_record_id!=null && custpage_record_id!="")
      {
        ...
        if(results)
        {
          ...
          if(custpage_record_type=="salesorder")
          ...
          var file = nlapiCreateFile(custpage_tranid + " return_label.pdf","PDF",imageXml);
