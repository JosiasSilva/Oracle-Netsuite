 ...
 * Script URL : /app/common/scripting/script.nl?id=952
 * Button URL : /app/site/hosting/scriptlet.nl?script=952&deploy=1
 * Save Search URL : /app/common/search/searchresults.nl?searchid=4453
 * Schedule Script URL : /app/common/scripting/script.nl?id=1046
 */

//Create a Suitelet Script that generate an Item Receipt
/*function GenerateItemReceipt(request,response)
{
	var form = nlapiCreateForm("Create Item Receipt of POs");
	...
	else
	{
		try
		{
			...
			var mySearch = nlapiLoadSearch(null,4453); //Save search id of POs list
			var searchresult = [];
			...
			{  
				...
				{
					var columns = searchresult[m].getAllColumns();
					var Id = searchresult[m].getId();
					var receivePO = searchresult[m].getValue(columns[4]);
                    if(receivePO == 'T')
                    {
						nlapiLogExecution("debug","receive PO is :"+m,Id);
...
function GenerateItemReceipt(request, response)
{
	...
	else if(request.getMethod() == 'POST')
	{
                 ...
                 params['custscript_savedsearch_id'] = 4453;
		var result = nlapiScheduleScript('customscript1046', 'customdeploy1',params);  // 1046 is internal id of the scheduled script that you wish to reschedule and 1 is deployment id
