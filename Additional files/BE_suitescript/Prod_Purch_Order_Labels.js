...
function Gen_Prod_PO_Label_SL(request,response)
{
    ...
    cols.push(new nlobjSearchColumn("lastname","createdfrom")
	...
	if(results)
	{
		...
		var zpl = '';
			zpl+= '^XA\n';
			zpl+= '^CF0,40\n';
			zpl+= '^FO20,30^FD' + results[0].getValue("tranid","createdfrom") + '^FS\n';
            zpl+= '^FO20,70^FD' + results[0].getValue("lastname","createdfrom") + '^FS\n';
			zpl+= '^FO300,120^FD' + tbwMessage + '^FS\n';
			zpl+= '^CFF,20\n';
			zpl+= '^FO20,130^FDPO #' + results[0].getValue("tranid") + '^FS\n';
			zpl+= '^FO20,170^FD' + results[0].getText("mainname") + '^FS\n';
			zpl+= '^FO20,210^FDDD:' + results[0].getValue("custbody6","createdfrom") + ' ^FS\n';
			zpl+= '^FO20,250^FDDTS:' + results[0].getValue("custbody110","createdfrom") + '^FS\n';
			if(results[0].getValue("custbody111","createdfrom")=="T")
				zpl+= '^FO20,290^FDLayaway^FS\n';
			if(displayDiMessage==true)
				zpl+= '^FO20,330^FDDD on Specific Date^FS\n';
			zpl+= '^FO20,370^FD' + prodPOCountMsg + '^FS\n';
            zpl+= '^CF0,60\n';
            zpl+= '^FO340,360^FD' + beEngravingStatus + '^FS\n';
			zpl+= '^XZ';
