...
function transferOrders()
 {
   var search_id='';
    try {
       ...
       // var mySearch = nlapiLoadSearch(null, 5802); // For Sandbox
	     search_id = nlapiGetContext().getSetting('SCRIPT', 'custscript_new_sold_items_save_search');
		var search_title = nlapiGetContext().getSetting('SCRIPT', 'custscript_new_sold_items_search_title');
		nlapiLogExecution('Debug','Search id',search_id);
      nlapiLogExecution('Debug','Search title',search_title);
        ...
        if (searchCount > 0) {
            ...
            if (itemArrLoc) {
				//var location_id = nlapiGetContext().getSetting('SCRIPT', 'custscript_new_sold_items_save_search');
			//	nlapiLogExecution('DEBUG','Location through parameter',location_id);
			//	itemArrLoc= itemArrLoc.filter(function(item) {
             //           return (item.actual_from_loc_id  == location_id);
			//	});
