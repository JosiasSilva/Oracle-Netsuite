        ...
        if ( relatedPOs != null ) {
          for ( var i = 0; i < relatedPOs.length; i++ ) {
            ...
            for ( var i = 1; i <= poItemCount; i++ ){
              ...
              //Added by Saurabh Mishra as per NS-1083
              var curr_item_stocknum_id = purchaseOrder.getLineItemValue('item', 'item', i);
              if (curr_item_stocknum_id == '1093360' || curr_item_stocknum_id == '1087131') {
                var SF_sent_date_for_resize_ring = '';
                var created_date = purchaseOrder.getFieldValue('createddate');
                created_date = created_date.split(' ');
                var date_created = created_date[0];
                var time = created_date[1];
                ...
                if (create_time < fix_time) {
                  date_created = nlapiStringToDate(date_created, "date");
                  nextBusinessDay = date_created;
                  SF_sent_date_for_resize_ring = businessDay_date(nextBusinessDay);
                } else if (create_time >= fix_time) {
                  date_created = nlapiStringToDate(date_created, "date");
                  nextBusinessDay = nlapiAddDays(date_created, 1);
                  SF_sent_date_for_resize_ring = businessDay_date(nextBusinessDay);
                 // var dateNeededInSFCurrentValue = nlapiStringToDate(purchaseOrder.getFieldValue('custbody59'));
                 // var dateNeededInSFResize = null;
                 // if (dateNeededInSFCurrentValue) {
                  //  var dateNeededInSFNewValue = new Date();
                  //  dateNeededInSFNewValue.setDate(dateNeededInSFCurrentValue.getDate() + 1);
                  //  dateNeededInSFResize = businessDay_date(dateNeededInSFNewValue);
                   // purchaseOrder.setFieldValue('custbody59', dateNeededInSFResize)
                  //}
                }
                purchaseOrder.setLineItemValue('item', 'custcol18', i, SF_sent_date_for_resize_ring);
