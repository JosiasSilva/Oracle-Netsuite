 ...
 * Script Name                          :   Layaway Payment Client V3.js
...
function ChargeOption(option) {
    ...
    for (var i = 1; i <= count; i++) {
		...
		var jsondata = document.getElementById('custpage_results_arr').value;
		nlapiLogExecution("Debug", "jsondata before", jsondata);
		jsondata=replaceAll(jsondata,'Pending',option);	
		jsondata=replaceAll(jsondata,'Fail',option);
		jsondata=replaceAll(jsondata,'Do not charge',option);
		jsondata=replaceAll(jsondata,'Successful',option);
		jsondata=replaceAll(jsondata,'Successful',option);
		jsondata=replaceAll(jsondata,'Charge',option);
		document.getElementById('custpage_results_arr').value=jsondata
		nlapiLogExecution("Debug", "jsondata After", document.getElementById('custpage_results_arr').value);*/
}
...
function BeforeDeposit() {
    var DepositArr = [];
    var count, type, amount, website_amt;
    var month = nlapiGetFieldValue('custpage_month');
 
        count = nlapiGetLineItemCount('custpage_list');
        for (var i = 1; i <= count; i++) {
            type = nlapiGetLineItemValue('custpage_list', 'custpage_charge', i);
            if (type == "Charge") {
                amount = nlapiGetLineItemValue('custpage_list', 'custpage_monthly_layaway_amount', i);
                if (parseFloat(amount) > 0 && amount != null && amount != "") {
                    website_amt = nlapiGetLineItemValue('custpage_list', 'custpage_website_balance', i);
