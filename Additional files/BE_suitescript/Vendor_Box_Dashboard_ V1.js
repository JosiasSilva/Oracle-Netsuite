...
function Vendor_Box_Dashboard(request,response)
{
  ...
  {
    var create_record=nlapiCreateRecord('customrecord_box_record_temp');
    create_record.setFieldValue('custrecord_get_data',JSON.stringify(request.getParameter("send_data")));
    nlapiSubmitRecord ( create_record);
    return true;
  }
  else if(request.getParameter("get_price"))
  {
      ...
      {    
        for(var x=1; x <= request.getLineItemCount("custpage_list"); x++)
        {
          ...
          if(request.getLineItemValue("custpage_list","custpage_list_changed",x)=="T")
          {
            ...
            nlapiLogExecution("debug","list_notes",list_notes  +'  > '+old_list_notes);
            nlapiLogExecution("debug","date_sent_from_sf",date_sent_from_sf  +'  > '+old_date_sent_from_sf);
            nlapiLogExecution("debug","list_status",list_status  +'  > '+old_list_status);
            nlapiLogExecution("debug","insurance_value",insurance_value  +'  > '+old_insurance_value);
            nlapiLogExecution("debug","list_gem_notif",list_gem_notif  +'  > '+old_list_gem_notif);
