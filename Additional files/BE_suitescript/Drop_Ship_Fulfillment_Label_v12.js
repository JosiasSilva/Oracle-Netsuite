...
{	
	...
	var labelObj = {
		...
		dsr : "yes",
		maskaddress : "",
		label_origin_address1 : "",
		label_origin_address2 : "",
		label_origin_city : "",
		label_origin_state : "",
		label_origin_zipcode : "",
	...
	if(results)
	{
		...
		var shipFromCols = [];
		shipFromCols.push(new nlobjSearchColumn("custrecord_use_vendor_ship_from_address"));
		shipFromCols.push(new nlobjSearchColumn("custrecord_drop_ship_vendor_link"));
		...
		if(shipFromResults)
		{
			if(shipFromResults[0].getValue("custrecord_use_vendor_ship_from_address")=="T" && results[0].getValue("custbody39","createdfrom")!=null && results[0].getValue("custbody39","createdfrom")!="")
			{
				var vendor = shipFromResults[0].getValue("custrecord_drop_ship_vendor_link");
				
				var vendorShipFromFilters = [];
				vendorShipFromFilters.push(new nlobjSearchFilter("internalid",null,"is",vendor));
				var vendorShipFromCols = [];
				vendorShipFromCols.push(new nlobjSearchColumn("shipaddress1"));
				vendorShipFromCols.push(new nlobjSearchColumn("shipaddress2"));
				vendorShipFromCols.push(new nlobjSearchColumn("shipcity"));
				vendorShipFromCols.push(new nlobjSearchColumn("shipstate"));
				vendorShipFromCols.push(new nlobjSearchColumn("shipzip"));
				vendorShipFromCols.push(new nlobjSearchColumn("shipcountrycode"));
				var vendorShipFromResults = nlapiSearchRecord("vendor",null,vendorShipFromFilters,vendorShipFromCols);
				if(vendorShipFromResults)
				{
					labelObj.ship_from_addressee = shipFromResults[0].getValue("attention","address");
					labelObj.ship_from_address1 = vendorShipFromResults[0].getValue("shipaddress1");
					labelObj.ship_from_address2 = vendorShipFromResults[0].getValue("shipaddress2");
					labelObj.ship_from_city = vendorShipFromResults[0].getValue("shipcity");
					labelObj.ship_from_state = vendorShipFromResults[0].getValue("shipstate");
					labelObj.ship_from_zipcode = vendorShipFromResults[0].getValue("shipzip");
					labelObj.ship_from_country = vendorShipFromResults[0].getValue("shipcountrycode");
					labelObj.ship_from_phone = shipFromResults[0].getValue("phone","address");
					labelObj.maskaddress = "YES";
					
					labelObj.label_origin_address1 = shipFromResults[0].getValue("address1","address");
					labelObj.label_origin_address2 = shipFromResults[0].getValue("address2","address");
					labelObj.label_origin_city = shipFromResults[0].getValue("city","address");
					labelObj.label_origin_state = shipFromResults[0].getValue("state","address");
					labelObj.label_origin_zipcode = shipFromResults[0].getValue("zip","address");
					labelObj.label_origin_country = shipFromResults[0].getValue("countrycode","address");
				}
				else
				{
					labelObj.ship_from_addressee = shipFromResults[0].getValue("attention","address");
		...
		if(scilResults)
		{
			...
			else
			{
				...
				nlapiLogExecution("debug","FedEx Limit",fedexLimit);
				...
				nlapiLogExecution("debug","Parcel Pro Limit",parcelProLimit);
		...
		else if(results[0].getValue("custbody39","createdfrom")!=null && results[0].getValue("custbody39","createdfrom")!="")
		{
			nlapiLogExecution("debug","Using Drop Ship Insurance Reference...");
		...
		if(results[0].getValue("custbody87","createdfrom")=="1" && (totalCost / 0.80) <= 250)
			labelObj.dsr = "";
	...
	{
		var templateID = "30504239";
		var folderID = "8762529";
	}
	else
	{
		var templateID = "30210486";
		var folderID = "8762529";
	}
