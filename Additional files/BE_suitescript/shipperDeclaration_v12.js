  ...
  for(var i=1; i<= itemCount; i++)
  {
    ...
    if(itemType == "InvtPart")
    {
      ...
      if(itemcat != 1 && itemcat != 12 && itemcat != 23 && itemcat != 30)
      {
		...
		//var itemdisplay = obj.getLineItemValue("item","item_display",i);//added by vinay NS-910
        itemArr.push({
          sublist_item:itemId,
		  //sublist_itemdisplay :itemdisplay,//added by vinay NS-910
    ...
    try
    {
      ...
      if(page=='home')
      {
        ...
        if(itemArr.length>0)
        {
		  ...
		  //sublist.addField('sublist_itemdisplay','textarea','Item Display').setDisplayType('hidden');//added by vinay NS-910
  ...
  else if(request.getMethod() == "POST")
  {
    ...
    try
    {
      if(request.getParameter('custpage_so_hide')==null)
      {
		...
		//sublist.addField('sublist_itemdisplay_two','textarea','Item Display').setDisplayType("hidden");//added by vinay NS-910
        var lines=[];			
        for(var i=1; i<=request.getLineItemCount("custpage_items"); i++)
        {
          ...
          lines.push({
            ...
            //sublist_itemdisplay_two:request.getLineItemValue("custpage_items","sublist_itemdisplay",i)//added by vinay NS-910			
      ...
      else
      {
        ...
        for(var i=1; i<=request.getLineItemCount("custpage_items_two"); i++)
        {
          lines.push({
           ...
           // sublist_itemdisplay_two : request.getLineItemValue("custpage_items_two","sublist_itemdisplay_two",i)//added by vinay NS-910			
...
function pdfView(lines,request,response,orderId)
{
  ...
  var soFields = ['entity', 'currency','billaddress', 'shipaddress','custbodyloose_gem_order','custbody110','custbody_shipper_bill_adds', 'tranid'];
  ...
  for(var i=0;i<lines.length;i++)
  {
	...
	var itemDisplay = nlapiLookupField('inventoryitem',itemId,'name');//added by vinay NS-910
	//var itemDisplay = lines[i].sublist_itemdisplay_two;//added by vinay NS-910
    ...
    {   
      itemArr1.push({itemDisplay:itemDisplay,itemName:itemName,itemAmt:itemAmt,itemCategory:itemCategory,item_qty:item_qty});
    }
    else
    {
      itemArr2.push({itemDisplay:itemDisplay,itemName:itemName,itemAmt:itemAmt,item_qty:item_qty});
    }
  }
  ...
  if(itemArr1.length>0)
  {
    ...
    var cnt=0;
    var item_qty_val=1;
	var itemDisplay = '';//added by vinay NS-910
    for(var j=0; j<itemArr1.length;j++)
    {
      ...
      if(catId==2)
      {
        if(itemDes=='')
        {
		  ...
		  itemDisplay=itemArr1[j].itemDisplay;//added by vinay NS-910
        }
        else
        {
          itemDes=itemDes+'<br/>'+itemArr1[j].itemName;
		  itemDisplay = itemDisplay+'<br/>'+itemArr1[j].itemDisplay;//added by vinay NS-910
        }
      }
      else
      {
        ...
        if(cnt==1)
        {
          ...
          {					    
            itemDes=itemDes+'<br/>'+itemArr1[j].itemName;
			itemDisplay = itemDisplay+','+itemArr1[j].itemDisplay;//added by vinay NS-910
          }
          else
          {					    
    ...
    itemArr.push({itemDisplay:itemDisplay,itemName:itemDes,itemAmt:totAmt1,item_qty:item_qty_val});
  }
  ...
  if(itemArr2.length>0)
  {
    ...
    {
      itemArr.push({itemDisplay:itemArr2[k].itemDisplay,itemName:itemArr2[k].itemName,itemAmt:itemArr2[k].itemAmt,item_qty:itemArr2[k].item_qty});
    }
  }
  //End//
  ...
  for(var n=0;n<itemArr.length;n++)
  {
	...
	var itemDisplay = itemArr[n].itemDisplay;//added by vinay NS-910
    ...
    htmlStr +="<td class='pt10' height='20' valign='middle' align='center' bgcolor='#fff' style='padding:10px !important; border-bottom:solid; border-right:solid'>"+itemDisplay+"</td>";
  ...
  htmlStr +="</table>";	
	...
	nlapiLogExecution("debug","isCheck_wbo",isCheck_wbo);

  if(isCheck_wbo=='T')
  {			
    htmlStr +="<table border='0' align='center' cellpadding='2' cellspacing='1' width='500' style='font-family:Arial, Helvetica, sans-serif'><tr>"
	htmlStr +="<td width='30px'><img src='https://system.na3.netsuite.com/core/media/media.nl?id=24381327&amp;c=666639&amp;h=c7941a99c822602baf43' id='chk_wto' name='chk_wto' checked='1' style='width:25px; height:25px;'/></td>";//" text='The goods did not confirm to contract specifications given by the importer to the manufacture or supplier, or because of a fault or defect in the goods.'/>";
  ...
  {
    htmlStr +="<table border='0' align='center'  cellpadding='2' cellspacing='1' width='500' style='font-family:Arial, Helvetica, sans-serif'><tr>";
	htmlStr +="<td width='30px'><img src='https://system.na3.netsuite.com/core/media/media.nl?id=24382247&amp;c=666639&amp;h=db99ca4b1de461c7d1ea' id='chk_wto' name='chk_wto' style='width:25px; height:25px;'/></td>";//" text='The goods did not confirm to contract specifications given by the importer to the manufacture or supplier, or because of a fault or defect in the goods.'/>";
    htmlStr +="<td class='pt10'>The goods did not confirm to contract specifications given by the importer to the manufacturer or supplier, or because of a fault or defect in the goods.</td>";
    htmlStr +="</tr></table>";		
  }
  ...
  htmlStr +="<tr><td colspan='3'></td></tr>";
  htmlStr +="<tr><td colspan='3'></td></tr>"; 		
  htmlStr +="<tr><td width='225'></td><td align='left' colspan='2' class='pt10'><b>Signature:</b> {{t:s;r:y;o:\"signer\";w:150;h:15;}}</td></tr>";	
  htmlStr +="<tr><td colspan='3'></td></tr>";		
  htmlStr +="<tr><td colspan='3'></td></tr>";		
  //htmlStr +="<tr><td colspan='3'></td></tr>";		
  ...
  if((today.getMonth() + 1) < 10)
  	dateStr += "0" + (today.getMonth() + 1);
  else
    dateStr += "" + (today.getMonth() + 1);
  if(today.getDate() < 10)
  	dateStr += "0" + today.getDate();
  else
  ...
  var fileNameStr="FSD_"+soRecord.tranid+"_"+dateStr+"_"+soId+".pdf";
  nlapiLogExecution("debug","File Name String",fileNameStr);
