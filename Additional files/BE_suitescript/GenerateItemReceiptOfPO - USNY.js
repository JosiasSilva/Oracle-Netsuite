 ...
 * Script URL : /app/common/scripting/script.nl?id=1016
 * Button URL : /app/site/hosting/scriptlet.nl?script=1016&deploy=1
 * Save Search URL : /app/common/search/searchresults.nl?searchid=4713
 */

//Create a Suitelet Script that generate an Item Receipt
/*function GenerateItemReceipt(request,response)
{
	var form = nlapiCreateForm("Create Item Receipt of POs for USNY");
	...
	else
	{
		try
		{
			...
			var mySearch = nlapiLoadSearch(null,4713); //Save search id of POs list
			var searchresult = [];
...
function GenerateItemReceipt(request, response)
{
	...
	else if(request.getMethod() == 'POST')
	{
                ...
                params['custscript_savedsearch_id1'] = 4713;
		var result = nlapiScheduleScript('customscript1054', 'customdeploy1',params);  // 1054 is internal id of the scheduled script that you wish to reschedule and 1 is deployment id
		...
		if(result == 'QUEUED')
		{
			...
			formSavedLabel.setDefaultValue("<html><body><span style=\"font-size:30px;color:green;\">Item Receipt creation of related POs is in processing.Please wait and check execution status of schedule script id #1054.</span></body></html>");
			response.writePage(form);
		}
