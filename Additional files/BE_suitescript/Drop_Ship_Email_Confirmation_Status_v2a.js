function Drop_Ship_Email_Confirmation_Status(type)
{
	if(type=="create")
	{
		try
		{
			...
			nlapiLogExecution("debug","PO Internal ID",po);
			...
			nlapiLogExecution("debug","Receipt Items",JSON.stringify(receiptItems));
			
			//Get sales order from PO
			var so = nlapiLookupField("purchaseorder",po,"createdfrom");
			nlapiLogExecution("debug","PO - Created From SO",so);
			...
			if(results)
			{
				...
				{
					nlapiLogExecution("debug","PO Internal ID",results[x].getValue("internalid",null,"group"));
