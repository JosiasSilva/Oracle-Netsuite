function SignNow_Send_Document_Btn(type,form)
{
	if(type=="view")
	{
		try
		{
			var documentId = nlapiGetFieldValue("custrecord_signnow_document_id");
			var status = nlapiGetFieldValue("custrecord_signnow_status");
			...
			else if(documentId!=null && documentId!="" && status!="3")
			{
				var url = nlapiResolveURL("SUITELET","customscript_signnow_check_status","customdeploy_signnow_check_status");
					url+= "&record=" + nlapiGetRecordId();
				
				form.addButton("custpage_check_status","Check Status","window.open('" + url + "','chkStatusWin','dependent=yes,width=600,height=600');");
			}
