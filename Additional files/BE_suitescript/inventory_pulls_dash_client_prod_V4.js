...
function page_Init(){
	...
	jQuery('#custpage_default_splits').append('<td class="tinytext" align="center" style="padding-top:10px" colspan="21"><span style="background-color:#ebf9d1;color:#000000">Layaway</span>&nbsp;•&nbsp;<span style="background-color:#d1f9eb;color:#000000">DD Tommorrow</span>&nbsp;•&nbsp;<span style="background-color:#FFFFFF;color:#999999">0 in SF</span>&nbsp;•&nbsp;<img src="/images/icons/highlights/reskin/exclamationMark.svg" border="0" align="absmiddle"><span style="background-color:#FFFFFF;color:#000000">DD Tomorrow - 0 in SF</span>&nbsp;•&nbsp;<span style="background-color:#FFFFFF;color:#ff0000">Customer Selected Specific Gem</span></td>');
	}
...
//Created by Babin for BE-13
function Print_so_label(type, name){
	
  var allrows = nlapiGetLineItemCount('custpage_default');
  nlapiLogExecution('DEBUG','All Sublist Items',allrows);
  
  var so_select = false;
  for (var i=1; i<=allrows; i++) {
   
    var chkbox = nlapiGetLineItemValue('custpage_default', 'custpage_pulled', i);
    if (chkbox == 'T') {
      var so_id = nlapiGetLineItemValue('custpage_default', 'custpage_sales_id', i);
      var tran_id = nlapiGetLineItemValue('custpage_default', 'custpage_sales_number', i);
      var cust_name = nlapiGetLineItemValue('custpage_default', 'custpage_customer', i);
      var item_id = nlapiGetLineItemValue('custpage_default', 'custpage_item_id', i);
      var tran_id1 = tran_id.match(/<a [^>]+>([^<]+)<\/a>/)[1];
      cust_name = cust_name.match(/<a [^>]+>([^<]+)<\/a>/)[1];
      //var tran_id1 = tran_id.slice(tran_id.indexOf(">"),tran_id.indexOf("<"));
      
      cust_name = cust_name.substr(cust_name.indexOf(" ")+1);
     
      //for validation
      so_select = true;
      
      var url = nlapiResolveURL("SUITELET","customscript_selected_so_label_zpl","customdeploy_selected_so_label_zpl");
	  url+= "&custscriptsoid=" + so_id + "&custscript5=" + tran_id1 + "&custscript_customer_name=" + cust_name + "&custscript_item_id=" + item_id;
      var window_name = 'printSOLblWin_'+i;

	  window.open(url,window_name,'height=400,width=400');
    }
  	...
  	nlapiLogExecution('DEBUG','Sublist Items',request_param);
  	nlapiLogExecution('DEBUG','Tran ID',tranId_param);

  	//var url = nlapiResolveURL("SUITELET","customscript_selected_so_label_zpl","customdeploy_selected_so_label_zpl");
	//url+= "&custscriptsoid=" + request_param;

	//window.open(url,'printSOLblWin','height=400,width=400');
}
