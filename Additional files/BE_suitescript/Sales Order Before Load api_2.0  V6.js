       ...
       function(search,log,serverWidget,runtime,record,url,redirect) {
         function beforeLoad(context) {
           if(context.type == context.UserEventType.VIEW|| context.type == context.UserEventType.EDIT)
           {
             ...
             var log_message ="So ID => "+SalesRecord.id +";\n Start Time => "+ new Date();
             ...
             if(get_value_cads)
             {
               ...
               try{Highlight_SO_Fields(search,form,serverWidget,SalesRecord,log); }
               catch(er){log_message+="\n Highlight_SO_Fields Error => "+er.message; }//  Script 607  //OK
               ...
               try{ BackStock_Automation_UE_BL(context,SalesRecord,log,url); }
...
{
  //displayFraudMessage(form,SalesRecord,serverWidget,log); This is commented and merged with the script #2099 (Missing Doc Alert)
...
function Cads_SO(form,serverWidget,search,SalesRecord,record,log) {
  // log.debug({ title: 'Cads_SO', details: 'OK'});
    ...
    {	
      ...
      if(ring_size_value)
      {
        ...
        if(cads.length>0)
        {
          ...
          //log.debug({ title: 'get_new_cads_get', details: get_new_cads_get});
          //log.debug({ title: 'get_new_cads_get', details: get_new_cads_get.length});

          var get_new_chkCadValue=chkCadValue.replaceAll(';',';\n')
          get_new_chkCadValue=get_new_chkCadValue.substring(0,3000);
          // log.debug({ title: 'get_new_chkCadValue', details: get_new_chkCadValue.length });
          // log.debug({ title: 'get_new_chkCadValue', details: get_new_chkCadValue });
  ...
  if(cads_index_value_check.length>0)
  {
    ...
    for (var x = 0; x < lineItemCount; x++) {
      ...
      if(index_value>=0)
      {
        ...
        //log.debug({ title: 'cads_get.length Value   => SO ID '+SalesRecord.id, details: set_cad_value.length});
        if(set_cad_value.length>=3000)
        {
          set_cad_value=set_cad_value.substring(0,3000);
        }
        //log.debug({ title: 'cads_get.length Value   change => SO ID '+SalesRecord.id, details: set_cad_value.length});
