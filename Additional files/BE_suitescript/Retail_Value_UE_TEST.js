...
function Retail_Value_UE(type)
{
	if(type=="create" || type=="edit")
	{
		try
		{
			...
			{
				nlapiLogExecution("debug","Order type is Wedding Band or Engraving");
				var filters = [];
				...
				if(results)
				{
					...
					for(var x=0; x < originalOrder.getLineItemCount("item"); x++)
					{
						...
						{
							nlapiLogExecution("debug","Original Order Line " + x+1,originalOrder.getLineItemValue("item","amount",x+1));
