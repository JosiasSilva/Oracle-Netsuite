function Push_Celigo_Job_Import_In_Last_30()
{
  try
  {
    ...
    //var mySearchOscarError = nlapiLoadSearch(null, 8495); // For Sandbox
    var mySearchOscarError = nlapiLoadSearch(null, 9566); // For Production
...
function To_Check_CeligoId(searchresultOscarSuccess,searchresultOscarError,celigo_id,chk_success_celigo_id,chk_error_celigo_id)
{
  try{
    ...
    nlapiLogExecution("Debug","Search Result Oscar Success (Length)",searchCountOscarSuccess);
    var searchCountOscarError = searchresultOscarError.length;
    nlapiLogExecution("Debug","Search Result Oscar Error (Length)",searchCountOscarError);
    var re_chk_error_celigo_id = false;
    ...
    {
      re_chk_error_celigo_id = true;
      GetDetailsOfCeligoJobImport(celigo_id);
    }
    nlapiLogExecution("Debug","Re- Check Error Celigo Id(T/F)",re_chk_error_celigo_id);
    if(re_chk_error_celigo_id == false)
    {
      //Push to website if error celigo id is false
      nlapiLogExecution("Debug","Error Celigo Id(T/F)",chk_error_celigo_id);
      if(chk_error_celigo_id == false)
      {
        GetDetailsOfErrorCeligoJobImport(celigo_id);
      }
    }
  }
  catch(ex)
...
function GetDetailsOfCeligoJobImport(celigo_id)
{
  ...
  {
    nlapiLogExecution('DEBUG','Error!! has been occurred while getting details from Celigo Job Import (fun 1)',ex.message);
  }

}
// function 2
function GetDetailsOfErrorCeligoJobImport(celigo_id)
{
  try
  {
    //Start
    nlapiLogExecution("DEBUG","Celigo Id (in schedule script) fun() 2", celigo_id);
    if(celigo_id !='' && celigo_id !=null)
    {
      var log_id = nlapiLookupField ('customrecord_celigo_job_import' , celigo_id , 'custrecord_celigo_ji_log' );
      nlapiLogExecution("DEBUG","Internal Id & Log Id of record for Celigo Import Job(Via Schedule Script) fun() 2","Internal Id of Celigo Import job: " + celigo_id +" & Log Id :"+ log_id );
      var order_id =nlapiLookupField('customrecord_celigo_job_import',celigo_id,'custrecord_celigo_ji_order_id');
      var filters = new Array();
      var columns = new Array();
      var display_error_message = '';
      ...
      filters.push(new nlobjSearchFilter('custrecord_celigo_logger_parent',null,'is',log_id));
      var serach_result_celigo_logger = nlapiSearchRecord("customrecord_celigo_logger",null,filters,columns);
      nlapiLogExecution("DEBUG","Error Message Information fun() 2",JSON.stringify(serach_result_celigo_logger));
      if(serach_result_celigo_logger)
      {
        if(serach_result_celigo_logger.length>0)
        {
          ...
          var internal_id = serach_result_celigo_logger[0].getValue('id');
          status_code = 101 // error 
          nlapiLogExecution("DEBUG","In Schedule Script having error for following celigo id fun() 2",celigo_id);
          Push_To_Oscar_Error(status_code,order_id,display_error_message,celigo_id,internal_id);
        }
      }
    }
  }
  ...
  {
    nlapiLogExecution('DEBUG','Error!! has been occurred while getting details from Celigo Job Import(function 2)',ex.message);
  }
...
function Push_To_Oscar_Error(status_code,order_id,display_error_message,celigo_id,internal_id)
{
  try
  {
    var jsonobj = null;
    ...
    if(responseData.getCode() == 200)
    {
      nlapiLogExecution('DEBUG','Import log has been pushed successfully to the website having error log with celigo Id (From Schedule Script):', celigo_id);
      Create_CR_Error_Sync_Celigo_Id_Via_Schedule_Script(celigo_id,internal_id);
    }
  }
  catch(ex)
  {
    nlapiLogExecution('DEBUG','Error!! has been occurred while pushing error log onto the website (From Schedule Script)',ex.message);
  }
}
function Create_CR_Success_Sync_Celigo_Id_Via_Schedule_Script(celigo_id)
{
  try
  {
    ...
    nlapiLogExecution('DEBUG','Information for Oscar Success Sync having celigo id:' + celigo_id , "Custom record created successfully for success via schedule script");
  }
  catch(ex)
  {
    nlapiLogExecution('DEBUG','Error!! has been occurred while creating custom record Success Sync(From Schedule Script)',ex.message);
  }
}
function Create_CR_Error_Sync_Celigo_Id_Via_Schedule_Script(celigo_id,internal_id)
{
  try
  {
    ...
    nlapiLogExecution('DEBUG','Information for Oscar Error Sync having celigo id:' + celigo_id , "Custom record created for error successfully via schedule script");
  }
  catch(ex)
  {
    nlapiLogExecution('DEBUG','Error!! has been occurred while creating custom record for Error (From Schedule Script)',ex.message);
  }
}