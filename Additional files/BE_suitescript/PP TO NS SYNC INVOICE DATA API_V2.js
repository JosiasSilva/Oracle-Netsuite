/**
	...
	@Last Modified Date 		:-  Jul 31, 2018
...
function PP_TO_NS_SYNC_INVOICE_DATA(datain) {
  try 
  {
    nlapiLogExecution('DEBUG', 'Invoice Request Data', JSON.stringify(datain));
          ...
          for (var z = 1; z <= count; z++) 
          {
            ...
            for(var u=0; u<Item_data.length; u++)
            {
              ...
              if (Item_data[u].Item_Id == Item && LineArr.indexOf(Item_data[u].Line_Id) == -1) 
              {
                ...
                // PP-349 map the portal vendor ct. wt. field to the NetSuite column Vendor Ct. Wt. 
                obj.setLineItemValue('item', 'custcolvendor_ct_wt', z, Item_data[u].vendor_ct_wt);
                try
                {
                ...
                {
                  nlapiLogExecution("DEBUG", "Vendor cost (Error) :",qwe.message);
                }
