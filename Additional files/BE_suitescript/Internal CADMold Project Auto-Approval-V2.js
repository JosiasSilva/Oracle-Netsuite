              ...
              for (var m = 0; m < data_sales_order.length; m++)
                {
                    try {
						...
						nlapiLogExecution("debug","parent item is ",get_parent_item[0].id);
						nlapiLogExecution("debug","Ring sub item Id ",SubItemHead);
                        if (get_parent_item) {
...
// New Code implemented by Sandeep
function create_po_so_update_cads(get_vendor, get_item, so_id, status_value,cad_result_id) {
    if (get_vendor && get_item && so_id) {

        var so_fields = nlapiLookupField('salesorder', so_id, ['custbody87', 'custbody227', 'custbody285']);
      ...
      nlapiLogExecution("debug","cad result id",cad_result_id);
        ...
        if (type_of_order.indexOf('1') > -1 && order_status.indexOf('8') > -1 && order_status.indexOf('6') == -1 && order_status.indexOf('7') == -1 && so_fields.custbody285 == 'F') {
