function TO_Satellite_PU_SS()
{
	...
	cols.push(new nlobjSearchColumn("memo","appliedToTransaction")); // zag: added : 11/28/18
	var filters = [];
	...
	if(results)
	{
			...
			for(var i=0; i < transfers.length; i++)
			{
				if(transfers[i].to_location == results[x].getValue("custbody_pickup_location"))
				{
					transfers[i].lines.push({
						...
						item_descr : results[x].getValue("memo","appliedToTransaction") // zag: added : 11/28/18
					});
		...
		for(var x=0; x < transfers.length; x++)
		{
			...
			for(var i=0; i < transfers[x].lines.length; i++)
			{
				...
				transferOrder.setCurrentLineItemValue("item","description",transfers[x].lines[i].item_descr); // zag: added : 11/28/18
			...
			nlapiLogExecution("debug","Transfers Order Created ",toId); // zag : Testing : 11/26/2018
          ...
          nlapiLogExecution("debug","itemfulfillment Created ",fulfillmentId); // zag : Testing : 11/26/2018
		}
	}
}
