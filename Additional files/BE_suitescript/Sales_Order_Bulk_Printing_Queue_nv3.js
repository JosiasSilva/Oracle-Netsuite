...
function Sales_Order_IA_Bulk_Printing_Queue()
{
  try
  {
    ...
    var printOption = nlapiGetContext().getSetting('SCRIPT', 'custscript_so_ia_print');
    nlapiLogExecution("Debug","Print Option",printOption);
    var email =false;
    var type ="bulkPrintingQueue"
    if(printOption==1)// From Sales Order File
    {
      try
      {
        ...
        if (searchCount > 0)
        {
          for (var j = 0; j < searchresult.length; j++)
          {
            ...
            //call function
            generate_SO_IA_Pdf_File(searchresult[j],email,type)
          }
        }
      }
      catch(ex)
    ...
    else if(printOption ==2)//Bulk Printing Queue
    {
      try
      {
        //var mySearch = nlapiLoadSearch(null, 7659); // For Sandbox
        var mySearch = nlapiLoadSearch(null, 7670); // For Production
        var searchresult = [];
        var resultset = mySearch.runSearch();
        var searchid = 0;
      ...
      {
        nlapiLogExecution("Debug","Error Saved Search 7670",ex.message);
      }
    }
  }
  catch(ex)
...
function generate_SO_IA_Pdf_File(searchresult,email,type)
{
  try
  {
    nlapiLogExecution("Debug","type",type);
    var orderId= searchresult.id;
    var pickupatBE = searchresult.getValue('custbody53');
    var shippingCountry=searchresult.getValue('shipcountry');
    nlapiLogExecution("debug","Shipping Country",shippingCountry);
    var formulatext=searchresult.getValue('formulatext');
    //var blockIA = searchresult.getValue('custitemblock_insurance_appraisal');
    var docNumber = searchresult.getValue('tranid');
    //Sales Order & IA PDF
...
//start to Calculate Halo Melee Count 
//new code
          ...
          if(haloParentItemId!='' && haloParentItemId!=null)
          {
            ...
            if(subItem5!='' && subItem5!=null)
            {
              ...
              if(subItemShape5=='1')
              {
                var gemStoneCarat=0;  
                var gemStoneCount=0;
                var checkHaloMeleeSize=nlapiLookupField("item",haloParentItemId,"custitem_sub_item_5",true);
            ...
            if(subItem6!='' && subItem6!=null)
            {
              ...
              if(subItemShape6=='1')
              {
                var gemStoneCarat=0;  
                var gemStoneCount=0;
                var checkHaloMeleeSize=nlapiLookupField("item",haloParentItemId,"custitem_sub_item_6",true);
            ...
            if(subItem7!='' && subItem7!=null)
            {
              ...
              if(subItemShape7=='1')
              {
                var gemStoneCarat=0;  
                var gemStoneCount=0;
                var checkHaloMeleeSize=nlapiLookupField("item",haloParentItemId,"custitem_sub_item_7",true);
...
// Block-2 start to Calculate Eternity Melee Count 
