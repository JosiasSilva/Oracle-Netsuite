...
{    
	...
	{ 		
		...
		{			
			...
			var drop_ship_checklist = Drop_Ship_Data(POObj);
			...
			if(response.code==200)
			{
				...
				try
				{
                  ...
                  nlapiLogExecution("debug","Error in submitted from portal to NS is : ",ex.message); 	
                }				
			}
			else
			{			
...
function Drop_Ship_Data(POObj)
{
	...
	try
	{
		...
		if(SOID != null && SOID != '')
		{
			...
			if(ItemsShippingSeparately != ''&& ItemsShippingSeparately != null)
			{
				...
				if(ItemsShippingSeparatelyArr.indexOf(8))
				{
					drop_ship.push('Cert Kept for Drop Ship');
				}
				if(ItemsShippingSeparatelyArr.indexOf(2) || ItemsShippingSeparatelyArr.indexOf(9))
				{
	...
	nlapiLogExecution("debug","Drop_Ship_Data for PO #"+POObj.id,JSON.stringify(drop_ship)); 
