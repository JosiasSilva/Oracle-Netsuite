...
function IA_Print_Queue(request,response)
{
  ...
  else
  {
    ...
    */
    ...
    var templateId_Statement_Of_Value_Print_Queue_IA_Template = nlapiGetContext().getSetting("SCRIPT","custscript_st_value_print_queue_id");
    var templateFile_Statement_Of_Value_Print_Queue_IA_Template = nlapiLoadFile(templateId_Statement_Of_Value_Print_Queue_IA_Template);
    var template_Statement_Of_Value_Print_Queue_IA_Template = templateFile_Statement_Of_Value_Print_Queue_IA_Template.getValue();
    var statement_of_value_print_queue_ia_template = Handlebars.compile(template_Statement_Of_Value_Print_Queue_IA_Template);

    //Print Queue IA for Earrings & Pendants

    var templateId_print_ia_queue_for_earrings_pendants = nlapiGetContext().getSetting("SCRIPT","custscript_ia_queue_earring_pendant");
    var templateFile_print_ia_queue_for_earrings_pendants = nlapiLoadFile(templateId_print_ia_queue_for_earrings_pendants);
    var template_print_ia_queue_for_earrings_pendants = templateFile_print_ia_queue_for_earrings_pendants.getValue();
    var print_ia_queue_for_earrings_pendants = Handlebars.compile(template_print_ia_queue_for_earrings_pendants);
    ...
    for(var t=0; t < orders.length; t++)
    {
      ...
      var customer_name = nlapiEscapeXML(results[0].getValue("firstname","customer")) + " " + nlapiEscapeXML(results[0].getValue("lastname","customer"));
      ...
      var print_ia_queue_cyo_earrings = Print_IA_Queue_CYO_Earrings(order,currency,today,customer_name,print_ia_queue_for_earrings_pendants);
      nlapiLogExecution("Debug","Length of print ia queue v5 cyo earrings :",print_ia_queue_cyo_earrings.length);
      nlapiLogExecution("DEBUG","Print ia queue v5 cyo earrings JSON Array",JSON.stringify(print_ia_queue_cyo_earrings));
      nlapiLogExecution("DEBUG","Print ia queue v5 cyo earrings Array",print_ia_queue_cyo_earrings);
      if(print_ia_queue_cyo_earrings.length>0)
      {
        pageData = pageData.concat(print_ia_queue_cyo_earrings);
        continue;
      }
      var print_ia_queue_cyo_pendants = Print_IA_Queue_CYO_Pendants(order,currency,today,customer_name,print_ia_queue_for_earrings_pendants);
      nlapiLogExecution("Debug","Length of print ia queue v5 cyo pendants :",print_ia_queue_cyo_pendants.length);
      nlapiLogExecution("DEBUG","Print ia queue v5 cyo pendants JSON Array",JSON.stringify(print_ia_queue_cyo_pendants));
      nlapiLogExecution("DEBUG","Print ia queue v5 cyo pendants Array",print_ia_queue_cyo_pendants);
      if(print_ia_queue_cyo_pendants.length>0)
      {
        pageData = pageData.concat(print_ia_queue_cyo_pendants);
        continue;
      }
      ...
      if(orderType == 2 || orderType== 3)
      {
        ...
        var result = "For order #:" + tranid + ", IA has been blocked due to type of order having either repair or resize";
        pageData_no_print.push(result);
        continue;
      }
      // End Here
      ...
      nlapiLogExecution("DEBUG","Check more than one ring for Print Queue V5 JSON Array Length",(JSON.stringify(chk_morethan_onering_with_modifications_print_ia_queue_v5_template)).length);
      ...
      var chk_LooseDiamond_Print_Queue_IA_V5_Template =  chk_LooseDiamond_For_IA_Print_Queue_IA_V5_Template(order,currency,today,statement_of_value_print_queue_ia_template,customer_name);
      ...
      for(var x=0; x < results.length; x++)
      {
        ...
        if(category=="2")
        {
          ...
          var dmdHaloMeleeId='';
          var dmdStoneId='';
          var myParentItemId=results[x].getValue("item");  //order.getLineItemValue("item","item",x+1); 
          ...
          //if(item["parent.custitem9"]!=null && item["parent.custitem9"]!="")
          //  accentText = item["parent.custitem101"];
          if(isHaloStyle=='F' && (item["parent.custitem9"]==null || item["parent.custitem9"]==""))
          {
            accentText = item["parent.custitem101"];
          }
          if(accentText!="" && accentText!=null)
          {
            ...
            {
              nlapiLogExecution("Debug","Collection(Test)",collection);
              var accentTable = "<table cellpadding='0' cellmargin='0' cellborder='0'>";  
    ...
    var data_con=[];
    nlapiLogExecution("debug","pageData_no_print length",pageData_no_print.length);
    nlapiLogExecution("debug","pageData_no_category length",pageData_no_category.length);
    ...
    nlapiLogExecution("debug","pageData length",pageData.length);
    nlapiLogExecution("debug","pageData",pageData);
  ...
  if((dmd_Id!='' && dmd_ParentItemId!=null))
  {
    ...
    {                 
      ...
      else if((subItem2!='' && subItem2!=null) && grpItemArr.length>=1)
      {
        ...
        nlapiLogExecution("debug","totGemStone (Test)",totGemStone);
        nlapiLogExecution("debug","totGemCarat (Test)",totGemCarat);
      } 
      ...
      if(meleeSize==1 || meleeSize==2)
      {
        ...
        if(searchResult.length>0)
        {
          ...
          if( haloMeleeId > 0 && (haloMeleeSize!='' && haloMeleeSize!=null))                
          {
            ...
            {
              nlapiLogExecution("debug","haloItemId (Test 2)",haloItemId);
          ...
          var totMinCarat=0;
          nlapiLogExecution("debug","Stock Unit (Test )",stockUnit);
          nlapiLogExecution("debug","Gem Count(Test )",gemCount);
