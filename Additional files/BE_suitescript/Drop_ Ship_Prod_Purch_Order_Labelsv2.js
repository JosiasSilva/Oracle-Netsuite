...
function Gen_Prod_PO_Label_SL(request,response)
{
    ...
    cols.push(new nlobjSearchColumn("custbody_international_shipping","createdfrom"));
	...
	if(results)
	{
		...
		if(results[0].getValue("custbody348","createdfrom")=="1" || results[0].getValue("custbody348","createdfrom")=="7")
			beEngravingStatus = "E";
		...
		var zpl = '';
			zpl+= '^XA\n';
			zpl+= '^CFF,30\n';
			zpl+= '^FO20,180^FD' + results[0].getValue("tranid","createdfrom") + '^FS\n';
			zpl+= '^FO20,220^FD' + customerName + '^FS\n';
			zpl+= '^CF0,40\n';
			zpl+= '^FO20,40^FDPO #' + results[0].getValue("tranid") + '^FS\n';
