...
function createDeposit(fileObj,encoding)
{
	...
	try
	{
		...
		for(var x=0; x < lines.length; x++)
		{
			...
			var amount = parseFloat(lines[x]["Amount"]);
			...
			if(sublist=="Payments")
			{
				...
				for(var i=0; i < deposit.getLineItemCount("payment"); i++)
				{
					...
					nlapiLogExecution("debug","Payment sublist customer: " + sl_customer,"Sublist Amount: " + sl_amount + "  |  JSON Amt: " + amount);
					
					if(sl_customer.toUpperCase()==customer.toUpperCase() && sl_amount==amount)
					{
