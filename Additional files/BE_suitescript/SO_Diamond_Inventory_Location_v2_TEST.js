function SO_Diamond_Inventory_Location_TEST(type)
{
	if(type=="view")
	{
		try
		{
			...
			nlapiLogExecution("debug","containsAntiqueOrRing",containsAntiqueOrRing);
			nlapiLogExecution("debug","earringPendantFound",earringPendantFound);
				...
				for(var x=0; x < order.getLineItemCount("item"); x++)
				{
					...
					if(category=="7" && order.getLineItemValue("item","location",x+1)!="2")
					{
						...
						if(!deposits && name.charAt(name.length-1)=="Y")
						{
							...
							nlapiLogExecution("debug","Setting Location to SF");
						}
						else
						{
							order.setLineItemValue("item","location",x+1,location);
							nlapiLogExecution("debug","Setting Location to " + location);
						}
