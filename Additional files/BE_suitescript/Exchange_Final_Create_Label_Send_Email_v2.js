function  Exchange_Final_Create_Label_Send_Email(form,dropOff)
{
  try
  {
    var  CC =[];
    ...
    var return_label_insurance_rtn_item =0;
  ...
  try
  {
    ...
    if(items_return_ra.length>0 || items_exchange_ra.length>0)
    {
      if(rma==null || rma=="")
      {
        ...
        ra.setFieldValue("tranid",orderNumber+"EX");
        ...
        //if(result!=null)
        //ra.setFieldValue("salesrep",original_salesrep); //1. Sales rep will always go on the return auth commented as per client feedback
        ...
        ra.setFieldValue("custbody87","4"); //Type of Order = Exchange [added by feedback document of 20 jun 2017]
        ...
        for(var i=0;i<items_exchange_ra.length;i++)
        {
          ...
          ra.setCurrentLineItemValue("item","amount",items_exchange_ra[i].saleamount);
        ...
        // ra.setFieldValue("custbody_return_label_insurance",return_label_insurance_rtn_item);
        // nlapiLogExecution("Debug","Return Label Insurance Value",return_label_insurance_rtn_item);
  ...
  try
  {
    ...
    if(items_return_resize.length>0 || items_return_repair.length>0 || items_return_cust_be.length>0 || new_items.length>0)
    {
      ...
      newOrder.setFieldValue("tranid",orderNumber + "EX");
      //Check if transaction ID already exists
     ...
     // newOrder.setFieldValue("salesrep",original_salesrep);
      ...
      newOrder.setFieldValue("custbody87","4"); //Type of Order = Exchange [added by feedback document of 20 jun 2017]
      ...
      if(items_return_ra.length>0 || items_exchange_ra.length>0)
        newOrder.setFieldValue("custbody138",7);//return label status
      else
        newOrder.setFieldValue("custbody138",'');//return label status
      ...
      if(orNotes!='')
        ...
        newOrder.setFieldValue("custbody254",''); //BLANK OR NOTES
      ...
      // commented by client feedback doc of 12 jun 2017
      ...
      var type ="exchange";
      var retlblStatus = createReturnLabelPdf(shipService[1],shipService[0],"exchange",orderID);    
      nlapiLogExecution("Debug","Tracking No",retlblStatus[0]);
      nlapiLogExecution("Debug","File Id",retlblStatus[1]);
      if(rmaID != null && rmaID !="")
      {
