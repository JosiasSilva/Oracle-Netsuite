		...
		for(var i=0; i < results1.length; i++)
		{
			try
			{
				...
				if(hasAppointments==false)
				{
					...
					{
						if(results1[i].getValue(apptLocationInvField) > 0)
							hasInventoryInApptLocation = true;
						if(results1[i].getValue(apptLocationInvField) > 1)
					...
					if(results1[i].getValue("custrecord_appointment_item_qty_avail") > 1 && hasInventoryInApptLocation==false)
					{
					...
					if(fromLocations!=null && fromLocations.length > 0)
					{
						...
						else
						{
							...
							else
							{
								...
								nlapiSubmitField(results1[i].getRecordType(),results1[i].getId(),"T");
							}
						}
					}
				}
