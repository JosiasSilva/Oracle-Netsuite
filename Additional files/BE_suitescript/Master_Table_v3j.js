...
function Master_Table_CS(type,name)
{
	...
	nlapiLogExecution("debug","= Requested",sameDayAsRequested);
		...
		if(hasAppointments==false)
		{
			...
			if(nlapiGetCurrentLineItemValue("recmachcustrecord_appointment_item_parent","custrecord_item_qty_avail_den") > 0)
				fromLocations.push("7"); //Denver
			
			//Check if inventory in appointment location
			var apptLocationInvField = null;
			switch(location)
			{
				case "1":
					apptLocationInvField = "custrecord_appointment_item_qty_avail";
					break;
				case "2":
					apptLocationInvField = "custrecordappointment_item_qty_avail_la";
					break;
				case "3":
					apptLocationInvField = "custrecord_item_qty_avail_boston";
					break;
				case "4":
					apptLocationInvField = "custrecord_item_qty_avail_chicago";
					break;
				case "5":
					apptLocationInvField = "custrecord_item_qty_avail_sd";
					break;
				case "6":
					apptLocationInvField = "custrecord_item_qty_avail_dc";
					break;
				case "7":
					apptLocationInvField = "custrecord_item_qty_avail_den";
					break;
			}
			
			var hasInventoryInApptLocation = false;
			var hasMultipleInventoryInApptLocation = false;
			
			if(apptLocationInvField!=null)
			{
				if(nlapiGetCurrentLineItemValue("recmachcustrecord_appointment_item_parent",apptLocationInvField) > 0)
					hasInventoryInApptLocation = true;
				if(nlapiGetCurrentLineItemValue("recmachcustrecord_appointment_item_parent",apptLocationInvField) > 1)
					hasMultipleInventoryInApptLocation = true;
			}
			
			if(hasMultipleInventoryInApptLocation == true)
			{
				nlapiLogExecution("debug","Appt Location Has > 1 Inventory (Non-Unique)","No conflict will occur.");
				return true;
			}
			
			if(nlapiGetCurrentLineItemValue("recmachcustrecord_appointment_item_parent","custrecord_appointment_item_qty_avail") > 1 && hasInventoryInApptLocation==false)
			{
			...
			if(fromLocations!=null && fromLocations.length > 0)
			{
				...
				if(results)
				{
					...
					if(hoursInt < 14)
					{
						...
						{
							alert("This item is not available for your appointment. It may be sold, on hold, on another appointment at the same time, or in another location. Please select another item.");
				...
				{
					if(fromLocations.length==1 && location == fromLocations[0])
					{
						nlapiLogExecution("debug","Location/Appointment are same place. No master table exists, no conflict.");
					}
					else
					{
						//No inventory is found for item, setting as conflict
						nlapiSetCurrentLineItemValue("recmachcustrecord_appointment_item_parent","custrecord_conflict","T",true,true);
					}
				}
			}
		...
		else
		{
			...
			else
			{
				...
				else if(sameDayAsRequested==true && otherApptLocation!=location)
				{
