    ...
    for(var i = 1; i<=line_count; i++)
    {
      ...
      if(rec_type== "Purchase Order")
      {
        ...
        if(link_po)
        {
          ...
          Push_BE_Engraving_Status_to_Portal(link_po,type);
        }
      }
    }
  }
...
nlapiLogExecution("audit","FLOStart",new Date().getTime());
function Push_BE_Engraving_Status_to_Portal(poId,type)
{
  try
  {
    ...
    if(new_be_engraving_status!='' && new_be_engraving_status!=null)
    {
      if(new_be_engraving_status != old_be_engraving_status)
      {
        ...
        var obj = PushPOToPortal(poId,"2166");
        obj = JSON.parse(obj);
        if(obj.status == "OK")
        {
  ...
  {		
    ...
    if(soId != null && soId != "")
    {
      ...
      if(shipTo != null && shipTo != "")
      {
        ...
        SendNStoPortal(shipAddress,poId,"2166");
      }
    }
    else
    {
...
function SendNStoPortal(shipAddress,poId,scriptId)
{
 ...
 // var url = "https://partner.brilliantearth.com/api/production/po/"+poId+"/customer-address/"; 	// live portal
  var url = "https://testportal.brilliantearth.com/api/production/po/"+poId+"/customer-address/"; 	//test portal

  //Setting up Headers 
  var headers = new Array(); 
  headers['http'] = '1.1';    
  headers['Accept'] = 'application/json';       						
  //headers['Authorization']= 'token d499ecd8233123cacd3c5868939fe167b7bf0837';// live						  
