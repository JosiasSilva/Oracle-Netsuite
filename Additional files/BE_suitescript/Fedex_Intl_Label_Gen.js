function Gen_Fedex_Label(shipId,autoIF)
{
	...
	if(results)
	{
		...
		//If Country is Canada, Guam, Northern Mariana Islands, Puerto Rico or Australia bill Third Party (194375506) else bill Receipient	
		if(results[0].getValue("shipcountry")=="CA" || results[0].getValue("shipcountry")=="GU" || results[0].getValue("shipcountry")=="MP" || results[0].getValue("shipcountry")=="PR" || results[0].getValue("shipcountry")=="AU")
		...
		if(results[0].getValue("custbody87")=="2" || results[0].getValue("custbody87")=="3" || results[0].getValue("custbody87")=="4")
			postSale = true;
		...
		for(var x=0; x < results.length; x++)
		{
			...
			else if(postSale==true && results[x].getValue("custcol_category")=="12" && results[x].getValue("memo")!=null && results[x].getValue("memo")!="")
			{
	...
	//Terms of Sale (Use DDP for Canada, Guam, Northern Mariana Islands, Puerto Rico and Australia. Everything else use DDU)
	if(shipCountry=="CA" || shipCountry=="GU" || shipCountry=="MP" || shipCountry=="PR" || shipCountry=="AU")
	{
	...
	if(nlapiGetContext().getEnvironment()=="PRODUCTION")
	{
		var templateID = "16713471";
		var folderID = "8762529";
	}
	else
	{
		var templateID = "15611301";
		var folderID = "407735";
	}
	...
	if(nlapiGetContext().getEnvironment()=="SANDBOX")
	{
		...
		responseFile.setFolder("7975542");
