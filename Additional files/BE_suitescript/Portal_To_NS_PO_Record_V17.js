...
function UpdatePORecord(datain)
{
  ...
  //ended logic
  //PP-336
  var new_updated_date_time_on_ns ='';
  var old_date_time  ='';
  try
  {
    var type = "xedit";
    var poId = datain.po_id;
    var ns_updated_time ='';
    ns_updated_time = nlapiLookupField("purchaseorder",poId,'custbody_updated_datetime_on_ns');
    nlapiLogExecution("debug","Updated date & time(NS) is :",ns_updated_time);
    ...
    //Start Here PP-336
    var portal_updated_time = datain.updated_datetime_on_ns;
    nlapiLogExecution("debug","Updated date and time(Portal) is :",portal_updated_time);
    old_date_time = ns_updated_time;
    // End Here PP-336
    ...
    //Start Here PP-343 set value for the following fields Request to Push and Vendor Push Reason 
    // Request to Push
    if(datain.request_to_push == false)
      poObj.setFieldValue("custbody_request_to_push",'F');
    else if(datain.request_to_push == true)
      poObj.setFieldValue("custbody_request_to_push",'T');
    // Vendor Push reason
    poObj.setFieldValue("custbody_vendor_push_reason", datain.push_reason);
    // End Here
    ...
    //Addded : ZAG : 03/04/2019 : Escalated PO : PP-373
    if(datain.escalated_po == false)
      poObj.setFieldValue("custbody_escalated_po",'F');
    else if(datain.escalated_po == true)
      poObj.setFieldValue("custbody_escalated_po",'T');
    ...
    // Added below code for PP-336
    // Start Here
    nlapiLogExecution('DEBUG',"Old date & time for PO having id is : "+ id, old_date_time);
    Updated_Date_Time(type,id);
    // End Here
    if(id > 0)
    {
      if (datain.attachments != null && datain.attachments != "undefined")
      {
        ...
        for(var j=0; j< datain.attachments.length; j++)
        {
          ...
          if(url != null && url != '')
          {
            nlapiLogExecution('DEBUG',"Step A");
            url = url.split('/');
            var extn = url[url.length-1].split('.');
            extn = extn[extn.length-1];
            nlapiLogExecution('DEBUG',"Extension",extn);
            ...
            nlapiLogExecution('DEBUG',"step 3A ");
            var response = nlapiRequestURL(datain.attachments[j],null,headers);
            //var response = nlapiRequestURL(datain.attachments[j]);
            nlapiLogExecution('DEBUG',"step 3B ");
            ...
            if(response.code == 200)
            {
              ...
              if(GetExtnType(extn) != "")
              {
                ...
                nlapiLogExecution('DEBUG',"final step ","file has been attached successfully");
              }
            }
          }
        }
    ...
    if(itemReceipt==1)
    {
      ...
      nlapiLogExecution("debug","item Receipt Info :",JSON.stringify(err));
