function  Resize_Final_Create_Label_Send_Email(form)
{
  ...
  try
  {
    ...
    nlapiLogExecution("Debug","Resize Order Delivery Information else block",JSON.stringify(retObj));
    ...
    {
      var newRingSize = request.getLineItemValue("custpage_customer_item_returning_be", "custpage_rtn_item_newsize", i);
      nlapiLogExecution("debug","Final Submission New Ring Size",newRingSize);
    ...
    soRec.setFieldValue("custbody87",2);//Type Of Order =Resize by feedback document of 20 jun 2017
    ...
    for(var i=1; i<=request.getLineItemCount("custpage_customer_item_returning_be"); i++)
    {
      ...
      // nlapiLogExecution("debug","item link sku value ",item_link_sku_val);
      ...
      soRec.setCurrentLineItemValue("item","item",1093360);//RESIZE ITEM
    ...
    nlapiLogExecution("Debug","resizeOrderID",resizeOrderID);
    nlapiSubmitField('salesorder',soid,"custbody_resize_requested",'T');
    var shipService = setShipService(resizeOrderID);	
    ...
    var type="resize";
    var retlblStatus = createReturnLabelPdf(shipService[1],shipService[0],type,resizeOrderID);    
    nlapiLogExecution("Debug","Tracking No",retlblStatus[0]);
    nlapiLogExecution("Debug","File Id",retlblStatus[1]);
    nlapiSubmitField('salesorder',resizeOrderID,['custbody123','custbody137'],[retlblStatus[0],retlblStatus[1]]);
    ...
    sendEmailWithAttachment(retlblStatus[1],CC,resizeOrderID,toEmail,emailType,copyEmailTemplateInternalId);
  }
  catch(err)
  {
    nlapiLogExecution("error","Resize Page Processing Error in else block final page submission","Details: " + err.message);
  }
}