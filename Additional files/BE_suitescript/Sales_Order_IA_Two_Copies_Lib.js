function Sales_Order_IA_Two_Copies(orderId,email,type)
{
  ...
  xml += '<td align="left" class="freightDispPro  pt10" width="3in"><p align="left" text-align="left">${item.description}</p></td>';
  ...
  var pages = 0;
  //var templateID = nlapiGetContext().getSetting("SCRIPT","custscript_ia_template_file_id");
  ...
  else
  {
    ...
    {
      xml += "</body></pdf>";
      nlapiLogExecution("Debug","xml 1",xml);
    }
    else
    {
      nlapiLogExecution("error","chk Category",chkCategory);
      if(chkCategory==true)
      {
        xml += "</body></pdf>";
        nlapiLogExecution("Debug","xml 2",xml);
      }
      else
      {
        var errorMsg = "";
