...
{
  nlapiLogExecution('Error', 'Yelp Integration JSON Data', JSON.stringify(datain));
  var err = new Object();
  err = {
    "status": "failed",
  ...
  try
  {
    ...
    if(datain.email)
    {
      ...
      nlapiLogExecution('Error','Result JSON',JSON.stringify(Result));
      if(Result)
      {
        nlapiLogExecution('Error', 'Existing customer Id : '+ Result[0].getId());
        ...
        nlapiLogExecution('Error','Customer with Yelp Info',"Customer record has been updated with Yelp Info successfully");
