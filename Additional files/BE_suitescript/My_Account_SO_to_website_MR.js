 ...
 */
define(['N/search', 'N/record', 'N/runtime','N/https','N/format'],
    /**
	...
	function map(context) {
		log.debug("Inside  map",'====START====');
    	try {
    		
    		var searchResults = JSON.parse(context.value);
            log.debug("Inside  map","Search Results ="+JSON.stringify(context.value)); 
            var soId       = searchResults.id;
            log.debug("Inside map",'Transaction Id ='+soId); 
            if(soId != null && soId != '')
			{
            	//checkApproveOrCanelledOrChangedField(soId);
    			...
    			log.debug('Inside onRequest', 'Salesorder Status:'+soStatus+'/OrderType ='+orderType+'/MyAccount WebInfo='+MyAccount_WebInfo);
    			try {
    				if((orderType == 1 || orderType == 5) && soStatus!="Pending Approval"  && MyAccount_WebInfo==false)
					{
						log.debug("Inside checkApproveOrCanelledStatus",'MyAccount_WebInfo ='+MyAccount_WebInfo); 
						var isChange = checkApproveOrCanelledOrChangedField(soId);
						log.debug("Inside checkApproveOrCanelledStatus",'isChange ='+isChange);
						if(isChange == true )
						{
							MyAccount_WebInfo = true;
						}	
					}
    				if((orderType == 1 || orderType == 5) && soStatus!="Pending Approval"  && MyAccount_WebInfo==true)
					{
						PushSODataNStoPortal(soObj);
						log.debug("Inside map",'SO Data Successfully Pushed to Website ='+soId); 
					}
    				else if((orderType == 1 || orderType == 5) && soStatus=="Pending Approval")
    				{
    					MyAccount_WebInfo=false;	
					...
					{
						log.debug("Inside map",'SO Data UnSuccessfully Pushed to Website ='+soId); 
					}	
				} catch (e) {
					// TODO: handle exception
					log.debug("Inside map","Error for SO ID is "+soId+","+er.massage); 
				}
			}
			log.debug("Inside  map",'====END====');	
		} catch (e) {
			// TODO: handle exception
	...
	function checkApproveOrCanelledOrChangedField(soId)
	{
        ...
        if(searchRecords != "")
        {
        	...
        	for (var i = 0; i < searchRecords.length; i++) {
    			...
    			log.debug("System Notes",'oldValue ='+oldvalue+'/newvalue ='+newvalue+'/field ='+field);
    			...
    			else if((field == 'tranid' || field == 'custbody71' || field == 'custbody6' || field == 'custbody53' || field == 'custbody_pickup_location' || field == 'taxitem' || field == 'taxrate' || field == 'taxtotal' || field == 'discounttotal' || field == 'shippingcost' || field == 'subtotal' || field == 'custbody_truebalanceamount' || field == 'orderstatus' || field == 'custbody310' || field == 'custbody69') && (oldvalue != newvalue))
        		{
    				isChangeFlag = true;
    				break;
    			}	
        ...
        log.debug("System Notes",'isChangeFlag ='+isChangeFlag);
        if(isChangeFlag == false)
        {
        	var filters1 = search.createFilter({
    			name: 'internalid',
    			operator: search.Operator.IS,
    		...
    		var searchLineRecords = search.create({
    			type: search.Type.SALES_ORDER,
    			filters: [filters1],
    			columns: [columns1,columns2,columns3,columns4]
    		}).run().getRange({ start: 0, end: 1000 });
            
            if(searchLineRecords != "")
            {
            	log.debug("Line System Notes", searchLineRecords.length+'/searchRecords='+searchLineRecords.length);
            	for (var i = 0; i < searchLineRecords.length; i++) {
            		var soId = searchLineRecords[0].id;
        			//log.debug("API Test",'soId ='+soId);
        			var oldvalue = searchLineRecords[i].getValue({
        				name: 'oldvalue',
        				join: 'linesystemnotes'
        				});
        			var newvalue = searchLineRecords[i].getValue({
        				name: 'newvalue',
        				join: 'linesystemnotes'
        				});
        			var field = searchLineRecords[i].getValue({
        				name: 'field',
        				join: 'linesystemnotes'
        				});
        			log.debug("Line System Notes",'oldValue ='+oldvalue+'/newvalue ='+newvalue+'/field ='+field);
        			if((field == 'amount' || field == 'memo' ) && (oldvalue != newvalue))
            		{
        				isChangeFlag = true;
        				break;
        			}	
    		...
    		if(soId!='' && soId!=null)
    		{
                ...
                for (var int = 0; int < itemCount; int++) {
                    ...
                    {							 
						...
						log.debug("Inside  PushSODataNStoPortal",'gift_item ='+gift_item);
				        if(gift_item==true)
					    {
				        	Objitem= {
                ...
                log.debug("Inside  PushSODataNStoPortal",'Customer Deposit');
              //Customer Deposit
				...
				log.debug("Inside  PushSODataNStoPortal",'Customer Deposit length ='+searchResult.length);
				...
				log.debug("Inside Item Fulfillment",'searchRecords ='+searchRecords);	
				if(searchRecords != null && searchRecords != "")
				{
