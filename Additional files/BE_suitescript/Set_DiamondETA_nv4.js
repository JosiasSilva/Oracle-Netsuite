...
function SetDiamondETA(type)
{
  try
  {
    ...
    if(type == "create" || type == "edit" || type == "xedit")
    {
      ...
      var cdpArrField = ["custrecord_diamond_eta","custrecord_custom_diamond_request_type","custrecord_diamond_status","custrecord_action_needed","custrecord165"];
      ...
      //Added by Shruti Gupta

      var actionNeeded = cdpFieldVal.custrecord_action_needed;
      var old_actionNeeded=nlapiGetOldRecord().getFieldValue("custrecord_action_needed").split('|');
      var new_actionNeeded=nlapiGetNewRecord().getFieldValue("custrecord_action_needed").split('|');
      nlapiLogExecution("debug","Old Action Needed is :",old_actionNeeded);
      nlapiLogExecution("debug","New Action Needed is :",new_actionNeeded);

      //Ended by Shruti Gupta
      ...
      if(old_diamond_eta == new_diamond_eta || (old_diamond_eta == null && new_diamond_eta == ''))
      {
        ...
        //  if( (old_diamondStatus != 1 && new_diamondStatus == 1 && (new_portalReqType ==3 || new_portalReqType ==4 || portalReqType == 3 || portalReqType == 4)) || (old_itemStatus != 2 && new_itemStatus == 2 && itemStatus == 2  && old_diamondStatus != new_diamondStatus && (new_portalReqType == 3 || new_portalReqType==4 || portalReqType == 3 || portalReqType == 4)))	//Changed New condition	
          //if( (old_diamondStatus != 1 && new_diamondStatus == 1 && (new_portalReqType ==3 || new_portalReqType ==4 || portalReqType == 3 || portalReqType == 4))  && actionNeeded.indexOf('3')>=0 ||new_actionNeeded.indexOf('3')>=0)	//Added by Shruti Gupta
          {
