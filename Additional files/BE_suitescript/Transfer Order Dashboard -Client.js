var main_url='/app/site/hosting/scriptlet.nl?script=2363&deploy=1';
...
function Field_Style()
{
  ...
  var incoming_url=main_url;
  var outgoing_url=main_url;
  if(nlapiGetFieldValue('custitem_incoming_location_to'))
  {
    incoming_url+='&to_location='+nlapiGetFieldValue('custitem_incoming_location_to');
  }
  if(nlapiGetFieldValue('custitem_outgoing_location_from'))
  {
    outgoing_url+='&from_location='+nlapiGetFieldValue('custitem_outgoing_location_from');
  }
  // Get_Update_Records(outgoing_url,'custitem_list_outgoing_rcords','records_outgoing','records_outgoing_page');
  Get_Update_Records(incoming_url,'custitem_list_incoming_rcords','records_incoming','records_incoming_page');
}

function Get_Update_Records(url,type,field_records,field_select)
{
  url+='&machine_records='+type;
  jQuery.ajax({
    type: 'GET',
    ...
    success: function(get_return_value) { 
      ...
      if(type=='custitem_list_outgoing_rcords')
      {
...
function page_index_change(val)
{
  ...
  {
    machine='custitem_list_incoming';
  }
  else
  {
    machine='custitem_list_outgoing';
  }
  var url=main_url+'&r=T&machine='+machine+'&from_location='+nlapiGetFieldValue('custitem_outgoing_location_from')+'&to_location='+nlapiGetFieldValue('custitem_outgoing_location_to')+'&search_document='+nlapiGetFieldValue('custitem_outgoing_search')+'&page_index='+val.value;
...
function refreshmachine(val)
{
  ...
  var restUrl = nlapiResolveURL('RESTLET', 'customscript_transfer_order_service', 'customdeploy_transfer_order_service');
  jQuery.ajax({
    type: 'POST',
    contentType: "application/json;",
    url:restUrl,
  ...
  jQuery.ajax({
    ...
    success: function(get_return_value) { 
      ...
      else
      {
        if(val=='custitem_outgoing_packing_slip')
        {
          alert('No Available Creating Packing Slips');
        }
        else
        {
          alert('No Available Creating Shipping Labels');
        }
      }
    }
  });
...
function Fulfillment_Records()
{
  ...
  for (var t = 1; t <= nlapiGetLineItemCount('custitem_list_outgoing'); t++) {
    ...
    var Transfer_Order_status = nlapiGetLineItemValue('custitem_list_outgoing', 'custitem_outgoing_fulfill_check', t);
    if (Transfer_Order_status == 'T') {
      trans_id.push(nlapiGetLineItemValue('custitem_list_outgoing', 'custitem_outgoing_internalid', t));
    }
  }
...
{
  document.getElementById("id_wait").style='';
  document.getElementById("id_wait_message").innerText="Creating  Box Record's "+send_data_qty+" out of "+temp_data_obj.length;
...
{
  var url_val=main_url+'&r=T&machine=custitem_list_outgoing&from_location='+nlapiGetFieldValue('custitem_outgoing_location_from')+'&to_location='+nlapiGetFieldValue('custitem_outgoing_location_to')+'&search_document='+nlapiGetFieldValue('custitem_outgoing_search')+'&search_ship_date='+nlapiGetFieldValue('custitem_outgoing_ship_date');
  document.getElementById('server_commands').src =url_val
  Get_Update_Records(url_val,'custitem_list_outgoing_rcords','records_outgoing','records_outgoing_page');
}

function refreshmachine_incoming()
{
  var url_val=main_url+'&r=T&machine=custitem_list_incoming&from_location='+nlapiGetFieldValue('custitem_incoming_location_from')+'&to_location='+nlapiGetFieldValue('custitem_incoming_location_to')+'&search_document='+nlapiGetFieldValue('custitem_incoming_search')+'&search_ship_date='+nlapiGetFieldValue('custitem_incoming_ship_date');
  document.getElementById('server_commands').src =url_val;
  Get_Update_Records(url_val,'custitem_list_incoming_rcords','records_incoming','records_incoming_page');
}
