function VRA_Overhaul_Diamonds(request,response)
{
	...
	var filterExp = [
		...
		"and",["internalid","anyof",["960696","1979064"]]
	];
	var cols = [];
	...
	var results = nlapiSearchRecord("item",null,filterExp,cols);
	if(results)
	{
	...
	//Create VRA's - One Per Vendor up to $70K
	for(var x=0; x < returns.length; x++)
	{
		var vra = nlapiCreateRecord("vendorreturnauthorization");
		var vraTotal = 0.00;
	...
	{
		var vraIF = nlapiTransformRecord("vendorreturnauthorization",vraIDs[x],"itemfulfillment",{recordmode:"dynamic",nsi:"1965157"});
		...
		nlapiSubmitRecord(vraIF,true,true);
	...
	for(var x=0; x < vraIDs.length; x++)
	{
		var billCredit = nlapiTransformRecord("vendorreturnauthorization",vraIDs[x],"vendorcredit");
		...
		if(billCredit.getLineItemCount("item") > 0)
		{
			...
			nlapiSubmitRecord(billCredit,true,true);
		}
		else
		{
	...
	for(var x=0; x < returns.length; x++)
	{
		for(var i=0; i < returns[x].items.length; i++)
		{
			if(returns[x].items[i].po_status!="")
			{
				var poFound = false;
	...
	{
		var poRec = nlapiLoadRecord("purchaseorder",ptc[x].id);
	...
	if(exceptions.length > 0)
	{
		...
		nlapiSendEmail(nlapiGetUser(),"diaops@brilliantearth.com","Diamonds Not Returned Report",emailBody);
...
{
	var vraIF = nlapiTransformRecord("vendorreturnauthorization",8311273,"itemfulfillment",{recordmode:"dynamic",nsi:"1965157"});
	...
	if(vraIF.getFieldValue("shipcountry")=="US")
		...
		vraIF = nlapiTransformRecord("vendorreturnauthorization",8311273,"itemfulfillment",{recordmode:"dynamic"});
	...
	if(integratedShipping==true)
	{
		...
		if(costTotal > 50000)
			...
			vraIF.setCurrentLineItemValue("packagefedex","reference1fedex","BEF8956"+(parseInt(costTotal))+"X1560 "+nlapiLookupField("vendorreturnauthorization",8311273,"tranid")); //Reference Information
	...
	nlapiSubmitRecord(vraIF,true,true);
	nlapiLogExecution("debug","Finishing Fulfilling VRA ID " + 8311273);
}
