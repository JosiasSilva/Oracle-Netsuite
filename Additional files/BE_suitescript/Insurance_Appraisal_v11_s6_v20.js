...
function Insurance_Appraisal(request,response)
{
  try
  {
    ...
    var templateId_Statement_Of_Value = nlapiGetContext().getSetting("SCRIPT","custscript_statement_of_value_temp_id");
    ...
    var Statement_Of_Value_Temp = Handlebars.compile(template_Statement_Of_Value);

    // IA for Earrings & Pendants [NS-1128]

    var templateId_ia_for_earrings_pendants = nlapiGetContext().getSetting("SCRIPT","custscript_ia_for_earrings_pendants");
    var templateFile_ia_for_earrings_pendants = nlapiLoadFile(templateId_ia_for_earrings_pendants);
    var template_ia_for_earrings_pendants = templateFile_ia_for_earrings_pendants.getValue();
    var ia_for_earrings_pendants = Handlebars.compile(template_ia_for_earrings_pendants);
    ...
    var soFields = nlapiLookupField("salesorder",orderId,["custbody87","tranid"]);
    //Exchange Order type =4,Order#
    var orderType = soFields.custbody87;
    var order_tran_id = soFields.tranid;
    nlapiLogExecution("Debug","Exchange Order Type Value",orderType);
    // Exchange Order Type = 4
    ...
    //Start Here [NS-1128]
    var cyo_earrings = CYO_Earrings(order,currency,pages,email,customer_name,today,xml,ia_for_earrings_pendants);
    nlapiLogExecution("Debug","Return value of cyo earrings is :",cyo_earrings);
    if(cyo_earrings == true)
      return true;

    var cyo_pendants = CYO_Pendants(order,currency,pages,email,customer_name,today,xml,ia_for_earrings_pendants);
    nlapiLogExecution("Debug","Return value of cyo pendants is :",cyo_pendants);
    if(cyo_pendants == true)
      return true;
    // End Here

    var chkLD =  chkLooseDiamondForIA(order,currency,pages,email,customer_name,today,xml,Statement_Of_Value_Temp,chkCategory);
    nlapiLogExecution("Debug","Return value of chkLooseDiamondForIA() fun",chkLD);
    if(chkLD == true)
      return true;

    var chkmorethanoneringwithmodification = Chk_MoreThan_OneRing_With_Modifications(order,xml);
    nlapiLogExecution("Debug","Return value of Chk_MoreThan_OneRing_With_Modifications() fun",chkmorethanoneringwithmodification);
    if(chkmorethanoneringwithmodification== true)
      return true;

    var  ChkOneRingWithModifications = Chk_OneRing_With_Modifications(order,xml,pages,today,customer_name,currency,email,iaTemp,isIABlock,isIABlockCode,iaBlockMsg,countgemStoneArr,chkCategory,chkExchangeOrderType,chkGemStoneForExchange,chkRingForExchange);
    if(ChkOneRingWithModifications==true)
      return true;

    /*var  ia_for_finished_jewelry = IA_For_Finished_Jewelry(order,xml,pages,today,customer_name,currency,email,iaTemp,chkCategory);
      nlapiLogExecution("Debug","Return value(T/F) of IA_For_Finished_Jewelry()",ia_for_finished_jewelry);
      if(ia_for_finished_jewelry == true)
      return true;*/

    if(chkExchangeOrderType == true)
    {
      var ia_for_exchange =  IA_For_Exchange_One(order,xml,pages,today,customer_name,currency,email,iaTemp,countgemStoneArr,chkCategory,chkGemStoneForExchange,chkRingForExchange,chkExchangeOrderType,isIABlockCode,iaBlockMsg,isIABlock);
      nlapiLogExecution("Debug","Return value(T/F) of IA_For_Exchange_One() fun",ia_for_exchange);
      if(ia_for_exchange == true)
        return true;
    }

    for(var x=0; x < order.getLineItemCount("item");
        x++)
    {
      ...
      if(category=="2")
      {
        ...
        // Added by Shiv per NS-402 assigned by Eric

        for(var i=0; i < order.getLineItemCount("item");
            i++)
        {
          if(x==i)
            continue;
        ...
        if((myParentItemId !='' && myParentItemId !=null) && (subItemId !='' && subItemId !=null) && (dmdStoneId !='' || dmdStoneId !=null) && (dShape=='1'))
        {
          ...
          {
            var createPOLinkCount = order.getLineItemCount('links');
            //find PO link on line item
          ...
          if(haloCheck=='T')
          {
            ...
            for(var j=0; j < accentTextArr.length; j++)
            {
              if(accentTextArr[j]!='' && accentTextArr[j]!=null)
              {
                ...
                //Added by Sandeep for NS-624
                if(index1>0 || index11>0)
                {
                  var index2 = accentTextArr[j].toLowerCase().indexOf("round");
                  //if Round
                  if(index2 > 0 )
                  {
        ...
        for(var z=0; z < order.getLineItemValue("item","quantity",x+1);
            z++)
        {
          if(pages > 0)
          {
      ...
      else if(category=="3")
      {
        ...
        nlapiLogExecution("debug","accentText-3 : "+accentText,accentText);
        // Added for UT
      ...
      else if(category=="5" || category =="4" || category == "34")
      {
        ...
        {
          for(var sr =0; sr<searchResult_FinishedJewelry.length;
              sr++)
          {
    ...
    if(haloMeleeCollection=='T' && diamondShape=='1') // Check for Halo Settings  & diamond Shape
    {
      ...
      if(subItem1!='' && subItem1!=null)
      {
        ...
        if(sunbItemShape1=='1')
        {
          grpItemArr.push(
            {
              subItemId:subItem1,gemStone:gemStoneCount,gemCarat:gemStoneCarat
            }
          );
        }
      ...
      if(subItem2!='' && subItem2!=null)
      {
        ...
        if(subItemShape2=='1')
        {
          ...
          grpItemArr.push(
            {
              subItemId:subItem2,gemStone:gemStoneCount,gemCarat:gemStoneCarat
            }
          );
        }
      }
      if(subItem3!='' && subItem2!=null)
      {
        ...
        if(subItemShape3=='1')
        {
          ...
          grpItemArr.push(
            {
              subItemId:subItem3,gemStone:gemStoneCount,gemCarat:gemStoneCarat
            }
          );
        }
      }
      if(subItem4!='' && subItem4!=null)
      {
        ...
        if(subItemShape4=='1')
        {
          ...
          grpItemArr.push(
            {
              subItemId:subItem4,gemStone:gemStoneCount,gemCarat:gemStoneCarat
            }
          );
        }
      }
      if(subItem5!='' && subItem5!=null)
      {
        ...
        if(subItemShape5=='1')
        {
          ...
          grpItemArr.push(
            {
              subItemId:subItem5,gemStone:gemStoneCount,gemCarat:gemStoneCarat
            }
          );
        }
      }
      if(subItem6!='' && subItem6!=null)
      {
        ...
        if(subItemShape6=='1')
        {
          ...
          grpItemArr.push(
            {
              subItemId:subItem6,gemStone:gemStoneCount,gemCarat:gemStoneCarat
            }
          );
        }
      }
      if(subItem7!='' && subItem7!=null)
      {
        ...
        if(subItemShape7=='1')
        {
          ...
          grpItemArr.push(
            {
              subItemId:subItem7,gemStone:gemStoneCount,gemCarat:gemStoneCarat
            }
          );
        }
      }
      ...
      {
        meleeSize=2;
        // If more than 1 melee Size stone found
      ...
      if(meleeSize==1 || meleeSize==2)
      {
        ...
        filters[0] = nlobjSearchFilter("custrecord_center_shape",null,'is',1);
        //Shape= Round
        ...
        if(searchResult.length>0)
        {
          ...
          for(i=0;i<searchResult.length;i++)
          {
            ...
            haloMeleeArr.push(
              {
                hmId:hmId,hmSize:hmSize,hmGemCount:hmGemCount,hmMeleeSize:hmMeleeSize
              }
            );
          ...
          if( haloMeleeId > 0 && (haloMeleeSize!='' && haloMeleeSize!=null))
          {
            ...
            filters[0] = new nlobjSearchFilter('type',null,'is','InvtPart');
            //Item type will be Inventory Item
            filters[1] = new nlobjSearchFilter('itemid',null,'is',haloMeleeSize);
            //itemname
            var results = nlapiSearchRecord('item', null, filters, new nlobjSearchColumn('itemid'));
            nlapiLogExecution("debug","JSON Results ",JSON.stringify(results));
            var haloItemId='';
            var haloItemName='';
            for (var i in results)
            {
              ...
              nlapiLogExecution("debug","haloItemId (Test ) 1",haloItemId);
              nlapiLogExecution("debug","haloItemName (Test ) 2",haloItemName);
              break;
            }
          ...
          nlapiLogExecution("debug","stockUnit (Test )",stockUnit);
          //end halo melee size
          var totMinCarat=0;
...
/*
* ---Old Function for Single Stone Size----
function getHaloMeleeDetail(dmd_Id,dmd_ParentItemId)
{
var haloMeleeResult=[];
if((dmd_Id!='' && dmd_ParentItemId!=null))
...
function getEternityMeleeDetail(myItemId)
{
  ...
  if(myItemId!='' && myItemId!=null)
  {
    ...
    {
      var ringSize = nlapiLookupField("item",myItemId,"custitem2",true);
      //get ring Size;
      ...
      if(ringSize.length >1)
      {
        ...
        fracPart=ringSize.split('.')[1];
        // Get fraction part of ring size
        fracPart='0.'+fracPart;
      ...
      //custitemeternitymeleecountsize6
      ...
      if(isEternity=='1');
      {
        ...
        if((stockUnit !='' && stockUnit != null) && (NoOfGemStone !='' && NoOfGemStone != null))
        {
          ...
          eternityMeleeArr.push(
            {
              NoOfGemStone:NoOfGemStone,
...
function MeleeItemCountOn_PO_Temp(order, createPOLinkCount)
{
  var haloCheck='F';
  var meleeArr=[];
...
function rtnNewMeleeItemTable(meleeResult,newTempTable,accentText)
{
  ...
  if(meleeResult[0].roundMeleeShape=='1')
  {
    ...
    newTempTable += "<td class='freightDispPro pt10'> Diamond </td>";
    newTempTable += "</tr>";
    newTempTable += "<tr>";
