function item_fall_dashboard_suitelet(request,response){
	try{
		if(request.getMethod()=="GET"){
			...
			var list = form.addSubList('custpage_default', 'list','Sales Orders',null);
						...
						/* if(layaway_chk=='T'){						 
							...
							if(layaway_chk=='T'){
								...
								if(qty_SF<1){
									...
									show_list=list_show_grey(show_list,exclamation,chk_layaway,so_id,so_num,item_id,item_name,item_desc,qty_SF,on_hand,available,in_transit,po_id,po_text,date_received,diamond_ETA,delivery_date,cs_fulfill,pickUp_loc,category,loose_gem,del_date_tommorrow,layaway_chk);					
								}
								else{
									nlapiLogExecution("debug","test 11");
									show_list=list_show_white(show_list,exclamation,chk_layaway,so_id,so_num,item_id,item_name,item_desc,qty_SF,on_hand,available,in_transit,po_id,po_text,date_received,diamond_ETA,delivery_date,cs_fulfill,pickUp_loc,category,loose_gem,del_date_tommorrow,layaway_chk);
								}								
							}
							else{
								var dd = new Date(delivery_date);
								var dd_tom = new Date(del_date_tommorrow);
								if(qty_SF<1 && dd<dd_tom){
									chk_layaway='Yes2';
									exclamation=html;
									show_list=list_show_grey(show_list,exclamation,chk_layaway,so_id,so_num,item_id,item_name,item_desc,qty_SF,on_hand,available,in_transit,po_id,po_text,date_received,diamond_ETA,delivery_date,cs_fulfill,pickUp_loc,category,loose_gem,del_date_tommorrow,layaway_chk);
								}
								else if(qty_SF<1 && dd>=dd_tom){
									chk_layaway='No';
									show_list=list_show_grey(show_list,exclamation,chk_layaway,so_id,so_num,item_id,item_name,item_desc,qty_SF,on_hand,available,in_transit,po_id,po_text,date_received,diamond_ETA,delivery_date,cs_fulfill,pickUp_loc,category,loose_gem,del_date_tommorrow,layaway_chk);
								}
								else if(qty_SF>=1 && dd>=dd_tom){
...
function list_show_grey(show_list,exclamation,chk_layaway,so_id,so_num,item_id,item_name,item_desc,qty_SF,on_hand,available,in_transit,po_id,po_text,date_received,diamond_ETA,delivery_date,cs_fulfill,pickUp_loc,category,loose_gem,del_date_tommorrow,layaway_chk){
	show_list.push({
...
function list_show_white(show_list,exclamation,chk_layaway,so_id,so_num,item_id,item_name,item_desc,qty_SF,on_hand,available,in_transit,po_id,po_text,date_received,diamond_ETA,delivery_date,cs_fulfill,pickUp_loc,category,loose_gem,del_date_tommorrow,layaway_chk){
	show_list.push({
					"custpage_html" : exclamation,
					"custpage_layaway_highlight" :chk_layaway,
