          ...
          for(var i=0; i < orders.length; i++)
          {
            ...
            for(var j=0; j < Search.length; j++)
            {
              ...
              if(search_order==find_search)
              {
                ...
                //New added by ajay 21March 2017
				...
				orders[i].custpage_date_of_last_payment= "<a target='_blank' title='Click here to link Item' href='/app/accounting/transactions/custdep.nl?id=" + Search[j].id + "' style='color: #255599;    text-decoration: none;' >" + Search[j].getValue('trandate')+ "</a>";;
				if(diffDays > 60)
				{
					orders[i].custpage_date_of_last_payment=  "<a target='_blank' title='Click here to link Item' href='/app/accounting/transactions/custdep.nl?id=" + Search[j].id + "' style='color: red;    text-decoration: none;' >" + Search[j].getValue('trandate')+ "</a>";
				}
                SNo++;
                break;
              }
          ...
          if(month.indexOf(MonthNo)>=0 || month.length == 0)
          {
            ...
            if(so_arr.indexOf( AllRows[rs].id)==-1)
            {
              if(AllRows[rs].getValue(allCol[22])=="T")
              {
                ...
                nlapiLogExecution("debug","row index true is :",indexRow);
                indexRow++;
              }
              else if(AllRows[rs].getValue(allCol[22])=="F")
              {
                ...
                nlapiLogExecution("debug","row index false is :",indexRow);
                indexRow++;
              }
              else
              {
                ...
                nlapiLogExecution("debug","row index empty is :",indexRow);
                indexRow++;
              }
            ...
            else
            {
              if(count > 0)
              {
                if(AllRows[rs].getValue(allCol[22])=="T")
                {
                  ...
                  nlapiLogExecution("debug","row index pop true is :",indexRow);
                  indexRow++;
                  count = 0;
                }
                else if(AllRows[rs].getValue(allCol[22])=="F")
