function Matched_Wedding_Band_DropOff_One(dropOff,form)
{
  ...
  try
  {
    ...
    nlapiLogExecution("debug","(POST) New Item Info Retrieved",JSON.stringify(new_items));
    ...
    newOrder.setFieldValue("tranid",so_num + "WB");
    ...
    /*
     * Updated by client feedback doc 20 jun 2017
     * 5 = Matched Wedding Band
     */ 
    newOrder.setFieldValue("custbody87",5);
    ...
    for(var i=1; i<=request.getLineItemCount("custpage_items"); i++)
    {
      ...
      if(shipping_to_be == 'T')
      {
        ...
        //newOrder.setCurrentLineItemValue("item","custcol_full_insurance_value_2",insurance);
        newOrder.setCurrentLineItemValue("item","custcol_full_insurance_value",insurance);
        ...
        //nlapiLogExecution("debug","step 01");
        newOrder.commitLineItem("item");
      }
    }

    newOrder.setFieldValue('custbody_center_stone_link',centerStoneLink);
    //nlapiLogExecution("debug","center stone link",centerStoneLink);
  ...
  {
    nlapiLogExecution("error","Matched Wedding Band Page Processing Error in if block","Details: " + err.message);
    return true;
  }
}