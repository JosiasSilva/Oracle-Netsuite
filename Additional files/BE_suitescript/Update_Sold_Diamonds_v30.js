	...
	try
	{
		...
		nlapiLogExecution("debug","New CS Status is : "+new_CsStatus);
			...
			{                           
			   ...
			   var mapping_logic = cdpObj.getFieldValue('custrecordoverride_mapping_logic');// added by ajay 24Jan 2017
			   ...
			   { 
				...
				{   
					...
					{						
						pushCDPtoPortal(cdpId,resultArr,portalRequestTypeId,item_status,action_needed,mapping_logic,cdpObj,onVendorPortal,requestType,paid,groupB,assetAcct,diamond,soStatus,old_PortalReqType,old_ItemStatus,vendorId,vendorPortal,manual_override,paymentTerms,new_ItemStatus,cs_status,old_CsStatus,new_CsStatus,gr_notes,cs_notes,diamond_notes);
...
function SendJsonData(cdpId,vendorId,portalRequestTypeId,actNeedStr,item_status,vendorPortal,manual_override,paymentTerms,requestType,gr_notes,cs_notes,diamond_notes)
{
		...
		if(onVendorPortal=='F' || onVendorPortal=='T')  
		{
				...
				if(soStatus != null && soStatus != "" && soStatus == "fullyBilled")
				{
					if(old_PortalReqType == 2 && map_PortalReqType == 3)
					{
						if(old_ItemStatus == 2) //confirmed
						{
						  ...
						  jsonobj = SendJsonData(cdpId,vendorId,portalRequestTypeId,actNeedStr,item_status,vendorPortal,manual_override,paymentTerms,requestType,gr_notes,cs_notes,diamond_notes);								      
						}
						else
						{
							portalRequestTypeId = map_PortalReqType;	
			...
			if(jsonobj != null)
			{
				...
				if(response.code=='200')
				{
                    ...
                    cdpObj.setFieldValue('custrecord238','886');
...
function GetPortalRequestVal(arr,diamond,paymentTerms,portalReqType,reqType,paid,grA,grB,itemStatus,actionNeeded,csStatus,assetAcct)
	{		
		...
		if((reqType == 1 && paid < 20)&&((diamond.indexOf("AN") != -1) || (diamond.indexOf("Y") != -1 || diamond.indexOf("AY") != -1 || diamond.indexOf("A") != -1) ))// 3,4 (Sold)
		{
		...
		//Remove AN logic by ajay 18Nov 16
		else if((reqType == 7 && paid >= 20 && csStatus != 4)&&((diamond.indexOf("AN") != -1)||(diamond.indexOf("A") != -1 || diamond.indexOf("Y") != -1) )) // 9 (Replacement(diamond unavailable))
		{
			if(diamond.indexOf("A") != -1 || diamond.indexOf("Y") != -1 || diamond.indexOf("AN") != -1)
			{
				portalReqType = 2;
				portalReqTypeText = "Hold";
            }
