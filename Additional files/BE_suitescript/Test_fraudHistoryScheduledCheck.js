function AutoFraudScheduledCheck() {
    try {
        //var fraudSearch = nlapiLoadSearch(null, 4216); // Sandbox
        var fraudSearch = nlapiLoadSearch(null, 4393); // Production
        ...
        // nlapiLogExecution("debug", 'old Record Length : ' + searchid);

        //var new_mySearch = nlapiLoadSearch(null, 4215);// Sandbox
        var new_mySearch = nlapiLoadSearch(null, 4416); //Production        
        var searchresult = [];
        var new_resultset = new_mySearch.runSearch();
        var new_searchid = 0;
        do {
            var new_resultslice = new_resultset.getResults(new_searchid, new_searchid + 1000);
            if (new_resultslice != null && new_resultslice != '') {
                for (var rs in new_resultslice) {
                    RunFraudHistoryCheck(new_resultslice[rs].id, searchResultArr);
                    new_searchid++;
                }
            }
        } while (new_resultslice.length >= 1000);
        // nlapiLogExecution("debug", "New Record Length : " + new_searchid);
    }
    catch (err) {
...
function RunFraudHistoryCheck(recordId, searchResultArr) {
