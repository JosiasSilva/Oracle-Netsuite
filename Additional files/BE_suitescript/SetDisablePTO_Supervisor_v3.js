...
function Check_PTO_Status(type)
{
	...
	var pto_approver = nlapiLookupField('employee',nlapiGetUser(),'custentity_pto_approver');
	nlapiLogExecution('debug','PTO Approver?',pto_approver);
	if (pto_supervisor != null)
    {
       ...
       if ((current_user != null) && (current_user != ''))
       {
		...
		if((supervisor == current_user) || (pto_arr.indexOf(current_user) != -1) || (fixed_User.indexOf(current_user) != -1) || (pto_approver=='T'))
		{
