 ...
 * Comments 			:	// 02/14/2019 - Inoday - NS-1498 - Edited the the 'bodyMsg' parameter that didn't have well defined tags.
 *						:	// 02/20/2019 - Inoday - NS-1498 - Edited the the 'bodyMsg' parameter and added the <html> and <body> tags in the variable.
 * SB SS URL 			:	https://system.netsuite.com/app/common/scripting/script.nl?id=855
 * Production SS URL	:	https://system.na3.netsuite.com/app/common/scripting/script.nl?id=855
 */
  ...
  try
  {
    ...
    for(var i=0; i<retData.length; i++)
    {
      try
      {
		...
		// instead of retData[i].ews_id using JSON.parse(retData[i].custom_fields)[9][0]
        // if((retData[i].status ==0 || retData[i].status ==2) && (retData[i].ews_id ==null  || retData[i].ews_id ==''))
		if((retData[i].status ==0 || retData[i].status ==2) && (JSON.parse(retData[i].custom_fields)[9][0] == null  || JSON.parse(retData[i].custom_fields)[9][0] == ''))
        {
          ...
          if(OptionalFirstClick  !=null  && OptionalFirstClick[0] != 'undefined' && OptionalFirstClick[0] != undefined)
          {
          ...
          var parseWurfl = null;
          if(OptionalWurfl  !=null  && OptionalWurfl[0] != 'undefined' && OptionalFirstClick[0] != undefined)
          {
          ...
          if(clientphone.length < 22)
          {
            ...
            { 
			  ...
			  filters[3] = new nlobjSearchFilter('title', null, 'doesnotstartwith', 'CANCELLED'); // Changed in NS-1554
              if(type!=null && type!='')
              {
                filters[4] = new nlobjSearchFilter('custevent_appointment_type', null, 'is', type); // changed in NS-1554
              }
              ...
              else
              {
                ...
                //New Logic Start Here To Prevent Creating Duplicate Leads
...
function Create_Appointment(leadId,obj_data,token)
{
  ...
  //var Lead_data=nlapiLookupField('lead',leadId, ['altname','email','phone']);// old code
  var Lead_data=nlapiLookupField('customer',leadId, ['altname','email','phone','custentity107']);
  //var customerEmail =Lead_data['email'];	// old code	
  var customerPhone = obj_data.clientphone;	
  // NS-1358 URGENT - Setster script/cookie issue
  var AttributionPath = obj_data.AttributionPath;
  var multipath_cookie_data = Lead_data['custentity107'];
  // [NS-1284]
  /*
     if the field on the Lead page called Phone (id: phone) is blank but the field on the appointment page called Customer Phone Number (id: custevent_customer_phone_numer) is not          blank then the field from the appointment page will sync to the lead page
  */
  var customerPhoneOnLead = Lead_data['phone'];
  ...
  try
  {
    ...
    else if(retData_type=="FINE JEWELRY &amp; VINTAGE")
    {
    ...
    survey_appointment_linking(apptmtId); // Added as part of NS-1287 on 08/16/2018
    ...
    if(apptmtId)
    {
      ...
      // nlapiSubmitField ( 'lead' , leadId , 'custentity_set_setster_data' ,JSON.stringify(update_obj_data) , true ) ;// old code
      if(multipath_cookie_data == '' || multipath_cookie_data == null)
        nlapiSubmitField('customer', leadId , 'custentity107' , AttributionPath ,true);//NS-1358
      if(customerPhoneOnLead =='' || customerPhoneOnLead == null)
        nlapiSubmitField('customer', leadId , ['custentity_set_setster_data','custentity_email_status','phone'] , [JSON.stringify(update_obj_data),9,customerPhone],true);//[NS-1284]
      else
        nlapiSubmitField('customer' , leadId , ['custentity_set_setster_data','custentity_email_status'] ,[JSON.stringify(update_obj_data),9], true);//[NS-1171]

      //var dataToSend = {"ews_id":  apptmtId};
      var dataToSend = {"ews_id":  apptmtId,"custom_fields": {"9": apptmtId}};//NS-1111 Added Custom Fields 20th August.
...
function SendMailToCustomer(apptId,setsId) {
  try {       
    ...
    if (customerMail) {
      ...
      nlapiLogExecution('debug','Customer ID',companyId);
      ...
      // commneted on 10/6/2018 by saurabh in concern of NS-1374 nlapiSendEmail(20561,customerMail, "Your Brilliant Earth Appointment Request", bodyMsg,null,"eito@brilliantearth.com",companyId,null,true);

      //nlapiSendEmail(20561, "eaito0916@gmail.com", "Your Brilliant Earth Appointment Request", bodyMsg,null,null,null,null,true);

      //added by saurabh 10/6/2018 in concern of NS-1374 
      var customerObject={};
