...
Script Name :		PO Receive Dashboard - Client_V2.js
Sandbox SS URL :    https://system.netsuite.com/app/common/scripting/script.nl?id=2117    
production SS URL:  https://system.na3.netsuite.com/app/common/scripting/script.nl?id=2117
Description:		highlighting/bold Text a rows based on certain conditions
...
function page_Init(){
   ...
   jQuery('input[name^="custpage_delivery_date_flag"]').each(function(){
		...
		if(deliverydate_flag=='1'){
			...
			jQuery(this).style('background-color','#f2e53a','important');
		  });
		}
	  });
	  	  // end code added by Bhutani for NS-1240 on 27/6/18

	 // Addition by Bharath - NS-1437 on 11/27/2018
	 
	  jQuery('input[name^="custpage_to_be_tested_flag"]').each(function(){
		var tobeTested_flag = jQuery(this).val();
		//alert(tobeTested_flag);
		if(tobeTested_flag== '1'){
			jQuery(this).parent().parent().children().each(function(){
	  ...
	  jQuery('input[name^="custpage_del_date_flag"]').each(function(){
		var delDate_flag = jQuery(this).val();
		//alert(delDate_flag);
		if(delDate_flag=='1'){
			jQuery(this).parent().parent().children().each(function(){
			jQuery(this).style('background-color','#f2e53a','important');
      ...
      jQuery('input[name^="custpage_del_date_mon"]').each(function(){
		var delDate_mon = jQuery(this).val();
	    
		if(delDate_mon=='1'){
            console.log(delDate_mon);
			jQuery(this).parent().parent().children().each(function(){
			jQuery(this).style('background-color','#f2e53a','important');
		  });
		}
   ...
   jQuery('input[name^="custpage_del_date_diff"]').each(function(){
		var delDate_diff = jQuery(this).val();
	...
	// Code change ends - Bharath NS-1437
}
