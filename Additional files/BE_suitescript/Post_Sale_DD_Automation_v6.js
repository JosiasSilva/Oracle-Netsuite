				...
				if(hours < 14)
				{
					...
					if(isExchange==true)
					{
						if(poRec.getFieldValue("custbody82")=="T")
							nlapiSubmitField("purchaseorder",nlapiGetRecordId(),["custbody_linked_to_post_sale_order","custbody_exchange_po"],["T","T"]);
						else
					...
					{
						if(poRec.getFieldValue("custbody82")=="T")
							nlapiSubmitField("purchaseorder",nlapiGetRecordId(),["custbody_po_ship_date","custbody_linked_to_post_sale_order","custbody59"],[shipDate,"T",dateNeededInSF]);
						else
	...
	else if(type=="edit")
	{
		try
		{
			...
			if(soID!=null && soID!="")
			{
				...
				nlapiLogExecution("debug","Is Exchange? " + isExchange);
				
				if(shipDate!=oldShipDate)
				{
					if(poRec.getFieldValue("custbody_linked_to_post_sale_order")=="T")
					{
						nlapiLogExecution("debug","Ship Date != Old Ship Date");
						...
						filters.push(new nlobjSearchFilter("createdfrom",null,"is",soID));
						filters.push(new nlobjSearchFilter("mainline",null,"is","T"));
						filters.push(new nlobjSearchFilter("custbody82",null,"is","F"));
						var results = nlapiSearchRecord("purchaseorder",null,filters);
						if(results)
						{
							for(var x=0; x < results.length; x++)
				...
				else if(dateNeededInSF!=olddateNeededInSF)
				{
					if(poRec.getFieldValue("custbody_linked_to_post_sale_order")=="T")
					{
						nlapiLogExecution("debug","Date Needed in SF != Old Date Needed in SF");
