...
function IA_Print_Queue(request,response)
{
  ...
  var templateId_print_ia_queue_v5_for_earrings_pendants = nlapiGetContext().getSetting("SCRIPT","custscript_ia_queue_v5_earring_pendant");
  var templateFile_print_ia_queue_v5_for_earrings_pendants = nlapiLoadFile(templateId_print_ia_queue_v5_for_earrings_pendants);
  var template_print_ia_queue_v5_for_earrings_pendants = templateFile_print_ia_queue_v5_for_earrings_pendants.getValue();
  var print_ia_queue_v5_for_earrings_pendants = Handlebars.compile(template_print_ia_queue_v5_for_earrings_pendants);
  ...
  for(var k=0;k<salesOrder_data.length;k++)
  {
    ...
    //Start Here [NS-1128]
    var print_ia_queue_v5_cyo_earrings = Print_IA_Queue_V5_CYO_Earrings(order,currency,today,customer_name,print_ia_queue_v5_for_earrings_pendants);
    nlapiLogExecution("Debug","Length of print ia queue v5 cyo earrings :",print_ia_queue_v5_cyo_earrings.length);
    nlapiLogExecution("DEBUG","Print ia queue v5 cyo earrings JSON Array",JSON.stringify(print_ia_queue_v5_cyo_earrings));
    nlapiLogExecution("DEBUG","Print ia queue v5 cyo earrings Array",print_ia_queue_v5_cyo_earrings);
    if(print_ia_queue_v5_cyo_earrings.length>0)
    {
      pageData = pageData.concat(print_ia_queue_v5_cyo_earrings);
      continue;
    }
    var print_ia_queue_v5_cyo_pendants = Print_IA_Queue_V5_CYO_Pendants(order,currency,today,customer_name,print_ia_queue_v5_for_earrings_pendants);
    nlapiLogExecution("Debug","Length of print ia queue v5 cyo pendants :",print_ia_queue_v5_cyo_pendants.length);
    nlapiLogExecution("DEBUG","Print ia queue v5 cyo pendants JSON Array",JSON.stringify(print_ia_queue_v5_cyo_pendants));
    nlapiLogExecution("DEBUG","Print ia queue v5 cyo pendants Array",print_ia_queue_v5_cyo_pendants);
    if(print_ia_queue_v5_cyo_pendants.length>0)
    {
      pageData = pageData.concat(print_ia_queue_v5_cyo_pendants);
      continue;
    }
