//function PDF_file_Create(get_line_item,vendor_text,box_rec_Id,total_insu,packing_slip_id,po_push,type_call) // zag: commented
function PDF_file_Create(get_line_item,vendor_text,box_rec_Id,total_insu,packing_slip_id,po_push,box_counter,type_call) // zag: Added
{
  if(packing_slip_id)
  {
  ...
  nlapiLogExecution('DEBUG',"get Obj",JSON.stringify(get_obj));
  if(get_obj)
  {
    ...
    //xml += Get_Fotter(vendor_text,box_rec_Id,get_obj_box).replaceAll("&",""); // zag: commented
	xml += Get_Fotter(vendor_text,box_rec_Id,get_obj_box, box_counter).replaceAll("&",""); // zag: Added
    xml += '</macro>';
    xml += '</macrolist>';   
    xml += '</head>';
    ...
    //  xml +='<br />';
    var get_body_two= Get_Body_two(get_obj,type_call).replaceAll("&","");;
    xml +=get_body_two;//Get_Body_two(get_obj).replaceAll("&","");
    ...
    try
    {
      ...
      nlapiLogExecution("debug","Submit box_rec_Id : " + box_rec_Id);
    }
    catch(err)
    {
      nlapiLogExecution("debug","Box Record Id=>"+ box_rec_Id+ "  Error in File ID",err.message);
    }
  }
...
function Create_obj_data(get_line_item,vendor_text,box_rec_Id,total_insu)
{
  ...
  try
  {
    ...
    for(var p=0;p<get_line_item.length;p++)
    {
	  ...
	  var ship_date=get_line_item[p].ship_date;
      var po_doc_insert ='';
      if(po_doc){po_doc_insert=po_doc.replace("Purchase Order #", ""); }
      var get_desc=get_line_item[p].description;
      if(!get_desc)
      {
        get_desc='';
      }
...
function Get_Header(get_obj_box)
{
  ...
  xml += '<tr>';
  //xml += '<td height="30" align="left"><img width="180"  height="35" src="https://system.sandbox.netsuite.com/core/media/media.nl?id=image_url" /></td>'; // ZAG : commented
  // ZAG : Start
  if(nlapiGetContext().getEnvironment()=="PRODUCTION")
     xml += '<td height="30" align="left"><img width="180"  height="35" src="https://system.na3.netsuite.com/core/media/media.nl?id=image_url" /></td>';
  else
	xml += '<td height="30" align="left"><img width="180"  height="35" src="https://system.netsuite.com/core/media/media.nl?id=image_url" /></td>';
  // ZAG : End
...
// function Get_Fotter(vendor_text,box_rec_Id,get_obj_box) // ZAG : commented
function Get_Fotter(vendor_text,box_rec_Id,get_obj_box, box_counter) // ZAG : Added
{
  ...
  xml += '</td>';
  //xml += '<td></td>'; // ZAG : commented
  xml += '<td> Box # ' + box_counter + '</td>'; // ZAG : Added
...
function Get_Body_two(get_obj,type_call)
{
  ...
  xml += '<td width="13%" align="center">Ship Date</td>';
  ...
  for(var a=0;a<line_item.length;a++)
  {
    ...
    {
      var get_description=line_item[a].description;
      var new_get_description = get_description.replace(/(\r\n|\n|\r)/gm," ");
      if(type_call=='1')
      {
        var box_rec_line_temp=nlapiCreateRecord('customrecord_vendor_box_item', {recordmode: 'dynamic'});
        box_rec_line_temp.setFieldValue('custrecord_vbi_item_description', decodeURIComponent(decodeURIComponent(line_item[a].description)));
        line_item[a].description= box_rec_line_temp.getFieldValue('custrecord_vbi_item_description');
      }
    }
    else
    {
	...
	xml += '<td align="center" style="border: 1px solid #959799;">'+line_item[a].ship_date+'</td>';
...
{
  // nlapiLogExecution("debug","Get_Sorting_SB");
  var obj_line=[];
  var obj_line_two=[];
  var obj_line_three=[];
  var po_order=[];
  ...
  for(var a=0;a<get_line_item.length;a++)
  {
    if(get_line_item[a].sublist_type=='1')
    {
      obj_line.push({
		...
		ship_date:get_line_item[a].ship_date,
  ...
  {
    nlapiLogExecution("debug",JSON.stringify(po_order));
    ...
    for(var a=0;a<po_order.length;a++)
    {
      ...
      if(index_number>=0)
      {
        ...
        obj_line.push({
		  ...
		  ship_date:obj_line_two[index_number].ship_date,
  ...
  for(var a=0;a<obj_line_three.length;a++)
  {
    obj_line.push({
	  ...
	  ship_date:obj_line_three[a].ship_date,
  ...
  //nlapiLogExecution("debug",JSON.stringify(obj_line));
  return obj_line;
}