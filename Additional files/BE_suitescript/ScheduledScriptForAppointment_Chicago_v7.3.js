...
function SendMailToCustomer(apptId,setsId) {
    try {       
			...
			var hash = nlapiRequestURL('http://www.setster.com/api/v2/appointment/hash?appointment_id='+setsId,'GET');
			nlapiLogExecution('debug','hash data',hash.getBody());
			var hashdata = JSON.parse(hash.getBody());
			var hashnum = hashdata['data'];
			nlapiLogExecution('debug','Hash Number',hashnum);

            if (customerMail) {
                        var companyId = newAppointment.getFieldValue("company");
                        var companyFName = nlapiLookupField('customer', companyId, 'firstname');              
				        var linkReschedule = "https://brilliantearth.setster.com/?id=" + setsId + "&h=" + hashnum + "&act=reschedule";
                        var linkCancel = "https://brilliantearth.setster.com/?id=" + setsId + "&h=" + hashnum + "&act=cancel";
						var linkShare="https://www.brilliantearth.com/appointment-preferences/";
                        var bodyMsg;
						if(apptType==2 || apptType==28){
						 bodyMsg="Hi " + companyFName + "," + "<p>Thank you for scheduling an appointment with Brilliant Earth! Your appointment is scheduled for "+dateString+" at "+apptTime+" in "+calendarloctext+". <p>Cancel this appointment. (<a href=" + linkCancel + ">Cancel</a>)<br>Modify this appointment (<a href=" + linkReschedule + ">Reschedule</a>)<p>Please call us at 800-691-0952 with any questions or to make changes within 24 hours of your appointment.<p>We look forward to your visit!<br>-Brilliant Earth Team";
						}
						else{
                        bodyMsg="Hi " + companyFName + "," + "<p>Thank you for scheduling an appointment with Brilliant Earth! Your appointment is scheduled for "+dateString+" at "+apptTime+" in "+calendarloctext+". If you have not already done so, please <a href= "+linkShare+">share</a> your jewelry preferences. This will enable us to provide you with the best possible experience at your upcoming appointment.<p>Cancel this appointment. (<a href=" + linkCancel + ">Cancel</a>)<br>Modify this appointment (<a href=" + linkReschedule + ">Reschedule</a>)<p>Please call us at 800-691-0952 with any questions or to make changes within 24 hours of your appointment.<p>We look forward to your visit!<br>-Brilliant Earth Team";
