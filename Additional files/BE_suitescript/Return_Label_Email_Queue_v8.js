 ...
 * Script Type   : Suitscript (Suitelet Script)
 * Script Name   : Return Label Email Queue
 * Created Date  : July 28, 2018
 * Last Modified Date : 
 * Comments : 
 * SS URL : 
 * Script URL:https://system.netsuite.com/app/common/scripting/script.nl?id=1085&whence=
 */
...
function Return_Label_Email_Queue(request, response) {
    if (request.getMethod() == "GET") {
        try {
            ...
            var send_mail_Btn = form.addSubmitButton("Send Emails");
            //Added by Priti on 27th July, 2018

            form.addButton('custpage_sendascss_button', 'Send as CSS', 'SendAsCSS();');
        ...
        // nlapiLogExecution('debug', "Button is " + event.srcElement.id + " " + event.srcElement.name + " " + event.srcElement.label);
        //Load email configurations
        var config = [];
        ...
        for (var x = 0; x < request.getLineItemCount("custpage_orders"); x++) {
            if (request.getLineItemValue("custpage_orders", "custpage_order_rsl_status", x + 1) == "5") //Create and Email Label to Customer
            {
                ...
                for (var i = 0; i < config.length; i++) {
                    if (config[i].order_type == request.getLineItemValue("custpage_orders", "custpage_email_type", x + 1)) {
						...
						nlapiLogExecution("debug", "nlapiSubmitField", "Inside Email customer label"); // zag : testing
                ...
                if (templateFound == false) {
                    if (request.getLineItemValue("custpage_orders", "custpage_email_template", x + 1) != null && request.getLineItemValue("custpage_orders", "custpage_email_template", x + 1) != "") {
						...
						nlapiLogExecution("debug", "nlapiSubmitField", "Inside Template found"); // zag : testing
                        nlapiSubmitField(request.getLineItemValue("custpage_orders", "custpage_order_record_type", x + 1), request.getLineItemValue("custpage_orders", "custpage_order_id", x + 1), ["custbody138"], ["3"]); //10 Units
                    }
                }
            }
...
function Gen_Return_Label(internalid, recordType, transId, shipMethod, insuranceAmount) {
    ...
    //ZAG : Added : 02/12/19
	//var typeOfOrder = nlapiLookupField(recordType, internalid, "custbody87"); // commented
	var objSO = nlapiLookupField(recordType, internalid, ["custbody87", "custbody223", "custbody_created_from"]); //added
	var typeOfOrder = objSO.custbody87;
	var qaChecked = objSO.custbody223;
	var OrigSOId = objSO.custbody_created_from;
	var custFormId = '';
	if(OrigSOId){
		var objOrigSO  = nlapiLookupField("salesorder",OrigSOId,["custbody223","customform"]); // ZAG : Added
		//var qaChecked = objOrigSO.custbody223;
		custFormId = objOrigSO.customform;
	}
	
  	nlapiLogExecution("debug","Gen_Return_Label:", "qaChecked: " + qaChecked + " : customform : " + custFormId + " : typeOfOrder : " + typeOfOrder);
	if(custFormId == '152' || (qaChecked=="T" && typeOfOrder == '2')){ 
		shipToLocation = "2"; // SF
		nlapiLogExecution("debug", "Inside IF. shipToLocation: ", shipToLocation); // zag : testing
	}else{
	//ZAG : Ended : 02/12/19
		var filters = [];
		...
		nlapiLogExecution("debug", "Inside ELSE. shipToLocation: ", shipToLocation); // zag : testing
	} //ZAG : Added : 02/12/19
	...
	nlapiLogExecution("debug", "labelObj", JSON.stringify(labelObj)); // zag : testing
	...
	nlapiLogExecution("debug", "nlapiSubmitField", "Inside Gen_Return_Label. fileID: " + fileID); // zeg : Test
