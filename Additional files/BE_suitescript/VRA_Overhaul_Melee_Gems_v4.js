...
function VRA_Overhaul_Melee_Gems(request,response)
{
	...
	//Create UOM Map record to get unit IDs from text
	var uomMap = nlapiCreateRecord("customrecord_uom_map",{recordmode:"dynamic"});
	...
	filters.push(new nlobjSearchFilter("status",null,"anyof",["PurchOrd:B","PurchOrd:D","PurchOrd:E","PurchOrd:F","PurchOrd:G"]));
	...
	if(results)
	{
		for(var x=0; x < results.length; x++)
		{
			...
			for(var i=0; i < returns.length; i++)
			{
				...
				{
					var uom = "";
					if(results[x].getValue("unit")!=null && results[x].getValue("unit")!="")
					{
						uomMap.setFieldText("custrecord_uom_map_units",results[x].getValue("unit"));
						uom = uomMap.getFieldValue("custrecord_uom_map_units");
					}
					
					returns[i].lines.push({
						item : results[x].getValue("item"),
						qty : results[x].getValue("quantityuom"),
	...
	//{
		...
		//{
			...
			//{
				...
				//{
					//ir.setLineItemValue("item","itemreceive",r+1,"T");
					//break;
				//}
			//}
		//}
	...
	for(var x=0; x < returns2.length; x++)
	{
		...
		for(var i=0; i < returns2[x].items.length; i++)
		{
			...
			var rate = returns2[x].items[i].amount / returns2[x].items[i].qty;
			...
			vra.setCurrentLineItemValue("item","rate",rate);
