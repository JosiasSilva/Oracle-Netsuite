  ...
  if (records) {
    ...
    while (len > 0) {
      var temp = records.filter(function(obj) {
        return (obj.vendor_id == records[0].vendor_id);
      });
      ...
      while (tempLen > 0) {
        ...
        for (var j = 0; j < arrResult.items.length; j++) {
          arrSendRecords[arrSendRecords.length - 1].items.push({
            ...
            carats: arrResult.items[j].carats
          });
      ...
      records = removeByAttr(records, 'vendor_id', records[0].vendor_id);
      len = records.length;
    }
  }
