 ...
 * Change Log:
 * 
 * v6 - Updated so that If Owned Reason = Any Value, then Memo vs Owned = Own (previously would only set if Owned Reason = Preset)
 */
function Owned_Paid_Sold_Fields_SS()
{
	var testItem = nlapiGetContext().getSetting("SCRIPT","custscript_upd_ops_test_item_id");
	
	var search = nlapiLoadSearch("item", "customsearch_upd_owned_memo_flds");
	
	if(testItem!=null && testItem!="")
	{
		var newFilter = new nlobjSearchFilter("internalid",null,"is",testItem);
...
function checkDetails(item_id)
{
	...
	filters.push(new nlobjSearchFilter("status",null,"noneof",["SalesOrd:C","SalesOrd:H"])); //10 UNITS
	var cols = [];
	...
	if(ownedReason!=null && ownedReason!="")
		owned = true;
	if(paid==true)
		owned = true;
	if(sold==true)
...
function checkGovernance()
{
	...
	if(context.getRemainingUsage() < 7000)
	{
