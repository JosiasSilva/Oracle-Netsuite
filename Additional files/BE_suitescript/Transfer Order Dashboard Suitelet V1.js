...
function Transfer_Order_Dashboard(request, response)
{
   ...
   if(ck_location!=null && ck_location!='')
	...
	list_outgoing.addField("custitem_outgoing_box_chk_new", "text",'sandeep').setDisplayType("hidden");
	...
	{	
			...
			//var result_outgoing=nlapiSearchRecord(null,4604,filter_outgoing);
			//var result_incoming=nlapiSearchRecord(null,4604,filter_incoming);
			   ...
			   for(var s=0;s<result_outgoing.length;s++)
				{
				  ...
				  if(!boxnumber_check)
				  {
				   ...
				   outgoing_array.push({
							 ...
							 custitem_outgoing_box_chk_new:boxnumber_check
							 });
...
function Item_Fulfillment(idtransferorder,line_id_value)
{
   try{
	   var itemfulfillment = nlapiTransformRecord("transferorder",idtransferorder,"itemfulfillment");
	   itemfulfillment.setFieldValue('carrierform','fedex');	  
	   itemfulfillment.setFieldValue('shipmethod',1965157);
	    itemfulfillment.setFieldValue('shipstatus','C');
	   itemfulfillment.setFieldValue('generateintegratedshipperlabel','T');
	   itemfulfillment.setFieldValue('deliveryconffedex',5);
	   itemfulfillment.setFieldValue('custompackagefedex',1);
	   itemfulfillment.setFieldValue('reference1fedex','T'+itemfulfillment.getFieldValue('sonum') ); 
	   var insurance_value_get=0;
	   try{insurance_value_get=parseFloat(nlapiLookupField ( 'transferorder' , idtransferorder , 'custbody_insurance_total'));}catch(er){}	 
	   if(insurance_value_get<50000 ){
	   itemfulfillment.setFieldValue('useinsuredvaluefedex','T');
	   itemfulfillment.setFieldValue('insuredValueCurrencyLabelfedex',50000);
	   itemfulfillment.setFieldValue('reference1fedex','BEF8956'+insurance_value_get+'X1560-T'+itemfulfillment.getFieldValue('sonum'));
	   }
	   else{
       itemfulfillment.setFieldValue('useinsuredvaluefedex','T');	   
	   itemfulfillment.setFieldValue('insuredValueCurrencyLabelfedex',insurance_value_get);
	   var value_chnage='RE2014'+insurance_value_get-50000+'200305zT'+itemfulfillment.getFieldValue('sonum');
	   itemfulfillment.setFieldValue('reference1fedex',value_chnage);	   
	   }	   
	   nlapiSubmitRecord(itemfulfillment ,true,true);
   }
   catch(er){nlapiLogExecution ( 'debug', 'error', er.message);}
