...
function calcSOContributionMargin(type)
{
	if(type=="create" || type=="edit" || type=="approve")
	{
		...
		try
		{
			...
			//Exit if contribution margin already exists
			if(nlapiGetNewRecord().getFieldValue("custbody_contribution_margin")!=null && nlapiGetNewRecord().getFieldValue("custbody_contribution_margin")!="")
				return true
		...
		catch(err)
		{
			...
			emailErrorLog("Error finding sales order internal ID on sales order #" + nlapiGetNewRecord().getFieldValue("tranid") + ". Details: " + err.message)
			return true
		}
...
function calcSOContributionMargin_VendorBill(type)
{
	if(type=="create" || type=="edit")
	{
		...
		catch(err)
		{
			...
			emailErrorLog("Error finding associated sales orders on vendor bill #" + nlapiGetNewRecord().getFieldValue("tranid") + ". Details: " + err.message)
			return true
		}
		...
		catch(err)
		{
			...
			emailErrorLog("Error calculating contribution margin on sales order ID" + salesOrders[x] + ". Details: " + err.message)
			return true
		}
	}
}
