...
function TO_SO_Count_UE(type)
{
	if(type=="create")
	{
		try
		{
			...
			if(results)
			{
				nlapiSubmitField("transferorder",nlapiGetRecordId(),"custbody_to_so_count",results.length);
			}
			else
			{
				nlapiSubmitField("transferorder",nlapiGetRecordId(),"custbody_to_so_count",0.00);
			}
		}
		catch(err)
		{
			nlapiLogExecution("error","Error Calculating SO# Field","Details: " + err.message);
		}
	}
}
