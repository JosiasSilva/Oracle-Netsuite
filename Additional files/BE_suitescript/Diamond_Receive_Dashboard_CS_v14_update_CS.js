    ...
    try
    {
      ...
      if(vend_label.print==true)
      {
        ...
        if(results)
        {
          ...
          zpl+= '^XA\n';
          zpl+= '^CF0,40\n'
          zpl+= '^FO20,20^FD' + results[0].getValue("tranid","createdfrom") + "^FS\n";
          zpl+= '^CF0,45\n';
          zpl+= '^FO20,70^FD' + vend_label.poid + '^FS\n';
          zpl+= '^CFF,35\n';
          zpl+= '^FO20,110^FD' + vend_label.vendor + '^FS\n';
          zpl+= '^FO20,160^FD' + results[0].getValue("custitem27","item") + ", " + results[0].getText("custitem5","item") + ", " + results[0].getText("custitem7","item") + ", " + results[0].getText("custitem19","item") + '^FS\n';
          zpl+= '^FO20,200^FDCert # ' + results[0].getValue("custitem46","item") + "^FS\n";
          ...
          //else
            //zpl+= 'No';
          zpl+= '^FS\n';
          zpl+= '^FO20,240^FDLouped^FS\n';
          zpl+= '^BY2,5,75\n'; //Added by Sandeep NS-1386
          zpl+= '^FO50,300^BC^FD'+vend_label.poid.split('#')[1]+'^FS\n'; //Added by Sandeep NS-1386
          if(vend_label.multi_diamond==true)
          zpl+= '^FO150,280^FD1 of 2^FS\n';
          //zpl+= '^BY2,5,55\n';
          //zpl+= '^FO80,320^BC^FD' + results[0].getValue("tranid") + '^FS\n';
          zpl+= '^XZ';
        }
      }
...
function Save_Validate()
{
  ...
  {
    if(nlapiGetLineItemValue('custpage_list', 'custpage_certificate_included', x))
    {
      return true;
    }
  }
...
function PageInit()
{
  ...
  //ZAG: Added : NS 1484
   jQuery('input[name^="custpage_imagery_needed"]').each(function(){
    if(jQuery(this).val()=="T")
    {
      jQuery(this).parent().parent().children().each(function(){
        jQuery(this).style('background-color','#a2f2a7','important');
      });
    }
  });
  //ZAG: Ended
