...
function Gen_Layaway_ZPL_Label_SL(request,response)
{
	var filters = [];
	...
	if(results)
	{
		...
		var zpl = '';
			...
			//zpl+= '^FO20,80^FD' + results[0].getValue("lastname","entity") + ', '+ results[0].getValue("firstname","entity") +'^FS\n';
            zpl+= '^FO20,60^FD' + customerName +'^FS\n';
            zpl+= '^FO20,180^FDLAYAWAY' +'^FS\n';
			//zpl+= '^FO20,140^FD' + results[0].getText("custbody140") + '^FS\n';
                        //zpl+= '^FO20,180^FD' + results[0].getText("custbody194") + '^FS\n';
                        //zpl+= '^FO20,220^FDDD: ' + results[0].getValue("custbody6") + '^FS\n';
                        zpl+= '^BY2,1,60'
                        zpl+= '^FO60,280^BC^FD' + results[0].getValue("tranid") + '^FS\n';
