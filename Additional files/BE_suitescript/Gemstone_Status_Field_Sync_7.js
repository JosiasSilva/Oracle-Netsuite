							...
							if(isGemPO && location=="2")
							{
								...
								else
								{
									if(ddChanged)
										...
										nlapiSubmitField("purchaseorder",results[x].getValue("purchaseorder",null,"group"),["custbody103","custbody82"],[gemstone_status,delivery_date_firm]);
								}
							}
							else
							{
		...
		else if(type=="xedit")
		{
			...
			else if(rectype=="purchaseorder")
			{
				...
				else if(date_needed_in_sf!=null && date_needed_in_sf!="")
				{
					...
					nlapiLogExecution("debug","Location",order.location);
					nlapiLogExecution("debug","Vendor Type",order["vendor.custentity4"]);
