 ...
 * Script URL : /app/common/scripting/script.nl?id=1020
 * Button URL : /app/site/hosting/scriptlet.nl?script=1020&deploy=1
 * Save Search URL : /app/common/search/searchresults.nl?searchid=4725
 */

//Create a Suitelet Script that generate an Item Receipt
/*function GenerateItemReceipt(request,response)
{
	var form = nlapiCreateForm("Create Item Receipt of POs for HR");
	...
	else
	{
		try
		{
			...
			var mySearch = nlapiLoadSearch(null,4725); //Save search id of POs list
			var searchresult = [];
...
function GenerateItemReceipt(request, response)
{
	...
	else if(request.getMethod() == 'POST')
	{
                ...
                params['custscript_savedsearch_id5'] = 4725;
		var result = nlapiScheduleScript('customscript1058', 'customdeploy1',params);  // 1058 is internal id of the scheduled script that you wish to reschedule and 1 is deployment id
