 ...
 * FILE NAME   			: IA_For_CYO_Pendants.js
...
function CYO_Pendants(order,currency,pages,email,customer_name,today,xml,ia_for_earrings_pendants)
{
  ...
  {
    return IA_CYO_Pendants(order,currency,pages,email,customer_name,today,xml,ia_for_earrings_pendants);
  }
  else 
    return false;
}
function IA_CYO_Pendants(order,currency,pages,email,customer_name,today,xml,ia_for_earrings_pendants)
{
  ...
  if(cyo_pendants_pdf_arr.length>0)
  {
    ...
    xml += ia_for_earrings_pendants(cyo_pendants_pdf_json_obj);
  }
  xml += "</body></pdf>";
  nlapiLogExecution("DEBUG","xml for CYO Pendants",xml);
  var file = nlapiXMLToPDF(xml);
  response.setContentType("PDF","Insurance_Appraisal_Pendants.pdf","inline");
