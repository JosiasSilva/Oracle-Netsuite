function Gen_Fedex_Label(shipId,autoIF)
{
	...
	cols.push(new nlobjSearchColumn("custitem20","item")); //Category
	...
	if(results)
	{
		...
		for(var x=0; x < results.length; x++)
		{
			...
			{
				if(shipment.getLineItemValue("item","orderline",i+1) == results[x].getValue("line"))
				{
					onIF = true;
					break;
				}
		...
		for(var x=0; x < results.length; x++)
		{
			...
			nlapiLogExecution("debug","Category",results[x].getValue("custitem20","item"));
			...
			if(results[x].getValue("custitem20","item")=="2")
			{
				...
				for(var d=0; d < results.length; d++)
				{
					...
					{
						for(var c=0; c < shipment.getLineItemCount("item"); c++)
						{
							if(shipment.getLineItemValue("item","item",c+1) == results[d].getValue("item"))
							{
	...
	{
		//Prevent international labels from creating when country is DDU on automatically created labels only. Buttons will still generate.
		return true;
	}
	...
	if(errors.length == 0)
	{
		...
		var pdfFileObj = nlapiXMLToPDF(pdfxml);
			pdfFileObj.setName(labelObj.invoice_number + "_intlabel_" + last4 + ".pdf");
			pdfFileObj.setFolder(folderID);
			pdfFileObj.setIsOnline(true);
		var pdfFileId = nlapiSubmitFile(pdfFileObj);
		
		var ciFile = nlapiCreateFile(labelObj.invoice_number + " commercial_invoice_" + last4 + ".pdf","PDF",commercialInvoiceXml);
		...
		if(autoIF==false)
		{
			response.setContentType("PDF",labelObj.invoice_number + "intlabel_" + last4 + ".pdf","inline");
