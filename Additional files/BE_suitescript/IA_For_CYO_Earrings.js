 ...
 * FILE NAME   			:	IA_For_CYO_Earrings.js
 ...
 * Suite Script URL 	:   https://debugger.sandbox.netsuite.com/app/common/scripting/script.nl?id=364&whence=
 * File URL	Sandbox		:   https://debugger.sandbox.netsuite.com/app/common/media/mediaitem.nl?uploadrectype=script&id=22328004
 * File Url Production  :
 */

function CYO_Earrings(order,currency,pages,email,customer_name,today,xml,ia_for_earrings_pendants)
{
  ...
  {
    return IA_CYO_Earrings(order,currency,pages,email,customer_name,today,xml,ia_for_earrings_pendants);
  }
  else 
    return false;
}
function IA_CYO_Earrings(order,currency,pages,email,customer_name,today,xml,ia_for_earrings_pendants)
{
  ...
  if(cyo_earrings_pdf_arr.length>0)
  {
    ...
    xml += ia_for_earrings_pendants(cyo_earrings_pdf_json_obj);
  }
  xml += "</body></pdf>";
  nlapiLogExecution("DEBUG","xml for CYO earrings",xml);
  var file = nlapiXMLToPDF(xml);
  response.setContentType("PDF","Insurance_Appraisal_Earrings.pdf","inline");
