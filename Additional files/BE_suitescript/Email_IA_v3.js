function Email_IA(type)
{
	if(type=="create")
	{
		try
		{
                         nlapiLogExecution("error","Type: " +type,type); // 2b remove after test;	
			var template = nlapiGetFieldValue("template");		
                        nlapiLogExecution("error"," template-1: "+template , template ); // 2b remove after test;	
                        if(template=="490")
			{
				var transaction = nlapiGetFieldValue("transaction");
				 nlapiLogExecution("error"," SO Id: "+transaction , transaction ); // 2b remove after test;	
				var filters = [];
...
function Generate_IA_Form(record)
{
	...
	for(var x=0; x < order.getLineItemCount("item"); x++)
	{
		...
		if(category=="2")
		{
			...
			if(accentText!="" && accentText!=null)
			{
				...
				for(var i=0; i < accentText.length; i++)
				{
					...
					accentTable += "<tr>";
					accentTable += "<td width='150px' class='freightDispPro pt10'>" + nlapiEscapeXML(line[0]) + ":</td>";
					accentTable += "<td class='freightDispPro pt10'>" + nlapiEscapeXML(line[1]) + "</td>";
					accentTable += "</tr>";
				}
			...
			*/	
			...
			var pdf = {
				logo : nlapiEscapeXML(nlapiGetContext().getSetting("SCRIPT","custscript_ia_email_logo")),
			...
			if(email)
				xml += "<img align='center' src='" + nlapiGetContext().getSetting("SCRIPT","custscript_ia_email_logo") + "' />";
			else
		...
		else if(category=="3")
		{
			...
			*/
			var pdf = {
				logo : nlapiGetContext().getSetting("SCRIPT","custscript_ia_email_logo"),
				title : nlapiGetContext().getSetting("SCRIPT","custscript_ia_email_title"),
				//date : formatDate(order.getFieldValue("trandate")),
				date : formatDate(todaysDate),
				name : nlapiEscapeXML(nlapiLookupField("customer",order.getFieldValue("entity"),"altname")),
