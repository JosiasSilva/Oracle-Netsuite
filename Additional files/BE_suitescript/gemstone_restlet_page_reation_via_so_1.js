 ...
 * Edited Date   :  June 26, 2018 
 * Edited By     :  Priti Tiwari(priti.tiwari@inoday.com)
 ***/
            ...
            for (var i = 1; i <= item_count; i++) {
                ...
                if ((category == 15 || category == 18 || category == 20 || category == 31 || category == 14 || category == 8 ||
                    ...
                    if (search) {
                        ...
                        nlapiLogExecution('DEBUG', 'Record not matched, so it will generate new record.');
                        ...
                        //added by Priti on 26th June, 2018
                        var itm_pref_loc = nlapiLookupField('inventoryitem', item_id, 'preferredlocation');
                        var filter = new Array();
                        filter.push(new nlobjSearchFilter('internalid', null, 'anyof', item_id));
                        filter.push(new nlobjSearchFilter('inventorylocation', null, 'anyof', itm_pref_loc));

                        var columns = new Array();
                        columns.push(new nlobjSearchColumn('locationquantitycommitted'));
                        ...
                        if (search) {
                            var qtyOnHand = search[0].getValue('locationquantityonhand');
                            var qtyCommitted = search[0].getValue('locationquantitycommitted');
                            if (qtyOnHand) qtyOnHand = parseInt(qtyOnHand);
                            if (qtyCommitted) qtyCommitted = parseInt(qtyCommitted);

                            nlapiLogExecution('debug', 'Qty on hand', qtyOnHand);
                            nlapiLogExecution('debug', 'Qty committed', qtyCommitted);
                            if (qtyOnHand >= qtyCommitted) {
                                gemResPagecreation.setFieldValue('custrecord_grp_cs_status', '7'); //sales Rep
                                nlapiLogExecution('debug', 'cs status updated ', '7');
                            }
                        }
                        // Ended by Priti
