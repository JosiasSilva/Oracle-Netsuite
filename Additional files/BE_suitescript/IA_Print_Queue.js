function IA_Print_Queue(request,response)
{
	if(request.getMethod()=="GET")
	{
		...
		filters.push(new nlobjSearchFilter("custbody87",null,"noneof",["2","3"]));
		var cols = [];
		...
		if(results)
		{
			for(var x=0; x < results.length; x++)
			{
				orders.push({
					...
					custpage_date_to_ship : results[x].getValue("custbody110")
				});
			}
		}
		...
		list.addField("custpage_date_to_ship","date","Date To Ship");
	...
	else
	{
		...
		xml += "</macrolist>";
		xml += "</head>";
		xml+= "<body size='letter' header='nlheader' header-height='30px'>";
