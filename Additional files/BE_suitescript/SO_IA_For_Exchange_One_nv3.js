function SO_IA_Exchange_One(order,pages,today,customer_name,currency,email,iaTemp,countgemStoneArr,chkCategory,chkGemStoneForExchange,chkRingForExchange,chkExchangeOrderType,type)
{
  ...
  for(var x=0; x < order.getLineItemCount("item"); x++)
  {
    ...
    if(category=="2")
    {
      ...
      if(chkExchangeOrderType == true && chkGemStoneForExchange == false)
      {
        ...
        if(originalSOId == null || originalSOId =='') return '';
      ...
      nlapiLogExecution("Debug","GemStone Length For Exchange",countgemStoneArr.length);
  ...
  if(chkExchangeOrderType == true && chkRingForExchange == false)
  {
    ...
    var so_ia_exchange_for_two = SO_IA_For_Exchange_Two(order,pages,today,customer_name,currency,email,iaTemp,countgemStoneArr,type,chkCategory);
    nlapiLogExecution("DEBUG","Return value of IA_For_Exchange_Two() fun",so_ia_exchange_for_two);
    if(so_ia_exchange_for_two !='')
      return so_ia_exchange_for_two;
    else
      return '';
  }
