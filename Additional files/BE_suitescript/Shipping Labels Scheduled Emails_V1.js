	...
	var results;
        ...
        if (results.length != 0){
			...
			for(var c = 0; results != null && results != '' && c < results.length; c++) {
              	var so_id = results[c].getId();
              	nlapiLogExecution('Debug', 'Marking custbody_fedex_label_printed as checked.', so_id);
				nlapiSubmitField('itemfulfillment',so_id,'custbody_fedex_label_printed','T');
			}
