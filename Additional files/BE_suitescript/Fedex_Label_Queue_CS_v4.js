...
function PageInit()
{
  jQuery('div[id^="csfulfill_status"]').each(function(){

    var csfullfill_status=jQuery(this).text();
    if(csfullfill_status)
    {
      if(csfullfill_status !="Item(s) in Production"  )
      {

        jQuery(this).parent().parent().children().each(function(){
          jQuery(this).style('background-color','#f2d3bf','important');
          //jQuery(this).style('display','none');
        });
      }
    }
  });
...
function Fedex_Label_Queue_CS_FC(type,name,linenum)
{
  ...
  if(name=='custpage_droup_ship')
  {
    var url = nlapiResolveURL("SUITELET","customscript_fedex_label_print_queue","customdeploy_fedex_label_print_queue");
    url+= "&drop_value=" + nlapiGetFieldValue('custpage_droup_ship');
    window.onbeforeunload = null;
    window.location.href = url;
  }
  else if(name=="custpage_allow_reprinting")
  {
    try
    {
      var allowReprint = 'F';
  ...
  else if(name=="custpage_page")
  {
    ...
    window.onbeforeunload = null;
    window.location.href = nlapiResolveURL("SUITELET","customscript_fedex_label_print_queue","customdeploy_fedex_label_print_queue") + "&page=" + page+"&reprint=" + allowReprint;
  }
  else if(type=="custpage_labels" && name=="custpage_print")
  {
    var selected = 0;
