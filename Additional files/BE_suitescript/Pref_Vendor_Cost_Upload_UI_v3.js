...
function updateMatrixPricingUI(request,response)
{
	if(request.getMethod()=="GET")
	{
		...
		{
			//Show User UI Form
			var form = nlapiCreateForm("Pref Vendor/Cost CSV Upload");
			var fld = form.addField("custpage_csv_file","file","CSV File");
			fld.setMandatory(true);
			
			fld = form.addField("custpage_exclude_14k","checkbox","Do Not Update 14K Items");
		...
		{
			nlapiLogExecution("error","Pref Vendor/Cost CSV Upload UI Error","Details: " + err.message);
			return true;
		}
	}
	else
	{
		try
		{
			...
			params["custscript_csv_cost_update_user"] = nlapiGetUser();
			params["custscript_csv_filename"] = fileName;
			params["custscript_exclude_14k"] = request.getParameter("custpage_exclude_14k");
			...
			fld.setDefaultValue("<p>Your CSV upload of matrix costs is underway and in progress. You will be notified upon completion or should any errors be encountered.</p>");
			response.writePage(form);
		}
		catch(err)
		{
			nlapiLogExecution("error","Pref Vendor/Cost CSV Upload UI POST Error","Details: " + err.message);
		}
	}
}
