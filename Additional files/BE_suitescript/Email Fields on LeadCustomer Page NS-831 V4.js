function Update_Email(type) {
  try {
        var currentContext = nlapiGetContext(); 
    var executionContext = currentContext.getExecutionContext();
    nlapiLogExecution('debug', 'context', executionContext);
    Update_Email_Update1(nlapiGetRecordId());
     nlapiLogExecution('debug', 'Started with Record Id', nlapiGetRecordId());
  } catch (e) {
    nlapiLogExecution('debug', 'Error in P', e.message);
  }

  try {
    //addEmailHyperlinkToLeadAndCustomer(type);
  } catch (ex) {
    nlapiLogExecution('debug', 'Error in S', ex.message);
  }
}
...
function Update_Email_Update1(get_Message_id)
{
  ...
  {
    nlapiLogExecution ( 'debug' , 'er' ,get_Message_id +"  =>  "+ er.message ) 
  }

}
function addEmailHyperlinkToLeadAndCustomer(recordType,recordId) {
  nlapiLogExecution('debug', 'email links recordType  and recordId', recordType + " " +recordId );
...
function getHtmlData(email_recipient_array, customer_id) {
  ...
  {
    nlapiLogExecution('debug','error occured when generating links for customer or employee: details is -',ex.message);
  }
}
