function item_fall_dashboard_suitelet(request,response)
{
			...
			var searchresult = nlapiSearchRecord(null,'customsearch_item_falls_dashboard');
			 if(searchresult){
				...
				for (var t = 0; t < searchresult.length; t++){
					...
					//var department='';
					var created_po= check_po(so_id,item_id);					
					if(created_po!=''){
						nlapiLogExecution("debug","test enter is");
						html1+="<tr>\
						<td style='background-color:#A6FAAF'><input type='checkbox' class='suiteletcheckbox' so-id='"+so_id+"' itemid='"+item_id+"' name='cbox' /></td>\
						<td style='background-color:#A6FAAF'>"+item_name+"</td>\
						<td style='background-color:#A6FAAF; display:none;' >"+item_id+"</td>\
						<td style='background-color:#A6FAAF'>"+item_desc+"</td>\
						<td style='background-color:#A6FAAF'><a href='/app/accounting/transactions/salesord.nl?id="+so_id+"&whence='>"+so_num+"</a></td>\
						<td style='background-color:#A6FAAF; display:none;'>"+so_id+"</td>\
						<td style='background-color:#A6FAAF;'><a href='/app/accounting/transactions/purchord.nl?id="+po_id+"&whence='>"+po_text+"</a></td>\
						</tr>";
					}
					else{
 ...
 //t += '//$("body").delegate(".suiteletcheckbox","click",function(){ var ischecked= $(this).is(":checked"); if(!ischecked) alert("uncheckd " + $(this).val());});';
			...
			<span style='background-color:#A6FAAF;font-size: 15px;'>Item to be pulled for production</span>\
			</td>\
			</tr>\
			</body>"+t+" " +dd+"</html>";
...
function check_po(so_id,item_id)
{
	var chk_created_po='';
	var filter = [];
		var column = [];	
		filter.push(new nlobjSearchFilter('internalid', null, 'is', so_id));		
		filter.push(new nlobjSearchFilter('internalid', 'item', 'is', item_id));      
		...
		if(PO_Result){
		  var col = PO_Result[0].getAllColumns();
		   chk_created_po=PO_Result[0].getValue(col[0]);
		}
	return chk_created_po;
}