...
var suiteletUrl='/app/site/hosting/scriptlet.nl?script=2170&deploy=1';
...
function Change_Data_Search(name)
{
  ...
  else if(name=='custitem_outgoing_search')
  {
    ...
    Get_Update_Records('records_outgoing',null,true);
  }
}
...
function Droup_select_field(numOrders,page_index,field_records,field_select)
{
  ...
  if(document.getElementById(field_records))
  {
...
function Fulfillment_Records()
{
  ...
  if(trans_id.length>0)
  {
    ...
    document.getElementById("id_wait").style='';
    document.getElementById("id_wait_message").innerText='Updating  '+(responce_records.length)+' of '+trans_id.length+' Records...';
    Get_Update_Records_Service(trans_id,0,'all_trans_fulfill','custitem_list_outgoing','refress_call')
  }
}

function Get_Update_Records_Service(trans_id,index_trans,type_call,sublist_id,type_call_metod)
{
  var objsend_data={
    type:type_call,
  ...
  jQuery.ajax({
    ...
    success: function(get_return_value) {
      ...
      {
        Get_Update_Records_Service(trans_id,responce_records.length,type_call,sublist_id,type_call_metod);
      }
      else
      {
        document.getElementById("id_wait").style='display:none';
        ...
        {
          if(type_call_metod=='refress_call')
          {
            debugger;
...
function Number_of_box_create(insu_value)
{
  debugger;
  var get_insu_val=[];
  if(insu_value<=70000)
...
function Create_Box_Record_Obj(record_item)
{
  ...
  if(record_item)
  {
    ...
    for(var a=0;a<record_item.length;a++)
    {
      ...
      var get_line_item=record_item[a].line_item;
      var chek_number_of_insurence=Number_of_box_create(get_insu_amount);
...
{ 
  ...
  Get_Update_Records(outgoing_url,'records_outgoing',null,true);
}
