function Pickup_Automation_SL(request,response)
{
	if(request.getMethod()=="GET")
	{
		var order = request.getParameter("order");
		var customer = request.getParameter("customer");
		var puid = request.getParameter("puid");
		
		var form = nlapiCreateForm("Pick-up Appointment",true);
		...
		fld = form.addField("custpage_pu_receipt_metadata","select","PU Receipt Metadata","customrecord_pu_receipt_metadata");
		fld.setDisplayType("hidden");
		fld.setLayoutType("startrow","none");
		...
		var filters = [];
		if(order!=null && order!="")
			filters.push(new nlobjSearchFilter("custrecord_pu_receipt_salesorder",null,"is",order));
		else if(puid!=null && puid!="")
			filters.push(new nlobjSearchFilter("internalid",null,"is",puid));
		filters.push(new nlobjSearchFilter("status","custrecord_pu_receipt_item_fulfillment","noneof","ItemShip:C"));
		var results = nlapiSearchRecord("customrecord_pu_receipt_metadata",null,filters);
		if(results)
		{
			fld.setDefaultValue(results[0].getId());
			puRecId = results[0].getId();
		}
		else
		{
			var puRec = nlapiCreateRecord("customrecord_pu_receipt_metadata");
			puRec.setFieldValue("custrecord_pu_receipt_salesorder",order);
			puRec.setFieldValue("custrecord_pu_receipt_customer",customer);
			puRecId = nlapiSubmitRecord(puRec,true,true);
			
			fld.setDefaultValue(puRecId);
		}
		
		var puRec = nlapiLoadRecord("customrecord_pu_receipt_metadata",puRecId);
		...
		fld.setLayoutType("startrow","none");
		if(puRec!=null && puRec!="")
			fld.setDefaultValue(puRec.getFieldValue("custrecord_pu_receipt_salesorder"));
		...
		fld.setLayoutType("startrow","none");
		if(puRec!=null && puRec!="")
			fld.setDefaultValue(puRec.getFieldValue("custrecord_pu_receipt_customer"));
		
		fld = form.addField("custpage_fulfillment","select","Item Fulfillment","itemfulfillment");
		fld.setDisplayType("hidden");
		fld.setLayoutType("startrow","none");
		if(puRec!=null && puRec!="")
			fld.setDefaultValue(puRec.getFieldValue("custrecord_pu_receipt_item_fulfillment"));
		
		fld = form.addField("custpage_items","longtext","Item Info");
		fld.setDisplayType("hidden");
		fld.setLayoutType("startrow","none");
		if(puRec!=null && puRec!="")
			fld.setDefaultValue(puRec.getFieldValue("custrecord_pu_receipt_items"));
		...
		if(puRec!=null && puRec!="" && puRec.getFieldValue("custrecord_pu_receipt_state")!=null && puRec.getFieldValue("custrecord_pu_receipt_state")!="")
		{
			fld.setDefaultValue(puRec.getFieldValue("custrecord_pu_receipt_state"));
			currentState = puRec.getFieldValue("custrecord_pu_receipt_state");
		}	
		...
		if(puRec!=null && puRec!="")
		{
			var pendings = puRec.getFieldValue("custrecord_pu_receipt_pending_info");
			if(pendings!=null && pendings!="")
			{
				pendings = JSON.stringify(pendings);
				
				if(currentState=="collect_ring" && pendings.pending_showroom_ring==false)
					currentState = "collect_id";
				if(currentState=="collect_id" && pendings.pending_id==false)
					currentState = "collect_finance";
				if(currentState=="collect_finance" && pendings.pending_finance_receipt==false)
					currentState = "collect_affidavit";
				if(currentState=="collect_affidavit" && pendings.pending_affidavit==false)
					currentState = "collect_balance";
				if(currentState=="collect_balance" && pendings.pending_balance==false)
					currentState = "cash_carry";
					
				nlapiLogExecution("debug","Current State",currentState);
					
				fld.setDefaultValue(currentState);
			}
		}
		
		
		fld = form.addField("custpage_pending","longtext","Pending Info");
		fld.setDisplayType("hidden");
		fld.setLayoutType("startrow","none");
		if(puRec!=null && puRec!="")
			fld.setDefaultValue(puRec.getFieldValue("custrecord_pu_receipt_pending_info"));
		
		fld = form.addField("custpage_qa_employee","text","QA Employee Notes");
		fld.setDisplayType("hidden");
		fld.setLayoutType("startrow","none");
		if(puRec!=null && puRec!="")
			fld.setDefaultValue(puRec.getFieldValue("custrecord_pu_receipt_qa_employee"));
		
		fld = form.addField("custpage_collected_id","checkbox","ID Checked");
		fld.setDisplayType("hidden");
		fld.setLayoutType("startrow","none");
		if(puRec!=null && puRec!="")
			fld.setDefaultValue(puRec.getFieldValue("custrecord_pu_receipt_id_checked"));
		
		fld = form.addField("custpage_finance_received","checkbox","Finance Receipt Received");
		fld.setDisplayType("hidden");
		fld.setLayoutType("startrow","none");
		if(puRec!=null && puRec!="")
			fld.setDefaultValue(puRec.getFieldValue("custrecord_pu_receipt_finance_rcpt_recvd"));
		
		fld = form.addField("custpage_affidavit_signed","checkbox","Affidavit Signed");
		fld.setDisplayType("hidden");
		fld.setLayoutType("startrow","none");
		if(puRec!=null && puRec!="")
			fld.setDefaultValue(puRec.getFieldValue("custrecord_pu_receipt_affidavit_signed"));
		
		fld = form.addField("custpage_affidavit_approved","checkbox","Affidavit Approved");
		fld.setDisplayType("hidden");
		fld.setLayoutType("startrow","none");
		if(puRec!=null && puRec!="")
			fld.setDefaultValue(puRec.getFieldValue("custrecord_pu_receipt_affidavit_reviewed"));
			
		fld = form.addField("custpage_go_to_mgr_approval","checkbox","Go To Manager Approval");
		fld.setDisplayType("hidden");
		fld.setLayoutType("startrow","none");
			
		if(currentState=="manager_approval")
		{
			fld = form.addField("custpage_manager_approval","inlinehtml","Header");
			fld.setDefaultValue("<ul><li style='font-size: 14px;'>Manager to approve balance of </li></ul>");
			
			var adjustments = nlapiLookupField("salesorder",puRec.getFieldValue("custrecord_pu_receipt_salesorder"),["custbody294","custbody_write_off_adjust_amt","custbody163"]);
			
			fld = form.addField("custpage_comped_approved_mgr","currency","Comped Amount Approved (Manager)");
			fld.setDefaultValue(adjustments.custbody294);
			
			fld = form.addField("custpage_comped_approved_ops","currency","Comped Approved (Ops)");
			fld.setDefaultValue(adjustments.custbody_write_off_adjust_amt);
			fld.setDisplayType("inline");
			
			fld = form.addField("custpage_manager_notes","text","Manager Notes");
		}
	...
	{
		var pu_metadata = request.getParameter("custpage_pu_receipt_metadata");
		...
		var pu_state = request.getParameter("custpage_state");
		var pending = request.getParameter("custpage_pending");
		var qa_employee = request.getParameter("custpage_qa_employee");
		var collected_id = request.getParameter("custpage_collected_id");
		var finance_received = request.getParameter("custpage_finance_received");
		var affidavit_signed = request.getParameter("custpage_affidavit_signed");
		var affidavit_approved = request.getParameter("custpage_affidavit_approved");
		
		nlapiLogExecution("debug","Current State",pu_state);
		
		var nextState = "";
		
		if(pu_state=="init")
		{
			//nlapiSubmitField("customrecord_pu_receipt_metadata",pu_metadata,["custrecord_pu_receipt_items","custrecord_pu_receipt_state"],[items]);
		...
		else if(pu_state=="collect_finance")
		{
			pending = JSON.parse(pending);
		...
		else if(pu_state=="collect_affidavit" || pu_state=="review_affidavit")
		{
		...
		else if(pu_state=="collect_balance" && request.getParameter("custpage_go_to_mgr_approval")=="F")
		{
			nextStep = "cash_carry";
			
			nlapiSubmitField("customrecord_pu_receipt_metadata",pu_metadata,["custrecord_pu_receipt_state"],[nextStep]);
		}
		else if(pu_state=="collect_balance" && request.getParameter("custpage_go_to_mgr_approval")=="T")
		{
			nextStep = "manager_approval";
			
			nlapiSubmitField("customrecord_pu_receipt_metadata",pu_metadata,["custrecord_pu_receipt_state"],[nextStep]);
		}
		else if(pu_state=="manager_approval")
		{
			...
			nlapiSubmitField("salesorder",order,["custbody294","custbody163","custbody162","custbody265"],[request.getParameter("custpage_comped_approved_mgr"),newNotes,"1","20"]);
			
			nextStep = "collect_balance";
			
			nlapiSubmitField("customrecord_pu_receipt_metadata",pu_metadata,["custrecord_pu_receipt_state"],[nextStep]);
		}
		else if(pu_state=="cash_carry")
		{
		...
		else if(pu_state=="sign_pu_receipt")
		{
			...
			fld.setDefaultValue("<p style='font-size: 14px;'>The email will be sent to the follwoing email address: " + nlapiLookupField("customer",request.getParameter("custpage_customer"),"email") + ". If this email is not correct, please do not send the email.</p><p style='font-size: 14px;'>Pick-up receipt has been successfully completed. You may now close the window.</p>");
...
function checkCreateFulfillment(items,fulfillmentId,orderId,metadata,customer)
{
	...
	if(ffsToCreate.length > 0)
	{
		...
		var templateXML = nlapiLoadFile("29154728");
